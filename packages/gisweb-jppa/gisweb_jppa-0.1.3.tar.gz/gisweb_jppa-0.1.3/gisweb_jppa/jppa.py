...
class Jppa:
    ...
    async def pagamentoImmediato(self, soggetto: ISoggetto, debito: IDebito, notificaOK: str, notificaKO: str, testXml:bool=True):
    ...
    async def infoPagamento(self, deb:IChiaveDebito = None, xml_response: str = None):
        if xml_response:
            soup = BeautifulSoup(xml_response, 'xml')
            ret = await self.parseResponse(soup.find('DatiDettaglioRichiesta').string)
            deb = IChiaveDebito(idpos=ret.get('IDPos'), iddeb=ret.get('IDDeb'), codice=ret.get('CodiceTipoDebito'))
