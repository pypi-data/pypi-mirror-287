    ...
    def forward(self, input, labels):
        ...
        if self.cond:
            ...
            def f(i,j):
                ...
                mu = np.array([10,32])
                den = (2*np.pi)*np.sqrt(np.linalg.det(sigma))
                exp = np.transpose(x-mu)
                exp = np.linalg.matmul(exp,np.linalg.inv(sigma))
                exp = np.linalg.matmul(exp,x-mu)
                exp = -0.5*exp
                exp = np.exp(exp)
                return exp/(den*scale)
            ...
            #ebd[labels.cpu().detach().numpy()==1]= 0.005*np.ones([1,64,64]).astype(float)
            ebd[labels.cpu().detach().numpy()==1] = x.astype(float)
