                ...
                for l in n[key].split(','):
                    if l == labelname:
                        ...
                        for i, item in enumerate(keyList):
                            if re.search('{}.+?'.format(
                                    list(filter(lambda x: x != 'iotiumlib', resource.__name__.rsplit('.')))),
                                    item.lower()):
...
def filter_networks(node_id=None, org_id=None, search=None, cluster_id=None, fields=None, is_wan_network=None,
                    is_default_network=None):
    out_dict = {key: value for key, value in locals().items() if value is not None}
    from iotiumlib import network
    if out_dict == {}:
        filters = None
    else:
        filters = out_dict
    network_details_op = network.getv2(filters=filters).Response.output
    if not network_details_op:
        return []
    else:
        return network_details_op
