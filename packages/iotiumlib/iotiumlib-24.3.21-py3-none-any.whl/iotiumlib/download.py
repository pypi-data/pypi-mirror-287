...
from iotiumlib.requires.api_endpoints import Download
...
class download(object):
    class event(object):
        ...
        def __init__(self, action, payload=None, filters=None, event_id=None):
            ...
            api = Download()
            self.uri = {
                list_downevent: api.v1_download_event,
                request_downevent: api.v1_download_event,
                delete_downevent: api.v1_download_event_id,
            }
    ...
    class activity(object):
        ...
        def __init__(self, action, payload=None, filters=None, activity_id=None):
            ...
            api = Download()
            self.uri = {
                list_downactivity: api.v1_download_activity,
                request_downactivity: api.v1_download_activity,
                delete_downactivity: api.v1_download_activity_id,
            }
    ...
    class node(object):
        ...
        def __init__(self, action, payload=None, filters=None, node_id=None):

            if payload is None:
                payload = {}

            def get_downloadcertificate(uri):
                return download.node.DownloadCertificate(self, method='getv2', uri=uri, filters=filters)

            _function_mapping = {
                'get': get_downloadcertificate
            }

            api = Download()
            self.uri = {
                get_downloadcertificate: api.v1_node_id_download
            }
            ...
            _wrapper_fun = _function_mapping[action]
            args = '{}_downloadcertificate'.format(action)
            _wrapper_fun(self.uri[eval(args)])

        def DownloadCertificate(self, method, uri, filters=None):
            respOp = dict()
            paramRequired = checkforUriParam(uri)
            if paramRequired:
                for param in paramRequired:
        ...
        @staticmethod
        def certificate(node_id, download_type, cloud_provider):
            return download.node(action='get',
                                 filters={"download_type": download_type, "cloud_provider": cloud_provider},
                                 node_id=node_id)
