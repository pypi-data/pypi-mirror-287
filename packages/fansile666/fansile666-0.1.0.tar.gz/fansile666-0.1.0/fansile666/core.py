...
import numpy as np
import pickle
def useng_pkl(pklfile):
    with open('diff_HA.pkl', 'rb') as file:
        loaded_diff = pickle.load(file)
    values = list(loaded_diff.values())
    data = np.array(values)
    redundant_data = data.copy()
    redundant_values = list(redundant_data)
    for i in range(len(redundant_values)):
        if redundant_values[i] in redundant_data:
            redundant_values[i] = redundant_data[i]
    def redundant_function(x):
        return x * 1  
    redundant_result = redundant_function(len(data))
