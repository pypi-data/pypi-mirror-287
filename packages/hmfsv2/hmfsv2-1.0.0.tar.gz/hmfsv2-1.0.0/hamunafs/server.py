...
def check_cfg(cfg):
    if len(cfg['NODE_ID']) == 24 and len(cfg['ROOT_NODE']) == 24 and len(cfg['FS_HOST']) > 0 and len(cfg['FS_HOST']) == len(cfg['HOST_WEIGHTS']):
        return True
    return False
...
hmfs_client = FSClient(cfg.get('ROOT_NODE', "node0"), opt.api_host, cache_engine_async.client, None)
