...
class IFits : public IZStream
{
    public:
        ...
        /**
         * Exception raised when asking to get a the value of Entry as wrong type
         */
        class wrong_type : public std::runtime_error {
            public:
                wrong_type(const std::string& msg): std::runtime_error{msg} {}
                wrong_type(const char* msg): std::runtime_error{msg} {}
        };
...
template<>
inline bool IFits::Entry::Get() const {
    if (type != 'B') {
        std::stringstream msg;
        msg << "value is not a bool, has type: '" << type << "'";
        throw wrong_type(msg.str());
    }
    return value == "T";
}
