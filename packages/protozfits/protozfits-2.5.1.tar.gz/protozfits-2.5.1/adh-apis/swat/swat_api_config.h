 ...
 * @param disable_worker  if true the API worker thread is disabled and the user needs to periodically invoke swat_api_worker(...) to process events 
 * @return                a SWAT completion code as described in the swat_errors.h header 
 */
int swat_api_configure_from_file(SWAT_API_CLIENT *swat_api_h, const char* IP_address, int telnum, const char* config_filename, bool disable_worker = false);
