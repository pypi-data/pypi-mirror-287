...
from .agent_executor import new_smart_wallet_factory, \
    ...
    SubscriptionUsage, new_usage_contract
from .agent_link import AgentLink
from .agent_logger import monitor_new_onchain_log, OnChainLog, record_log, record_log_sync
from .models import AgentMetadata, SubscriptionPlan, SubscriptionPeriodEnum, UsagePlan
...
class LAgent:
    ...
    # agent usage plan
    usage_plan: List[UsagePlan] | None
    ...
    def __init__(self, name: str, private_key: str = None, message_hash: str = None, signature: str = None,
                 ...
                 image: str | None = None,
                 payable: bool = False,
                 subscription_plan: List[SubscriptionPlan] | None = None,
                 usage_plan: List[UsagePlan] | None = None):
        try:
        ...
        if payable:
            if not any([subscription_plan, usage_plan]):
                raise ValueError("subscription_plan or usage_plan must be provide when payable is true")
            if subscription_plan and usage_plan:
                raise ValueError("subscription_plan and usage_plan cannot exists at the same time")
    ...
    def _check_aa_wallet_and_subscription(self):
        ...
        self.subscription_usage = new_usage_contract(account=self.wallet, account_address=self.wallet_address)
        ...
        if self.usage_plan:
            logging.info("checking subscription usage plan...")
            self.subscription_usage.set_usage_price(self.aa_wallet_address, self.usage_plan)
