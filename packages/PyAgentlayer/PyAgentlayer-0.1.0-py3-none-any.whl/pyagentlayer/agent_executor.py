...
from .models import SubscriptionPlan, SubscriptionPeriodEnum, UsagePlan
...
subscription_usage_address = Web3.to_checksum_address('0xc670E823Ec67371868c904aA216A4906b2062726')
...
with open(os.path.join(os.path.dirname(__file__), "abi/SubscriptionUsage.json"), "r") as f:
    subscription_usage_abi = json.loads("".join(f.readlines()))
...
class SubscriptionUsage(AbstractExecutor):
    def is_subscribed(self, subscriber, target_agent_address):
        return self.contract.functions.isSubscribed(subscriber, target_agent_address).call()

    def total_usage(self, subscriber, target_agent_address):
        return self.contract.functions.userUsages(subscriber, target_agent_address).call()

    def set_usage_price(self, target_agent_address, usage_prices: List[UsagePlan]):
        params = [{"count": x.count, "price": int(math.pow(10, 18) * x.price_in_agent)} for x in usage_prices]
        tx = self.contract.functions.setUsagePrice(target_agent_address, params).build_transaction({
            "from": self.account.address,
            "nonce": self.contract.w3.eth.get_transaction_count(self.account.address)
        })
        signed_tx = self.account.sign_transaction(tx)
...
def new_usage_contract(account: Account = None, account_address: str = None):
    web3 = Web3(HTTPProvider(rpc_endpoint))
    contract = web3.eth.contract(address=subscription_usage_address, abi=subscription_usage_abi)
    return SubscriptionUsage(contract=contract, account=account, account_address=account_address)
