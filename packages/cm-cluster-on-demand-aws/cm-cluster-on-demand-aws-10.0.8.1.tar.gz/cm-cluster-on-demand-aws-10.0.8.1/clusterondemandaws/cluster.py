...
class Cluster:
    ...
    def __init__(self, aws_session: boto3.session.Session, name: str,
                 head_node_image: CODImage | None = None, vpc: Vpc | None = None,
                 head_nodes: list[Instance] | None = None) -> None:
    ...
    def find(cls, aws_session, names):
        ...
        for cluster_name, head_nodes in cluster_name_to_head_nodes.items():
            ...
            if len(set(h.vpc.id for h in head_nodes)) != 1:
                ...
                continue
            yield cls(aws_session, cluster_name, vpc=head_nodes[0].vpc, head_nodes=head_nodes)
