...
from folumo.betterGUI.constants import MOUSEBUTTONDOWN, MOUSEMOTION, MOUSEBUTTONUP
...
class Slider(Element):
    def __init__(self, xy, wh, min_val, max_val, current_val, color, handle_color, on_change=None):
        super().__init__(xy)
        self.wh = wh
    ...
    def render(self) -> Canvas:
        ...
        pygame.draw.rect(newCanvas.surf, self.color, (0, self.wh[1] // 2 - 2, self.wh[0], 4))
...
class ToggleSwitch(Element):
    def __init__(self, xy, wh, is_on, color_on, color_off, handle_color, on_toggle=None):
        super().__init__(xy)
        self.wh = wh
        self.is_on = is_on
    ...
    def render(self) -> Canvas:
        ...
        pygame.draw.rect(newCanvas.surf, color, (0, 0, self.wh[0], self.wh[1]), border_radius=15)

        handle_x = self.wh[0] - self.wh[1] if self.is_on else 0
        pygame.draw.circle(newCanvas.surf, self.handle_color, (handle_x + self.wh[1] // 2, self.wh[1] // 2),
                           self.wh[1] // 2)
...
class ProgressBar(Element):
    def __init__(self, xy, wh, min_val, max_val, current_val, bar_color, bg_color):
        super().__init__(xy)
        self.wh = wh
        self.min_val = min_val
        self.max_val = max_val
    ...
    def render(self) -> Canvas:
        newCanvas = Canvas(self.wh, True)
        pygame.draw.rect(newCanvas.surf, self.bg_color, (0, 0, self.wh[0], self.wh[1]))

        bar_width = int((self.current_val - self.min_val) / (self.max_val - self.min_val) * self.wh[0])
        pygame.draw.rect(newCanvas.surf, self.bar_color, (0, 0, bar_width, self.wh[1]))
...
class InputElement(Element):
    ...
    def render(self) -> Canvas:
        ...
        pygame.draw.rect(canvas.surf, self.border_color, (0, 0, *self.size), 2)

        display_text = self.text if self.input_type != 'password' else '*' * len(self.text)
        ...
        if self.active and self.cursor_counter < 30:
            ...
            pygame.draw.line(canvas.surf, self.text_color, (cursor_x, 5), (cursor_x, self.size[1] - 5), 2)

        return canvas


class Menu(Element):
    def __init__(self, xy, options: list[str], font_size=24, bg_color=(0, 0, 0), text_color=(255, 255, 255)):
        super().__init__(xy)
