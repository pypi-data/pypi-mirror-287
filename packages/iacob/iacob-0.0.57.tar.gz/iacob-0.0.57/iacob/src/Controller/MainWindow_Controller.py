...
class MainWindow_Controller:
    ...
    def AddGraphics_PieTab(self, action: QAction):
        ...
        if action is not None and action.text() not in self.displayedPies:
            ...
            graphicConnections_widget = GraphicWidget(self, action.text(), connectivities, "Connections")
            self.mainWindow_view.region_scroll_layout.addWidget(graphicConnections_widget)

            majorRegions = self.connGraph.GetAllMajorRegionsWithName(action.text())
            graphicMajorRegions_widget = GraphicWidget(self, action.text(), majorRegions, "MajorRegions")
            self.mainWindow_view.region_scroll_layout2.addWidget(graphicMajorRegions_widget)

            graphicMajorRegions_widgetBar = GraphicWidget(self, action.text(), majorRegions, "MajorRegionsBar")
            self.mainWindow_view.region_scroll_layout3.addWidget(graphicMajorRegions_widgetBar)

            connectionsType = self.connGraph.GetAllConnectionTypeWithName(action.text())
            graphicConnectionType_widget = GraphicWidget(self, action.text(), connectionsType, "ConnectionType")
            self.mainWindow_view.region_scroll_layout4.addWidget(graphicConnectionType_widget)

            graphicConnectionType_widgetBar = GraphicWidget(self, action.text(), connectionsType, "ConnectionTypeBar")
    ...
    def UpdateList(self):
        ...
        for edge in edges:
            ...
            area1 = QTableWidgetItem(str(edge[2]))
            area2 = QTableWidgetItem(str(edge[3]))
            region1 = QTableWidgetItem(str(edge[4]))
            region2 = QTableWidgetItem(str(edge[5]))
            connType = QTableWidgetItem(str(edge[6]))
            value = QTableWidgetItem(floatFormat.format(float(edge[7])))
            ...
            self.mainWindow_view.infoList_tableWidget.setItem(row, 2, area1)
            self.mainWindow_view.infoList_tableWidget.setItem(row, 3, area2)
            self.mainWindow_view.infoList_tableWidget.setItem(row, 4, region1)
            self.mainWindow_view.infoList_tableWidget.setItem(row, 5, region2)
            self.mainWindow_view.infoList_tableWidget.setItem(row, 6, connType)
            self.mainWindow_view.infoList_tableWidget.setItem(row, 7, value)
