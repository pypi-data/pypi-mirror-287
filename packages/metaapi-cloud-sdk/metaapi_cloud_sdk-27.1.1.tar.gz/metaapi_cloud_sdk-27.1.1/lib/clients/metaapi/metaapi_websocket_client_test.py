...
async def run_around_tests():
    ...
    client = MetaApiWebsocketClient(meta_api, domain_client, 'token', options)
    client.set_url('http://localhost:8080')
    client._socket_instances = {'vint-hill': {0: [], 1: []}, 'new-york': {0: []}}
    client._regions_by_accounts['accountId'] = {'region': 'vint-hill', 'connections': 1}
    client._regions_by_accounts['accountIdReplica'] = {'region': 'new-york', 'connections': 1}
    client._socket_instances_by_accounts = {0: {'accountId': 0, 'accountIdReplica': 0}, 1: {'accountId': 0}}
    client._accounts_by_replica_id['accountId'] = 'accountId'
    client._accounts_by_replica_id['accountIdReplica'] = 'accountId'
    client._account_replicas['accountId'] = {'vint-hill': 'accountId', 'new-york': 'accountIdReplica'}
    client._connected_hosts = {
    ...
    client._socket_instances['vint-hill'][0][0]['synchronizationThrottler']._accounts_by_synchronization_ids = {}
    client._socket_instances['vint-hill'][1][0]['synchronizationThrottler']._accounts_by_synchronization_ids = {}
    client._latency_service.on_connected = AsyncMock()
    client._latency_service.on_disconnected = MagicMock()
    client._latency_service.on_unsubscribe = MagicMock()
    client._latency_service.on_deals_synchronized = AsyncMock()
    client._latency_service.wait_connected_instance = AsyncMock(return_value='accountId:vint-hill:0:ps-mpa-1')
    client._latency_service.get_active_account_instances = MagicMock(return_value=[])
    global session_id
    ...
    client._packet_orderer.restore_order = MagicMock(side_effect=return_packet)
    yield
    await client.close()
...
class TestConnect:
    @pytest.mark.asyncio
    async def test_raise_validation_exception_if_connection_to_region_when_configure_another_one(self):
        """Should raise ValidationException if connection to region when configure another one."""
        domain_client.get_settings = AsyncMock(return_value={'domain': 'v3.agiliumlabs.cloud'})
        global client
        await client.close()
        client = MetaApiWebsocketClient(meta_api, domain_client, 'token', {**options, 'region': 'region1'})
        client.set_url('http://localhost:8080')

        try:
            await client.connect(0, 'region')
            pytest.fail()
    ...
    async def test_not_raise_exception_if_connection_to_same_region_as_configured(self):
        """Should not raise exception if connection to same region as configured."""
        domain_client.get_settings = AsyncMock(return_value={'domain': 'v3.agiliumlabs.cloud'})
        global client
        await client.close()
        client = MetaApiWebsocketClient(meta_api, domain_client, 'token', {**options, 'region': 'region1'})
        client.set_url('http://localhost:8080')
        await wait_for(client.connect(0, 'region1'), timeout=1)

    @pytest.mark.asyncio
    @patch('lib.clients.metaapi.metaapi_websocket_client.asyncio.wait_for', new=patch_wait_for(120))
    async def test_connect_to_any_region_if_it_restrained_in_config(self):
        """Should connect to any region if it is restrained in config."""
        domain_client.get_settings = AsyncMock(return_value={'domain': 'v3.agiliumlabs.cloud'})
        global client
        await client.close()
        client = MetaApiWebsocketClient(meta_api, domain_client, 'token', {**options, 'region': None})
        client.set_url('http://localhost:8080')
        await wait_for(client.connect(0, 'region1'), timeout=1)
        await wait_for(client.connect(0, 'region2'), timeout=1)


@pytest.mark.asyncio
async def test_change_client_id_on_reconnect():
    """Should change client id on reconnect."""
    with patch('lib.clients.metaapi.metaapi_websocket_client.asyncio.sleep', new=lambda x: sleep(x / 30)):
        with freeze_time() as frozen_datetime:
            ...
            async def connect(sid, environ):
                ...
                qs = parse_qs(environ['QUERY_STRING'])
                if (
                    environ['aiohttp.request'].headers['Client-Id'] != qs['clientId'][0]
                    or client_id == qs['clientId'][0]
                ):
                    pytest.fail()
...
async def test_retry_connection_if_timed_out():
    ...
    async def delayed_start():
        ...
        async def on_request(sid, data):
            if data['type'] == 'getPositions' and data['accountId'] == 'accountId' and data['application'] == 'RPC':
                await sio.emit(
                    'response',
                    {
                        'type': 'response',
    ...
    client._regions_by_accounts['accountId'] = {'region': 'vint-hill', 'connections': 1}
    client._accounts_by_replica_id['accountId'] = 'accountId'
    client._account_replicas['accountId'] = {'vint-hill': 'accountId'}
    client._connected_hosts = {'accountId:vint-hill:0:ps-mpa-1': 'ps-mpa-1'}
    client._latency_service.wait_connected_instance = AsyncMock(return_value='accountId:vint-hill:0:ps-mpa-1')
...
async def test_wait_for_connected_instance_before_sending_requests():
    ...
    async def on_request(sid, data):
        if data['type'] == 'getPositions' and data['accountId'] == 'accountId' and data['application'] == 'RPC':
            ...
            await sio.emit(
                'response', {'type': 'response', 'accountId': data['accountId'], 'requestId': data['requestId']}
            )
    ...
    client._latency_service.wait_connected_instance = AsyncMock(side_effect=wait_instance_func)
    actual = await client.get_positions('accountId')
    positions[0]['time'] = date(positions[0]['time'])
    positions[0]['updateTime'] = date(positions[0]['updateTime'])
    assert actual == positions
...
async def test_connect_to_shared_server():
    ...
    client._domain_client.get_settings = AsyncMock(return_value={'domain': 'v3.agiliumlabs.cloud'})
    client._socket_instances = {'vint-hill': {0: [{'connected': True, 'requestResolves': []}]}}
...
async def test_connect_to_dedicated_server():
    ...
    client._domain_client.get_settings = AsyncMock(
        return_value={
...
class TestAddAccountCache:
    @pytest.mark.asyncio
    async def test_add_account_cache(self):
        """Should add account cache."""
        with freeze_time() as frozen_datetime:
            with patch('lib.clients.metaapi.metaapi_websocket_client.asyncio.sleep', new=lambda x: sleep(x / 7200)):
                client = MetaApiWebsocketClient(
                    meta_api,
                    domain_client,
                    'token',
    ...
    @pytest.mark.asyncio
    async def test_delay_region_deletion(self):
        """Should delay region deletion if a request is made."""

        @sio.on('request')
        async def on_request(sid, data):
            if (
                data['type'] == 'getAccountInformation'
                and data['accountId'] == 'accountId2'
        ...
        with freeze_time() as frozen_datetime:
            with patch('lib.clients.metaapi.metaapi_websocket_client.asyncio.sleep', new=lambda x: sleep(x / 7200)):
                ...
                client._socket_instances = {'vint-hill': {0: [], 1: []}}
                client._regions_by_accounts['accountId'] = {
                    'region': 'vint-hill',
                    'connections': 1,
                    'lastUsed': datetime.now().timestamp(),
                }
                client._socket_instances_by_accounts = {0: {'accountId': 0}, 1: {'accountId': 0}}
                client._connected_hosts = {'accountId:0:ps-mpa-1': 'ps-mpa-1'}
                await client.connect(1, 'vint-hill')
                await client.connect(0, 'vint-hill')
                client._socket_instances['vint-hill'][0][0][
                    'synchronizationThrottler'
                ]._accounts_by_synchronization_ids = {}
                client._socket_instances['vint-hill'][1][0][
                    'synchronizationThrottler'
                ]._accounts_by_synchronization_ids = {}
                client._latency_service.on_connected = AsyncMock()
                client._latency_service.on_disconnected = MagicMock()
                ...
                client._latency_service.on_deals_synchronized = AsyncMock()
                client._latency_service.get_active_account_instances = MagicMock(return_value=[])
                client._latency_service.wait_connected_instance = AsyncMock(
                    return_value='accountId:vint-hill:0:ps-mpa-1'
                )
    ...
    @pytest.mark.asyncio
    async def test_clear_cache_including_accounts_with_several_replicas(self):
        ...
        with freeze_time() as frozen_datetime:
            with patch('lib.clients.metaapi.metaapi_websocket_client.asyncio.sleep', new=lambda x: sleep(x / 7200)):
                ...
                client.add_account_cache('accountId1', {'vint-hill': 'accountId1', 'new-york': 'accountId2'})
    ...
    @pytest.mark.asyncio
    async def test_clear_cache_added_on_sync_packet(self):
    ...
    @pytest.mark.asyncio
    async def test_update_account_cache(self):
        """Should update account cache."""
        client.add_account_cache(
            'accountId1', {'vint-hill': 'accountId1', 'new-york': 'accountId2', 'tokyo': 'accountId3'}
        )
        client.update_account_cache(
            'accountId1', {'vint-hill': 'accountId1', 'tokyo': 'accountId4', 'singapore': 'accountId5'}
        )
        assert client.account_replicas.get('accountId1') == {
...
@pytest.fixture
def cancel_account_stub():
    stub = MagicMock()
    client._subscription_manager.cancel_account = stub

    return stub


class TestOnAccountDeleted:
    @pytest.mark.asyncio
    async def test_delete_master_account(self, cancel_account_stub):
        """Should delete master account."""
        client.add_account_cache(
            'accountId1', {'vint-hill': 'accountId1', 'new-york': 'accountId2', 'tokyo': 'accountId3'}
        )

        client.on_account_deleted('accountId1')

        cancel_account_stub.assert_called_with('accountId1')
        client._latency_service.on_unsubscribe.assert_called_with('accountId1')
        assert client.get_account_region('accountId1') is None
        assert client.get_account_region('accountId2') is None
        assert client.get_account_region('accountId3') is None
        assert client.account_replicas.get('accountId1') is None

    @pytest.mark.asyncio
    async def test_delete_replica(self, cancel_account_stub):
        """Should delete replica."""
        client.add_account_cache(
            'accountId1', {'vint-hill': 'accountId1', 'new-york': 'accountId2', 'tokyo': 'accountId3'}
        )

        client.on_account_deleted('accountId2')
        cancel_account_stub.assert_called_with('accountId2')
        client._latency_service.on_unsubscribe.assert_called_with('accountId2')
        assert client.get_account_region('accountId1') == 'vint-hill'
        assert client.get_account_region('accountId2') is None
        assert client.get_account_region('accountId3') == 'tokyo'
        assert client.account_replicas.get('accountId1') == {'vint-hill': 'accountId1', 'tokyo': 'accountId3'}


class TestGetAccountInformation:
    @pytest.mark.asyncio
    async def test_retrieve_account(self):
        ...
        @sio.on('request')
        async def on_request(sid, data):
            assert data.get('refreshTerminalState') is None
            if (
                data['type'] == 'getAccountInformation'
                and data['accountId'] == 'accountId'
                and data['application'] == 'RPC'
            ):
                await sio.emit(
                    'response',
    ...
    @pytest.mark.asyncio
    async def test_set_refresh_terminal_state_flag(self):
        """Should set refreshTerminalState flag if it is specified."""

        @sio.on('request')
        async def on_request(sid, data):
            assert data.get('type') == 'getAccountInformation' and data.get('refreshTerminalState') is True
            await sio.emit(
                'response',
                {
                    'type': 'response',
        ...
        await client.get_account_information('accountId', {'refreshTerminalState': True})
...
class TestGetPositions:
    ...
    async def test_set_refresh_terminal_state_flag(self):
        ...
        @sio.on('request')
        async def on_request(sid, data):
            assert data.get('type') == 'getPositions' and data.get('refreshTerminalState') is True
            await sio.emit(
                'response', {'type': 'response', 'accountId': data['accountId'], 'requestId': data['requestId']}
            )

        await client.get_positions('accountId', {'refreshTerminalState': True})
...
class TestGetPosition:
    ...
    async def test_retrieve_position(self):
        ...
        async def on_request(sid, data):
            assert data.get('refreshTerminalState') is None
            if (
                data['type'] == 'getPosition'
                and data['accountId'] == 'accountId'
                and data['positionId'] == '46214692'
                and data['application'] == 'RPC'
            ):
                await sio.emit(
                    'response',
    ...
    async def test_set_refresh_terminal_state_flag(self):
        ...
        @sio.on('request')
        async def on_request(sid, data):
            assert data.get('type') == 'getPosition' and data.get('refreshTerminalState') is True
            await sio.emit(
                'response', {'type': 'response', 'accountId': data['accountId'], 'requestId': data['requestId']}
            )

        await client.get_position('accountId', '123', {'refreshTerminalState': True})
...
class TestGetOrders:
    ...
    async def test_retrieve_orders(self):
        ...
        async def on_request(sid, data):
            assert data.get('refreshTerminalState') is None
            if data['type'] == 'getOrders' and data['accountId'] == 'accountId' and data['application'] == 'RPC':
                await sio.emit(
                    'response',
                    {
                        'type': 'response',
    ...
    async def test_set_refresh_terminal_state_flag(self):
        ...
        @sio.on('request')
        async def on_request(sid, data):
            assert data.get('type') == 'getOrders' and data.get('refreshTerminalState') is True
            await sio.emit(
                'response', {'type': 'response', 'accountId': data['accountId'], 'requestId': data['requestId']}
            )

        await client.get_orders('accountId', {'refreshTerminalState': True})
...
class TestGetOrder:
    ...
    async def test_set_refresh_terminal_state_flag(self):
        ...
        @sio.on('request')
        async def on_request(sid, data):
            assert data.get('type') == 'getOrder' and data.get('refreshTerminalState') is True
            await sio.emit(
                'response', {'type': 'response', 'accountId': data['accountId'], 'requestId': data['requestId']}
            )

        await client.get_order('accountId', '123', {'refreshTerminalState': True})
...
async def test_retrieve_history_orders_by_ticket():
    ...
    async def on_request(sid, data):
        if (
            data['type'] == 'getHistoryOrdersByTicket'
            and data['accountId'] == 'accountId'
            and data['ticket'] == '46214692'
            and data['application'] == 'RPC'
        ):
            await sio.emit(
                'response',
...
async def test_retrieve_history_orders_by_position():
    ...
    async def on_request(sid, data):
        if (
            data['type'] == 'getHistoryOrdersByPosition'
            and data['accountId'] == 'accountId'
            and data['positionId'] == '46214692'
            and data['application'] == 'RPC'
        ):
...
async def test_retrieve_history_orders_by_time_range():
    ...
    async def on_request(sid, data):
        if (
            data['type'] == 'getHistoryOrdersByTimeRange'
            and data['accountId'] == 'accountId'
            and data['startTime'] == '2020-04-15T02:45:00.000Z'
            and data['application'] == 'RPC'
            and data['endTime'] == '2020-04-15T02:46:00.000Z'
            and data['offset'] == 1
            and data['limit'] == 100
        ):
            await sio.emit(
    ...
    actual = await client.get_history_orders_by_time_range(
        'accountId', date('2020-04-15T02:45:00.000Z'), date('2020-04-15T02:46:00.000Z'), 1, 100
    )
    history_orders[0]['time'] = date(history_orders[0]['time'])
    history_orders[0]['doneTime'] = date(history_orders[0]['doneTime'])
    assert actual == {'historyOrders': history_orders, 'synchronizing': False}
...
async def test_retrieve_deals_by_time_range():
    ...
    actual = await client.get_deals_by_time_range(
        'accountId', date('2020-04-15T02:45:00.000Z'), date('2020-04-15T02:46:00.000Z'), 1, 100
    )
    deals[0]['time'] = date(deals[0]['time'])
    assert actual == {'deals': deals, 'synchronizing': False}
...
async def test_execute_trade():
    ...
    trade = {
        ...
        'symbol': 'AUDNZD',
        'volume': 0.07,
        'expiration': {'type': 'ORDER_TIME_SPECIFIED', 'time': date('2020-04-15T02:45:00.000Z')},
    }
    response = {
        'numericCode': 10009,
    ...
    client._subscription_manager.is_subscription_active = MagicMock(return_value=True)
    await sio.emit(
        'synchronization',
        {'type': 'authenticated', 'accountId': 'accountId', 'host': 'ps-mpa-1', 'instanceIndex': 0, 'replicas': 1},
    )
...
@pytest.mark.asyncio
async def test_execute_trade_replica():
    """Should execute a trade via a replica account."""
    client._latency_service.get_active_account_instances = MagicMock(return_value=['accountId:new-york:0:ps-mpa-2'])
    trade = {
        'actionType': 'ORDER_TYPE_SELL',
        'symbol': 'AUDNZD',
        'volume': 0.07,
    ...
    async def on_request(sid, data):
        ...
        if (
            data['type'] == 'trade'
            and data['accountId'] == 'accountIdReplica'
            and data['application'] == 'application'
            and data['instanceIndex'] == 0
        ):
            await sio.emit(
...
async def test_fail_trade_on_old_api():
    ...
    trade = {'actionType': 'ORDER_TYPE_SELL', 'symbol': 'AUDNZD', 'volume': 0.07}
    response = {
        'error': 10006,
...
@pytest.mark.asyncio
async def test_connect_to_terminal_replica():
    """Should connect to MetaTrader terminal via a replica even if synced main."""
    client._latency_service.get_active_account_instances = MagicMock(return_value=['accountId:vint-hill:0:ps-mpa-1'])
    request_received = False
    ...
    await client.subscribe('accountIdReplica', 1)
    assert request_received
...
async def test_create_new_instance():
    ...
    for i in range(100):
        client._socket_instances_by_accounts[0]['accountId' + str(i)] = 0
        client._regions_by_accounts['accountId' + str(i)] = {'region': 'vint-hill', 'connections': 1}
        client._accounts_by_replica_id['accountId' + str(i)] = 'accountId' + str(i)

    @sio.on('request')
    async def on_request(sid, data):
        if (
            data['type'] == 'subscribe'
            and data['accountId'] == 'accountId101'
            and data['application'] == 'application'
            and data['instanceIndex'] == 0
        ):
            await sio.emit(
    ...
    client._regions_by_accounts['accountId101'] = {'region': 'vint-hill', 'connections': 1}
    client._accounts_by_replica_id['accountId101'] = 'accountId101'
...
async def test_return_error_if_failed():
    ...
    @sio.on('request')
    async def on_request(sid, data):
        if data['type'] == 'subscribe' and data['accountId'] == 'accountId' and data['application'] == 'application':
            nonlocal request_received
            request_received = True
        await sio.emit(
            'processingError',
            {'id': 1, 'error': 'NotAuthenticatedError', 'message': 'Error message', 'requestId': data['requestId']},
        )
...
async def test_retrieve_symbol_specification():
    ...
    specification = {'symbol': 'AUDNZD', 'tickSize': 0.00001, 'minVolume': 0.01, 'maxVolume': 100, 'volumeStep': 0.01}

    @sio.on('request')
    async def on_request(sid, data):
        if (
            data['type'] == 'getSymbolSpecification'
            and data['accountId'] == 'accountId'
            and data['symbol'] == 'AUDNZD'
            and data['application'] == 'RPC'
        ):
            await sio.emit(
                'response',
...
async def test_retrieve_symbol_price():
    ...
    price = {'symbol': 'AUDNZD', 'bid': 1.05297, 'ask': 1.05309, 'profitTickValue': 0.59731, 'lossTickValue': 0.59736}

    @sio.on('request')
    async def on_request(sid, data):
        if (
            data['type'] == 'getSymbolPrice'
            and data['accountId'] == 'accountId'
            and data['symbol'] == 'AUDNZD'
            and data['application'] == 'RPC'
            and data['keepSubscription']
        ):
            await sio.emit(
                'response',
                {'type': 'response', 'accountId': data['accountId'], 'requestId': data['requestId'], 'price': price},
            )
...
async def test_retrieve_candle():
    ...
    async def on_request(sid, data):
        if (
            ...
            await sio.emit(
                'response',
                {'type': 'response', 'accountId': data['accountId'], 'requestId': data['requestId'], 'candle': candle},
            )
...
async def test_retrieve_tick():
    ...
    async def on_request(sid, data):
        if (
            ...
            await sio.emit(
                'response',
                {'type': 'response', 'accountId': data['accountId'], 'requestId': data['requestId'], 'tick': tick},
            )
...
async def test_retrieve_book():
    ...
    book = {
        ...
        'brokerTime': '2020-04-07 06:45:00.000',
        'book': [
            {'type': 'BOOK_TYPE_SELL', 'price': 1.05309, 'volume': 5.67},
            {'type': 'BOOK_TYPE_BUY', 'price': 1.05297, 'volume': 3.45},
        ],
    }
    ...
    async def on_request(sid, data):
        if (
            ...
            await sio.emit(
                'response',
                {'type': 'response', 'accountId': data['accountId'], 'requestId': data['requestId'], 'book': book},
            )
...
async def test_send_uptime_stats():
    ...
    async def on_request(sid, data):
        if (
            data['type'] == 'saveUptime'
            and data['accountId'] == 'accountId'
            and data['uptime'] == {'1h': 100}
            and data['application'] == 'application'
        ):
            await sio.emit(
                'response', {'type': 'response', 'accountId': data['accountId'], 'requestId': data['requestId']}
...
class TestRefreshTerminalState:
    @pytest.mark.asyncio
    async def test_initiate_refreshing_terminal_state_and_return_symbols(self):
        """Should initiate refreshing terminal state and return symbols, initiated to refresh."""

        @sio.on('request')
        async def on_request(sid, data):
            assert (
                data.get('type') == 'refreshTerminalState'
                and data.get('application') == 'RPC'
                and data.get('accountId') == 'accountId'
            )
            await sio.emit(
                'response',
                {
        ...
        assert await client.refresh_terminal_state('accountId') == ['EURUSD', 'BTCUSD']


class TestRefreshSymbolQuotes:
    @pytest.mark.asyncio
    async def test_retrieve_throttled_quotes(self):
        """Should retrieve throttled quotes."""

        @sio.on('request')
        async def on_request(sid, data):
            assert (
                data.get('type') == 'refreshSymbolQuotes'
                and data.get('application') == 'RPC'
                and data.get('accountId') == 'accountId'
                and data.get('symbols') == ['EURUSD', 'BTCUSD']
            )
            await sio.emit(
                'response',
                {
                    ...
                    'accountId': data['accountId'],
                    'requestId': data['requestId'],
                    'refreshedQuotes': {'quotes': [{'symbol': 'EURUSD'}, {'symbol': 'BTCUSD'}], 'balance': 1100},
                },
            )

        assert await client.refresh_symbol_quotes('accountId', ['EURUSD', 'BTCUSD']) == {
...
class TestUnsubscribe:
    ...
    async def test_unsubscribe(self):
        ...
        client._latency_service.on_unsubscribe.assert_called_with('accountId')
    ...
    async def test_not_throw_errors_on_unsubscribe(self):
        ...
        async def on_request(sid, data):
            await sio.emit(
                'processingError',
                {'id': 2, 'error': 'NotFoundError', 'message': 'Account not found', 'requestId': data['requestId']},
            )
    ...
    async def test_repeat_unsubscription(self):
        ...
        with freeze_time() as frozen_datetime:
            frozen_datetime.move_to('2020-10-10 01:00:01.000')
            prices = [
                {'symbol': 'AUDNZD', 'bid': 1.05916, 'ask': 1.05927, 'profitTickValue': 0.602, 'lossTickValue': 0.60203}
            ]
...
class TestErrorHandling:
    ...
    async def test_handle_validation_exception(self):
        ...
        trade = {'actionType': 'ORDER_TYPE_SELL', 'symbol': 'AUDNZD'}
    ...
    async def test_handle_not_synchronized_exception(self):
        ...
        async def on_request(sid, data):
            await sio.emit(
                'processingError',
                {'id': 1, 'error': 'NotSynchronizedError', 'message': 'Error message', 'requestId': data['requestId']},
            )
    ...
    async def test_handle_other_exceptions(self):
        ...
        async def on_request(sid, data):
            await sio.emit(
                'processingError',
                {'id': 1, 'error': 'Error', 'message': 'Error message', 'requestId': data['requestId']},
            )
...
class TestConnectionStatusSynchronization:
    ...
    async def test_process_auth_sync_event(self, sub_active):
        ...
        await sio.emit(
            'synchronization',
            {'type': 'authenticated', 'accountId': 'accountId', 'host': 'ps-mpa-1', 'instanceIndex': 1, 'replicas': 2},
        )
        await future_close
        listener.on_connected.assert_called_with('vint-hill:1:ps-mpa-1', 2)
        client._latency_service.on_connected.assert_called_with('accountId:vint-hill:1:ps-mpa-1')
    ...
    async def test_not_send_request_to_mismatching_instances(self, sub_active):
        ...
        request_received_authenticated0 = False
        client._latency_service.get_active_account_instances = MagicMock(
            return_value=['accountId:vint-hill:1:ps-mpa-1']
        )
    ...
    async def test_process_auth_sync_event_with_session_id(self, sub_active):
        ...
        await sio.emit(
            'synchronization',
            {
                ...
                'replicas': 2,
                'host': 'ps-mpa-1',
                'sessionId': client._socket_instances['vint-hill'][0][0]['sessionId'],
            },
            connections[2],
        )
        await future_close
        assert listener.on_connected.call_count == 1
        listener.on_connected.assert_called_with('vint-hill:0:ps-mpa-1', 2)

    @pytest.mark.asyncio
    async def test_cancel_subscribe_on_authenticated_event(self, sub_active, cancel_account_stub):
        """Should cancel subscribe on authenticated event."""
        cancel_subscribe_stub = MagicMock()
        client._subscription_manager.cancel_subscribe = cancel_subscribe_stub
        client._socket_instances_by_accounts[0]['accountId2'] = 0
        client._socket_instances_by_accounts[1]['accountId2'] = 0
        client._regions_by_accounts['accountId2'] = {'region': 'vint-hill', 'connections': 1}
        client._accounts_by_replica_id['accountId2'] = 'accountId2'
        await sio.emit(
            'synchronization',
            {
                'type': 'authenticated',
    ...
    async def test_process_broker_connection_status_event(self, sub_active):
        ...
        await future_close
        listener.on_broker_connection_status_changed.assert_called_with('vint-hill:0:ps-mpa-1', True)
    ...
    async def test_call_disconnect(self, sub_active):
        ...
        with patch('lib.clients.metaapi.metaapi_websocket_client.asyncio.sleep', new=lambda x: sleep(x / 50)):
            ...
            client._latency_service.on_disconnected.assert_called_with('accountId:vint-hill:0:ps-mpa-1')
    ...
    async def test_close_stream_on_timeout(self, sub_active):
        ...
        with patch('lib.clients.metaapi.metaapi_websocket_client.asyncio.sleep', new=lambda x: sleep(x / 50)):
            ...
            listener.on_disconnected = FinalMock()
            listener.on_stream_closed = AsyncMock()
            client._subscription_manager.on_timeout = AsyncMock()
            client._subscription_manager.on_disconnected = AsyncMock()
            client.add_synchronization_listener('accountId', listener)
            await sio.emit(
                'synchronization',
                {
            ...
            await sio.emit(
                'synchronization',
                {'type': 'status', 'accountId': 'accountId', 'host': 'ps-mpa-2', 'connected': True, 'instanceIndex': 0},
            )
            await sleep(0.3)
            await sio.emit(
                'synchronization',
            ...
            listener.on_disconnected.assert_not_called()
            client._subscription_manager.on_timeout.assert_not_called()
            await sleep(0.3)
            listener.on_disconnected.assert_called_with('vint-hill:0:ps-mpa-2')
            client._subscription_manager.on_disconnected.assert_not_called()
            client._subscription_manager.on_timeout.assert_any_call('accountId', 0)
            client._subscription_manager.on_timeout.assert_any_call('accountId', 1)
    ...
    async def test_process_server_health_status(self, sub_active):
        ...
        await sio.emit(
            'synchronization',
            {'type': 'authenticated', 'accountId': 'accountId', 'host': 'ps-mpa-1', 'instanceIndex': 1, 'replicas': 1},
        )
        await sio.emit(
            'synchronization',
            {
        ...
        await future_close
        listener.on_health_status.assert_called_with('vint-hill:1:ps-mpa-1', {'restApiHealthy': True})
    ...
    async def test_process_disconnected_synchronization_event(self, sub_active):
        ...
        await future_close
        client._subscription_manager.on_disconnected.assert_any_call('accountId', 0)
        client._subscription_manager.on_disconnected.assert_any_call('accountId', 1)
        listener.on_disconnected.assert_called_with('vint-hill:0:ps-mpa-1')
        listener.on_stream_closed.assert_called_with('vint-hill:0:ps-mpa-1')
    ...
    async def test_on_stream_closed(self, sub_active):
        ...
        await sio.emit(
            'synchronization',
            {'type': 'authenticated', 'accountId': 'accountId', 'host': 'ps-mpa-1', 'instanceIndex': 0, 'replicas': 2},
        )
        await sio.emit(
            'synchronization',
            {'type': 'authenticated', 'accountId': 'accountId', 'host': 'ps-mpa-2', 'instanceIndex': 0, 'replicas': 2},
        )
        await sio.emit(
            'synchronization',
            {'type': 'disconnected', 'accountId': 'accountId', 'host': 'ps-mpa-1', 'instanceIndex': 0},
        ...
        await sio.emit(
            'synchronization',
            {'type': 'disconnected', 'accountId': 'accountId', 'host': 'ps-mpa-2', 'instanceIndex': 0},
        )
        await future_close
...
class TestTerminalStateSynchronization:
    ...
    async def test_accept_own_packets(self, sub_active):
        ...
        client._socket_instances['vint-hill'][0][0]['synchronizationThrottler'] = MagicMock()
        client._socket_instances['vint-hill'][0][0]['synchronizationThrottler'].active_synchronization_ids = [
            'synchronizationId'
        ]
        client._socket_instances['vint-hill'][0][0]['synchronizationThrottler'].schedule_synchronize = AsyncMock()
        listener = MagicMock()
    ...
    async def test_synchronize_with_metatrader_terminal(self, sub_active):
        ...
        async def on_request(sid, data):
            if (
                data['type'] == 'synchronize'
                and data['accountId'] == 'accountId'
                and data['host'] == 'ps-mpa-1'
                and data['startingHistoryOrderTime'] == '2020-01-01T00:00:00.000Z'
                and data['startingDealTime'] == '2020-01-02T00:00:00.000Z'
                and data['requestId'] == 'synchronizationId'
                and data['application'] == 'application'
                and data['instanceIndex'] == 0
            ):
                nonlocal request_received
        ...
        await client.synchronize(
            ...
            {'specificationsHashes': '1111', 'positionsHashes': '2222', 'ordersHashes': '3333'},
        )
        assert request_received
    ...
    async def test_process_sync_started_with_no_updates(self, sub_active):
        ...
        mock_throttler.schedule_synchronize = AsyncMock()
        mock_throttler.active_synchronization_ids = ['synchronizationId']
        client._socket_instances['vint-hill'][0][0]['synchronizationThrottler'] = mock_throttler
        hashes = {
        ...
        await sio.emit(
            'synchronization',
            {
                ...
                'accountId': 'accountId',
                'synchronizationId': 'synchronizationId',
                'specificationsHashIndex': 0,
                'positionsHashIndex': 1,
                'ordersHashIndex': 2,
                'instanceIndex': 0,
                'host': 'ps-mpa-1',
            },
        ...
        await asyncio.sleep(0.05)
        mock_throttler.schedule_synchronize.assert_called_with(
            'accountId',
            {
                'host': 'ps-mpa-1',
                'instanceIndex': 0,
                'requestId': 'synchronizationId',
                'startingDealTime': format_date(date('2020-01-01T00:00:00.000Z')),
                'startingHistoryOrderTime': format_date(date('2020-01-01T00:00:00.000Z')),
                'type': 'synchronize',
                'version': 2,
            },
            hashes,
        ...
        listener.on_synchronization_started.assert_called_with(
            'vint-hill:0:ps-mpa-1',
            specifications_hash='shash0',
            positions_hash='phash1',
            orders_hash='ohash2',
            synchronization_id='synchronizationId',
        )
        listener.on_positions_synchronized.assert_called_with('vint-hill:0:ps-mpa-1', 'synchronizationId')
        listener.on_pending_orders_synchronized.assert_called_with('vint-hill:0:ps-mpa-1', 'synchronizationId')
    ...
    async def test_synchronize_account_information(self, sub_active):
        ...
        await future_close
        listener.on_account_information_updated.assert_called_with('vint-hill:0:ps-mpa-1', account_information)
    ...
    async def test_synchronize_positions(self, sub_active):
        ...
        positions[0]['time'] = date(positions[0]['time'])
        positions[0]['updateTime'] = date(positions[0]['updateTime'])
        listener.on_positions_replaced.assert_called_with('vint-hill:0:ps-mpa-1', positions)
        listener.on_positions_synchronized.assert_called_with('vint-hill:0:ps-mpa-1', 'synchronizationId')
    ...
    async def test_synchronize_orders(self, sub_active):
        ...
        await future_close
        orders[0]['time'] = date(orders[0]['time'])
        listener.on_pending_orders_replaced.assert_called_with('vint-hill:0:ps-mpa-1', orders)
        listener.on_pending_orders_synchronized.assert_called_with('vint-hill:0:ps-mpa-1', 'synchronizationId')
    ...
    async def test_synchronize_history_orders(self, sub_active):
        ...
        history_orders[0]['time'] = date(history_orders[0]['time'])
        history_orders[0]['doneTime'] = date(history_orders[0]['doneTime'])
        listener.on_history_order_added.assert_called_with('vint-hill:0:ps-mpa-1', history_orders[0])
    ...
    async def test_synchronize_deals(self, sub_active):
        ...
        await sio.emit(
            'synchronization',
            {'type': 'deals', 'accountId': 'accountId', 'deals': deals, 'instanceIndex': 0, 'host': 'ps-mpa-1'},
        )
        await future_close
        deals[0]['time'] = date(deals[0]['time'])
        listener.on_deal_added.assert_called_with('vint-hill:0:ps-mpa-1', deals[0])
    ...
    async def test_process_synchronization_updates(self, sub_active):
        ...
        listener = MagicMock()
        listener.on_account_information_updated = AsyncMock()
        listener.on_positions_updated = AsyncMock()
        listener.on_position_updated = AsyncMock()
        listener.on_position_removed = AsyncMock()
        listener.on_pending_orders_updated = AsyncMock()
        ...
        listener.on_account_information_updated.assert_called_with('vint-hill:0:ps-mpa-1', update['accountInformation'])
        listener.on_positions_updated.assert_called_with(
            'vint-hill:0:ps-mpa-1', update['updatedPositions'], update['removedPositionIds']
        )
        listener.on_position_updated.assert_called_with('vint-hill:0:ps-mpa-1', update['updatedPositions'][0])
        listener.on_position_removed.assert_called_with('vint-hill:0:ps-mpa-1', update['removedPositionIds'][0])
        listener.on_pending_orders_updated.assert_called_with(
            'vint-hill:0:ps-mpa-1', update['updatedOrders'], update['completedOrderIds']
        )
        listener.on_pending_order_updated.assert_called_with('vint-hill:0:ps-mpa-1', update['updatedOrders'][0])
        listener.on_pending_order_completed.assert_called_with('vint-hill:0:ps-mpa-1', update['completedOrderIds'][0])
        listener.on_history_order_added.assert_called_with('vint-hill:0:ps-mpa-1', update['historyOrders'][0])
        listener.on_deal_added.assert_called_with('vint-hill:0:ps-mpa-1', update['deals'][0])

    @pytest.mark.asyncio
    async def test_call_updates_again_if_they_arrive_during_sync(self, sub_active):
        """Should call updates again if they arrive during sync."""
        mock_throttler = MagicMock()
        mock_throttler.active_synchronization_ids = ['ABC']
        client._socket_instances['vint-hill'][0][0]['synchronizationThrottler'] = mock_throttler
        ...
        listener.on_pending_order_completed = AsyncMock()
        listener.on_history_order_added = AsyncMock()
        listener.on_deal_added = AsyncMock()
        listener.on_deals_synchronized = AsyncMock()
        client.add_synchronization_listener('accountId', listener)
        await sio.emit(
            'synchronization',
            {
        ...
        await sio.emit(
            'synchronization',
            {**{'type': 'update', 'accountId': 'accountId', 'host': 'ps-mpa-1', 'instanceIndex': 0}, **update},
            connections[2],
        )
        await sleep(0.05)

        listener.on_account_information_updated.assert_called_once()
        listener.on_positions_updated.assert_called_once()
        listener.on_position_updated.assert_called_once()
        listener.on_position_removed.assert_called_once()
        listener.on_pending_orders_updated.assert_called_once()
        listener.on_pending_order_updated.assert_called_once()
        listener.on_history_order_added.assert_called_once()
        listener.on_deal_added.assert_called_once()
        ...
        assert listener.on_account_information_updated.call_count == 2
        assert listener.on_positions_updated.call_count == 2
        assert listener.on_position_updated.call_count == 2
        assert listener.on_position_removed.call_count == 2
        assert listener.on_pending_orders_updated.call_count == 2
        assert listener.on_pending_order_updated.call_count == 2
        assert listener.on_history_order_added.call_count == 2
        assert listener.on_deal_added.call_count == 2
        ...
        called_data = deepcopy(update)
        called_data['updatedPositions'][0]['time'] = date(called_data['updatedPositions'][0]['time'])
        called_data['updatedPositions'][0]['updateTime'] = date(called_data['updatedPositions'][0]['updateTime'])
        called_data['updatedOrders'][0]['time'] = date(called_data['updatedOrders'][0]['time'])
        called_data['historyOrders'][0]['time'] = date(called_data['historyOrders'][0]['time'])
        called_data['historyOrders'][0]['doneTime'] = date(called_data['historyOrders'][0]['doneTime'])
        called_data['deals'][0]['time'] = date(called_data['deals'][0]['time'])

        listener.on_account_information_updated.assert_called_with(
            'vint-hill:0:ps-mpa-1', called_data['accountInformation']
        )
        listener.on_positions_updated.assert_called_with(
            'vint-hill:0:ps-mpa-1', called_data['updatedPositions'], called_data['removedPositionIds']
        )
        listener.on_position_updated.assert_called_with('vint-hill:0:ps-mpa-1', called_data['updatedPositions'][0])
        listener.on_position_removed.assert_called_with('vint-hill:0:ps-mpa-1', called_data['removedPositionIds'][0])
        listener.on_pending_orders_updated.assert_called_with(
            'vint-hill:0:ps-mpa-1', called_data['updatedOrders'], called_data['completedOrderIds']
        )
        listener.on_pending_order_updated.assert_called_with('vint-hill:0:ps-mpa-1', called_data['updatedOrders'][0])
        listener.on_pending_order_completed.assert_called_with(
            'vint-hill:0:ps-mpa-1', called_data['completedOrderIds'][0]
        )
        listener.on_history_order_added.assert_called_with('vint-hill:0:ps-mpa-1', called_data['historyOrders'][0])
        listener.on_deal_added.assert_called_with('vint-hill:0:ps-mpa-1', called_data['deals'][0])
        assert listener.on_account_information_updated.call_count == 4
        assert listener.on_positions_updated.call_count == 4
        assert listener.on_position_updated.call_count == 4
        assert listener.on_position_removed.call_count == 4
        assert listener.on_pending_orders_updated.call_count == 4
        assert listener.on_pending_order_updated.call_count == 4
        assert listener.on_history_order_added.call_count == 4
        assert listener.on_deal_added.call_count == 4
        ...
        assert listener.on_account_information_updated.call_count == 5
        assert listener.on_positions_updated.call_count == 5
        assert listener.on_position_updated.call_count == 5
        assert listener.on_position_removed.call_count == 5
        assert listener.on_pending_orders_updated.call_count == 5
        assert listener.on_pending_order_updated.call_count == 5
        assert listener.on_history_order_added.call_count == 5
        assert listener.on_deal_added.call_count == 5

    @pytest.mark.asyncio
    async def test_retrieve_server_time(self):
        """Should retrieve server time from API."""
        server_time = {'time': '2022-01-01T00:00:00.000Z', 'brokerTime': '2022-01-01 02:00:00.000Z'}
    ...
    async def test_calculate_margin(self):
        ...
        margin = {'margin': 110}
        order = {'symbol': 'EURUSD', 'type': 'ORDER_TYPE_BUY', 'volume': 0.1, 'openPrice': 1.1}

        @sio.on('request')
        async def on_request(sid, data):
            if (
                data['type'] == 'calculateMargin'
                and data['accountId'] == 'accountId'
                and data['application'] == 'MetaApi'
                and json.dumps(data['order'] == json.dumps(order))
            ):
                await sio.emit(
                    'response',
                    {
...
class TestMarketDataSynchronization:
    ...
    async def test_not_retry_on_failure(self, sub_active):
        ...
        async def on_request(sid, data):
            ...
            if (
                request_counter > 0
                and data['type'] == 'subscribeToMarketData'
                and data['accountId'] == 'accountId'
                and data['symbol'] == 'EURUSD'
                and data['application'] == 'application'
                and data['instanceIndex'] == 1
            ):
                await sio.emit(
    ...
    async def test_subscribe_to_market_data_with_mt_terminal(self, sub_active):
        ...
        async def on_request(sid, data):
            if (
                data['type'] == 'subscribeToMarketData'
                and data['accountId'] == 'accountId'
                and data['symbol'] == 'EURUSD'
                and data['application'] == 'application'
                and data['instanceIndex'] == 0
                and data['subscriptions'] == [{'type': 'quotes'}]
            ):
                nonlocal request_received
                request_received = True
                await sio.emit(
    ...
    async def test_refresh_market_data_subscriptions(self, sub_active):
        ...
        async def on_request(sid, data):
            if (
                data['type'] == 'refreshMarketDataSubscriptions'
                and data['accountId'] == 'accountId'
                and data['application'] == 'application'
                and data['instanceIndex'] == 1
                and data['subscriptions'] == [{'symbol': 'EURUSD'}]
            ):
                nonlocal request_received
                request_received = True
                await sio.emit(
    ...
    @pytest.mark.asyncio
    async def test_finish_syncing_deals(self, sub_active):
        """Should finish synchronizing deals."""
        mock_throttler = MagicMock()
        mock_throttler.remove_synchronization_id = MagicMock()
        mock_throttler.active_synchronization_ids = ['ABC']
        client._socket_instances['vint-hill'][0][0]['synchronizationThrottler'] = mock_throttler
        listener = MagicMock()
        listener.on_synchronization_started = AsyncMock()
        listener.on_deals_synchronized = FinalMock()
        client.add_synchronization_listener('accountId', listener)
        await sio.emit(
            'synchronization',
            {
        ...
        await future_close
        listener.on_deals_synchronized.assert_called_with('vint-hill:0:ps-mpa-1', 'ABC')
        client._latency_service.on_deals_synchronized.assert_called_with('accountId:vint-hill:0:ps-mpa-1')
        mock_throttler.remove_synchronization_id.assert_called_with('ABC')

    @pytest.mark.asyncio
    async def test_finish_syncing_orders(self, sub_active):
        """Should finish synchronizing orders."""
        mock_throttler = MagicMock()
        mock_throttler.active_synchronization_ids = ['ABC']
        client._socket_instances['vint-hill'][0][0]['synchronizationThrottler'] = mock_throttler
        listener = MagicMock()
        listener.on_synchronization_started = AsyncMock()
        listener.on_history_orders_synchronized = FinalMock()
        client.add_synchronization_listener('accountId', listener)
        await sio.emit(
            'synchronization',
            {
        ...
        await future_close
        listener.on_history_orders_synchronized.assert_called_with('vint-hill:0:ps-mpa-1', 'ABC')

    @pytest.mark.asyncio
    async def test_downgrade_subscription(self, sub_active):
        """Should downgrade subscription."""
        listener = MagicMock()
        listener.on_synchronization_started = AsyncMock()
        listener.on_subscription_downgraded = FinalMock()
        client.add_synchronization_listener('accountId', listener)
        await sio.emit(
            'synchronization',
            {
        ...
        await future_close
        listener.on_subscription_downgraded.assert_called_with(
            'vint-hill:0:ps-mpa-1', 'EURUSD', None, [{'type': 'ticks'}, {'type': 'books'}]
        )
    ...
    async def test_synchronize_symbol_specifications(self, sub_active):
        ...
        await future_close
        listener.on_symbol_specifications_updated.assert_called_with('vint-hill:1:ps-mpa-1', specifications, ['AUDNZD'])
        listener.on_symbol_specification_updated.assert_called_with('vint-hill:1:ps-mpa-1', specifications[0])
        listener.on_symbol_specification_removed.assert_called_with('vint-hill:1:ps-mpa-1', 'AUDNZD')
    ...
    async def test_synchronize_symbol_prices(self, sub_active):
        ...
        listener.on_symbol_prices_updated.assert_called_with('vint-hill:1:ps-mpa-1', prices, 100, 200, 400, 40000, None)
        listener.on_candles_updated.assert_called_with('vint-hill:1:ps-mpa-1', candles, 100, 200, 400, 40000, None)
        listener.on_ticks_updated.assert_called_with('vint-hill:1:ps-mpa-1', ticks, 100, 200, 400, 40000, None)
        listener.on_books_updated.assert_called_with('vint-hill:1:ps-mpa-1', books, 100, 200, 400, 40000, None)
        listener.on_symbol_price_updated.assert_called_with('vint-hill:1:ps-mpa-1', prices[0])
...
class TestServerSideSynchronization:
    ...
    async def test_wait_for_server_side_sync(self):
        ...
        async def on_request(sid, data):
            if (
                data['type'] == 'waitSynchronized'
                and data['accountId'] == 'accountId'
                and data['applicationPattern'] == 'app.*'
                and data['timeoutInSeconds'] == 10
                and data['application'] == 'application'
                and data['instanceIndex'] == 1
            ):
                nonlocal request_received
...
class TestLatencyMonitoring:
    ...
    async def test_invoke_latency_listener(self, sub_active):
        ...
        async def on_request(sid, data):
            if (
                ...
                and data['symbol'] == 'AUDNZD'
                and data['application'] == 'RPC'
                and 'clientProcessingStarted' in data['timestamps']
            ):
                nonlocal timestamps
    ...
    async def test_measure_price_latencies(self, sub_active):
        ...
        assert actual_timestamps['serverProcessingFinished'] == date(
            prices[0]['timestamps']['serverProcessingFinished']
        )
        assert actual_timestamps['serverProcessingStarted'] == date(prices[0]['timestamps']['serverProcessingStarted'])
        assert actual_timestamps['eventGenerated'] == date(prices[0]['timestamps']['eventGenerated'])
        assert 'clientProcessingFinished' in actual_timestamps
    ...
    async def test_measure_update_latencies(self, sub_active):
        ...
        await future_close
        assert account_id == 'accountId'
        assert actual_timestamps['serverProcessingFinished'] == date(update['timestamps']['serverProcessingFinished'])
        assert actual_timestamps['serverProcessingStarted'] == date(update['timestamps']['serverProcessingStarted'])
        assert actual_timestamps['eventGenerated'] == date(update['timestamps']['eventGenerated'])
        assert 'clientProcessingFinished' in actual_timestamps
    ...
    async def test_process_trade_latency(self, sub_active):
        ...
        await client.trade('accountId', trade)
        assert account_id == 'accountId'
        assert actual_timestamps['clientExecutionStarted'] == date(timestamps['clientExecutionStarted'])
        assert actual_timestamps['serverExecutionStarted'] == date(timestamps['serverExecutionStarted'])
        assert actual_timestamps['serverExecutionFinished'] == date(timestamps['serverExecutionFinished'])
        assert actual_timestamps['tradeExecuted'] == date(timestamps['tradeExecuted'])
        assert 'clientProcessingFinished' in actual_timestamps
...
async def test_reconnect():
    ...
    with patch('lib.clients.metaapi.metaapi_websocket_client.asyncio.sleep', new=lambda x: sleep(x / 15)):
        trade = {'actionType': 'ORDER_TYPE_SELL', 'symbol': 'AUDNZD', 'volume': 0.07}
        response = {
            'numericCode': 10009,
            'stringCode': 'TRADE_RETCODE_DONE',
        ...
        client._packet_orderer.on_reconnected = MagicMock()
        client._subscription_manager.on_reconnected = MagicMock()
        request_counter = 0
        ...
        client._subscription_manager.on_reconnected.assert_called_with(0, 0, ['accountId'])
        client._packet_orderer.on_reconnected.assert_called_with(['accountId'])
        await client.trade('accountId', trade)
...
async def test_cancel_sync_on_disconnect():
    ...
    with patch('lib.clients.metaapi.metaapi_websocket_client.asyncio.sleep', new=lambda x: sleep(x / 15)):
        ...
        client._socket_instances['vint-hill'][0][1]['synchronizationThrottler']._accounts_by_synchronization_ids = {
            'ABC2': {}
        }
        client._socket_instances['new-york'][0][0]['synchronizationThrottler']._accounts_by_synchronization_ids = {
            'ABC3': {}
        }
        client._socket_instances['vint-hill'][1][0]['synchronizationThrottler']._accounts_by_synchronization_ids = {
            'ABC4': {}
        ...
        client._socket_instances_by_accounts[0]['accountId3'] = 0
        client._socket_instances_by_accounts[1]['accountId4'] = 0
        client.add_account_cache('accountId2', {'vint-hill': 'accountId2'})
        client.add_account_cache('accountId3', {'new-york': 'accountId3'})
        client.add_account_cache('accountId4', {'vint-hill': 'accountId4'})
        await sio.emit(
            'synchronization',
            {
                'type': 'synchronizationStarted',
        ...
        await sio.emit(
            'synchronization',
            {
                ...
                'instanceIndex': 1,
                'host': 'ps-mpa-1',
                'synchronizationId': 'ABC4',
            },
            connections[1],
        )
        await sleep(0.1)
        assert 'synchronizationId' in client._synchronization_flags
        assert 'ABC2' in client._synchronization_flags
        assert 'ABC3' in client._synchronization_flags
        assert 'ABC4' in client._synchronization_flags
        await sio.disconnect(connections[2])
        await sleep(0.1)
        assert 'synchronizationId' not in client._synchronization_flags
        assert 'ABC2' in client._synchronization_flags
        assert 'ABC3' in client._synchronization_flags
        assert 'ABC4' in client._synchronization_flags
...
async def test_process_packet_in_order():
    """Should process packets in order."""
    with patch('lib.clients.metaapi.metaapi_websocket_client.asyncio.sleep', new=lambda x: sleep(x / 60)):
        orders_call_time = 0
...
async def test_not_process_old_sync_packet_without_gaps_in_sn():
    ...
    client._subscription_manager.is_subscription_active = MagicMock(return_value=True)
    client._socket_instances['vint-hill'][0][0]['synchronizationThrottler'] = MagicMock()
    client._socket_instances['vint-hill'][0][0]['synchronizationThrottler'].active_synchronization_ids = ['ABC']
    ...
    await sio.emit(
        'synchronization',
        {
            ...
            'orders': [],
            'sequenceNumber': 2,
            'sequenceTimestamp': 1603124267181,
            'instanceIndex': 0,
            'host': 'ps-mpa-1',
            'synchronizationId': 'ABC',
        },
    ...
    client._socket_instances['vint-hill'][0][0]['synchronizationThrottler'].active_synchronization_ids = ['DEF']
    await sio.emit(
        'synchronization',
        {
            'type': 'synchronizationStarted',
            'accountId': 'accountId',
            'sequenceNumber': 3,
            'sequenceTimestamp': 1603124267190,
            'instanceIndex': 0,
            'host': 'ps-mpa-1',
            'synchronizationId': 'DEF',
        },
    )
    await sio.emit(
        'synchronization',
        {
            ...
            'orders': [],
            'sequenceNumber': 4,
            'sequenceTimestamp': 1603124267192,
            'instanceIndex': 0,
            'host': 'ps-mpa-1',
            'synchronizationId': 'ABC',
        },
    ...
    await sio.emit(
        'synchronization',
        {
            ...
            'orders': [],
            'sequenceNumber': 5,
            'sequenceTimestamp': 1603124267195,
            'instanceIndex': 0,
            'host': 'ps-mpa-1',
            'synchronizationId': 'DEF',
        },
...
async def test_process_queued_events_among_synchronization_packets():
    ...
    async def listener_event(
        arg1, specifications_hash=None, positions_hash=None, orders_hash=None, synchronization_id=''
    ):
        await asyncio.sleep(0.1)
...
@pytest.mark.asyncio
async def test_convert_forbidden_error_to_corresponding_error_class():
    """Should convert ForbiddenError to corresponding error class."""
    ...
    async def on_request(sid, data):
        if (
            ...
            await sio.emit(
                'processingError', {'error': 'ForbiddenError', 'message': 'test', 'requestId': data.get('requestId')}
            )

    try:
        await client.get_account_information('accountId')
        pytest.fail()
    except Exception as err:
        assert isinstance(err, ForbiddenException)
