...
def to_cdn(s, domain, bucket, region) -> typing.Optional[str]:
    """内部sdk存储"""
    if isinstance(s, str) and (domain) and (bucket):
        try:
            if 'beijing.myqcloud.com' in s:
                s = re.sub(r'(https?://)?({0}\.cos\.{1}\.myqcloud\.com)'.format(bucket, region), f'https://{domain}', s)
            return s
        except UnicodeDecodeError as e:
            raise CosClientError('your bytes strings and cdn request!')
    return s
...
def parse_object_canned_acl(result_acl, rsp_headers):
    ...
    public_read = {'Grantee': {'Type': 'Group', 'URI': 'http://cam.qcloud.com/groups/global/AllUsers'},
                   'Permission': 'READ'}
    if 'AccessControlList' in result_acl and result_acl['AccessControlList'] is not None and 'Grant' in result_acl[
        'AccessControlList']:
