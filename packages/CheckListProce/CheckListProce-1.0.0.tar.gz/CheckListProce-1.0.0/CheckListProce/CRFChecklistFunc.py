...
#     root.withdraw()
#     file_paths = []
#     if isinstance(files[-1],bool):
#         path = filedialog.askdirectory(title=files[0]) +'/CKL.xlsx'
...
def getFormName(page_text,lable_formName,lable_subj = None):
    ...
    if lable_subj:
        ...
        for line in lines:  
            if lable_formName in line:  
                parts = line.split(lable_formName, 1) 
                if len(parts) > 1:   
                    formName = parts[1].strip()  
                    return formName    
    return None
    
# def GetFormName_Rave(page_text, label_formName):  
#     lines = page_text.splitlines()  
#     for line in lines:  
#         if label_formName in line:  
#             parts = line.split(label_formName, 1)  # 分割一次，保留分隔符之后的部分  
#             if len(parts) > 1:  
#                 # 移除可能的前导空格  
#                 formName = parts[1].strip()  
#                 return formName  
...
def getFormList(path,for_rave):
    crf = None
    Lable_begin = "Protocol #"
    lable_formName = "dataset ="
    lable_formName_Rave = "Form:"
    lable_subj = "Site ID"
        ...
        for page in crf.pages: 
            if not for_rave:
                if Lable_begin in page.extract_text():             
                    ...
                    formName = getFormName(page.extract_text(), lable_formName,lable_subj)
                    Form_Dic.setdefault(formName, []).append(page.page_number)
            else:
                if page.page_number>1:
                    formName = getFormName(page.extract_text(), lable_formName_Rave)
                    Form_Dic.setdefault(formName, []).append(page.page_number)
                    # print("Page finish: " + str(page.page_number) + "  " + formName)
...
def OutFile_Docx(*args):
    for i in range(len(args)//2):
        doc = Document(args[2*i+1])
        dic = args[2*i]
        if doc.tables:
            table = doc.tables[1]
            while len(table.rows) > len(dic)+1:
                table.remove_row(-1)
