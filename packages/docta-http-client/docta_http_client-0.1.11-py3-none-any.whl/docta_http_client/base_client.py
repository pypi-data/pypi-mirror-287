...
class APIClient:
    ...
    def preference_data_cleaning(self, file_type: str, user_email: str, file_path: str, instruction_keys: Union[str, List[str]],
        ...
        for i in range(tqdm(int(num_chunks))):
            start = i * chunk_size
            end = min(start + chunk_size, file_info.st_size)
            chunk = file_bytes[start:end]
            chunk_md5 = get_md5(chunk)

            # 'folderType': 'text', 'fileType': 'text', dongze 说这个不重要, 我暂时记为 text
            fields = {
                'md5Value': file_md5,
        ...
        print('file uploaded successfully, running data validation, expected time: 1-2 minutes')

        # 后端接收到文件之后, 将文件上传到s3, 通知ML service, 进行扣费, 通知前端成功
        params = {
            'originalFilename': file_name,
            'fileType': file_type,
