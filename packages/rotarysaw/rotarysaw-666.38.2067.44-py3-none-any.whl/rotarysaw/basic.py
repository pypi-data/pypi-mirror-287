...
def rerun_delay(lmd, delay=30, *argv, **argk):
    if not hasattr(rerun_delay, 'lastrun'):
        rerun_delay.lastrun = {}
    h = hash(lmd)
    if h not in rerun_delay.lastrun:
        rerun_delay.lastrun[h] = monotonic()
    elif monotonic() - rerun_delay.lastrun[h] < delay:
        return

    rerun_delay.lastrun[h] = monotonic()
    return lmd(*argv, **argk)
