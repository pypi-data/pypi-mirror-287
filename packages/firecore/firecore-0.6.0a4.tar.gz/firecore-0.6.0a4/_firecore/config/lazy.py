...
def LazyCall(target: T) -> T:
    """
    I use fake type to make intellisense work
    """
    ...
    def f(**kwargs):
        if dataclasses.is_dataclass(_target):
            # omegaconf object cannot hold dataclass type
            # https://github.com/omry/omegaconf/issues/784
            target = _convert_target_to_string(_target)
        else:
            target = _target
...
_CFG_PACKAGE_NAME = "firecore._cfg_loader"
"""
...
def _patch_import():
    ...
    def find_relative_file(original_file: str, relative_import_path: str, level: int):
