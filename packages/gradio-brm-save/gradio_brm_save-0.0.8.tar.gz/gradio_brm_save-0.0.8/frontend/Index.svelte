	...
	onMount(() => {
		...
		window.addEventListener("message", async function(e: any) {
			...
			if (res.type === 'selectFilePath' && res.status === 'succeed') {
				const url = new URL('https://openapi.dp.tech/openapi/v1/open/file/upload/binary');
...
>
	{#if loading_status}
		...
		/>
	{/if}
	<iframe src="https://bohrium.dp.tech/app/function-panel" bind:this={iframeRef} style="box-sizing: content-box;width:{width}px;height: {height}px;padding: 24px;background: #ffffff;{visible ? '' : 'display: none;'}"></iframe>
</Block>
