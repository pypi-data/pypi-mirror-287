...
class AnyscaleClient(AnyscaleClientInterface):
    ...
    def upload_dataset(
        ...
        cloud_id, project_id = self._source_cloud_id_and_project_id(
            cloud_id, project_id
        )
        print(existing_dataset_id_or_name, description)

    def download_dataset(
        self, dataset_id_or_name: str, version: Optional[int]  # noqa: ARG002
    ) -> bytes:
        # TODO: replace with self._external_api_client.get_dataset_download_url()
        dataset_bytes_url = "https://example.com/dataset_bytes"
        response = requests.get(dataset_bytes_url)
        dataset_bytes = response.content
        return dataset_bytes

    def _source_cloud_id_and_project_id(
        self, cloud_id: Optional[str], project_id: Optional[str]
    ) -> Tuple[str, str]:
        """
