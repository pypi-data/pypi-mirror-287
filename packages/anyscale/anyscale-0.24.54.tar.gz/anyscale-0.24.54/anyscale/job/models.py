...
"""
    ...
    execution_mode: JobQueueExecutionMode = field(
        default=JobQueueExecutionMode.FIFO,
        metadata={
    ...
    def _validate_execution_mode(self, execution_mode: JobQueueExecutionMode):
        if not isinstance(execution_mode, JobQueueExecutionMode):
            raise TypeError(
...
"""
    ...
    def _validate_job_queue_spec(self, job_queue_spec: Optional[JobQueueSpec]):
        # NOTE: Structural validation is performed on the backend
        if job_queue_spec and not isinstance(job_queue_spec, JobQueueSpec):
            raise TypeError(
                f"'job_queue_spec' must be an instance of 'JobQueueSpec' (it is {type(job_queue_spec)})."
            )
...
"""
    ...
    def _validate_job_queue_config(self, job_queue_config: Optional[JobQueueConfig]):
        # NOTE: Structural validation is performed on the backend
        if job_queue_config and not isinstance(job_queue_config, JobQueueConfig):
            raise TypeError(
                "'job_queue_config' must be an instance of 'JobQueueConfig'."
            )
