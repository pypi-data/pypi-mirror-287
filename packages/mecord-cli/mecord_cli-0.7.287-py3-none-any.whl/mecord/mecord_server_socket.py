...
class MecordTaskExecutorThread(Thread):
    ...
    def next_task(self, gettask, ext):
        if ext == '-h':
            self.high_cur_counter -= 1
        elif ext == '-l':
            self.low_cur_counter -= 1
...
class MecordLongConnectThread(Thread):
    ...
    def receive_GetTask(self, body):
        ...
        if len(rsp.list) > 0:
            ...
            if len(task_list) > 0:
                ...
                # xy_pb.ReplyTask(self.service_country, task_list)
                self.TaskReply(task_list)
    ...
    def __init__(self, country, executor, mtype=''):
        ...
        extInfo["host_name"] = utils.get_hostname() + ('(H)' if self.mtype == '-h' else '')
        # self.extend_config = json.dumps(extInfo)
        self.extend_config = extInfo
        self.token = xy_pb.real_token(country)
        self.device_id = utils.generate_unique_id(self.mtype if self.mtype == '-h' else '')
    ...
    def socket_bypass(self):
        ...
        while self.is_running:
            ...
            if cur_pts - self.last_get_task_begin > 180:  # 超过3分钟没有主动发起拉任务
                self.GetTask()
                last_check_service_work_status_pts = cur_pts
            if self.executor.isWorking() == False and cur_pts - last_check_service_work_status_pts > 20:
                print(f"{current_thread().name}-{self.service_country} === trigger timeout getTask!")
                self.GetTask()
                last_check_service_work_status_pts = cur_pts
    ...
    def GetTask(self):
        if not os.path.exists(stop_thread_file):
            ...
            logger.info(f'{self.name} === waiting {req.limit} task, widgets: {req.widgets} -t {send_byte_time}')
            if self.mtype:
                print(f"{self.name} waiting next {req.limit} task, widgets: {req.widgets} -t {send_byte_time}")
            else:
                print(f"{self.name} waiting next {req.limit} task -t {send_byte_time}")
    ...
    def remake(self):
        self.is_running = False
        logger.info(f'{self.name} === remake')
        print(f"   {self.name} waiting stop")
        for t in self.THEADING_LIST:
            t.join()
        if self.service_country != "test":
...
class MecordShortConnectThread(Thread):
    ...
    def run(self):
        ...
        while self.is_running:
            ...
            if limit >= 1:
                logger.info(f'{self.name} === wait {limit} tasks -t {int(time.time())}, widgets: {self.widgets}')
                try:
                    reqid = str(uuid.uuid5(uuid.NAMESPACE_DNS, self.name + (self.mtype if self.mtype == '-h' else '') + str(int(time.time()*1000))))
                    datas, timeout = xy_pb.GetTask(self.service_country, self.widgets, limit=limit, keep_alive=False, reqid=reqid)
                    if len(datas) > 0:
                        logger.info(f'{self.name} {reqid} === get {len(datas)} task -t {int(time.time()*1000)}')
                        task_list = []
                        for it in datas:
                            task_list.append(it['taskUUID'])
                            self.executor.appendTask(it, timeout, self.service_country, self.taskCallback, self.next_task, self.map, self.mtype)
                        logger.info(f'{self.name} === reqid {reqid} GetTaskList {str(task_list)} -t {int(time.time()*1000)}')
                        # xy_pb.TaskReply(self.service_country, task_list)
