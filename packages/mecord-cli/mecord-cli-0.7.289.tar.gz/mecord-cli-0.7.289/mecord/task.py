...
def runTask(it, timeout, service_country, name, _appendTask, gettask, ext, next_task, map):
    ...
    executeSuccess, result_obj = executeLocalPython(taskUUID, local_cmd, params, timeout, service_country, name, _appendTask)
    #result
    ...
    next_task(gettask, ext)
    return is_ok, msg, json.dumps(result_obj["result"], separators=(',', ':')), True
...
def executeLocalPython(taskUUID, cmd, param, timeout, service_country, name, _appendTask):
    ...
    try:
        ...
        pid = process.pid
        _appendTask(taskUUID, service_country, name, pid)
