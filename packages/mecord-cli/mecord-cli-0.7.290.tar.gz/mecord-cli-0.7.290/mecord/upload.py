...
def upload(src, taskUUID, _country=None, timeout=300):
    import os
    ...
    for j in range(4):
        ...
        if len(ossurl) > 0:
            break
    print(f'getossurl_time: {int(time.time()) - getossurl_time}')
    print(f'Accelerate ossurl: {ossurl}')
    headers = dict()
    ...
    if res.status_code == 200:
        ...
        ossurl = ossurl.replace('mecord-web-sg.oss-accelerate.aliyuncs.com', 'mecord-web-sg.oss-ap-southeast-1.aliyuncs.com')
        print(f'Direct ossurl: {ossurl}')
        s = requests.session()
        s.keep_alive = False
        ...
        if _res.status_code == 200:
            ...
            raise Exception(f"upload file fail! (Direct link) res = {_res}")
    else:
        raise Exception(f"upload file fail! res = {res}")

def upload_byte(byte, suffix, taskUUID, _country=None, timeout=300):
    from mecord import store
    from mecord import taskUtils
    from mecord import xy_pb
    import requests
    ...
    content_type = ''
    for j in range(4):
        ossurl, content_type = xy_pb.GetOssUrl(country, suffix)
        if len(ossurl) == 0:
            if j == 3:
                raise Exception(f"oss server is not avalid, msg = {content_type}")
            else:
