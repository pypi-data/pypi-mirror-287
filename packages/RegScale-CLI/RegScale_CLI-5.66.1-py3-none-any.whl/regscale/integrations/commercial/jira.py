...
def sync_regscale_and_jira(
    ...
    jira_objects = fetch_jira_objects(
        jira_client=jira_client, jira_project=jira_project, jql_str=jql_str, jira_issue_type=jira_issue_type
    )
...
def fetch_jira_objects(
    jira_client: JIRA, jira_project: str, jql_str: str = None, jira_issue_type: str = "Issue"
) -> list[jiraIssue]:
    """
    ...
    page_size = 100
    jira_objects = []
    if jira_issue_type != "Issue":
        validate_issue_type(jira_client, jira_issue_type)
    logger.info("Fetching %s(s) from Jira...", jira_issue_type.lower())
    ...
    logger.info("%i %s(s) retrieved from Jira.", len(jira_objects), jira_issue_type.lower())
    return jira_objects
