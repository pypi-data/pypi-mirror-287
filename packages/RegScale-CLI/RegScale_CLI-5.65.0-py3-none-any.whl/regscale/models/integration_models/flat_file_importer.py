...
from typing import Any, Callable, Dict, Generator, Iterator, List, Optional, Sequence, TextIO, Tuple, Union
...
from regscale.models import Metadata, regscale_models
...
class FlatFileIntegration(ScannerIntegration):
    # TODO: I should probably have the ability to override asset_identifier_field, and title when create a FlatFileIntegration
    ...
    def fetch_assets(self, *args: Any, **kwargs: Any) -> Iterator[IntegrationAsset]:
        ...
        self.num_assets_to_process = kwargs.get("asset_count")
    ...
    def fetch_findings(self, *args: Tuple, **kwargs: dict) -> Iterator[IntegrationFinding]:
        ...
        self.num_findings_to_process = kwargs.get("findings_count")
...
class FlatFileImporter(ABC):
    ...
    def __init__(self, **kwargs: dict):
        ...
        flat_int.sync_findings(
            plan_id=self.attributes.parent_id,
            integration_findings=self.integration_assets,
            findings_count=len(self.data["vulns"]),
        )
        self.clean_up()
    ...
    def parse_finding(self, vuln: Vulnerability) -> Optional[IntegrationFinding]:
        ...
        try:
            ...
            name: str = vuln.plugInName
            cve = name if name and name.startswith("CVE") else vuln.cve
    ...
    def handle_vulnerability(
        ...
        """
        Override the handling the vulnerabilities for a finding.

        :param IntegrationFinding finding: The finding to handle
        :param regscale_models.Asset asset: The asset to associate the finding with
        :param regscale_models.ScanHistory scan_history: The scan history to associate the finding with
        :param Dict[str, regscale_models.Issue] existing_issues_dict: The existing issues dictionary
    ...
    def map_status_to_issue_status(self, status: str) -> str:
        """
        ...
        if status.lower() == "open":
            return "Open"
        elif status.lower() in ["resolved", "rejected", "closed", "completed"]:
            return "Closed"
        return "Open"
