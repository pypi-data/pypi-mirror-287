...
from regscale.core.app.utils.nessus_utils import software, cpe_xml_to_dict
...
class NessusIntegration(ScannerIntegration):
    ...
    def fetch_findings(self, *args: Tuple, path: str) -> Iterator[IntegrationFinding]:  # type: ignore
        """
        ...
        file_collection = self.find_nessus_files(path)
    ...
    def parse_finding(self, vuln: Any, asset_id: str) -> Optional[IntegrationFinding]:
        """
        Parses a Nessus vulnerability into an IntegrationFinding object.

        :param Any vuln: The Nessus vulnerability to parse
        ...
        try:
            ...
            return IntegrationFinding(
                control_labels=[],
                category="Nessus Vulnerability",
                title=f"{vuln.get('cve', '')}: {synopsis}",
                issue_title=f"{vuln.get('cve', '')}: {synopsis}",
                description=vuln.get("description"),
                severity=severity,
                status=status,
                asset_identifier=asset_id,
                external_id=vuln.get("plugin_text", "Unknown"),
    ...
    def fetch_assets(self, *args: Any, path: str) -> Iterator[IntegrationAsset]:  # type: ignore
        ...
        for file in file_collection:
            ...
            cpe_items = cpe_xml_to_dict(root)
            for asset in assets:
    ...
    def parse_asset(self, asset: Dict[str, Any]) -> IntegrationAsset:
        ...
        software_inventory = [
            {
                "name": software.get("title"),
                "version": software.get("version"),
                "references": software.get("references", []),
            }
            for software in asset.get("software_inventory", [])
        ]

        return IntegrationAsset(
            name=asset.get("name", ""),
            identifier=asset.get("tenable_id") or asset.get("fqdn", ""),
    ...
    def get_vulnerability_data(cls, file_vuln: Any) -> dict:
        ...
        severity = regscale_models.Vulnerability.determine_cvss3_severity_text(cve_base_score)
        return {
    ...
    @staticmethod
    def find_nessus_files(path: str) -> List[Path]:
        """
        Find all .nessus files in the given folder path.

        :param path: Path to the folder containing Nessus files
        :return: List of Path objects for .nessus files
        """
        return list(Path(path).glob("*.nessus"))
