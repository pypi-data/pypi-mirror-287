...
from regscale.models import Vulnerability
...
def create_regscale_vulnerability(row: pd.Series, match: List[Asset]) -> Vulnerability:
    """
    Create a vulnerability with a row of vuln data and matching asset list for RegScale.

    :param pd.Series row: Row of vulnerability data
    :param List[Asset] match: Matching asset list
    :return: RegScale vulnerability object
    :rtype: Vulnerability
    ...
    for asset in match:
        cves = row["cve"]
        distro = row["operatingSystem"]
        cvss3_score = float(row["cvssV3BaseScore"]) if row["cvssV3BaseScore"] else 0.0
        severity = Vulnerability.determine_cvss3_severity_text(cvss3_score) if cvss3_score else "low"
        regscale_vuln = Vulnerability(
            ...
            lastSeen=epoch_to_datetime(row["lastSeen"]),
            firstSeen=epoch_to_datetime(row["firstSeen"]),
            daysOpen=None,
            dns=row["dnsName"],
            mitigated=None,
            ...
            vprScore=None,
            tenantsId=0,
            title=row["description"][:255],
            description=row["description"],
            plugInText=row["pluginText"],
...
def create_sc_scan_history(ssp_id: int, tenable_df: pd.DataFrame) -> None:
    """
    Patch function to create Tenable SC scan history in RegScale. This will be superseded by ScannerIntegration

    :param int ssp_id: RegScale System Security Plan ID
    :param pd.DataFrame tenable_df: Dataframe of Tenable data
    :return: None
    :rtype: None
    """
    app = Application()
    scans = []
    vulns = []
    existing_assets = Asset.get_all_by_parent(parent_id=ssp_id, parent_module="securityplans")
    tenable_df["last_scan_dtm"] = pd.to_datetime(tenable_df["last_scan"])
    scanned_ips = tenable_df.nunique()["ips"]
    scanned_assets = set()

    # loop through dataframe
    for index, row in tenable_df.iterrows():
        match = [ass for ass in existing_assets if ass.name == row["dnsName"]]
        vulns.extend(create_regscale_vulnerability(row, match))
        if match:
            for asset in match:
                scanned_assets.add(asset)
    for scanned_asset in scanned_assets:
        # subset by dnsName
        asset_df = tenable_df[tenable_df["dnsName"] == scanned_asset["name"]]
        severities = (asset_df["severity"].value_counts()).to_dict()
        count_info = severities.get("Info", 0)
        count_low = severities.get("Low", 0)
        count_medium = severities.get("Medium", 0) + severities.get("Moderate", 0)
        count_high = severities.get("High", 0)
        count_critical = severities.get("Critical", 0)
        max_last_scan = asset_df["last_scan_dtm"].max()

        # Create a scan
        scan = ScanHistory(
            **{
                "id": 0,
                "scanningTool": "Tenable SC",
                "scanDate": max_last_scan.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        scans.append(scan)
    res = ScanHistory.batch_create(scans)
    # update vuln scan ids
    for new_scan in res:
        scan_vulns = [vuln for vuln in vulns if vuln.parentId == new_scan.parentId]
        for vuln in scan_vulns:
            # update scan id
            vuln.scanId = new_scan.id
            # update record in vulns list
            vulns[vulns.index(vuln)] = vuln
    Vulnerability.batch_create(vulns)
...
def q_vuln(query_id: int, ssp_id: int, create_issue_from_recommendation: bool) -> list:
    ...
    create_sc_scan_history(ssp_id, tenable_df)
...
def trend_vulnerabilities(
    ...
    # plotting graph
    import matplotlib.pyplot as plt  # Optimize import performance
...
def sync_vulns(regscale_ssp_id: int = 0) -> None:
    ...
    latest_scan: int = get_last_pull_epoch(regscale_ssp_id)
    vulns = client.exports.vulns(vpr_score={"gte": cvss2_min}, since=latest_scan, state=["OPEN", "REOPENED"])
    # Chunks are typically 200MB a piece, so we can use this to determine a rough estimate of the space needed
    console.print(
        f"The space available on the temp share ({tempfile.gettempdir()}) for caching Tenable data is "
        f"{str(round(determine_available_space() / (1024 ** 3), 1))} GB.",
    )
    if latest_scan:
        app.logger.info(
            "Building a list of Nessus Reports with a severity >= %s and updated since %s.",
            minimum_severity,
            datetime.fromtimestamp(latest_scan),
        )
    else:
