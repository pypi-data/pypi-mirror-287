...
class Prisma(ContainerScan):
    ...
    def create_issue(self, **kwargs: dict) -> Optional[Issue]:
        """
        Create an issue from a row in the Prisma csv file
        ...
        fix_status: str = self.mapping.get_value(dat, FIX_STATUS)
        cve: str = self.mapping.get_value(dat, CVE_ID)
        cvss3_score: str = self.mapping.get_value(dat, self.cvss3_score)
        ...
        iss = Issue(
            ...
            pluginId=dat[VULNERABILITY_ID] if VULNERABILITY_ID in dat else dat[CVE_ID],
            ...
            extra_data={"link": dat.get(VULNERABILITY_LINK)},
    ...
    def default_mapping() -> Mapping:
        """
        Default mapping for the Prisma csv file if one is not provided
        """
        mapping_dict = {
            "Registry": "Registry",
