...
class Asset(RegScaleModel):
    ...
    def get_map(cls, plan_id: int, key_field: str = "otherTrackingNumber") -> dict[str, "Asset"]:
        """
        Get the asset map for the asset
...
class AssetMapping(RegScaleModel):
    ...
    def _get_additional_endpoints() -> ConfigDict:
        ...
        return ConfigDict(  # type: ignore
            ...
            get_mappings_as_assetss="/api/{model_slug}/getMappingsAsAssets/{intId}/{strSortBy}/{strDirection}/{intPage}/{intPageSize}",
        )

    def find_by_unique(self, parent_id_field: Optional[str] = None) -> Optional["AssetMapping"]:  # type: ignore
        """
        Find a ComponentMapping by its unique fields

        :param Optional[str] parent_id_field: The parent ID field, defaults to None
        :raises ValueError: If componentId or assetId is not set
        :return: ComponentMapping object
        :rtype: Optional[AssetMapping]
        """

        if not self.componentId or not self.assetId:
            raise ValueError("Component ID and Asset ID are required")
        mappings: list[AssetMapping] = self.find_mappings(self.componentId)
    ...
    def get_all_by_parent(
        ...
        """
        Get all assets by parent id and parent module
    ...
    def find_mappings(cls, component_id: int) -> List[T]:
        """
        Find assets with graph for a given component
        ...
        search_data = f"""query {{
            assetMappings(skip: 0, take: 50, where: {{componentId: {{eq: {component_id}}} }}) {{
                items {{
                {cls.build_graphql_fields()}
                }}
                totalCount
        ...
        response = cls._api_handler.graph(query=search_data)
        assets = cast(List[T], cls._handle_graph_response(response))
        return assets

    @classmethod
    def _handle_list_response(cls, response: Response, component_id: int) -> List[T]:
        """
