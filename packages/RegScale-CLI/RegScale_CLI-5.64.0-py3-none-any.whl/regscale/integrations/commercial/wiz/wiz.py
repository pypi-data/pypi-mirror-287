...
def inventory(
    ...
    scanner.project_ids = wiz_project_id.split(",")
    scanner.sync_assets(
        plan_id=regscale_id,
...
@click.option(
    "--wiz_project_id",
    help="Enter the project ID for Wiz",
    default=None,
    required=False,
)
@regscale_id(help="RegScale will create and update issues as children of this record.")
...
def sbom(
    ...
    """
    from regscale.integrations.commercial.wiz import WizSbomIntegration

    wiz_project_id = wiz_project_id or os.environ.get("WizProjectID")
    sbom_integration = WizSbomIntegration(
        wiz_project_id=wiz_project_id,
        regscale_id=regscale_id,
        regscale_module=regscale_module,
        filter_by_override=filter_by_override,
...
def vulnerabilities(
    ...
    # wiz_integration = WizIntegration()
    # wiz_integration.fetch_vulnerabilities(
    #     wiz_project_id, regscale_id, regscale_module, client_id, client_secret, filter_by_override
    # )
    from regscale.integrations.commercial.wizv2.scanner import WizVulnerabilityIntegration
