...
from typing import List, Optional, Any, Iterator, Union, Tuple, TypeVar, Generic, Dict, Set
...
from regscale.models import regscale_models, OpenIssueDict, DataDataType
...
class ScannerIntegration(ABC):
    ...
    def get_cci_to_control_map(self) -> dict[str, set[int]]:
        ...
        with self.cci_to_control_map_lock:
            ...
            self.cci_to_control_map = regscale_models.map_ccis_to_control_ids(parent_id=self.plan_id)  # GraphQL Query
    ...
    def get_finding_other_identifier(self, finding: IntegrationFinding) -> str:
        """
        Gets the other identifier for the finding
        ...
        prefix += finding.cve or hash(finding.external_id).__str__() or finding.rule_id
    ...
    def update_asset_if_needed(self, asset: IntegrationAsset, existing_asset: regscale_models.Asset) -> None:
        """
        ...
        if is_updated:
            ...
            existing_asset.save()
            logger.debug("Updated asset %s", asset.identifier)
        else:
            logger.debug("%s is already up to date", asset.identifier)

        self.create_asset_data_and_link(existing_asset, asset)
        self.create_or_update_ports_protocol(existing_asset, asset)
    ...
    def create_new_asset(
        ...
        new_asset = new_asset.create_or_update()
        logger.debug("Created new asset with identifier %s", asset.identifier)

        # Get existing software inventory for this asset
        existing_software = regscale_models.SoftwareInventory.get_all_by_parent(
            parent_id=new_asset.id, parent_module=None
        )
        existing_software_dict = {(s.name, s.version): s for s in existing_software}

        # Create or update software from the scan
        for software in asset.software_inventory:
            if not (software_name := software.get("name")):
            ...
            if (software_name, software_version) not in existing_software_dict:
                software_inventory = regscale_models.SoftwareInventory(
                    name=software_name,
                    parentHardwareAssetId=new_asset.id,
                    version=software_version,
                    references=software.get("references", []),
                )
                software_inventory.create_or_update()
            else:
                existing_software_dict.pop((software_name, software_version))

        # Remove software that is no longer in the scan
        for software_obj in existing_software_dict.values():
            software_obj.delete()
    ...
    def create_or_update_ports_protocol(
        self, asset: regscale_models.Asset, integration_asset: IntegrationAsset
    ) -> None:
        """
    ...
    def create_or_update_issue_from_finding(
        ...
        issue = regscale_models.Issue(
            ...
            otherIdentifier=self.get_finding_other_identifier(finding),
        ...
        if ScannerVariables.issueCreation.lower() != "perasset":
            ...
            existing_issues = regscale_models.Issue.find_by_other_identifier(self.get_finding_other_identifier(finding))
            delimiter = "\n"
    ...
    def handle_passing_finding(
        ...
        for issue in existing_issues:
            ...
            if (
                issue.otherIdentifier == self.get_finding_other_identifier(finding)
    ...
    def process_checklist(self, finding: IntegrationFinding) -> None:
        ...
        :rtype: None
        """
        logger.info("Processing checklist %s", finding.external_id)
        ...
        if not found_checklist:
            logger.debug("Creating checklist for %s", finding.external_id)
            ...
            logger.debug("Updating checklist for %s", finding.external_id)
    ...
    def update_regscale_checklists(self, findings: List[IntegrationFinding]) -> None:
        ...
        loading_findings = self.finding_progress.add_task(
            "[#f8b737]Creating and updating checklists from %s.",
            self.title,
        )
        progress_lock = threading.Lock()
    ...
    def update_regscale_findings_and_vulnerabilities(self, findings: Iterator[IntegrationFinding]) -> None:
        """
        ...
        for finding in findings:
            self.process_finding(finding, existing_issues_dict, scan_history, current_vulnerabilities)
    ...
    def get_existing_issues_dict(self) -> Dict[str, regscale_models.Issue]:
        return {
            f"{i.otherIdentifier}_{i.assetIdentifier}": i
    ...
    def process_finding(
        ...
        """
        self.process_control_findings(finding)
    ...
    def create_or_get_control_test(
        self, finding: IntegrationFinding, control_implementation_id: int
    ...
    def handle_vulnerability(
        ...
        if finding.cve:
            ...
            logger.warning("No CVE found for finding %s", finding.title)
        return vulnerability_id

    def update_regscale_vulnerabilities(self, findings: Iterator[IntegrationFinding]) -> None:
        """
        Updates RegScale vulnerabilities based on the integration findings.

        :param Iterator[IntegrationFinding] findings: The integration findings
        """
        logger.info("Updating RegScale vulnerabilities...")
        ...
        vulnerabilities = regscale_models.Vulnerability.batch_create(
            items=self.findings, progress_context=self.finding_progress
        )
        if vulnerabilities:
            logger.info("Created total of %d vulnerabilities.", len(vulnerabilities))
        issues = regscale_models.Issue.batch_create(items=self.issues, progress_context=self.finding_progress)
        if issues:
            logger.info("Created total of %d issues.", len(issues))
        ...
        if self.issues_updated:
            ...
            logger.info("Updated total of %d issues.", len(issues))
    ...
    def close_outdated_vulnerabilities(self, current_vulnerabilities: Dict[int, Set[int]]) -> None:
        ...
        """
        closed_count = 0
        all_vulnerabilities: List[regscale_models.Vulnerability] = []
        all_vulnerabilities_ids: Set[int] = set()
        for asset_id, vulnerability_ids in current_vulnerabilities.items():
            all_vulnerabilities = regscale_models.Vulnerability.get_all_by_parent(
                parent_id=self.plan_id, parent_module=regscale_models.SecurityPlan.get_module_string()
            )
            all_vulnerabilities_ids.update(vulnerability_ids)

        for vuln in all_vulnerabilities:
            if vuln.status != regscale_models.VulnerabilityStatus.Closed and vuln.id not in all_vulnerabilities_ids:
    ...
    def close_outdated_issues(self, current_vulnerabilities: Dict[int, Set[int]]) -> None:
        """
        Closes issues that are not associated with current vulnerabilities for each asset.

        This method performs the following steps:
        1. Retrieves all assets for the current security plan.
        2. For each asset, gets all associated issues.
        3. Checks each open issue against the current vulnerabilities.
        4. If an issue is no longer associated with any current vulnerabilities, it is closed.
        5. Ensures that the scanning tool matches before closing vulnerability mappings.
        ...
        # Retrieve all assets for the security plan
        all_assets = regscale_models.Asset.get_all_by_parent(
            parent_id=self.plan_id, parent_module=regscale_models.SecurityPlan.get_module_string()
        )

        for asset in all_assets:
            # Get all issues associated with the current asset
            asset_issues: List[regscale_models.Issue] = regscale_models.Issue.get_all_by_parent(
                parent_id=asset.id, parent_module=regscale_models.Asset.get_module_string()
            )

            # Get the set of current vulnerability IDs for this asset
            current_vuln_ids = current_vulnerabilities.get(asset.id, set())

            closed_count += self.process_asset_issues(asset_issues, current_vuln_ids)

        logger.info("Closed %d outdated issues.", closed_count)

    def process_asset_issues(self, asset_issues: List[regscale_models.Issue], current_vuln_ids: Set[int]) -> int:
        """
        Processes issues for an asset and closes outdated ones.

        :param List[regscale_models.Issue] asset_issues: List of issues for the asset
        :param Set[int] current_vuln_ids: Set of current vulnerability IDs for the asset
        ...
        for issue in asset_issues:
            ...
            if self.should_close_issue(issue, current_vuln_ids):
                issue.status = regscale_models.IssueStatus.Closed
    ...
    def should_close_issue(self, issue: regscale_models.Issue, current_vuln_ids: Set[int]) -> bool:
        """
        Determines if an issue should be closed based on vulnerability mappings.

        :param regscale_models.Issue issue: The issue to check
        :param Set[int] current_vuln_ids: Set of current vulnerability IDs
        :return: True if the issue should be closed, False otherwise
        :rtype: bool
        """
        if self.regscale_version < "5.64.0" and self.regscale_version != "localdev":
            return True

        for vuln_mapping in regscale_models.VulnerabilityMapping.find_by_issue(
            issue.id, status=regscale_models.VulnerabilityStatus.Open
        ):
            scan_history = regscale_models.ScanHistory.get_object(object_id=vuln_mapping.scanId)
            if issue.vulnerabilityId not in current_vuln_ids and scan_history.scanningTool == self.title:
                vuln_mapping.status = regscale_models.VulnerabilityStatus.Closed
                vuln_mapping.save()
            else:
                return False
        return True
    ...
    def sync_findings(cls, plan_id: int, **kwargs: dict) -> None:
        ...
        with instance.finding_progress:
            ...
            if cls.type == ScannerIntegrationType.CHECKLIST:
                instance.update_regscale_checklists(findings=findings)
            else:
                instance.update_regscale_findings_and_vulnerabilities(findings=findings)
            if instance.errors:
