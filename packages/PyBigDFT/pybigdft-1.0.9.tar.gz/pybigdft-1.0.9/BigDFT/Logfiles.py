...
def plot_wfn_convergence(wfn_it, gnrm_cv, label=None, ax=None):
    """
    ...
    if ax is None:
        ...
        ax = ax
    import numpy
    ax.semilogy(numpy.ravel(wfn_it), label=label, marker='o',
                linestyle='solid')
...
class Logfile:
    ...
    def to_json(self, outfile):
        ...
        if len(self) == 0:
            ...
            timestr = str(jsondict['Timestamp of this run'])
            jsondict['Timestamp of this run'] = timestr
        else:
            jsondict = []
            for lf in self:
                dt = lf.log.copy()
