...
import json
from typing import Optional, Union, Any, MutableMapping, TypeVar
...
class Instance:
    ...
    def getTransplant(self, donor: Donor, recipient: Recipient) -> Transplant:
        """Get a specific transplant between the given donor and recipient."""
        for t in self.transplants:
            if t.donor == donor and t.recipient == recipient:
                return t
        raise KEPDataValidationException(
            f"No transplant from {donor} to {recipient} found"
        )
    ...
    def writeFile(self, filename: str, file_format: OutputFormat) -> None:
        ...
        if file_format == OutputFormat.JSON:
            ...
            self.writeFileXml(filename)

    def writeFileXml(self, filename: str) -> None:
    ...
    def writeFileYaml(self, filename: str) -> None:
...
class DynamicInstance(Instance):
    ...
    def is_ill(self, participant: Donor | Recipient, period: int) -> bool:
        """Return True if the given participant is ill in the given period.

        :param participant: The participant in question.
        :param period: The period in question.
        :return: True if and only if the given participant is ill in the given period.
        """
        if isinstance(participant, Recipient):
            return period in self.recipient_temporary_departures[participant.id]
        return period in self.ndd_temporary_departures[participant.id]
