...
class PTR():
    ...
    def __init__(self, input_file, output_file, num_objectives, num_proposals, ptr_ranges, output_res,
        ...
        if self.hard == None: self.hard = False
    ...
    def calc_ai(self, t_train, X_all, train_actions, test_actions):
        ...
        if self.hard == False:
            ...
            for i in range(self.num_proposals):
                ...
                actions.append(test_actions[index[i]])
        ...
        if self.hard == True:
            ...
            for i in range(len(index)):
                if all([cdfs_all[index[i]][j] > 0.5 for j in range(len(cdfs_all[0]))]):
                    new_index.append(index[i])

            for i in range(len(index)):
                if len(new_index) < self.num_proposals and index[i] not in new_index:
                    new_index.append(index[i])
            ...
            for i in range(self.num_proposals):
                ...
                actions.append(test_actions[new_index[i]])
