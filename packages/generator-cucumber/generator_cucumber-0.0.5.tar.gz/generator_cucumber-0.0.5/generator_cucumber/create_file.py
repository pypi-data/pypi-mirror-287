...
def create_cucumber_file(**params):
    ...
    # проверяем наличие папки под cache feature
    if os.path.isdir("__featurecache__") != True:
        os.mkdir("__featurecache__")
    # проверяем наличие папки под хранения изменений feature
    if os.path.isdir("__featurecachechanges__") != True:
        os.mkdir("__featurecachechanges__")
    ...
    if os.path.isfile(f'{params["name_file"]}.feature'):
        ...
        if file_date == updated_at:
            print('Еріс НЕ менялся')
        else:
            print('Еріс именялся\n')

            # сравниваем локальный файл (txt) с еріс
            cache_file_read = open(f'__featurecache__/{params["name_file"]}.txt', 'r')
            ...
            if menu_entry_index == 0:
                ...
                print(f'Отмена обновлений-файла {params ["name_f1le"]}.feature')
    else:
        # формирование текста
        # create_cucumber_text(**params)
        # проверяем создан ли файл cache_changes
        if os.path.isfile(f'__featurecache__/{params["name_file"]}.txt'):
            print('\n) Такой уже есть', f'__featurecache__/{params["name_file"]}.txt')
        else:
