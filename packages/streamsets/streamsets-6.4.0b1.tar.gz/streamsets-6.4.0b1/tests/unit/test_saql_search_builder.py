...
@pytest.mark.parametrize(
    'saql_search_type',
    (
        SAQLSearch.PipelineType.PIPELINE,
        SAQLSearch.PipelineType.FRAGMENT,
        SAQLSearch.JobType.JOB_TEMPLATE,
        SAQLSearch.JobType.JOB_INSTANCE,
        SAQLSearch.JobType.JOB_DRAFT_RUN,
    ),
)
def test_add_filter_default_values(mocker, saql_search_json, saql_search_type):
    mock_control_hub = mocker.Mock()
