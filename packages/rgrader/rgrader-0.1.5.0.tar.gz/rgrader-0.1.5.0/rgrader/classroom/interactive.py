    ...
    def control(self, c):
        ...
        match c:
            case curses.KEY_UP if self.y > 0:
                self.y -= 1
            case curses.KEY_DOWN if self.y < self.visible_vertical_range:
                self.y += 1
            ...
            case curses.KEY_LEFT if self.x > 0:
                self.x -= 1
            case curses.KEY_DOWN if self.y == self.visible_vertical_range:
                self.start_y = min(self.start_y + 1, len(self.lines) - self.visible_vertical_range - 1)
            case curses.KEY_UP if self.y == 0:
            ...
            case 94:  # ^
                ...
                self.start_x = 0
            case 71:  # G
                self.start_y = max(0, len(self.lines) - self.visible_vertical_range)
                self.y = self.visible_vertical_range - 1
            case 103:  # g
                self.start_y = 0
                self.y = 0
            case 114:  # r
...
class SummaryPanel(Panel):
    ...
    def draw(self):
        ...
        for i, rule in enumerate(self.storage.grading_schema.rules):
            ...
            result_string = "No Data" if test_result is None else f"{test_result.gained_points}/{test_result.total_points}"
