...
def summary_cmd_handler(
    ...
    for event in events:
        ...
        event.output(args)
    logger.info(
        f"Found {len(events)} events ({len(open_events)} open, {len(closed_events)} closed) "
        f"with a closed total of {td(seconds=total_seconds).pretty_duration(units='h')} (double-counting overlaps), touching {len(touched_days)} days"
    )
