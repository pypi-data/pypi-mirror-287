...
def plot_bug_rate(start:datetime, end:datetime, issues:list[Issue], who:str,
    ...
    if as_jpg:
        if False:
            ...
            img_md = f"![](data:image/jpeg;base64,{img_base64})"
        else:
            ...
            plt.savefig('bugcount.png', format='png')
            img_md = f'![](bugcount.png)'
        return '', img_md
    else:
...
def create_report(org: str, repo: str, token: str, out: str|None=None, 
    ...
    if fmt == '.txt':
        ...
        if days == 7: # This is a real kludge, thanks to Github not supporting encoded images
                      # and not wanting to manage files. We reuse the same file name.
            script, chartdiv = plot_bug_rate(now-timedelta(days=xrange), now, issues,
                               repo, [bug_label], interval=1, as_jpg=(fmt!='.html'))
        else:
            script, chartdiv = '', ''
        if fmt == '.md':
            result = report + '\n\n' + termranks + '\n\n' + chartdiv
