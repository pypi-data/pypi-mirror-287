...
allowed_commands = ['setup', 'test', 'show_config']
...
dir_path = os.path.expanduser('~/.config/plotlyshare')
...
if command == 'setup':
	...
	''')
	time.sleep(0.5)
	base_url = c.input('[bold #FF84AC] DETA_APP_URL: [/bold #FF84AC]')
	if base_url.endswith('/'):
		base_url = base_url.rstrip('/')
	config['DETA_APP_URL'] = base_url
	...
	with c.status('Testing credentials...', spinner='shark'):
		...
		if passed:
			...
			c.print(f'Your config has been saved to {config_file_path}')
		else:
	...
	if old_config['setup_done']:
		...
		c.print('Please run [bold white on deep_pink3]python -m plotlyshare setup[/] first')
elif command == 'show_config':
	print(f'plotlyshare config saved at {config_file_path}:')
	pretty.pprint(old_config)