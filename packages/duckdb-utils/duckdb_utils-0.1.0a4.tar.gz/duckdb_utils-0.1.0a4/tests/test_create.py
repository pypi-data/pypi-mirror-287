...
def test_create_table(fresh_db):
    ...
    assert [
               ...
               {"name": "datetime_col", "type": "VARCHAR"},
           ] == [{"name": col.name, "type": col.type} for col in table.columns]
    assert 'CREATE TABLE test_table(text_col VARCHAR, float_col FLOAT, int_col INTEGER, bool_col INTEGER, bytes_col BLOB, datetime_col VARCHAR);' == table.schema
...
def test_create_table_compound_primary_key(fresh_db):
    ...
    assert (
               'CREATE TABLE test_table(id1 VARCHAR, id2 VARCHAR, "value" INTEGER, PRIMARY '
               'KEY(id1, id2));'
           ) == table.schema
...
@pytest.mark.parametrize("pk", ("id",))
def test_create_table_with_single_primary_key(fresh_db, pk):
    fresh_db.create_table(
        "foo", {"id": int}, pk=("id",)
    )
    #TODO: create_table should not be necessary here. That's a bug
    fresh_db["foo"].insert({"id": 1}, pk=pk)
    # print(fresh_db["foo"].schema)
    # assert (
...
def test_create_table_with_defaults(fresh_db):
    ...
    assert [{"name": "name", "type": "VARCHAR"}, {"name": "score", "type": "INTEGER"}] == [
        {"name": col.name, "type": col.type} for col in table.columns
    ]
    assert ('CREATE TABLE players("name" VARCHAR DEFAULT(\'bob\'\'\'\'bob\'), score '
            'INTEGER DEFAULT(1));') == table.schema
...
def test_create_table_with_not_null(fresh_db):
    ...
    assert ('CREATE TABLE players("name" VARCHAR NOT NULL, score INTEGER DEFAULT(3) NOT '
            'NULL);') == table.schema
...
#     )
#     assert 'CREATE TABLE test_table(id INTEGER PRIMARY KEY, "ref" INTEGER, );' == table.schema
#
#
