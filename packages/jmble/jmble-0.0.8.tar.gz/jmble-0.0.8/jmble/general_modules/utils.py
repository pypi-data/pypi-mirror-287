...
def _set_next_append(
    ...
    if path[0].isdigit() and options.create_lists:
        obj.insert(int(key), [])
    else:
        obj.insert(int(key), {})
    ...
    set_nested(obj[-1], path, value, options)
...
def _set_next_list_item(
    ...
    set_nested(obj[int(key)], path, value, options)
...
def _set_next_nested(
    ...
    set_nested(obj.get(key), path, value, options)
...
def set_nested(
    ...
    if len(path) == 1:
        ...
        if isinstance(obj, list) and key.isdigit():  # true
            ...
            _set_next_nested(obj, path, value, key, options)
