...
class SqlTextArea(TextArea):
    ...
    BINDINGS = [
        Binding(bindings.execute_query, "execute", "Execute"),
        Binding(bindings.open_autocomplete, "autocomplete", "Autocomplete"),
        Binding(bindings.format_query, "format", "Format"),
        Binding(bindings.format_all, "format_all", "Format all"),
        Binding(bindings.select_query, "select", "Select query"),
        Binding(bindings.copy_selection, "copy", "Copy selection"),
        Binding(bindings.close_autocomplete, "close", "Close Autocomplete", show=False),
    ]
    ...
    def on_key(self, event: events.Key):
        if self.is_autocomplete_open:
            if event.key == "enter":
                self.post_message(Autocomplete.Select())
