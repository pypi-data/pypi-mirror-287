...
class EditorPane(TabPane):
    BINDINGS = [
        Binding(bindings.select_database, "select_database", "Select database"),
        Binding(bindings.switch_layout, "switch_layout", "Switch layout"),
        Binding(bindings.save, "save", "Save"),
        Binding(bindings.close_tab, "close", "Close"),
    ]
    ...
    async def _prompt_save_path(self) -> Path | None:
        dialog = TextPromptDialog("File path", placeholder="foo.sql")
