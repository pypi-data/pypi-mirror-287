...
def compute_ewm_long_short_filter(data: pd.DataFrame,
    ...
    r_val = data.to_numpy()
    init_value = np.zeros(r_val.shape[1])
    if short_span is not None:  # use short + long filter
        ...
        long_signal = weight_long * ewm_recursion(a=r_val * np.sqrt(1 - long_lambda) / (1 - np.sqrt(long_lambda)),
                                                  ewm_lambda=np.sqrt(long_lambda),
                                                  init_value=init_value)
        short_signal = weight_short * ewm_recursion(a=r_val * np.sqrt(1 - short_lambda) / (1 - np.sqrt(short_lambda)),
                                                    ewm_lambda=np.sqrt(short_lambda),
                                                    init_value=init_value)
        filter = long_signal - short_signal

    else:
        filter = ewm_recursion(a=r_val * np.sqrt(1.0 - long_lambda) / (1.0 - np.sqrt(long_lambda)),
                               ewm_lambda=np.sqrt(long_lambda),
                               init_value=init_value)

    if warmup_period is not None:   # set to nan first nonnan in warmup_period
        if isinstance(warmup_period, float):
            warmup_period = np.full(r_val.shape[1], warmup_period)
            if np.any(warmup_period > 0):
                for p, idx in zip(warmup_period, range(filter.shape[1])):
                    ind = np.argwhere(np.isfinite(r_val[:, idx]))
                    if ind.size > p:
                        filter[:ind[p][0], idx] = np.nan
                    else:
                        filter[:, idx] = np.nan
