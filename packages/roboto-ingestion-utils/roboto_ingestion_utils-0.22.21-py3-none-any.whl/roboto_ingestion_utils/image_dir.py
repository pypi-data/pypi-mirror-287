...
def image_dir_to_mcap(input_dir: Path,
    ...
    fname = str(output_dir / f"{topic_name}_{input_dir.name}.mcap")
    ...
    if encoding == "protobuf":
        ...
        IMAGE_MSG_WRIST_IMAGE_LEFT = """
        uint8[] wrist_image_left
        """

        IMAGE_MSG_EXTERIOR_IMAGE_1_LEFT = """
        uint8[] exterior_image_1_left
        """

        IMAGE_MSG_EXTERIOR_IMAGE_2_LEFT = """
        uint8[] exterior_image_2_left
        """
        ...
        if "wrist_image_left" in input_dir.name:
            img_msg = IMAGE_MSG_WRIST_IMAGE_LEFT
        elif "exterior_image_1_left" in input_dir.name:
            img_msg = IMAGE_MSG_EXTERIOR_IMAGE_1_LEFT
        elif "exterior_image_2_left" in input_dir.name:
            img_msg = IMAGE_MSG_EXTERIOR_IMAGE_2_LEFT
        else:
            print(input_dir.name)
            print("Error: image directory name must contain one of wrist_image_left, exterior_image_1_left, or exterior_image_2_left")
            img_msg = IMAGE_MSG_WRIST_IMAGE_LEFT

        add_types.update(get_types_from_msg(img_msg, 'custom_msgs/msg/image_msg'))

        typestore = get_typestore(Stores.LATEST)
        typestore.register(add_types)

        ImageMsg = typestore.types['custom_msgs/msg/image_msg']

        msgtype = ImageMsg.__msgtype__
        msgdef, _ = typestore.generate_msgdef(msgtype) # returns tuple def, checksum
        print("aaa")
        with open(fname, "wb") as stream:
            writer = MCAPWriter(stream)
            writer.start()
            schema_id = writer.register_schema(name="custom_msgs/msg/image_msg",
            ...
            for i, (timestamp, image) in enumerate(zip(timestamps, sorted_img_files)):
                ...
                if "wrist_image_left" in input_dir.name:
                    message = ImageMsg(wrist_image_left=image_msg)
                elif "exterior_image_1_left" in input_dir.name:
                    message = ImageMsg(exterior_image_1_left=image_msg)
                elif "exterior_image_2_left" in input_dir.name:
                    message = ImageMsg(exterior_image_2_left=image_msg)
                else:
                    print(input_dir.name)
                    print(
                        "Error: image directory name must contain one of wrist_image_left, exterior_image_1_left, or exterior_image_2_left")
                    message = ImageMsg(wrist_image_left=image_msg)



                raw_data = typestore.serialize_cdr(message, 'custom_msgs/msg/image_msg')
