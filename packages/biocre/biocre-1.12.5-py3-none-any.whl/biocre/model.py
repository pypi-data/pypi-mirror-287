...
def meanz(p,
          log_p=False):
    #This function from metap package
    p = np.array(p)
    keep = (p > 0) & (p < 1)
    if np.sum(keep) < 2:
        print("Warning: Must have at least two valid p values")
        return {"z": np.nan, "p": np.nan, "validp": p[keep]}
    else:
        if np.sum(keep) != len(p):
            print("Warning: Some studies omitted")
        zvals = norm.isf(p[keep])
        zp = np.mean(zvals) / (np.std(zvals, ddof=1) / np.sqrt(len(p[keep])))
        if log_p:
            combined_p = norm.logsf(zp)
        else:
            combined_p = norm.sf(zp)
        return combined_p

def process_row(row):
    p_values = [row['P.adj_Gene'], row['P.adj_Peak']]
    result = meanz(p_values)
    return result
...
def calculate_pvalue(l,
    ...
    for l_chr in tqdm(l):
        ...
        melted_chr['P.adj_Gene'] = [gene_p_values[i] for i in gene_index]
        ...
        #melted_chr['Combine_p_value'] = Parallel(n_jobs=n_jobs)(
        #   delayed(process_row)(row) for index, row in melted_chr.iterrows())
