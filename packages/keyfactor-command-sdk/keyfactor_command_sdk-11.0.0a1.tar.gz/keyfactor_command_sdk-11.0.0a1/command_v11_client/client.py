...
class Client:
    ...
    _idp_args: Dict[str, Any] = field(factory=dict, kw_only=True)
    ...
    def get_httpx_client(self) -> httpx.Client:
        ...
        if len(self._idp_args) > 0:
            token_url = self._idp_args["tokenURL"]
            client_id = self._idp_args["clientId"]
            client_secret = self._idp_args["clientSecret"]
            data = {
            ...
            token_response = requests.post(token_url, data=data, headers=oauthheaders)
            token_data = token_response.json()
            access_token = token_data["access_token"]
            self._client.headers["Authorization"] = f"Bearer {access_token}"
        self._client.headers["X-Keyfactor-Requested-With"] = "APIClient"
        return self._client
