...
def corrected_meta(output: Selected | Extracted) -> Image.Meta:
  match output.tag:
    case 'selected':
      return Image.Meta(boxes=Image.Meta.GridCoords(coords=output.grid_coords, model=output.model))
    case 'extracted':
...
def image_metas(output: Selected | Extracted):
  ...
  corrected = ImgOutput(img=output.corrected, meta=corrected_meta(output))
