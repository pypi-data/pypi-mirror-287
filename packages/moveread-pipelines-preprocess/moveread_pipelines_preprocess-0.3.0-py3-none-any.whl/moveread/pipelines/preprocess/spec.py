...
class Preprocess(Workflow[Input, Any, Queues, Params, Artifacts, Pipelines]): # type: ignore
  ...
  def run(self, queues: Queues, params: Params) -> Artifacts:
    logger, images = params['logger'], params['images']
    app = api(
      corr_api=self.pipelines['correct'].run(queues['correct'], corr.Params(images=images, logger=logger.prefix('[CORRECT]'))),
    ...
    procs = {
      'extract': self.pipelines['extract'].run(queues['extract'], extr.Params(images=images, logger=logger.prefix('[EXTRACT]'))),
