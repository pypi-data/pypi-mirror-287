...
def capture_screen_with_cursor():
    ...
    if (image_width, image_height) != (logic_width, logic_height):
        resized_image = image.resize((logic_width, logic_height), Image.Resampling.LANCZOS)
...
def _create_axui_node(node, nodes: set = None, depth: int = 0, bbox: tuple = None):
    ...
    node_role_name = role.lower().replace(" ", "_").replace(",", "") if role else "unknown_role"
