        ...
        for installation_id, group in grouped:
            ...
            try:
                ...
                control_result_df = pandas.merge(group, control_result_df[[
                    'ObjectPropertyId', 'Value', 'ControlStatus']], left_on=column_with_object_property_id, right_on='ObjectPropertyId', how='left')

                new_order = ['ControlName', 'InstallationId',
                             'TimestampLocal', 'PivotKey', 'ControlStatus', 'InfoColumns']

                control_result_df['ControlStatus'].fillna(0, inplace=True)
                control_result_df['ControlStatus'] = control_result_df['ControlStatus'].astype(
                    int)
...
def __send_control(api_inputs: ApiInputs, installation_id: Union[uuid.UUID, str], df: pandas.DataFrame, has_priority: bool, session_id: uuid.UUID,
    ...
    if 'ContinueValue' in data_frame.columns:
        data_frame['IsExtendControl'] = (data_frame['ExistsInControlCache'] == True) & (
            data_frame['ContinueValue'] > -1)
    else:
        data_frame['IsExtendControl'] = False

    send_condition = ((data_frame['ExistsInControlCache'] == False) & (data_frame['Value'] > -1)) | \
        ((data_frame['ExistsInControlCache'] == True)
         & (data_frame['ContinueValue'] > -1))

    df_matrix = data_frame.copy()
    # Remove rows that were not going to send
    data_frame = data_frame[send_condition]

    df_matrix['ControlStatus'] = ControlStatus.NoControlRequired.value
    df_matrix.loc[(df_matrix['ExistsInControlCache'] == True) & (
        df_matrix['ContinueValue'] <= -1), 'ControlStatus'] = ControlStatus.NotSentToService.value

    if data_frame.empty:
        logger.info("No controls to be sent. Updating Cache.")
    ...
    # If Extend Control
    data_frame.loc[(data_frame['ExistsInControlCache'] == True) & (
        data_frame['ContinueValue'] > -1), 'Value'] = data_frame['ContinueValue']

    acknowledgement_df = data_frame.copy()

    if is_send_control:
        result_status, result_df = send_sensors_acknowledgement(
            api_inputs=api_inputs, df=acknowledgement_df, has_priority=has_priority, session_id=session_id, installation_id=installation_id)
        if result_df.empty:
            df_matrix['ControlStatus'] = ControlStatus.NotSentToService.value
            return df_matrix

    control_request_df = data_frame.copy()

    if default_control_value_column in control_request_df.columns:
        required_columns.append(default_control_value_column)

    if has_priority:
        required_columns.append('Priority')

    control_request_df.drop(control_request_df.columns.difference(
        required_columns), axis=1, inplace=True)

    control_request_df = control_request_df.rename(columns={'ObjectPropertyId': 'sensorId',
                                                            'Value': 'controlValue',
                                                            'TTL': 'duration',
                                                            default_control_value_column: 'defaultControlValue'})

    if has_priority:
        if not 'Priority' in control_request_df:
            logger.error(
                f"The dataframe does not have the column 'Priority'.")
        else:
            control_request_df = control_request_df.rename(
    ...
    if is_send_control:
        ...
        # We merge the results sensors to df_matrix to determine if the sensors has a IsExtendControl and ExistsInControlCache
        control_results = pandas.merge(control_results, df_matrix[[
            'ObjectPropertyId', 'IsExtendControl', 'ExistsInControlCache']], on='ObjectPropertyId', how='left')
    ...
    df_matrix['ControlStatus'] = control_results['ControlStatus'].fillna(
        df_matrix['ControlStatus']).astype(int)

    df_matrix.loc[(df_matrix['IsExtendControl'] == True) &
                  (df_matrix['ControlStatus'] == True), 'ControlStatus'] = ControlStatus.ControlResent.value
    df_matrix.loc[(df_matrix['ExistsInControlCache'] == True) &
                  (df_matrix['IsExtendControl'] == False), 'ControlStatus'] = ControlStatus.NotSentToService.value
    ...
    df_matrix.drop(
        columns=['IsExtendControl', 'ExistsInControlCache'], inplace=True)
    ...
    if has_priority and 'priority' in df_matrix:
        df_matrix = df_matrix.rename(columns={'priority': 'Priority'})
...
def add_control_component(name: str, column_with_object_installation_id: str, column_with_object_property_id: str, column_with_label: str, column_with_value: str, column_with_continue: str = None, column_with_default_value: str = None, timeoutMinutes: int = 0, priority: int = 8):
    ...
    ----------
        ...
        timeoutMinutes (int, optional):
            Time until relinquish happens. Defaults to 0.
...
def send_sensors_acknowledgement(api_inputs: ApiInputs, df: pandas.DataFrame, has_priority: bool, session_id: uuid.UUID, installation_id: uuid.UUID):

    control_columns_required = ['ObjectPropertyId', 'Value', 'TTL', 'Priority']
    acknowledgement_df = df.copy()
    acknowledgement_df.drop(acknowledgement_df.columns.difference(
        control_columns_required), axis=1, inplace=True)

    # We convert these columns to the required payload property names
    acknowledgement_df = acknowledgement_df.rename(columns={'ObjectPropertyId': 'id',
                                                            'Value': 'v', 'TTL': 'dsecs'})

    if has_priority:
        if not 'Priority' in acknowledgement_df:
            logger.error(
                f"has_priority is set to True, but the dataframe does not have the column 'Priority'.")
            return 'Missing Priority column', pandas.DataFrame()
        else:
            acknowledgement_df = acknowledgement_df.rename(
                columns={'Priority': 'p'})

    json_payload = {
        "sensors": acknowledgement_df.to_dict(orient='records'),
        "email": api_inputs.email_address,
        "userid": api_inputs.user_id,
        "sessionId": str(session_id)
    }
...
def update_control_cache(api_inputs: ApiInputs, control_cache_key: str, control_cache_df: pandas.DataFrame, control_results_df: pandas.DataFrame):
    ...
    control_sensors_for_relinquish = df[
        df['ControlStatus'] == ControlStatus.NotSentToService.value]
    sensors_to_relinquish = control_sensors_for_relinquish['ObjectPropertyId']

    control_sensors_sent = df[
        (df['ControlStatus'] == ControlStatus.ControlSuccessful.value) |
        (df['ControlStatus'] == ControlStatus.ControlResent.value)]

    # Filter out the entries that are already in control_cache_df with the ones in control_sensors_sent that was sent
    if not control_sensors_sent.empty:
        new_entries = control_sensors_sent[~control_sensors_sent['ObjectPropertyId'].isin(
    ...
    if not sensors_to_relinquish.empty:
        control_cache_df = control_cache_df[~control_cache_df['ObjectPropertyId'].isin(
            sensors_to_relinquish)]

    df_cache_cleaned = control_cache_df[control_cache_df['ObjectPropertyId'].notna() & (
        control_cache_df['ObjectPropertyId'] != '')]
