...
def upload(path: Annotated[str, typer.Option(help="folder or file path")]):
	...
	if project is None:
		console.print("No project selected")

	path = os.path.expanduser(path)
	
	console.print(f"[green]Uplaod to project: [underline]{project.get('title')}({project.get('name')})[/green][/underline]")
	...
	if os.path.isdir(path):
		...
		docs = glob.glob(os.path.join(path, '*.docx'))
		files.extend(docs)
	else:
		files.append(path)
