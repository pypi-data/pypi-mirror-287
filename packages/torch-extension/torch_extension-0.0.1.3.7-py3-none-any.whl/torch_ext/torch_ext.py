...
def apply_from_dim(func: Callable, tensor: torch.Tensor, dim = 0, otypes: Iterable[_dtype] = None):
    '''
    Select a dim of an `torch.Tensor` and apply a function.
    '''
    slices = (slice(None, None, None),) * dim
    if len(otypes) > 1:
        return tuple(torch.as_tensor(item, dtype=otypes[i], device=tensor.device) for i, item in enumerate(zip(*[func(tensor[slices + (i,)]) for i in range(tensor.shape[dim])])))
    elif len(otypes) == 1:
        return torch.as_tensor([func(tensor[slices + (i,)]) for i in range(tensor.shape[dim])], otypes[0], device=tensor.device)
    else:
        [func(tensor[slices + (i,)]) for i in range(tensor.shape[dim])]

def min_dims(tensor: torch.Tensor, dims: Sequence[int], keepdim=False, *, out: Union[torch.Tensor, Tuple[torch.Tensor, ...], Sequence[torch.Tensor], None] = None):
    from .private import _min_max_dims
    return _min_max_dims(torch.min, tensor, dims, keepdim, out=out)

def max_dims(tensor: torch.Tensor, dims: Sequence[int], keepdim=False, *, out: Union[torch.Tensor, Tuple[torch.Tensor, ...], Sequence[torch.Tensor], None] = None):
    from .private import _min_max_dims
    return _min_max_dims(torch.max, tensor, dims, keepdim, out=out)
...
def map_range(
    ...
    min_value: torch.Tensor = min_dims(tensor, dims=dim, keepdim=True)
    max_value: torch.Tensor = max_dims(tensor, dims=dim, keepdim=True)
    ...
    tensor = (tensor - min_value).masked_fill(max_min_equal_mask, torch.tensor(scalar_default_value(scalar_default, eps)).to(tensor.dtype))
    return (tensor / max_min_difference * (interval[1] - interval[0]) + interval[0]).to(dtype)
...
def map_ranges(
    ...
    max_min_equal_mask = max_min_difference == 0
    max_min_difference.masked_fill_(max_min_equal_mask, 1)
    normed = (tensor - min_value) / max_min_difference
    results = np.empty(len(intervals), dtype=torch.Tensor)
    for i, interval in enumerate(intervals):
        normed = normed.masked_fill(max_min_equal_mask, torch.tensor(scalar_default_value(scalar_default, eps)).to(tensor.dtype))
        results[i] = (normed * (interval[1] - interval[0]) + interval[0]).to(dtype)
    return results

def gamma(input: torch.Tensor, *, out: torch.Tensor | None = None):
    return torch.exp(torch.lgamma(input, out=out), out=out)

def gamma_div(left: torch.Tensor, right: torch.Tensor, *, out: torch.Tensor | None = None):
...
def arith_gamma_prod(arith_term: torch.Tensor, arith_base: torch.Tensor, ratio_base: torch.Tensor):
...
def linspace(start, stop, num, dtype: Optional[_dtype] = None):
...
def linspace_at(index, start, stop, num, dtype: Optional[_dtype] = None):
    start = torch.as_tensor(start)
    stop = torch.as_tensor(stop)
    num = torch.as_tensor(num)
    common_difference = torch.as_tensor((stop - start) / (num - 1).to(dtype=dtype))
...
def linspace_cover(index, start, stop, num, dtype: Optional[_dtype] = None):
    start = torch.as_tensor(start)
    stop = torch.as_tensor(stop)
    num = torch.as_tensor(num)
...
def linspace_cumprod_at(index, start, stop, num, dtype: Optional[_dtype] = None):
    start = torch.as_tensor(start)
    stop = torch.as_tensor(stop)
    num = torch.as_tensor(num)
    common_difference = (stop - start) / (num - 1).to(dtype=dtype)
    ...
    torch.pow(start[zero_common_difference_mask].view(left_required_shape), n, out=result[combine_mask_index(result_index_prefix, zero_common_difference_mask)])
...
def linspace_cumprod_cover(index, start, stop, num, dtype: Optional[_dtype] = None):
    start = torch.as_tensor(start)
    stop = torch.as_tensor(stop)
    num = torch.as_tensor(num)
    common_difference = (stop - start) / (num - 1).to(dtype=dtype)
...
def invert(tensor: torch.Tensor):
    shape_length = len(tensor.shape)
    if shape_length > 2:
        dims = (shape_length - 2, shape_length - 1)
    else:
        dims = (shape_length - 1,)
    min_values = min_dims(tensor, dims=dims, keepdim=True)
    max_values = max_dims(tensor, dims=dims, keepdim=True)
    return max_values - tensor + min_values
