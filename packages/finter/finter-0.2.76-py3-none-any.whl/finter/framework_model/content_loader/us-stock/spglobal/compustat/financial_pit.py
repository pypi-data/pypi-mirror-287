...
def safe_apply_fiscal(x):
    if pd.isna(x):
        return x
    return max(int(k) for k in x.keys())


def safe_apply_value(x):
    if pd.isna(x):
        return x
    return x[str(max(int(k) for k in x.keys()))]
    ...
    def get_df(
        ...
        **kwargs
        ...
        if mode == "unpivot":
            ...
            trading_dates = sorted(iter_trading_days(start, end, "us"))
            max_fiscal = raw.applymap(safe_apply_fiscal).reindex(trading_dates)
            raw = raw.applymap(safe_apply_value).reindex(trading_dates)
            return raw[max_fiscal == max_fiscal.cummax()]
