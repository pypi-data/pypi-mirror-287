...
def decode_all_frames(file_path: str, framerate: int = 10, use_gpu: bool = True, log_progress: bool = False) -> dict:
    ...
    if log_progress:
        ...
        pbar.set_description('Reading log ')
    process = subprocess.Popen(
        [
            f'ffmpeg {"-hwaccel cuda" if use_gpu else ""} -i "{file_path}" -filter_complex [0]fps=fps={framerate}[s0] \
    -map [s0] -an -f null - -v debug 2>&1 | grep Parsed_fps'
        ],
