...
class SQLDataStore:
    ...
    def get_or_create_request_metadata_in_store(
        ...
        stored_metadata: Optional[RequestMetadataEntry] = session.execute(stmt).scalar_one_or_none()
        if stored_metadata is not None:
            return stored_metadata

        metadata_entry = RequestMetadataEntry(
    ...
    def get_execute_action_metadata(self, job: JobEntry) -> ExecutedActionMetadata:
        worker_name = (job.active_leases[0].worker_name if len(job.active_leases) > 0 else "") or ""
