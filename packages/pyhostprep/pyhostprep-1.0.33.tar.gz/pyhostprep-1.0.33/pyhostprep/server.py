...
class CouchbaseServer(object):
    ...
    def cluster_ca_load(self):
        FileManager().make_dir(self.ca_path)
        key_file = os.path.join(self.ca_path, "ca-cert.key")
        cert_file = os.path.join(self.ca_path, "ca-cert.pem")
        ...
        try:
            ...
            raise ClusterSetupError(f"CA load failed: {err}")
