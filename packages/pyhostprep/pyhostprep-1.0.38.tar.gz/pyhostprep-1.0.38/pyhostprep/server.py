...
class CouchbaseServer(object):
    ...
    def cluster_ca_load(self):
        ca_key = os.path.join(Path.home(), "ca.key")
        ca_cert = os.path.join(Path.home(), "ca.pem")
    ...
    def host_cert_load(self):
        ...
        if os.path.exists(ca_key) and os.path.exists(ca_cert):
            ...
            node_key, node_cert = CertMgr.certificate_standard(ca_cert_pem, ca_key_pem, alt_name=name_alt, alt_ip_list=ip_alt)
            ...
            try:
                api.api_post("/node/controller/reloadCertificate", "")
            except Exception as e:
                logger.error(f"Failed to load node cert: {e}")
                raise ClusterSetupError(f"Node cert load failed")
