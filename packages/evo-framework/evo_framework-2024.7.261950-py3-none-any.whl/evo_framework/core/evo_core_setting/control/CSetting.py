    ...
    def doInit(self):
        try:
            ...
            if IuText.StringEmpty(settingsEnv):
                IuLog.doWarning(__name__,"CYBORGAI_SETTINGS_ENV_EMPTY copy in .env first start")
            else:
                mapSettingsTmp =  IuSettings.doDecryptSettings(settingsEnv)
                
                if mapSettingsTmp is None:
                    IuLog.doError(__name__, "ERROR_DECRYPT|CYBORGAI_SETTINGS_ENV")
                    sys.exit(1)
                ...
                if IuText.StringEmpty(passwordEnv):
                    IuLog.doError(__name__, "ERROR_REQUIRED|CYBORGAI_PASSWORD_ENV")
                    sys.exit(1)
                
                if len(passwordEnv) <16:
                    IuLog.doError(__name__, "ERROR_PASSWORD|PASSWORD_LENGTH<16_ENV")
                    sys.exit(1)
