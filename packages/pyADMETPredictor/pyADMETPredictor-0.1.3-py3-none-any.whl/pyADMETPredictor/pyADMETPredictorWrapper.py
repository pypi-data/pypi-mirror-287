    ...
    def execute(self) -> dict:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if output_file.suffix == ".sdf":
                ...
                pass
                with open(output_file.as_posix()) as file:
                    lines = [line.replace('"', '').replace('<NONE>', '') for line in file]
                
                df = pd.read_csv(StringIO('\n'.join(lines)), sep="\t")
                mols = [Chem.MolFromSmiles(item) for item in df["SMILES"].tolist()]
                properties = df.columns
                records = df.to_records(index=False)
                for m, r in zip(mols, records):
                    if m is not None:
                        for idx, p in enumerate(properties):
                            m.SetProp(p, str(r[idx]))
            ...
            ret_content = {'mols': mols, 'log_content': log_content}
