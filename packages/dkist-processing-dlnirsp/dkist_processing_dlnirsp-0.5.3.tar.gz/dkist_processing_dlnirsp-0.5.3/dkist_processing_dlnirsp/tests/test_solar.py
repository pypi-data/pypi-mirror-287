...
def solar_task_for_basic_corrections(
    ...
    with SolarCalibration(
        ...
        assign_input_dataset_doc_to_task(
            task,
            DlnirspTestingParameters(dlnirsp_group_id_file_jband=jband_group_id_file_parameter),
        )
...
def test_compute_average_gain(solar_task_for_basic_corrections, solar_signal):
    """
...
def test_compute_characteristic_spectra(solar_task_with_full_corr, groups_by_slitbeam):
    ...
    # but their median is equal to the slitbeam number. Thus, after computing the characteristic spectra, all groups
    # in the slitbeam should be equal to the slitbeam number.
    ...
    for slitbeam, groups_in_slit in groups_by_slitbeam.items():
        for g in groups_in_slit:
            ...
            padded_group = char_spec[idx]
            np.testing.assert_equal(
                padded_group[~np.isnan(padded_group)], slitbeam / slitbeam_median
            )


def test_solar_task_completes(solar_task_for_basic_corrections, mocker):
    """
