...
from dkist_processing_dlnirsp.tests.conftest import tag_obs_on_mosaic_and_modstate
...
def science_task_with_data(
    ...
    with ScienceCalibration(
        ...
        yield task, science_obs_time, num_modstates, num_X_tiles, num_Y_tiles, num_mosaic
...
@pytest.fixture
def dummy_ifu_spatial_shape(dummy_slit_shape) -> tuple[int, int]:
    return (10, 15)


@pytest.fixture
def dummy_ifu_pos(dummy_slit_shape, dummy_ifu_spatial_shape) -> tuple[np.ndarray, np.ndarray]:
    x_pos = (
        np.zeros(dummy_slit_shape, dtype=np.float64)
...
def test_science_task_completes(
    ...
    num_observe_frames = write_observe_frames_to_task(
        ...
        tag_func=tag_obs_on_mosaic_and_modstate,
    )
...
def test_ifu_remapping(
    ...
    expected_shape = (dummy_slit_shape[1], *dummy_ifu_spatial_shape, 1)  # 1 from stokes axis
    assert remapped_data.shape == expected_shape
    for x_pos in range(dummy_ifu_spatial_shape[0]):
        for y_pos in range(dummy_ifu_spatial_shape[1]):
