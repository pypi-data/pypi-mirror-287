...
// FrozenDict // TODO: NOT IMPLEMENTED YET
#define FDICT_E 45
#define FDICT_1 46
#define FDICT_2 47
#define FDICT_D 48
...
// DateTime
#define DATETIME_DT 49 // DT for DateTime objects
#define DATETIME_TD 50 // TD for TimeDelta objects // TODO: NOT IMPLEMENTED YET
#define DATETIME_D  51 // D  for Date objects
#define DATETIME_T  52 // T  for Time objects
...
{
    // Check whether it's a frozenset
    if (is_frozenset == 1)
    {
        // Convert it to a regular set to get the size and items
        PyObject *set = PySet_New(value);
        Py_DECREF(value);
        value = set;
    }

    // The number of items in the set
    Py_ssize_t num_items = PySet_Size(value);

    // This will be stacked onto by the item bytes
    PyObject *bytes;
    if (is_frozenset == 1)
    {
        bytes = list_type_metadata(num_items, FSET_E, FSET_1, FSET_2, FSET_D);
    }
    else
    {
        bytes = list_type_metadata(num_items, SET_E, SET_1, SET_2, SET_D);
    }
