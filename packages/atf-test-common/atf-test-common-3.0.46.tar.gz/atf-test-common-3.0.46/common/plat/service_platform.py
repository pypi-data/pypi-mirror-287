...
class ServicePlatForm(object):
    ...
    @classmethod
    def getATFContant(self, projectAlice):
        try:
            header = {'token': projectAlice}
            response = APIDriver.http_request(url=Constant.ATF_PYTHON_URL + f'/get/constant',header=header,
                                              method='get',
                                              )
            content = json.loads(response.content)
            return content
    ...
    def getProjectDataSingle(self, projectName, projectKey):
        ...
        try:
            content = APIDriver.http_request(url=f"{Constant.ATF_URL_API}/getProjectData/{projectName}/{projectKey}",
            method='get')
    ...
    def getProjectData(self, projectName, projectKey):
        ...
        for i in range(1, Constant.RETRY_TIME):
            ...
            if type(_result) != None:
                ...
                time.sleep(1)
            if i == Constant.RETRY_TIME:
                logger.info(f"{Constant.ATF_URL_API}/getProjectData/{projectName}/{projectKey}" + "获取不到value")
                _result = None
        return _result
    ...
    def setProjectData(self, projectKey, value, type=1):
        ...
        if get_system_key('ProjectAlice') is not None and get_system_key('ProjectAlice').strip() != '':
            ...
            return APIDriver.http_request(
                url=f"{Constant.ATF_URL_API}/setProjectData/{projectName}/{projectKey}/{value}/{type}",
                method='get'
            )
    ...
    def getRedisKey(self, key):
        ...
        try:
            ...
            logger.info(f"{Constant.ATF_PYTHON_URL}/redis/get/key获取不到{key}")
    ...
    def runDeploy(self, jobName, _pramater):
        ...
        :return:
        """
        _tempdata = APIDriver.http_request(url=f"{Constant.ATF_URL_API}/jenkins/runDeploy/{jobName}",
