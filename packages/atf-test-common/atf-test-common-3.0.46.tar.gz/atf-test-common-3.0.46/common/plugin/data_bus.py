...
class DataBus(object):
    ...
    def get_Alice(cls):
        ...
        :return:
        """
        return get_system_key(Constant.PROJECT_ALICE)
    ...
    def get_data_atf(cls):
        ...
        _info = ServicePlatForm.getATFContant(projectAlice)
        for key in _info:
            if DataProcess.isNotNull(get_system_key(Constant.RUN_TYPE)):
                cls.set_key(key, _info[key].strip())
            else:
                cls.set_key(key, _info[key].strip(), False)
...
if __name__ == '__main__':
    os.environ[Constant.PROJECT_ALICE]='M64'
