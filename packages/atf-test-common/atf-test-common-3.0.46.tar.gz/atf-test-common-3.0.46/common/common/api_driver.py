...
class APIDriver(object):
    ...
    def http_request(cls, url, method, parametric_key=None, header=None, data=None, file=None, cookie=None, _auth=None,
        ...
        if parametric_key is None:
            header,header_desc = cls.getCommontHeader(header,url)
    ...
    @classmethod
    def getCommontHeader(self, header, url:str):
        if header is None:
        ...
        if url.find(Constant.ATF_PYTHON_URL) >= 0 and url.find('get/constant') < 0:
            from common.plugin.data_bus import DataBus
            _alice = DataBus.get_Alice()
            DataBus.get_data_atf()
            token = DataBus.get_key('PYTHON_JWT').strip()+'@' + _alice.strip()
            header['Authorization'] = f'Token {token}'
        if url.find(Constant.ATF_API) >=0:
            from common.plugin.data_bus import DataBus
            _alice = DataBus.get_Alice()
            token = DataBus.get_key('PYTHON_JWT').strip() +'@' + _alice.strip()
            header['Authorization'] = f'Token {token}'
