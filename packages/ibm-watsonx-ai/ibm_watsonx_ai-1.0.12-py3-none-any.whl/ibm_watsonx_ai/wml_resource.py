...
class WMLResource:
    ...
    def _handle_response(
        ...
        if response.status_code == expected_status_code:
            ...
            if json_response:
                try:
                    ...
                    raise WMLClientError(
                        "Failure during parsing json response: '{}'".format(
                            response.text
                        ),
                        e,
                    )
            ...
            raise ApiRequestFailure(
                "Failure during {}.".format(operationName), response
            )
    ...
    def _check_if_lib_or_def(self, lib_id: str) -> str | None:
        lib_details = requests.get(
            self._client.service_instance._href_definitions.get_custom_library_href(
                lib_id
            ),
            headers=self._client._get_headers(),
        )
        ...
        if def_details.status_code == 200:
            ...
            raise WMLClientError(
                "The LIB_ID provided is not a valid model_definition/library"
            )
