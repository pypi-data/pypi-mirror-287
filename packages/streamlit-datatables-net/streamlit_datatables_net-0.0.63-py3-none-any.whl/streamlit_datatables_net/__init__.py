...
if not _RELEASE:
    ...
    with content_column:
        with st.container(border=True):
            ...
            dt_click = st_datatable(None, props=props, column_order=column_order, ajax_setup=ajax_setup, ajax_reload=ajax_reload,
                                    key="ajax_table", on_select="rerun", enable_diagnostics=True)
            st.write(dt_click)
        ...
        with st.container(border=True):
            ...
            with st.expander("Code"):
                with st.echo():
                    ...
                    with dt_container:
                        dt_response = st_datatable(
                            data, key="example_2", date_format='%Y-%b-%d', on_select="ignore", boolean_to_string=True, enable_diagnostics=True)
                        st.write(dt_response)
