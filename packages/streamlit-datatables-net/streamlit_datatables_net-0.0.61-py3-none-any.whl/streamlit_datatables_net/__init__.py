...
def st_datatable(data: pd.DataFrame | list[dict] = None, column_order: List[str] = None, props: Dict = None, dt_config: str = "default", class_name: str = None, js_scripts: List[str] = None, stylesheets: List[str] = None, disabled: bool = False, enable_top_navigation: bool = False, date_format: str = None, is_datetime: bool = False, export_file_name: str = None, on_select: Optional[Union[Literal["ignore"], Literal["rerun"], Callable]] = "rerun", on_change: Optional[Callable] = None, on_click: Optional[Callable] = None, ajax_setup: Dict = None, enable_diagnostics: bool = False, args: Optional[Tuple[Any, ...]] = None,
    ...
    disabled:
        An optional boolean, which disables the component and sets the component value to an empty dict if set to True. The default is False.
    ...
    if "ajax" in props:
        if "columns" not in props:
            st.warning(
                "Either columns or column_order must be included in props for Ajax data sources.")
            return {}
    ...
    if not key and props.get("stateSave"):
        print("st_datatable error: key is required when stateSave is enabled")
        return

    component_value = _component_func(
        props=props, class_name=class_name, js_scripts=js_scripts, stylesheets=stylesheets, key=key, table_id=table_id, on_change=on_change, disabled=disabled, date_format=luxon_date_format, export_file_name=export_file_name, ajax_setup=ajax_setup, enable_diagnostics=enable_diagnostics, default=None)
...
if not _RELEASE:
    ...
    with content_column:
        with st.container(border=True):
            ...
            column_order = ["postId", "id", "name", "email", "body"]
            ajax_setup = {
                "headers": {
                    'Content-type': "application/x-www-form-urlencoded",
                    'X-CSRFToken': "csrf_token"
            ...
            st_datatable(None, props=props, column_order=column_order, ajax_setup=ajax_setup,
                         key="ajax_table", enable_diagnostics=False)
        ...
        with st.container(border=True):
            ...
            with st.expander("Code"):
                with st.echo():
                    ...
                    props["scrollX"] = True
                    props["stateSave"] = True
                    props["stateDuration"] = -1
                    props["select"] = {"style": "single", "info": True}
                    props["layout"] = {
                    ...
                    with dt_container:
                        ...
                        dt_response = st_datatable(
                            big_data, column_order=column_order, props=props, key="example_1", dt_config="default", date_format="%m/%d/%Y", is_datetime=True, enable_top_navigation=False, export_file_name="Example 1", on_select=on_select, enable_diagnostics=False)
                ...
                with st.echo():
                    ...
                    columns = [
                        ...
                        {"data": "salary", "title": "Salary",
                            "render": ['number', ',', '.', 0, '$']},
                        {"data": "", "title": "Actions",
                            "column_type": "actions", "actions": ["view", "edit", "delete", "pdf", "download"]},
                    ...
                    with dt_container:
                        ...
                        if on_select == "callable":
                            on_select = my_on_click_function
                        dt_response = st_datatable(
                            data, props=props, key="example_1b", enable_top_navigation=False, date_format="%b %d %Y", is_datetime=True, on_select=on_select, enable_diagnostics=False)
                        st.write(dt_response)
        ...
        with st.container(border=True):
            ...
            with st.expander("Code"):
                with st.echo():
                    ...
                    with dt_container:
                        dt_response = st_datatable(
                            data, key="example_2", date_format='%Y-%b-%d', on_select="ignore")
                        st.write(dt_response)
