...
def get_render_function(file_path, function_name):
    with open(file_path, 'r') as file:
        ...
        if body_start_idx == 0:
            raise ValueError(
                f"Function body for {function_name} not found in {file_path}")
        ...
        if open_braces != 0:
            raise ValueError(
                f"Unmatched braces in function {function_name} in {file_path}")
