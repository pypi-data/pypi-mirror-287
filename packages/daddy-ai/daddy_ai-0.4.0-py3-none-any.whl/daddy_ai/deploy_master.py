...
[Service]
ExecStart=/usr/bin/sudo /usr/bin/python3 -c 'from daddy_ai.deploy_master import DeployMaster; setup = DeployMaster(); setup.deploy("{command}", "{script_path}", {time_slots})'
Restart=always
User=root
...
"""
    ...
    def _run_with_sudo(self, service_content, service_file_path, password):
        ...
        cmd = f"echo '{password}' | sudo -S bash -c 'echo \"{service_content_escaped}\" > {service_file_path}'"
        subprocess.run(cmd, shell=True, check=True)
        subprocess.run(f"echo '{password}' | sudo -S systemctl daemon-reload", shell=True, check=True)
        subprocess.run(f"echo '{password}' | sudo -S systemctl enable daddy-ai-deploymaster.service", shell=True, check=True)
        subprocess.run(f"echo '{password}' | sudo -S systemctl start daddy-ai-deploymaster.service", shell=True, check=True)
