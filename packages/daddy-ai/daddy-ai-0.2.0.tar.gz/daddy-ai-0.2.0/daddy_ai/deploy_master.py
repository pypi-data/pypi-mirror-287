...
class DeployMaster:
    ...
    def run_command(self, command, script_path):
        full_command = f"python {os.path.join(script_path, command)}"
    ...
    def _setup_reboot(self, command, script_path, time_slots):
        ...
        service_file_path = '/etc/systemd/system/deploymaster.service'
        try:
    ...
    def _run_with_sudo(self, service_content, service_file_path, password):
        ...
        subprocess.run(f"echo '{password}' | sudo -S systemctl enable deploymaster.service", shell=True)
        subprocess.run(f"echo '{password}' | sudo -S systemctl start deploymaster.service", shell=True)
