...
class Timescale:  # pylint: disable=R0902
        ...
        try:
            with self.dbcursor() as cur:
                psycopg2.extras.execute_values(
                    cur,
                    """INSERT INTO request(time,run_id,greenlet_id,loadgen,name,request_type,response_time,success,testplan,response_length,exception,pid,url,context, prod_name, prod_version, client_cpu_usage) VALUES %s""",
                    samples,
                    template="(%(time)s, %(run_id)s, %(greenlet_id)s, %(loadgen)s, %(name)s, %(request_type)s, %(response_time)s, %(success)s, %(testplan)s, %(response_length)s, %(exception)s, %(pid)s, %(url)s, %(context)s, %(prod_name)s, %(prod_version)s,  %(client_cpu_usage)s)",
                )
    ...
    def on_request(
        ...
        **kwargs,
        ...
        sample["client_cpu_usage"] = psutil.cpu_percent()/psutil.cpu_count()
