...
class VannaBase(ABC):
    ...
    def generate_sql(self, question: str,general_input: str, allow_llm_to_see_data=False, **kwargs):
        """
        Generate a SQL query that answers a question using the LLM.
        ...
        print(f"Allow LLM to see data: {allow_llm_to_see_data}")
        ...
        question_sql_list = [item for item in question_sql_list if item is not None]

        print(f"Similar Question SQL List: {question_sql_list}")
        #self.log(message=question_sql_list)
        ddl_list = self.get_related_ddl(question, **kwargs)
        print(f"DDL List: {ddl_list}")
        #self.log(message=ddl_list)
        doc_list = self.get_related_documentation(question, **kwargs)
        print(f"Documentation List: {doc_list}")
        #self.log(message=doc_list)
        prompt = self.get_sql_prompt(
            general_rules=general_input,
        ...
        length=0
        #print(f"Generated SQL Prompt: {prompt}")
        ...
        concatenated_content = ''.join([p['content'] for p in prompt if 'content' in p])
        if encoding.encode(concatenated_content) is not None:
            length = len(encoding.encode(concatenated_content))
        ...
        if 'intermediate_sql' in llm_response:
            ...
            if allow_llm_to_see_data:
                ...
                print(f"Extracted Intermediate SQL: {intermediate_sql}")
                try:
                    ...
                    #print(f"Final SQL Prompt: {prompt}")
        ...
        return length,self.extract_sql(llm_response)
    ...
    def generate_summary(self, question: str, df: pd.DataFrame, **kwargs) -> str:
        ...
        **Example:**
        ...
        message_log = [
            self.system_message(
                f"You are a data summarization assistant. The user asked the following question: '{question}'. make sure the response is in the same language as the question. \n\nBelow is a pandas DataFrame with the query results: \n{df.to_markdown()}\n\n"
            ),
            self.user_message(
                f"Please provide a brief summary of the data based on the question asked. Do not include any additional explanation or change the language."
            ),
        ]

        # examples = [
        #     {
        #         "question":"top 3 sold tracks",
        #         "summary":"The top 3 selling tracks are 'Tres-Zap' with 20 sales, 'Temp' with 19 sales and 'Latlux' with 19 sales."
        #     },
        # ]
        ...
        return summary,len(encoding.encode(''.join([p['content'] for p in message_log if 'content' in p])))
    ...
    def get_sql_prompt(
        ...
        **kwargs,
        ...
        ddl_list = [item for item in ddl_list if item is not None]           
        ...
        doc_list = [item for item in doc_list if item is not None]
        ...
        initial_prompt += (
            ...
            "6. If the query needs joining make sure to allias each table and refrence the correct feild if same feild appear in diffrent tables."
            "7. Later on their is plotly code generator that will be used for vizaulization, so in case you get a followup about ploting just reply with the same sql as provided before. \n"

        )
        initial_prompt += "=== General SQL Generation Rules \n" + general_rules
        ...
        question_pattern = re.compile(r'"question":\s*(.*?),\s*"sql":')
        sql_pattern = re.compile(r'"sql":\s*(.*)')


        for item in question_sql_list:
            questions = question_pattern.search(item).group(1) if question_pattern.search(item) else None
            sql = sql_pattern.search(item).group(1) if sql_pattern.search(item) else None
            
            if questions and sql:
                message_log.append(self.user_message(questions))
    ...
    def generate_plotly_code(
        ...
        return self._sanitize_plotly_code(self._extract_python_code(plotly_code)),len(encoding.encode(''.join([p['content'] for p in message_log if 'content' in p])))
