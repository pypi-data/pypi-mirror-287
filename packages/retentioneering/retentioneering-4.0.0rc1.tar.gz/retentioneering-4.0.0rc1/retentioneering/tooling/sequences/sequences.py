...
class Sequences:
    ...
    def fit(
        ...
        if groups:
            group_0 = list(groups[0]) if isinstance(groups[0], set) else groups[0]
            group_1 = list(groups[1]) if isinstance(groups[1], set) else groups[1]
        ...
        if self.groups:
            ...
            vec_data = vec_data[vec_data.index.isin(list(groups[0]) + list(groups[1]))]  # type: ignore
