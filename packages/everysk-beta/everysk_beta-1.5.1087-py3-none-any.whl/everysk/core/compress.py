...
def compress(obj: Any, protocol: str = 'zlib', serialize: str | None = 'pickle') -> bytes:
    """
    ...
    Args:
        obj (Any): The Python object to compress
        protocol (str): The serialization protocol to use. Default is 'zlib'. Options are 'zlib' and 'gzip'.
        serialize (str): The serialization format to use. Default is 'pickle'. Options are 'json', 'pickle' and None.
    ...
    result = obj
    if serialize:
        result = dumps(obj, protocol=serialize, use_undefined=True)
...
def decompress(data: bytes, protocol: str = 'zlib', serialize: str | None = 'pickle') -> Any:
    """
    ...
    if serialize:
        result = loads(result, protocol=serialize, use_undefined=True)

    return result

def zip_directory_to_str(path: str, path_name: str, ignore_files: list = None, ignore_roots: list = None) -> str:
    """
    This function takes a directory path as input and creates a zip file in memory.

    Args:
        path (str): The path to the directory to zip.
        path_name (str): The name of the root folder inside the zip file.
        ignore_files (list): A list of file names to ignore when zipping the directory.
        ignore_roots (list): A list of patterns to ignore when zipping the directory.
    ...
    with zipfile.ZipFile(memory_zip, 'w', zipfile.ZIP_DEFLATED) as zip_file:
        for root, _, files in os.walk(path):
            ...
            if any(fnmatch.fnmatch(root, pattern) for pattern in ignore_roots):
                continue
            for file in files:
