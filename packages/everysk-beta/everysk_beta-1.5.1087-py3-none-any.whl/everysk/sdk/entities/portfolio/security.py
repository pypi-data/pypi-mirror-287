...
from everysk.core.fields import StrField, FloatField, DateField, ListField
...
from everysk.sdk.entities.base import BaseSDK, _parser_out
...
class Security(BaseDict, BaseSDK):
    ...
    def to_legacy_dict(self, with_internals: bool = True) -> dict:
        """
        Convert the Security object to a dictionary representation.

        This method converts the Security object and its attributes to a dictionary representation.
        By default, it includes internal attributes such as 'display', 'comparable', and 'previous_quantity'.
        If 'with_internals' is set to False, these internal attributes are excluded from the output dictionary.

        Args:
            with_internals (bool, optional): Include internal attributes in the dictionary. Defaults to True.

        Returns:
            dict: A dictionary representation of the Security object.
        ...
        Example usage:
            ...
            >>> security.market_price = 150.0
            >>> security.name = 'Apple Inc.'
            >>> security_dict = security.to_legacy_dict()
            >>> security_dict
            {
                'symbol': 'AAPL',
                'quantity': 100,
        ...
        """
        dct: dict = _parser_out(self.to_dict(add_class_path=False), with_internals=with_internals)
        if not with_internals:
            dct.pop('display')
            dct.pop('comparable')
            dct.pop('previous_quantity')
