...
class Securities(EntityList):
    ...
    def to_legacy_dict(self, with_internals: bool = True) -> list[dict]:
        """
        ...
        Example usage:
            ...
            ...                  {'symbol': 'GOOGL', 'name': 'Alphabet Inc.', 'price': 2800.0}]
            >>> securities_collection = Securities(security_data)
            >>> securities_dict = securities_collection.to_legacy_dict()
            >>> len(securities_dict)
            2
        """
        return [security.to_legacy_dict(with_internals=with_internals) for security in self]
