...
class HXtexture(HXcomponent):
    def __init__(self, object, size:list[int]=[32, 32], color:list[int]=[25, 60, 80], path:str=None, render_layer:str="background"):
...
class HXanim(HXcomponent):
    def __init__(self, object, flip_speed:float, dimensions:list[int], loop:bool=False, loop_delay:bool=False, delay:int|float=4, scale:list[int]=[1,1], sheet_path:str=None, dir_path:str=None):
        super().__init__(object, [])
        self.nframe:int = 0
    ...
    def load_sheet(self, sheet_path:str, dimensions:list[int], scale:list[int]=[1,1]):
        ...
        frame_textures = [HXtexture(self.object, dimensions) for _ in frame_images]
