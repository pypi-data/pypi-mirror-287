...
class HXrenderer:
    ...
    def render(self, sgrid:HXsgrid, cursor:HXcursor, window:HXwindow, zoom:float, offset=pg.math.Vector2(0,0), show_colliders:bool=False, show_rects:bool=False, show_grid:bool=False, show_nodes:bool=False):
        ...
        def get_transform(object):
            if object.has_component(HXtransform):
                return object.get_component(HXtransform)
        
        try:
            sorted_layer = sorted(self.layer_array, key=lambda obj: get_transform(obj).get_rect().centery)
            sorted_objects.extend(sorted_layer)
        except(AttributeError): 
            # No Transform Found In Renderer For Y Sort!
            pass
        ...
        for object in sorted_objects:
            ...
            if object.has_component(HXtransform):
                ...
                if object.has_component(HXtexture):
                    ...
                    window.display.blit(flip_surface(tex.image, otransform.negx, False), draw_loc)
                
                elif object.has_component(HXanim):
                    anim = object.get_component(HXanim)
                    anim.rect = otransform.get_rect(offset)
                    window.display.blit(flip_surface(anim.image, otransform.negx, False), draw_loc)
