...
def get_api_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    health_check = args.health_check_script and {
        "type": "script",
...
def update_job(args: argparse.Namespace, api: ToolforgeClient) -> None:
    ...
    response = api.patch("/jobs/", json=payload)
    _display_messages(response["messages"])
...
def _delete_and_wait(api: ToolforgeClient, names: Set[str]):
    ...
    raise ToolforgeError(message="Timed out while waiting for old jobs to be deleted")
