...
from samgis_web.utilities.constants import (OUTPUT_CRS_STRING, DRIVER_RASTERIO_GTIFF, N_MAX_RETRIES, N_CONNECTION,
                                            N_WAIT, ZOOM_AUTO, BOOL_USE_CACHE)
from samgis_web.utilities.type_hints import tuple_ndarray_transform
...
def download_extent(w: float, s: float, e: float, n: float, zoom: int or str = zoom_auto_string,
    ...
    """
    try:
        from samgis_web import contextily_tile
        from samgis_web.io_package.coordinates_pixel_conversion import _from4326_to3857
...
def get_transform_raster(raster: ndarray, raster_bbox: TupleFloat) -> tuple_ndarray_transform:
    ...
    try:
        ...
        app_logger.debug(f"raster: type {type(raster)}, raster_ext:{type(raster_bbox)}, {raster_bbox}.")
        rgb = fromarray(raster.astype(np_uint8)).convert('RGB')
        np_rgb = np_array(rgb)
...
def reshape_as_image(arr: ndarray | ma.MaskedArray) -> ndarray:
    """
    Double swapping of image axis, needed in crop_raster().

    Args:
        arr: numpy raster array
