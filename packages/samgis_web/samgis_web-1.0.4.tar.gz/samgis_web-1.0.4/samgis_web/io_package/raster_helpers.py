...
def write_raster_png(arr, transform, prefix: str, suffix: str, folder_output_path="/tmp") -> tuple[Path, Path]:
    from pathlib import Path
    from rasterio.plot import reshape_as_raster

    output_filename = Path(folder_output_path) / f"{prefix}_{suffix}.png"
    msg = MSG_WRITE_TMP_ON_DISK + f"PNG, coords/prefix {prefix}, suffix {suffix}, shape:{arr.shape}, {len(arr.shape)}."
