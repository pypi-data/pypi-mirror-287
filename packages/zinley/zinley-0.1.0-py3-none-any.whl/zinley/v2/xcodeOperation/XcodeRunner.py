...
class XcodeRunner:
    def __init__(self, directory_path, api_key, endpoint, deployment_id, max_tokens):
        ...
        self.self_healing = SelfHealingAgent(os.path.join(directory_path), api_key, endpoint, deployment_id, max_tokens)
        self.bugExplainer = BugExplainer(os.path.join(directory_path), api_key, endpoint, deployment_id, max_tokens)
    ...
    async def run_xcode_project(self, basename, scheme, max_retries=10):
        ...
        while retries < max_retries:
            self.self_healing.clearConversationHistory()
            self.bugExplainer.clearConversationHistory()

            self.bugExplainer.initialSetup()
            self.self_healing.initialSetup()
            ...
            try:
                ...
                if e.returncode == 70:
                    print("Build failed with exit status 70. Cleaning and retrying...")
                    cleaned = False
                    retries += 1
                    continue
