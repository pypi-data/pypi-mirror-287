...
class BugExplainer:
    ...
    def initialSetup(self):
        prompt = (
            "You are a senior iOS developer working as a bug-fixing agent. Analyze the provided project context and current errors, and provide detailed steps to fix the project in a structured manner. "
    ...
    async def get_bugFixed_suggest_requests(self, bug_log_path, files, overview):
        ...
        filtered_lists = [item for item in files if item]
...
async def main():
    ...
    max_tokens = 1024
    bug_log_path = "path/to/bug_log.txt"
    files = ["file1.swift", "file2.swift"]
    overview = "Project overview text"

    bug_explainer = BugExplainer(directory_path, api_key, endpoint, deployment_id, max_tokens)
    bug_explainer.initialSetup()
    plan = await bug_explainer.get_bugFixed_suggest_requests(bug_log_path, files, overview)
    print(plan)
