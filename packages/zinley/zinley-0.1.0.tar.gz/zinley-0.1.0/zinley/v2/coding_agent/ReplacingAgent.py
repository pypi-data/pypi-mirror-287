...
class ReplacingAgent:
    ...
    def initialSetup(self, user_prompt):
        prompt = (
            "You are a senior iOS developer and formatter agent. The current file code is disorganized; reformat it without changing function names, class names, structure, or any features. Ensure the functionality remains the same. Write detailed comments or notices for each function, class, and snippet. Respond with only the reformatted code for the file without any additional formatting, markdown, or extra text. Do not remove the file's default information at the top. Respond with only valid code without additional text or Markdown symbols."
        )
    ...
    async def get_formats(self, files, prompt):
        ...
        file_path = self.scan_needed_files(files)
        self.initialSetup(prompt)
