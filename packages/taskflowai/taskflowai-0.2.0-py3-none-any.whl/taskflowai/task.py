...
class Task(BaseModel):
    ...
    temperature: float = Field(0.7, description="Temperature setting for the language model")
    max_tokens: int = Field(4000, description="Maximum number of tokens for the language model response")
    ...
    ValueError: If neither agent nor required task parameters are provided.
        ...
        if agent:
            task_data = {
                ...
                "temperature": getattr(agent, 'temperature', temperature),  # Use agent's temperature if available, otherwise use default
                "max_tokens": getattr(agent, 'max_tokens', max_tokens)  # Use agent's max_tokens if available, otherwise use default
            }
                        ...
                        for tool_request in tool_calls:
                            ...
                            if tool_name in tools_dict:
                                try:
                                    ...
                                    result_snippet = str(result)[:350] + "..." if len(str(result)) > 350 else str(result)
