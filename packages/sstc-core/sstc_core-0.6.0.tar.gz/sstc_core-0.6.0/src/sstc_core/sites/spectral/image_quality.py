...
def assess_image_quality(image):
    ...
    quality_assessment_results = {
        ...
        'flag_blur': detect_blur(image),  # Add blur detection as well
        'flag_snow': detect_snow(image),  # Add snow detection as well
        'flag_rain': detect_rain(image),  # Add rain detection as well
        'flag_water_drops': detect_water_drops(image),  # Add water drops detection as well
        'flag_dirt': detect_dirt(image),  # Add dirt detection as well
        'flag_obstructions': detect_obstructions(image),  # Add obstructions detection as well
...
def load_weights_from_yaml(yaml_file, station_name='default', platform_id='default'):
    """
...
def calculate_quality_index(weights, quality_assessment_results):
    """
    ...
    Parameters:
        ...
        quality_assessment_results (dict): Dictionary containing results from the image quality assessment.
    ...
    flag_brightness = quality_assessment_results.get('flag_brightness', 0)
    flag_blur = quality_assessment_results.get('flag_blur', False)
    flag_snow = quality_assessment_results.get('flag_snow', False)
    flag_rain = quality_assessment_results.get('flag_rain', False)
    flag_water_drops = quality_assessment_results.get('flag_water_drops', False)
    flag_dirt = quality_assessment_results.get('flag_dirt', False)
    flag_obstructions = quality_assessment_results.get('flag_obstructions', False)
    flag_glare = quality_assessment_results.get('flag_glare', False)
    flag_fog = quality_assessment_results.get('flag_fog', False)
    flag_rotation = quality_assessment_results.get('flag_rotation', False)
    flag_birds = quality_assessment_results.get('flag_birds', False)
