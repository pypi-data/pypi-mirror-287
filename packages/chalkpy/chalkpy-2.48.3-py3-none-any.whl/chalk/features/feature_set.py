...
class ClassSource:
    ...
    def __init__(self, c: Any):
        super().__init__()
        try:
            self.filename: Final = inspect.getfile(c)
        except:
            self.filename = None

        try:
            if notebook.is_defined_in_module(c):
                self.class_source: Final = inspect.getsource(c)
            # TODO (rkargon) Try to get source from notebook cells if possible
            else:
                self.class_source = None
        except:
            self.class_source = None
        self.dedent_source: Final = self.class_source and textwrap.dedent(self.class_source)
        try:
            self.tree: Final = get_class_ast(c)
        except:
            self.tree = None
...
class FeaturesMeta(type):
    ...
    features: List[Feature]  # pyright: ignore[reportUninitializedInstanceVariable]
