...
from typing import Any, Generic, Iterable, Optional, Protocol, Sequence, Type, TypeVar, Union, cast, overload
...
class PrimitiveFeatureConverter(Generic[_TPrim]):
    ...
    @classmethod
    def from_pyarrow_to_protobuf(cls, values: Union[pa.Array, pa.ChunkedArray]) -> list[pb.ScalarValue]:
    ...
    def from_primitive_to_protobuf_single(self, value: _TPrim) -> pb.ScalarValue:
        pa_arr = self.from_primitive_to_pyarrow([value])
        pb_values = self.from_pyarrow_to_protobuf(pa_arr)
        if len(pb_values) != 1:
            raise ValueError(f"Expected exactly one value, but got {len(pb_values)}")
        return pb_values[0]
    ...
    @classmethod
    def from_protobuf_to_pyarrow(cls, scalars: list[pb.ScalarValue]) -> pa.Array:
        pa_scalars = [cls._from_pb_to_pa(x) for x in scalars]
        py_objects = [x.as_py() for x in pa_scalars]
        arr_type = pa_scalars[0].type if pa_scalars else None
        ans = pa.array(py_objects, type=arr_type)
        assert isinstance(ans, pa.Array)
        return ans
    ...
    @classmethod
    def _from_pb_to_pa(cls, pb_value: pb.ScalarValue) -> pa.Scalar:
        if pb_value.HasField("null_value"):
            return pa.scalar(value=None, type=cls.convert_proto_dtype_to_pa_dtype(pb_value.null_value))
        ...
        if pb_value.HasField("struct_value"):
            name_to_pa_scalar = {
                field.name: cls._from_pb_to_pa(field_value)
    ...
    @classmethod
    def _from_pa_to_pb(cls, value: pa.Scalar) -> pb.ScalarValue:
        ...
        if isinstance(value, pa.StructScalar):
            ...
            for name, pa_scalar in value.items():
                pb_scalar = cls._from_pa_to_pb(pa_scalar)
                fields.append(
                    pb.Field(
                        name=name,
...
class FeatureConverter(PrimitiveFeatureConverter[_TPrim], Generic[_TPrim, _TRich]):
    ...
    def from_rich_to_protobuf(
        ...
        return self.from_primitive_to_protobuf_single(self.from_rich_to_primitive(value, missing_value_strategy))
