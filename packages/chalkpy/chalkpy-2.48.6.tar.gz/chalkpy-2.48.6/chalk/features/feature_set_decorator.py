...
from chalk.streams import Windowed, get_duration_secs
...
def _process_class(
    ...
    for name, annotation in raw_cls_annotations.items():
        ...
        if isinstance(annotation, Windowed):
            ...
            if wind._materialization is not None:
                ...
                if bucket_duration_raw is None:  # pyright: ignore [reportUnnecessaryComparison] -- user provided
                    error_builder.add_diagnostic(
                        message=(
                            f"Windowed feature '{namespace}.{name}' has a materialization, but no bucket_duration. "
                            "Please provide a bucket_duration, like '1h', in the dictionary under the keyword argument "
                            "'materialization'."
                        ),
                    ...
                    try:
                        get_duration_secs(bucket_duration_raw)
                    except Exception:
                        error_builder.add_diagnostic(
                            message=(
                                f"Windowed feature '{namespace}.{name}' "
                                f"has an invalid bucket_duration '{bucket_duration_raw}'. "
                                "Please use a valid duration, like '10m' or '1h'. "
            ...
            for bucket in wind._buckets:
                ...
                try:
                    bucket_seconds = get_duration_secs(bucket)
                except Exception:
                    ...
                    error_builder.add_diagnostic(
                        message=(
                            f"Windowed feature '{namespace}.{name}' has an invalid duration '{bucket}'. "
                            f"Please use a valid duration, like '10m' or '1h'. "
                            f"Read more at https://docs.chalk.ai/docs/duration"
                        ),
                        label="invalid duration",
