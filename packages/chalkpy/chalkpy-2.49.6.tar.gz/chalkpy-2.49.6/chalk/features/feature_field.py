...
from chalk.features.filter import Filter, TimeDelta, MultipleClauseJoinException
...
if TYPE_CHECKING:
    ...
    from chalk.streams._windows import MaterializationWindowConfig, GroupByWindowed
...
class WindowConfigResolved:
    ...
    # aggregation: Literal["sum", "count", "mean", "std", "var", "min", "max"]
    aggregation: str
...
class Feature(Generic[_TPrim, _TRich]):
    window_materialization: MaterializationWindowConfig | None
    ...
    def __init__(
        ...
        window_materialization: "Optional[MaterializationWindowConfig]" = None,
        ...
        self.window_materialization: MaterializationWindowConfig | None = window_materialization
    ...
    def converter(self) -> FeatureConverter:
        ...
        if feature_typ is not None:
            ...
            non_scalar_types = (
                feature_typ.typ.as_feature(),
                feature_typ.typ.as_features_cls(),
                feature_typ.typ.as_dataframe(),
            )
            non_null_non_scalar_type = next((typ for typ in non_scalar_types if typ is not None), None)
            if non_null_non_scalar_type is not None:
                raise TypeError(
                    (
                        "When annotating a feature's type with another feature to specify a join, the feature being "
                        f"used as a type must be a scalar type itself. Here, feature '{self.fqn}' is annotated with "
                        f"type '{feature_typ.fqn}', but '{feature_typ.fqn}' is annotated with complex type "
                        f"{non_null_non_scalar_type}. Feature '{feature_typ.fqn}' should be annotated with "
                        "a type 'str' or 'int', for example."
                    )
                )
    ...
    def dummy_feature(self) -> Feature:
