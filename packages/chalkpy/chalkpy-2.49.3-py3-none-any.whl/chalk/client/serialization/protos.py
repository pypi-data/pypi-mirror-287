...
from chalk.client._internal_models.models import INDEX_COL_NAME, TS_COL_NAME
...
class OnlineQueryConverter:
    ...
    def online_query_bulk_response_decode_to_single(
        ...
        oat = ".__chalk_observed_at__"
        res: List[FeatureResult] = []

        if len(response_proto.scalars_data) > 0:
            root_ts: Optional[dt.datetime] = None
            table = pa.Table.from_batches([arrow_ipc_to_record_batch(response_proto.scalars_data)])
            if len(table) != 1:
                raise ValueError(f"Expected exactly one scalar data row in response, found {len(table)}")

            for col_name in table.column_names:
                if col_name.endswith(oat):
                    root_ts = table[col_name][0].as_py()
                    break

            for col_name in table.column_names:
                if (
                    col_name.startswith("__chalk__.__metadata__.")
                    or col_name.endswith(oat)
                    or col_name in (TS_COL_NAME, INDEX_COL_NAME)
                ):
                    continue

                converter = PrimitiveFeatureConverter(
                    name=col_name,
                    is_nullable=table.schema.field(col_name).nullable,
                    pyarrow_dtype=table.schema.field(col_name).type,
                )
                converted_col = converter.from_pyarrow_to_primitive(table[col_name])
                assert (
                    len(converted_col) == 1
        ...
        for output_name, output_bytes in response_proto.groups_data.items():
            ...
            table = pa.Table.from_batches([arrow_ipc_to_record_batch(output_bytes)])

            for col_name in table.column_names:
                if col_name.endswith(oat):
                    namespace_ts = table[col_name][0].as_py()
                    break
            ...
            for col_name in table.column_names:
                ...
                values.append(converter.from_pyarrow_to_primitive(table[col_name]))
