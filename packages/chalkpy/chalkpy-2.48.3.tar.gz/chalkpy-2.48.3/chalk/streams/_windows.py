...
class Windowed(Generic[TRich], metaclass=_WINDOWED_METACLASS):
    ...
    def _to_feature(self, bucket: Optional[Union[int, str]]):
        ...
        if bucket is None:
            name = self._name
        elif get_duration_secs(bucket) not in self.buckets_seconds:
        ...
        return Feature(
            ...
            window_materialization=self._materialization,
        )
    ...
    def __init__(
        ...
        expression: Optional[Underscore],
        materialization: Optional[MaterializationWindowConfig],
    ):
        super().__init__()
        self._kind = kind
...
class MaterializationWindowConfig(TypedDict):
...
class GroupByWindowed(Generic[TRich], metaclass=_WINDOWED_METACLASS):
    ...
    def __getitem__(self, item: Type[Any]):
        ...
        for x in ensure_tuple(item):
            if isinstance(x, str):
                duration_seconds = get_duration_secs(x)
...
def windowed(
    *buckets: str,
    ...
    materialization: Optional[MaterializationWindowConfig] = None,
) -> Windowed[TRich]:
