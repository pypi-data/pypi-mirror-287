...
async def get_email(event: Event, email: str = ArgStr()):
    ...
    result, cookie_dict = await api_client.get_login_token()
    login_dict[user_id]["email"] = email
    login_dict[user_id]["cookies"] = cookie_dict

    result, cookie_dict = await api_client.login(email=email, cookies=cookie_dict)
    if result["code"] == 200:
...
async def get_code(event: Event, code: str = ArgStr()):
    ...
    result, cookies_dict = await api_client.post_code(
    ...
    if result["code"] == 200:
        # 获取当前角色信息
        mates_info, new_cookies_dict = await api_client.get_mate_page(cookies=cookies_dict)
