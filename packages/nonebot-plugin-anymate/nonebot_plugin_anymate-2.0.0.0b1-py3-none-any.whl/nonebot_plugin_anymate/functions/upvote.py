...
async def upvote_func_by_hand(event: Event):
    ...
    await upvote.send(f"共有{len(mateId_list)}个账户\n{str(mateId_list)}")
    # 获取replyId、UUID
    explore_posts, temp = await api_client.get_explore_post(5)
    ...
    for mateId in mateId_list:
        ...
        for i in range(len(number_list)):
            ...
            # 通过remember更新token
            temp, cookies = await api_client.get_token_by_remember(cookies=cookies)

            # 点赞
            result, new_cookies = await api_client.upvote(
...
async def upvote_func_auto():
    ...
    for mateId in mateId_list:
        ...
        # 获取replyId、UUID
        explore_posts, temp = await api_client.get_explore_post()
        replyId_list = []
        last_explore_post_list = explore_posts["page"]["data"]
        for last_explore_post in last_explore_post_list:
            replyId_list.append(last_explore_post["reply"]["id"])
