...
async def check_in_func_by_hand(event: Event):
    ...
    await check_in.send(f"共有{len(mateId_list)}个账户\n{str(mateId_list)}")
    ...
    for mateId in mateId_list:
        ...
        result, new_cookies = await api_client.check_in(cookies=new_cookies)
...
async def check_in_func_auto():
    ...
    for mateId in mateId_list:
        ...
        result, new_cookies = await api_client.check_in(cookies=cookies)
