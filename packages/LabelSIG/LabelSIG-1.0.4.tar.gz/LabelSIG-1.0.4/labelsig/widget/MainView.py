...
class BatchVisualizeThread(QThread):
    ...
    def run(self):
        ...
        for i, filename in enumerate(self.filenames):
            ...
            self.label_info.setText(f"[{bar}] {flag} Drawing: {filename}")
...
class LoadFolderThread(QThread):
    ...
    def import_other_files(self):
        ...
        self.main_window.label_info.setText("[{}] All files imported!".format('█' * bar_length))
...
def get_dict(path_dict, qlabel=None, bar_length=50):
    ...
    if qlabel is not None:
        qlabel.setText("[{}] All files processed!".format('█' * bar_length))
        if total_files == 0:
            qlabel.setText("[{}] No files found!".format('█' * bar_length))
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def init_ui_elements(self):
        ...
        self.init_table(message='Initialization Complete')
