...
class CriticClient(AbstractClient):
    ...
    def rep_to_obj(self, replica_dict):
        if 'PreviousScore' in replica_dict.keys():
            ReplicaItem(Body=replica_dict['Body'], Role=replica_dict['Role'], DateTime=replica_dict['DateTime'], PreviousScore=replica_dict['PreviousScore'])
        return ReplicaItem(Body=replica_dict['Body'], Role=replica_dict['Role'], DateTime=replica_dict['DateTime'], PreviousScore=None)
