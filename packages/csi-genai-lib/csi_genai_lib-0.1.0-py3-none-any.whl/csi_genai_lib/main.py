...
from csiMVP.Common.init import CFG, post_status_message, get_primary_ip, gethostname
...
class MainHandler:
    ...
    def __init__(self, **kwargs) -> None:
        self.sqs_message = {"R.ID": "", "event.time": datetime.now(timezone.utc).isoformat(), "R.VERSION": VERSION,
                            'R.API': '', 'C.REQ': {}, 'C.RSP': {}}
    ...
    def new_request(self, **kwargs) -> None:
        ...
        self.dest_host = kwargs.get('hostname') or gethostname()
        ...
        if self.query is not None:
            self.sqs_message.update({'R.ARGS': f'{self.query}'})
            self.sqs_message['C.REQ'].update({'query': f'{self.query}'})

        if self.user_session is not None:
            self.sqs_message.update({'user.session': self.user_session})
        ...
        if self.req_tap is not None:
            ...
            if self.req_tap.result is not None:
                self.sqs_message['C.REQ'].update({"sha256": self.req_tap.sha256})
                if isinstance(self.req_tap.result, str):
                    self.sqs_message['C.REQ'].update({"body": self.req_tap.result})
                elif self.reqB64encode:
                    self.sqs_message['C.REQ'].update({"body": base64.b64encode(self.req_tap.result).decode('utf-8')})
                    self.sqs_message['C.REQ'].update({"base64": True})
                else:
                    self.sqs_message['C.REQ'].update({"body": self.req_tap.result.decode('utf-8')})
                pmsg = self.sqs_message["C.REQ"]["body"][0:80].replace(CRLF, '')
    ...
    def post_event(self) -> None:
        ...
        message = json.dumps(self.sqs_message, cls=extEncoder)
    ...
    def update_response(self, **kwargs) -> None:
        ...
        self.apirsp = kwargs.get('response') or None
        rsp_type = kwargs.get('mime_type') or 'application/json; charset=utf-8'

        data = True if self.apirsp is not None else False
        alen = len(self.apirsp) if data else 0
        rsp_params = {
            "code": 200,
            "base64": False,
            "sha256": "",
        ...
        if int(alen) <= 0:
            ...
            self.rsp_tap = StreamTap(self.apirsp, chunk_size=alen, uid=xuid)
            self.req_tap.read()
            ...
            if self.rsp_tap.result is not None:
                ...
                pmsg = self.sqs_message["C.RSP"]["body"][0:80].replace(CRLF, '')
                print(f'[{self.req_id[-12:]}] RESP Data tap result => {pmsg}')

        self.sqs_message.update({'C.RSP': rsp_params})
