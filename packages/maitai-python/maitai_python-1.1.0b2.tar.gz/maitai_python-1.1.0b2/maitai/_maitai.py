...
class Maitai:
    def __init__(
        ...
        *,
        ...
        self.chat = Chat(self.client, client_params)
...
class Completions:
    def __init__(self, client: Optional[openai.Client] = None, client_params: Optional[ClientParams] = None):
        self.client = client
        self.client_params = client_params or ClientParams()
    ...
    def create(
        ...
        *,
        ...
        tools: Union[Tools, Iterable[Union[openai_chat_types.ChatCompletionToolParam, ToolFunction]], openai.NotGiven] = openai.NOT_GIVEN,
        ...
        if server_side_inference:
            ...
            response = maitai.Inference.infer(chat_request, callback, response_timeout)
            if stream:
            ...
            if self.client is None:
                self.client = openai.OpenAI(
                    default_query=self.client_params.default_query or None,
                    default_headers=self.client_params.default_headers or None,
                    base_url=self.client_params.base_url or None,
                )
            ...
            if stream:
                return _process_openai_stream(session_id, reference_id, intent, application_ref_name, messages, response, evaluation_enabled, completion_params, callback)
            else:
...
def _process_openai_stream(session_id: Union[str, int],
                           ...
                           callback: Optional[EvaluateCallback] = None, ) -> Iterable[MaitaiChunk]:
    full_body = ""
    proto_messages = openai_messages_to_proto(messages)
    last_chunk = None
    for chunk in stream:
...
def _process_inference_stream(stream: Iterable[InferenceStreamResponse]) -> Iterable[MaitaiChunk]:
