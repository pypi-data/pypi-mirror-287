...
def get_chat_completion_params(*,
                               ...
                               tools: Union[Iterable[Union[openai_types.ChatCompletionToolParam, ToolFunction]], openai.NotGiven] = openai.NOT_GIVEN,
    ...
    if stream_options != openai.NOT_GIVEN and stream_options:
        params.stream_options = chat_types.StreamOptions(include_usage=stream_options.get("include_usage"))
    ...
    if tools != openai.NOT_GIVEN:
        params.tools = []
        for tool in tools:
            if isinstance(tool, Callable) and not isinstance(tool, ToolFunction):
