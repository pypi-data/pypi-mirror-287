...
class PromptStatus(betterproto.Enum):
    BENIGN = 0
    MALICIOUS = 1
...
@dataclass(eq=False, repr=False)
class PromptguardRequest(betterproto.Message):
    company_id: int = betterproto.int64_field(1)
    request_id: str = betterproto.string_field(2)
    messages: List["ChatMessage"] = betterproto.message_field(3)


@dataclass(eq=False, repr=False)
class PromptguardResponse(betterproto.Message):
    input_safety_score: float = betterproto.float_field(1)
    status: "PromptStatus" = betterproto.enum_field(2)
...
class SessionMessage(betterproto.Message):
    ...
    application_action_id: int = betterproto.int64_field(3)
    ...
    instructions: str = betterproto.string_field(8)
    date_created: int = betterproto.int64_field(9)
...
class EvaluationServiceStub(betterproto.ServiceStub):
    ...
    async def evaluate_input(
        self,
        promptguard_request: "PromptguardRequest",
        *,
        timeout: Optional[float] = None,
        deadline: Optional["Deadline"] = None,
        metadata: Optional["MetadataLike"] = None
        ...
        return await self._unary_unary(
            "/chat.EvaluationService/EvaluateInput",
            promptguard_request,
            PromptguardResponse,
            timeout=timeout,
            deadline=deadline,
...
class EvaluationServiceBase(ServiceBase):
    ...
    async def evaluate_input(
        self, promptguard_request: "PromptguardRequest"
    ) -> "PromptguardResponse":
        raise grpclib.GRPCError(grpclib.const.Status.UNIMPLEMENTED)
    ...
    async def __rpc_evaluate_input(
        self, stream: "grpclib.server.Stream[PromptguardRequest, PromptguardResponse]"
    ) -> None:
        request = await stream.recv_message()
        response = await self.evaluate_input(request)
        await stream.send_message(response)
    ...
    def __mapping__(self) -> Dict[str, grpclib.const.Handler]:
        return {
            ...
            "/chat.EvaluationService/EvaluateInput": grpclib.const.Handler(
                self.__rpc_evaluate_input,
                grpclib.const.Cardinality.UNARY_UNARY,
                PromptguardRequest,
                PromptguardResponse,
            ),
