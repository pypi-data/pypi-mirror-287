...
class MysqlDbCursor:
    ...
    async def remove(self, model: Type[_BModel], where: str, *args: Any, tableName: str = '') -> int:
        return await self.execute(
            f'''
            DELETE
            FROM
                `{tableName or model.tableName()}`
            WHERE
                {where}
            ''',
            *args,
...
class ConditionMaker:
    ...
    def __init__(self):
        self._whereAry: list[str] = []
        self._args: list[Any] = []

    def add(self, sql: str, *args: Any):
        self._whereAry.append(sql)
        self._args.extend(args)

    @property
    def sql(self) -> str:
        if self._whereAry:
            return f'{self._statement} ' + ' AND '.join([f'({x})' for x in self._whereAry])
        else:
            return ''
