...
def load_dotenvs():
    ...
    for cur_path in paths:
        if not os.path.isfile(cur_path):
            print(f"WARNING: env path {cur_path} does not exist")
            continue
        dotenv.load_dotenv(cur_path, override=True)

        if do_check:
            print(f"Checking environment integrity on {cur_path}")
            # Check environment variables are consistent between .env and .env.example
            check_environment_variable_integrity(cur_path)
    return paths
