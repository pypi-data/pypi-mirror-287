...
from .CoveredWarrantData import CoveredWarrantData
...
class SubscribeCoveredWarrantEvents:
    ...
    def _data_handler(self, message):
        if message is not None:
            ...
            self.return_data = CoveredWarrantData(self.df[-1:])
            if self.callback:
    ...
    def _join_groups(self):
        if self.connected:
            self.hub_connection.send("JoinGroup", [f"Realtime.CoveredWarrant.{self.ticker}"])
            print(f"Joined group: Realtime.CoveredWarrant.{self.ticker}")
        else:
