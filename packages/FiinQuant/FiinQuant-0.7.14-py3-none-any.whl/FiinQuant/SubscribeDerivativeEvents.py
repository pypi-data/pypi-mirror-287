...
class SubscribeDerivativeEvents:
    def __init__(self, access_token: str, ticker: str, callback: callable):
        ...
        self.derivative = pd.DataFrame(columns=[
            ...
            'ForeignTotalRoom','ForeignCurrentRoom','OpenInterest'])
        self.ticker = ticker
        self._stop_event = threading.Event()
    def _data_handler(self, message):
        if message is not None:
            self.derivative.loc[len(self.derivative)] = message[0]['data'][0].split('|')
            self.return_data = DerivativeData(self.derivative[-1:])
            if self.callback:
    ...
    def _join_groups(self):
        if self.connected:
            self.hub_connection.send("JoinGroup", [f"Realtime.Derivative.{self.ticker}"])
            print(f"Joined group: Realtime.Derivative.{self.ticker}")
        else:
