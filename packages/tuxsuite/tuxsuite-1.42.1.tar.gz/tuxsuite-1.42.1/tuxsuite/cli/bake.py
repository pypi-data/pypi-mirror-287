...
def handle_get(options, _, config):
    ...
    if options.json:
        ...
        ret = get_storage(config, oebuild.download_url).json()
...
def setup_parser(parser):
    ...
    grp.add_argument(
        ...
        "--list-artifacts",
        action="store_true",
        help="List the oebuild artifacts.",
    )
    grp.add_argument(
        "-d",
        "--download-artifacts",
        type=pathlib.Path,
        help="Download the oebuild artifacts at specified path",
    )
    ...
    t.add_argument(
        ...
        help="Path/URL to build_definition.json",
        nargs=1,
        type=file_or_url,
    )
    bake_cmd_options(t)
