...
def init_worker(wksp, sconfig=None):
    wrun_cmd, wexist, wls, wdownload, wupdate, wremove = make_workspace_run_cmd(wksp, sconfig=sconfig)
    class _: pass
    _.wrun_cmd = wrun_cmd
    _.wexist = wexist
    _.wls = wls
...
def download_a_docker(wksp, dockername, localfilename, dockerfilename, ziptype=None):
    # 将 docker 镜像下载到本地
    wrun_cmd, wexist, wls, wdownload, wupdate, wremove = make_workspace_run_cmd(wksp)
...
def install_gogs(sconfig=None):
    make_global_ssh(sconfig)
    wrun_cmd, wexist, wls, wdownload, wupdate, wremove = make_workspace_run_cmd(workspace)
...
def install(imagename):
    if imagename == "gogs" or imagename == 'gogs/gogs:latest': 
        update_a_docker(workspace, 'gogs/gogs:latest', './gogs.tar.xz', 'gogs.tar.xz', ziptype='xz')
    if imagename == "nodejs" or imagename == 'node:alpine': 
        update_a_docker(workspace, 'node:alpine', './node21.tar.xz', 'node21.tar.xz', ziptype='xz')
    if imagename == "openresty_ja3" or imagename == 'cilame/openresty_ja3:latest': 
        update_a_docker(workspace, 'cilame/openresty_ja3:latest', './openresty_ja3.tar.xz', 'openresty_ja3.tar.xz', ziptype='xz')
...
def execute():
    argv = sys.argv
    tools = ['gogs','openresty_ja3','nodejs']
...
__all__ = [
    ...
    'save_in_server',
    'make_workspace_run_cmd',
    'download_a_docker',
    'update_a_docker',
    'install_gogs',
    'install_openresty_ja3',
    'install_nodejs',
]
