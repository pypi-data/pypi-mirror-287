        ...
        if do_tests_toggle.get():
            ...
            if not patch[0]:
                ...
                if gen_patches_toggle.get():
                    ...
                    ips_smc = bytes(
                        ips.Patch.create(bytearray(0x200) + randomizer.original_rom_data, bytearray(0x200) + patch[1]))
...
def diff_help():
    lines = [
        "Increased difficulty generally places required items deeper in dungeons, so more of the game has to be completed.",
        "",
...
def goal_help():
    lines = ["Goal determines the required conditions to beat the game:", "",
             "[DARK/APOCALYPSE/RANDOM] GAIA:", " - Collect the required Mystic Statues (if any)",
             " - Rescue Kara from her portrait using the Magic Dust",
             " - Acquire and use the Aura to gain Shadow's form",
             " - Talk to Gaia in any Dark Space to face and defeat the Gaia of your choice", "",
             "RED JEWEL HUNT:",
             "Collect the appropriate number of Red Jewels, by difficulty, and turn them into the Jeweler:",
...
def logic_help():
    lines = ["Logic determines how items and abilities are placed:", "",
             "COMPLETABLE:", " - All items and locations are accessible",
             " - Freedan abilities will only show up in dungeons", "",
             "BEATABLE:", " - Some non-essential items may be inaccessible",
             " - Freedan abilities will only show up in dungeons", "",
...
def enemizer_help():
    lines = ["The following enemy shuffle modes are available:", "",
             ...
             "FULL:", " - Enemies can show up in any dungeon with their normal stats", "", "INSANE:",
             " - Same as Full, but enemy stats are shuffled"]
    tkinter.messagebox.showinfo("Enemizer", "\n".join(lines))


def start_help():
    lines = ["You can change this setting to affect where you start the game.",
             "Where you start the game is also where Gaia sends you when you warp to start.", "",
...
def orb_rando_help():
    lines = [
        "Shuffles into the item pool the orbs that open doors and barriers, produced when some monsters are destroyed.",
