...
def option_ipe(args):
    ...
    if check_modifiers('-ipe',args,["-file","-limit","-last","-where","-contains"],help) == False: return
...
def option_logs(args):
    ...
    LOG Parsing
        Common modifiers:
            {utils.CYELLOW}-debuglevel M{utils.CEND} values XXS, XS, S, M, L. To control how big the log files are. The more information is logged the longer time it takes. 
                    XXS --> ApexCode: debug   Database: info
                    displays basic information and it is fast. M adds the stack trace, is the default. L can help detecting if there is an exception in the manage package. 
 ...
 """
    ...
    if check_modifiers('-logs',args,["-last","-tail","-store","-deletelogs","-limit","-limitinfo","-auto","-folder","-level","-loguser","-where","-inputfile","-file","-error","-noScreen","-allLimits","-debuglevel","-var","-downloadOnly","-all","-sqol"],help) == False: return
    ...
    if pc.loguser == None:
        pc.loguser = restClient.getConfigVar('loguser')

    if pc.debug_level == None: pc.debug_level = 'M'
    else: 
