...
from petdb.service.server import run, crypt
...
ExecStart={sys.executable} -m petdb.service run {{password_hash}} -p {{port}}
Restart=always
...
def init_service(password: str, port: int = 3944):
