...
def start_jobs_get_exchanges(params: dict = None, hook: Callable = None) -> None:
    ...
    """
    # load api key from secretfunctions
    api_key = secretfunctions.SecretFunctions().get_secret_key(key="EOD2PD_API_KEY", default=None)
    if api_key is None:
...
def get_symbols_prices_bulk(
    ...
    for exchange in exchanges:
        ...
        start_jobs_get_symbols_prices_bulk(exchange, start_date, end_date)
...
def get_symbols_prices(
    ...
    start_jobs_get_symbols_prices(symbols, start_date, end_date, freq)
    # wait until all jobs are done
    basefunctions.get_default_threadpool().get_input_queue().join()
    # get dataframes from output queue
    return eod2pd.utilityfunctions.get_dataframes_from_output_queue(dict_result=dict_result)
...
def get_symbols_dividends(
    ...
    start_jobs_get_symbols_dividends(symbols, start_date, end_date)
    # wait until all jobs are done
    basefunctions.get_default_threadpool().get_input_queue().join()
    # get dataframes from output queue
    return eod2pd.utilityfunctions.get_dataframes_from_output_queue(dict_result=dict_result)
...
def get_symbols_splits(
    ...
    start_jobs_get_symbols_splits(symbols, start_date, end_date)
    # wait until all jobs are done
    basefunctions.get_default_threadpool().get_input_queue().join()
    # get dataframes from output queue
    return eod2pd.utilityfunctions.get_dataframes_from_output_queue(dict_result=dict_result)
...
def send_message(content: EOD2PDMessageContent, hook: Callable = None) -> None:
    ...
    """
    timeout = secretfunctions.SecretFunctions().get_secret_key(key="EOD2PD_TIMEOUT", default=10)
