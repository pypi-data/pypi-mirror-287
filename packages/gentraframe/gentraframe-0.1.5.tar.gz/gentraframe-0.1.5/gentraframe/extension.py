...
from gentraframe.config import ModelConfig, TrainConfig
...
class CoraProvider:
    def run(self, input_dict: dict[str,Any], output_keys: list[str], **kwargs)-> Union[dict[str,Any], tuple, Any]:
        ...
        from_sets = input_dict['from_sets']
        ...
        input_dict['in_dim'] = in_dim
        input_dict['out_dim'] = out_dim
        input_dict['datasets'] = [self.my_cora.get_set(from_set=from_set, task=task)  for from_set in from_sets ] #type:ignore
...
class DefaultModelBuilder:
    ...
    def run(self, input_dict: dict[str,Any], output_keys: list[str],config: TrainConfig , **kwargs)-> Union[dict[str,Any], tuple, Any]:
