...
from gentraframe.trainerv2 import Trainer, SingleTrainer, TrainerConfig
...
class DefaultEvaluator(Trainer):
    ...
    def run(self, inpout: Type[EvalConfig],  outputs: list[str], config: TrainConfig, **kwargs) -> Union[dict[str,Any], tuple, Any]:
        task = inpout.task
        models = inpout.models
        datasets = inpout.datasets
        model_path = inpout.model_paths[0]
        ...
        test_loader =self.get_test_loader(datasets, train_args=train_args, **kwargs)  #DataLoader(datasets[-1], batch_size=train_args.batch_size, shuffle=False) #type:ignore
        ...
        # gather metrics
        test_metric_fn_dict= self.get_test_metric_fn_dict(task=task, **kwargs)  #get_metric_fn_collection(prefix='test', task = task) 
