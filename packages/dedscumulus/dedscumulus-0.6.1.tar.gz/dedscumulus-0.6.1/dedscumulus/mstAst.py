...
def getAssetTypes(DB_CRED,logger,trust='no',encrypt='yes'):
    current_row=''
    assetTypes={}
    try:
        ...
        while errors<3:
            try:
                with pyodbc.connect('Driver='+DB_CRED['driver']+';Server=tcp:'+DB_CRED['db_server']+',1433;Database='+DB_CRED['database']+';Uid='+DB_CRED['db_user']+';Pwd={'+DB_CRED['db_password']+'};Encrypt='+encrypt+';TrustServerCertificate='+trust+';Connection Timeout=30') as conn:
...
def getBrzAssetIds(DB_CRED,LOGGER,trust='no',encrypt='yes'):
    assetIds={}
    try:
        with pyodbc.connect('Driver='+DB_CRED['driver']+';Server=tcp:'+DB_CRED['db_server']+',1433;Database='+DB_CRED['database']+';Uid='+DB_CRED['db_user']+';Pwd={'+DB_CRED['db_password']+'};Encrypt='+encrypt+';TrustServerCertificate='+trust+';Connection Timeout=30') as conn:
            with conn.cursor() as cursor:
...
def executeCommand(DB_CRED,sql,logger,trust='no',encrypt='yes'):
...
def executeSelect(DB_CRED,sql,logger,trust='no',encrypt='yes',pandas_df=False):
    selectRows=[]
    try:
...
def getAssetParameter(DB_CRED,assetId,parameter,logger,subParameter=None,trust='no',encrypt='yes'):
    try:
...
def getColumnEntry(DB_CRED,getWhat,tableFrom,whereFilter,equalsWhat,string,logger,trust='no',encrypt='yes'):
    try:
...
def getSetting(CREDS,version,settingName,systemShortName,logger,trust='no',encrypt='yes'):
    ...
    try:
        ...
        while errors<3:
            try:
                ...
                conn_string='Driver='+CREDS['driver']+';Server=tcp:'+CREDS['db_server']+',1433;Database='+CREDS['database']+';Uid='+CREDS['db_user']+';Pwd={'+CREDS['db_password']+'};Encrypt='+encrypt+';TrustServerCertificate='+trust+';Connection Timeout=30'
...
def dfToDB2(CREDS,table,df,chunkSize,logger,trust='yes',encrypt='yes'):
    try:
        try:
        ...
        conn_str=urllib.parse.quote_plus(r'Driver='+CREDS['driver']+';Server=tcp:'+CREDS['db_server']+',1433;Database='+CREDS['database']+';Uid='+CREDS['db_user']+';Pwd='+CREDS['db_password']+';Encrypt='+encrypt+';TrustServerCertificate='+trust+';Connection Timeout='+timeout+';')
...
def insertSLALog(DB_CRED,service,function,factor,ex_time,logger,trust='no',encrypt='yes'):
    try:
        errors=0
        sql="Not set"
        error="Not set"
