...
def _format_stack(task, complete=False):
    ...
    for f in _get_stack(task.coro):
        ...
        name = co.co_name
        if not complete and Path(filename).parent == dirname:
            continue
...
def _where(task):
    ...
    for f in _get_stack(task.coro):
        ...
        co = f.f_code
        filename = co.co_filename
        if Path(filename).parent == dirname:
            continue
        ...
        try:
            ...
            while self._finished or self._running:
                ...
                while self._finished:
                    ...
                    if self.completed is None and not (
                        (self._wait is object) and (not task.exception) and (task.result is None)
                    ):
                        self.completed = task
                    ...
                    if (
                        task.exception
                        or (self._wait is any)
                        or ((self._wait is object) and (task.result is not None))
                    ):
                        return
