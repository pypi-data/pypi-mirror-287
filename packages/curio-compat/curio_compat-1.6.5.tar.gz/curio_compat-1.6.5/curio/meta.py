...
def from_coroutine(level=2, _cache={}):  # noqa: B006
...
def iscoroutinefunction(func):
    ...
    return (
        ...
        or hasattr(func, "_awaitable")
        or _isasyncgenfunction(func)
    )
...
def instantiate_coroutine(corofunc, *args, **kwargs):
    ...
    if isinstance(corofunc, collections.abc.Coroutine) or inspect.isgenerator(corofunc):
        ...
        ), "arguments can't be passed to an already instantiated coroutine"
        return corofunc
...
def awaitable(syncfunc):
    ...
    def decorate(asyncfunc):
        if inspect.signature(syncfunc) != inspect.signature(asyncfunc):
            raise TypeError(
                f"{syncfunc.__name__} and async {asyncfunc.__name__} have different signatures"
            )
