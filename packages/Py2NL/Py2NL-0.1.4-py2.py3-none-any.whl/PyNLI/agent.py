...
class AgentThread(threading.Thread):
    ...
    def set_agent(self):
        ...
        agent_chain = PromptTemplate(template=get_agent_template(), input_variables=["input"],
                                     partial_variables={"variables": st.session_state[self.key].parse_variables,
                                                        "modules": self.get_current_modules}) | agent_model
        ...
        interpret_yes_no_chain = PromptTemplate(template=get_yes_no_template(),
                                                input_variables=[]) | assistant_model | text_to_bool

        detect_loop_chain = PromptTemplate(template=get_detect_loop_template(),
                                           input_variables=["input", 'agent_scratchpad']) | assistant_model | text_to_bool
...
class CustomAgentExecutor(AgentExecutor):
    ...
    reinit: bool = False
    aborting:bool = False
    detect_loop_chain: RunnableSequence = None
    interpret_yes_no_chain: RunnableSequence = None
