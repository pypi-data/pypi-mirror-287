...
def pie_base(
    ...
    if output is None and not show:
        ...
        df_sort, cluster_size, cluster_list = get_real_predict_label(df=df, map_cluster=map_cluster, clusters=clusters, value=value)
        ...
        ax1.set_axis_off()
        ax1.pie(top_x, labels=[", ".join(cluster_list), "Other"], colors=colors, startangle=90, labeldistance=label_distance, pctdistance=pct_distance)
        ax1.pie([np.array(top_x).sum()], colors=['white'], radius=radius, startangle=90, wedgeprops=dict(width=radius, edgecolor='w'))
...
def pie_trait(
    ...
    def trait_plot(trait_: str, atac_cell_df_: DataFrame) -> None:
        ...
        if trait_ not in trait_cluster_map_key_list:
            ...
            raise ValueError(f"The key in `trait_cluster_map` does not contain the `{trait_}` trait and needs to be added")
