...
def set_colors(data: collection, big_rgb_value: int = 150) -> Tuple:
    log.info("Start setting color gradient information")
    new_data: list = list(set(data)).copy()
    new_data.sort()
    size = len(new_data)
    # get zero location
    index = get_index(0.0, new_data)
    # get zero index
    middle_index = index if isinstance(index, number) else index[1]

    colors = []
    bounds = []

    # judge colors size and bounds size
    if size >= big_rgb_value * 2:

        # Add colors smaller than zero
        for i in range(big_rgb_value + 1):
            colors.append(rgb2hex(i, big_rgb_value, i))

        # Add colors greater than zero
        for j in range(1, big_rgb_value + 1):
            colors.append(rgb2hex(big_rgb_value, big_rgb_value - j, big_rgb_value - j))

        colors_size = len(colors)
        # Sampling data
        after_size = size - middle_index
        colors_size_half = colors_size // 2
        # add binds
        rate = middle_index / colors_size_half
        rate_after = after_size / (colors_size - colors_size_half)

        # Add data less than zero
        for m in range(colors_size_half):
            bounds.append(new_data[math.floor(rate * m)])

        # Add data greater than zero
        for n in range(colors_size - colors_size_half):
            if size > middle_index + math.floor(rate_after * n):
                bounds.append(new_data[middle_index + math.floor(rate_after * n)])
            else:
                break

        if bounds[-1] != new_data[-1]:
            bounds.append(new_data[-1])

    else:
        # Obtain colors
        size_half = size // 2
        rate_before = 150 / size_half
        rate_after = 150 / (size - size_half)

        # Add colors smaller than zero
        for i in range(size_half + 1):
            rgb = math.floor(rate_before * i)
            colors.append(rgb2hex(rgb, big_rgb_value, rgb))

        # Add colors greater than zero
        for j in range(1, size - size_half):
            rgb = math.floor(rate_after * j)
            colors.append(rgb2hex(big_rgb_value, big_rgb_value - rgb, big_rgb_value - rgb))

        # Data does not need to change
        bounds = new_data

    # The purpose of adding a value to each number is to ensure that each value falls within a certain color range, rather than the color edge
    new_bounds = [k - 1e-10 for k in bounds]
    new_bounds.append(bounds[-1] + 1e-5)

    # judge size
    colors_size = len(colors)
    bounds_size = len(new_bounds)

    if colors_size == bounds_size - 2:
        colors.append(rgb2hex(big_rgb_value, 0, 0))
    elif colors_size < bounds_size - 1:
        log.error(f"There are {bounds_size - 1} color bins including extensions, but colors = {colors_size}; colors must equal or exceed the number of bins")
        raise ValueError(f"There are {bounds_size - 1} color bins including extensions, but colors = {colors_size}; colors must equal or exceed the number of bins")

    # Return the data required for drawing
    cmap = ListedColormap(colors)
    norm = BoundaryNorm(new_bounds, len(colors))
    return cmap, norm
