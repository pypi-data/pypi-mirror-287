...
def barcodes_add_anno(annotation_file: path, cell_annot: DataFrame, clusters: str = "clusters") -> DataFrame:
    ...
    if clusters is not None:
        if clusters not in list(cell_annotation_file.columns):
            log.error(f"The comment file must contain a column name with `{clusters}`, Try changing the `clusters` parameter")
        ...
        if clusters not in list(cell_annot.columns):
            cell_annot = cell_annot.rename(columns={f"{clusters}_y": clusters})
...
def read_barcodes_file(
    ...
    log.info("handle cells information")
    # read file
    cell_annot = pd.read_csv(barcodes_file, header=None, index_col=None)
...
def read_info_by_metadata(
    ...
    # read barcodes file
    cell_annot = read_barcodes_file(os.path.join(base_path, "barcodes.tsv"), clusters=clusters, barcode_split_character=barcode_split_character, annotation_file=annotation_file)
...
def read_sc_atac(
    ...
    if is_metadata:
        ...
        sc_atac: AnnData = read_info_by_metadata(
            base_path=resource,
        ...
        if str(resource).endswith(".h5"):
            sc_atac: AnnData = read_sc_atac_10x_h5(file=resource, barcode_split_character=barcode_split_character, peak_split_character=peak_split_character)
...
def read_variants(
    ...
    :param base_path: Path for storing mutation trait data;
        The file must contain the following names {id, chr, position, rsId, pp}, where ID represents the representative of the trait name;
    :param files: Collection of mutation trait data;
    :param labels: Classification labels for each trait or disease;
    :param column_map: The mapping of column names facilitates mapping the corresponding column names in the mutation file to the specified column name information, i.e. {id, chr, position, rsId, pp};
    ...
    for variant_file in new_files:
        ...
        if column_map is not None:
            ...
            if isinstance(keys[0], str):
                ...
                raise ValueError("The `keys` in `column_map` only support `None`, `str` type, and `int` type. When it is None or `str` type, it will be read with a header, and when it is `int` type, it will be read without a header.")
