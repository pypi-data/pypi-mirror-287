...
def pages_of(client, paginator_name, **kwargs):
    ...
    page_iterator = paginator.paginate(PaginationConfig=dict(PageSize=100), **kwargs)
...
def unnest(things, shape, subtree_key) -> Iterable[dict | str]:
    ...
    if path is None or path == " ":
        args = "with given key '{subtree_key}'" if subtree_key else "with first branching node"
        raise RuntimeError(f"Cannot determine the way to unnest data of shape {shape} {args}")
...
def unnest_single_thing(things):
    if isinstance(things, list):
        ...
        if len(things) == 1:
            return things[0]
        raise RuntimeError("The single option is set but result has {len(result)} records.")
    raise TypeError("The single option is set, which is meaningless for results of type '{type(result)}'.")


def _smart_query(client, operation, *, pagination=None, subtree=None, single=False, **kwargs):
...
def smart_query(session, api, operation, *, pagination=None, subtree=None, single=False, **kwargs):
