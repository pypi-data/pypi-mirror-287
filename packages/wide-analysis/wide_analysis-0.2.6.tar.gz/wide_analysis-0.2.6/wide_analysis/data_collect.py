...
def collect(mode, start_date=None, end_date=None, url=None, title=None, output_path=None):
    if mode not in ['date_range', 'date', 'title','existing']:
            raise ValueError("Invalid mode. Choose from ['date_range', 'date', 'title','existing']")
    if mode == 'existing':
    ...
    if mode in ['date_range', 'date', 'title']:
        ...
        print("Invalid input. Choose existing_data option from ['yes', 'no']")