...
async def tongyi_model_invoke(prompt_text_list: List[str]) -> List[str]:
    ...
    if context is None:
        raise ValueError("应用上下文对象ApplicationContext为空，请先执行bootstrap初始化")

    # 获取llm的配置
    llm_model_config = context.config_resolver.get_config('application.llm.model.tongyi')
    if llm_model_config is None:
        raise ValueError("llm model 的配置：application.llm.model.tongyi 不能为空")
