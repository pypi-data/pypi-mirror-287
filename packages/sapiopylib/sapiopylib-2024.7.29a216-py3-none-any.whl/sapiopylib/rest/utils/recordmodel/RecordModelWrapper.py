...
class AbstractRecordModel(ABC):
    ...
    def add_parent(self, parent_record: AbstractRecordModel) -> None:
        """
    ...
    def get(self, getter: AbstractRecordModelPropertyGetter[RecordModelPropertyType]) -> Optional[
        RecordModelPropertyType]:
        """
...
RecordModelType = TypeVar("RecordModelType", bound=AbstractRecordModel)
...
class RecordModelWrapperUtil:
    ...
    @staticmethod
    def wrap(py_record_model: PyRecordModel | AbstractRecordModel, clazz: Type[RecordModelType]) -> RecordModelType:
        if isinstance(py_record_model, PyRecordModel):
            return clazz(backing_model=py_record_model)
        elif isinstance(py_record_model, AbstractRecordModel):
    ...
    @staticmethod
    def wrap_list(py_record_model_list: Iterable[PyRecordModel | AbstractRecordModel], clazz: Type[RecordModelType])\
            -> List[RecordModelType]:
