...
from sapiopylib.rest.utils.recordmodel.RecordModelWrapper import AbstractRecordModel, RecordModelWrapperUtil
...
class LastSavedValueManager(RecordModelManagerBase):
    ...
    def get_last_saved_cache(self, model: PyRecordModel | AbstractRecordModel) -> dict[str, Any] | None:
        ...
        return self._model_data_cache.get(RecordModelWrapperUtil.unwrap(model))
    ...
    def is_loaded(self, model: PyRecordModel | AbstractRecordModel) -> bool:
        ...
        if model.is_new or model.is_new_in_sapio or model.record_id < 0:
            return True
        return RecordModelWrapperUtil.unwrap(model) in self._model_data_cache
    ...
    def load(self, models: list[PyRecordModel | AbstractRecordModel]) -> None:
        ...
        """
        all_type_to_load_list: list[PyRecordModel] = RecordModelWrapperUtil.unwrap_list(
