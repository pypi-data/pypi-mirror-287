...
class GenericPredictionSystem(pl.LightningModule):
    ...
    # def download_weights(self, url: str, path: str):
    #     """Download a model's weights from a given URL.

    #     Parameters
    #     ----------
    #     url : str
    #         URL to the model's weights.
    #     path : str
    ...
    #         f.write(r.content)
    #     print(f"Model weights downloaded at {path}")
    #     return path
    ...
    def download_weights(
        ...
        if self._check_integrity(Path(out_path) / Path(filename).stem / 'last.ckpt'):
