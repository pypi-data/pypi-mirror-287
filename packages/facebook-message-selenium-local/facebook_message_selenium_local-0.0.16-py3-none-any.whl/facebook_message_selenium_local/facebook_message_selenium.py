...
class FacebookMessageSelenium(MessageLocal, metaclass=MetaLogger):

    def __init__(self, is_test_data: bool = False) -> None:  # noqa
        # Don't call super().__init__(), we already have the message_local object when we call the "init" of
        # FacebookMessageSelenium

        self.browser = FacebookSeleniumUtil.chrome_login_facebook_messenger(leave_open=False)
        self._api_type_id = FacebookMessageSeleniumLocalConstants.FACEBOOK_MESSAGE_SELENIUM_API_TYPE_ID

    # TODO body is not part of compound_message_dict?
    def send(self, *, body: str = None, recipients: List[Recipient] = None, scheduled_timestamp_start: datetime = None,
        ...
        for recipient in recipients:
            ...
            user_external_dict = recipient.get_user_dict_by_system_id(FacebookMessageSeleniumLocalConstants.FACEBOOK_SYSTEM_ID)
            ...
            if user_external_dict == {"username": list, "user_external_id": list,
            ...
            for username in user_external_dict["username"]:
                ...
                try:
                    ...
                    input_username = self.browser.find_element(By.XPATH, '//div[@role="main"]//input[@type="search"]')
                    ...
                    time.sleep(random.randint(3, 6))
                    input_username = self.browser.find_element(By.XPATH, '//div[@role="presentation"]//input['
                                                                         '@type="search"]')
                ...
                time.sleep(random.randint(2, 6))
                message_bar = self.browser.find_element(By.XPATH, '//div[@role="main"]//div[@role="textbox"]')
                message_bar.click()
                ...
                #  recipient=recipient)
                return_value.append(
                    {"status": "success", "message": "Message sent successfully", "recipient": recipient, "body": body})

        return return_value

    # TODO Doesn't work- Can we fix it?
    def read(self, *, username: str, number_messages: int = 1) -> []:
        self.logger.info("Reading message from:", username)

        # Find conversation with username
        search_bar = self.browser.find_element(By.XPATH,
                                               '/html/body/div[1]/div/div[1]/div/div[3]/div/div/div[1]/div[1]/div['
                                               '1]/div/div/div/div[2]/div/div/div/div/div/label/input')
        search_bar.click()
        ...
        try:
            i = 1
            while True:
                xpath = ('/html/body/div[1]/div/div[1]/div/div[3]/div/div/div[1]/div[1]/div[2]/div/div/div/div['
                         '1]/div/div/div/div/div/div[2]/div/div/div/div[1]/div/div/div/div/div/div/div/div['
                         '3]/div/div[') + str(
                    i) + ']'
