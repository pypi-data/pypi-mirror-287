...
class StageNotSupported(Exception):
    """Indicates that an unsupported Filter stage was attempted to be run."""

    def __init__(self, stage: Stage):
        """Create a new StageNotSupported exception."""
        super().__init__(
...
class Filter:
    ...
    def filter_metadata(self, raw: bytes, metadata: dict) -> Dependencies:
        """Transform metadata."""
        raise StageNotSupported(Stage.METADATA)

    def filter_raw(
        self, raw: bytes, metadata: dict
    ) -> tuple[bytes, Dependencies]:
        """Transform raw data."""
        raise StageNotSupported(Stage.RAW)

    def filter_html(self, dom: BeautifulSoup, metadata: dict) -> Dependencies:
        """Transform an HTML DOM."""
        raise StageNotSupported(Stage.HTML)
...
class Pipeline:
    ...
    def __init__(self, html_output_encoding="utf-8"):
    ...
    def run(
        self,
        source: Source,
        until_stage: Optional[Stage] = None,
        metadata: dict = None
    ) -> tuple[bytes, Dependencies]:
