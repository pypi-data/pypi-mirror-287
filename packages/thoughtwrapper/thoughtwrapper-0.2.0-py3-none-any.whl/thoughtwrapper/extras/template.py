...
class Engine(Filter):
    ...
    def __init__(
        ...
        layout_wrapper_enabled: bool = False,
        ...
        if "loader" in env_params:
            ...
            _wrap_loader(env_params["loader"], self.dependencies)

        self.environment = jinja2.Environment(**env_params)
        """The Jinja2 Environment that is used by this filter.

        Be sure to read the documentation for :meth:`__init__` before
    ...
    @override
    def filter_raw(self, raw, metadata):
    ...
    def extract_wrap_params(self, metadata: dict) -> tuple[str, str]:
        ...
        """
        def lookup(d: dict, keys: str | list[str]):
