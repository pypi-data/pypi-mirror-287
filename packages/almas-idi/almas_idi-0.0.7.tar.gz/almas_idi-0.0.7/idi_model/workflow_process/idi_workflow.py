...
def before_activation_bloc(
    ...
    activated_ids = list(activation_data_before_activation["policy_no_"])
    combined_deposit_activation_df_before_activation = concat_all_dfs(
        pd.DataFrame(),
        deposit_data_before_activation,
        config["keys_df_mapping_deposit"],
        config["keys_df_mapping_activation"],
    ...
    combined_deposit_activation_df_before_activation["activation_status"] = (
        combined_deposit_activation_df_before_activation["policy_no_"].isin(
            activated_ids
        )
    )
    ...
    deposit_data_before_activation, activation_data_before_activation = (
        split_data(
            combined_deposit_activation_df_before_activation,
            config["run_settings"]["reporting_date"],
            config["run_settings"]["next_reporting_date"],
        )
    )
...
def convert_to_ifrs17_compliant(
    ...
    deposit_data_before_activation = deposit_df.copy(deep=True)
    activation_data_before_activation = activation_df.copy(deep=True)

    (
        df_quarterly_cohort_level_before_activation,
        df_monthly_policy_level_not_projected_before_activation,
    ) = before_activation_bloc(
        deposit_data_before_activation,
        activation_data_before_activation,
        cohort_df,
        monthly_loss_ratio_df,
