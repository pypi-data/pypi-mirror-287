...
def fetch_keysight_model(scope: Resource = None, ip_addr = None) -> Optional[str]:
    if scope is None and ip_addr is None:
        raise ValueError("Need either scope or ip_addr passed")

    scope = scope if ip_addr is not None else ResourceManager().open_resource(f"TCPIP0::{ip_addr}::inst0::INSTR")
