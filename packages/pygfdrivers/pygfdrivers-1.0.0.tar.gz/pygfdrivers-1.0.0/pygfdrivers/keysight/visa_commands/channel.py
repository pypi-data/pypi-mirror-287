...
from common.visa.visa_cmd import BaseVisaScopeCommand
from common.models.device.cmd import create_cmd_class, CmdClassType
from common.util.utilities import has_prop, has_setter, convert_value

from keysight.models.channel import KeysightChannelModel, KeysightChannelDataModel
...
class KeysightChannel(BaseVisaScopeCommand):
    def __init__(self, scope: VisaScope) -> None:
        super().__init__(scope)

        self.ch_cmds = init_ch_cmds()
        self.ch_fields = [field for field in self.source_fields if field.startswith('ch')]

        self.max_chs = 8 if self.is_mxr else 4
        if self.is_mxr:
            self.ch_cmds.add_cmd(key='ch_gain', cmd=':probe:external:gain')
            self.ch_cmds.update_cmd_args('ch_coupling', ['dc', 'dc50', 'dcfifty', 'ac'])

        _min = 0.1 if self.scope_model in ['DSOX2000', 'DSOX3000'] else 0.001
    ...
    def fetch_ch_config(self, active_chs: List[int], ch_info: Dict[str, KeysightChannelDataModel]) -> None:
        try:
            for ch in active_chs:
                self.ch_num = ch
                info = ch_info[str(ch)]
    ...
    def ch_gain(self, probe: float) -> None:
        probe = f"{probe},RATio" if self.scope_model == 'MXR' else probe
