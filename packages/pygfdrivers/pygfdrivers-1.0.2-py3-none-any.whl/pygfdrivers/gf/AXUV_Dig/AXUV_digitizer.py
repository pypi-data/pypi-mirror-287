...
from pygfdrivers.gf.AXUV_Dig.models.AXUV_digitizer_config import AXUVDigitizerConfig
...
class AXUV_digitizer(BaseDevice):
    def __init__(self, AXUV_config: AXUVDigitizerConfig = None) -> None:
        super().__init__(AXUV_config, "scope")
    ...
    def apply_configurations(self) -> None:
        self.name = self.config.scope.device_name
        self.hostname = self.config.scope.hostname
        self.username = self.config.scope.username
        self.password = self.config.scope.password
        self.remote_file = self.config.scope.remote_file
