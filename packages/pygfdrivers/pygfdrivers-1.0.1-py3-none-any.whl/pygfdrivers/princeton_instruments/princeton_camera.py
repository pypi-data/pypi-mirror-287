...
from common.base_device import BaseDevice, BaseModel
from common.util.utilities import has_setter

from princeton_instruments.picam.picam import *
...
class PrincetonCamera(BaseDevice):
    def __init__(self, camera_config: PrincetonCameraConfigModel = None, demo: bool = False) -> None:
        super().__init__(camera_config, 'camera')
        self.demo = demo
    ...
    def init(self):
        try:
            InitializeLibrary()
            self.conn_str = self.config.camera.device_conn_str
            self.connect()
        ...
        try:
            ...
            for _ , model in config:
                if isinstance(model, BaseModel):
                    for field, setting in model.dict().items():
    ...
    def commit_parameters(self):
        ...
        try:
            ...
            self.is_connected = False    
            self.log.error(f"Failed to commit parameters: {e}")    
        except Exception as e:
            self.log.error(f"Failed to commit parameters: {e}")
    ...
    def arm(self):
        ...
        try:
            ...
            self.is_connected = False
            self.log.error(f"Failed arming camera: {e}")
        except Exception as e:
            self.log.error(f"Failed arming camera: {e}")
    ...
    def get_acquisition_update(self):
    ...
    def disconnect(self):
        ...
        try:
            ...
            self.log.error(f"Disconnect attempt failed: {e}")
    ...
    def readout_count(self, count: int):
    ...
    def read_readout(self, frame_pointer):
        """
    ...
    def read_metadata(self, pointer):
        """
    ...
    def fetch_data(self):
        try:
            ...
            self.is_connected = False
            self.log.error(f"Failed fetching data: {e}")
        except Exception as e:
            self.log.error(f"Failed fetching data: {e}")
    ...
    def save_data_to_file(self):
        ...
        """
        file_format = self.config.camera.file_format
        path = self.config.camera.file_save_path
        if file_format == "PNG":
