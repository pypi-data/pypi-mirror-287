...
from common.device_map import device_map
from common.base_device import BaseDevice
from common.util.utilities import open_yaml
from common.util.logger_manager import LoggerManager, LOGGING_MODE
...
class ConfigManager:
    ...
    def get_config_name(config_obj: BaseModel, config_type: str) -> str:
        ...
        raise ValueError("Missing 'device_name' field.")
    ...
    def create_and_init_device_objs(self, device_configs: ValuesView, device_type: str) -> None:
        """
        ...
        """
        device_type_map = device_map.get(device_type)
        try:
            if device_type_map is None:
                raise ValueError(f"No mapping associated to device type '{device_type}'.")

            for device_config in device_configs:
                device_key = getattr(device_config, f"{device_type.rstrip('s')}").device_type

                device_class = device_type_map.get(device_key)
                if not device_class:
                    raise ValueError(f"No class or mapping associated to device key '{device_key}'.")

                device_obj = device_class(device_config, self.client_config)
                self.init_device_obj(device_obj)
                self.store_device_obj(device_obj, device_type)

        except Exception as e:
            self.log.error(f"Initializing '{device_type}' encountered error: {e}")
