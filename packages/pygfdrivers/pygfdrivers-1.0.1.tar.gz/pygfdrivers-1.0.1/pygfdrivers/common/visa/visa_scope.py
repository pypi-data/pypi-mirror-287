...
# custom packages
from common.base_scope import BaseScope
from common.util.utilities import convert_value
from common.models.device.cmd import CmdClassType
from common.util.fetch_device_models import fetch_keysight_model
...
class VisaScope(BaseScope, ABC):
    ...
    def init(self) -> None:
        try:
            self.conn_str = self.config.scope.create_resource_str()
            if self.conn_str is None:
                self.conn_str = getattr(self.config.scope, 'device_conn_str', None)
    ...
    def connect(self) -> None:
        try:
            ...
            self.scope = self.resourceManager.open_resource(f"{self.conn_str}")
            self.scope.read_termination = '\n'
            self.log.info(f"VISA device connected")
        except Exception as e:
    ...
    def get_device_model(self) -> None:
        if 'keysight' in self.scope_type:
            self.device_model = fetch_keysight_model(self.scope)
    ...
    def is_valid_setting(self, cmd_obj: CmdClassType, cmd_key: str, setting: Union[str, int, float]) -> None:
        ...
        if cmd_args is None:
            self.log.debug(f"mcd_key '{cmd_key}' does not have 'args' attribute.")
            return
    ...
    def is_in_range(self, max_value: Union[int, float], min_value: Union[int, float]) -> bool:
        raise NotImplementedError

    def is_valid_opt(self, opts: List) -> bool:
        raise NotImplementedError
