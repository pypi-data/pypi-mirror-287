...
from common.util.utilities import has_prop, has_setter
from common.visa.visa_cmd import BaseVisaScopeCommand
...
class KeysightAcquire(BaseVisaScopeCommand):
    def __init__(self, scope: VisaScope) -> None:
        ...
        max_segments = 250
        if self.scope_model in ['DSOX2000', 'DSO5000']:
            max_segments = 250
        elif self.scope_model in ['DSOX1200']:
            max_segments = 500
        elif self.scope_model in ['DSOX3000', 'DSOX4000', 'MXR']:
            max_segments = 1000
        self.acq_cmds.update_cmd_args('acq_segment_count', {'max': max_segments, 'min': 2})

        if self.scope_model in ['MXR', 'DSOX4000']:
    ...
    def acq_srate(self) -> float:
        _acq_srate = self.query(self.acq_cmds, 'acq_srate')
        return float(_acq_srate)
    ...
    def acq_srate_auto(self) -> bool:
        if self.scope_model in ['MXR', 'DSOX4000']:
            _acq_srate_auto = self.query(self.acq_cmds, 'acq_srate')
