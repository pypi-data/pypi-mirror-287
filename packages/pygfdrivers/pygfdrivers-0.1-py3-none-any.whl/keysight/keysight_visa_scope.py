...
from keysight.visa_commands.root import KeysightRoot
from keysight.visa_commands.common import KeysightCommon
from keysight.visa_commands.acquire import KeysightAcquire
from keysight.visa_commands.channel import KeysightChannel
from keysight.visa_commands.trigger import KeysightTrigger
from keysight.visa_commands.timebase import KeysightTimebase
from keysight.visa_commands.waveform import KeysightWaveform
from keysight.models.scope_config import KeysightScopeConfigModel


class KeysightVisaScope(VisaScope):
    def __init__(self, scope_config: KeysightScopeConfigModel = None) -> None:
        # Not all Keysight VISA devices are like this, so we may need something more dynamic than this in the future
        scope_config.scope.visa_board_num = 0
        scope_config.scope.visa_conn_type = 'ethernet'
        scope_config.scope.visa_resource_type = 'instr'
        scope_config.scope.eth_lan_device_name = 'inst0'
    ...
    def init(self) -> None:
        ...
        try:
            ...
            self.keysight_model = self.device_model
            self.max_chs = 8 if self.keysight_model == 'MXR' else 4
    ...
    def populateVoltValueForActiveChannels(self) -> None:
        self.fetch_metadata()
        self.fetch_data()

    def fetch_data(self) -> BaseModel:
