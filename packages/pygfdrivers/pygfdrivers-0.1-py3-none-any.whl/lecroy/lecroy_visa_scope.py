...
from lecroy.visa_commands.common import LecroyCommon
from lecroy.visa_commands.acquire import LecroyAcquire
from lecroy.visa_commands.channel import LecroyChannel
from lecroy.visa_commands.trigger import LecroyTrigger
from lecroy.visa_commands.timebase import LecroyTimebase
from lecroy.visa_commands.waveform import LecroyWaveform
...
class LecroyVisaScope(VisaScope):
    ...
    def arm(self):
        ...
        try:
            ...
            if self.arm_status:
                self.is_armed = True
                self.log.info("Scope armed.")
            else:
                self.is_armed = False
                self.log.info("Scope failed to arm.")
        except Exception as e:
