...
class Detector_UnevenMultiAccum(Detector_MultiAccum):
    ...
    def _power_value(self, snr):
        ...
        snr = np.max(snr,axis=0)

        power_grid=np.zeros_like(snr)
        power_grid[np.where((5 <= snr) & (snr <=  10))] = 0.4
        power_grid[np.where((10 <= snr) & (snr <=  20))] = 1.0
        power_grid[np.where((20 <= snr) & (snr <=  50))] = 3.0
        power_grid[np.where((50 <= snr) & (snr <= 100))] = 6.0
        power_grid[np.where(snr >= 100)] = 10.0

        return power_grid

    def slope_variance(self, rate, unsat_nframes, weights="optimal"):
        """
        ...
        It is very similar to romanisim/ramp.py fit_ramps_casertano_no_dq() in RomanISim
        ...
        weights: (str)
            Control for the weighting algorithm. One of "optimal", "simple", or "unweighted"
        """
        # first off, the inputs are actually frames; we need the resultants
        unsat_nresultants = self._frames_to_resultants(unsat_nframes)
        ...
        # This portion of the code is not from Casertano 2022 or RomanISIM (which handles cosmic rays by explicitly fitting partial ramps 
        # with custom Cython code)
        # Handle cosmic rays - they should not wipe out entire resultants, just make each one a little shorter.
        excess = np.vectorize(lambda a, b: self.exposure_spec.lastframe[a] - b)(unsat_nresultants, unsat_nframes)/self.exposure_spec.nresultants
        if excess.any() > 0:
            unsat_nresultants += 1
        # Remove the excess frames per pixel from the individual nreads and ntot entries for the last unsaturated resultant
        nreads -= excess
        ntotal -= excess

        # The source variance
        variance_per_pix = rate['fp_pix_variance']
        # add a dimension. Within this function, the dimensions are (resultant, y, x)
        variance_per_pix = np.expand_dims(variance_per_pix,0)

        tframe = self.exposure_spec.tframe

        # Calculate beginning time of each read
        t0 = np.insert(np.cumsum(ntotal, axis=0), 0, np.zeros_like(unsat_nresultants), axis=0)[:-1]
        # Calculate mean times for each resultant (Eq. 9)
        tmean = tframe * t0 + tframe * (nreads - 1) / 2 # would become negative if nreads is 0
        ...
        if weights == "optimal":
            ...
            simple_snr = var_per_resultant_per_pix / np.sqrt(self.rn**2 + var_per_resultant_per_pix)
            weighting_power = self._power_value(simple_snr)
            # Add necessary dimension
            weighting_power = np.expand_dims(weighting_power,0)
            # Calculate optimal weights using Equation 45 in Casertano et al.
            # Midpoint of all resultants
            tmid = (tmean[0,:,:] + tmean[-1,:,:]) / 2
            w_ij = ((1 + weighting_power) * nreads) / (1 + weighting_power * nreads) * np.abs(tmean - tmid) ** weighting_power
        elif weights == "simple":
            # Calculate weights based on number of reads, or P = 0
            w_ij = nreads * np.ones_like(var_per_resultant_per_pix)
        elif weights == "unweighted":
            # Set all weights equal to 1
            w_ij = np.ones_like(var_per_resultant_per_pix)
        else:
        ...
        k_i = np.zeros_like(w_ij)
        for y,val in enumerate(unsat_nresultants):
            for x,val in enumerate(unsat_nresultants[y]):
                values = self.weightcalc(unsat_nresultants[y,x],w_ij[:,y,x], tmean[:,y,x])
                k_i[:,y,x] = np.pad(values, (0,len(tmean)-len(values)), 'constant', constant_values=(0))

        slope_rn_var = self.rn_variance(k_i, ntotal)

        slope_var = np.zeros_like(rate['fp_pix_variance'])
        ...
        # Casertano+22 Eq 41
        slope_var = slope_rn_var + slope_var * rate["fp_pix_variance"]
    ...
    def weightcalc(self, uns, w_ij, tmean ):
        ...
        f_2 = np.sum(w_ij[:uns] * tmean[:uns] ** 2, axis=0)
        # Casertano+22 eq 36
        d   = f_2 * f_0 - f_1 ** 2

        # Casertano+22 eq 37
        k_i = (f_0 * tmean[:uns] - f_1) * w_ij[:uns] / d
    ...
    def get_unsaturated(self, slope):
        ...
        """
        # get the number of groups to saturation
        nframes_to_sat = self._frames_before_sat(slope)
    ...
    def _frames_to_resultants(self, unsat_frames):
        """
        ...
        """
        unsat = map(lambda i:bisect.bisect(self.exposure_spec.lastframe, i), unsat_frames.ravel())
    ...
    def _get_saturation_mask(self, rate):
        ...
        """
        saturation_mask = np.zeros(rate.shape)
        unsat_nresultants = self.get_before_sat(rate)
    ...
    def rn_variance(self, k_i, total_frames=None):
        """
        Readnoise Variance
        ...
        Args:
            K_i (_type_): _description_
            unsat_resultants (_type_, optional): _description_. Defaults to None.
        """
    ...
    def _frames_before_sat(self, slope):
        ...
        # number of frames is total time / frame time. If reference frames are used, nreference will equal 1 and all of the
        # frame counts will be lower by one frame (the reference frame). A frame_before_saturation of -1 is possible if the
        # source is bright enough to saturate during the reference frame.
        frames_before_saturation = np.floor(time_to_saturation / self.exposure_spec.tframe) - self.exposure_spec.nreference
    ...
    def get_before_sat(self, slope):
        ...
        frames_before_saturation = self._frames_before_sat(slope)

        resultants_before_saturation = self._frames_to_resultants(frames_before_saturation)
    ...
    def calc_cr_loss(self, nframes, pix_cr_rate):
        ...
        # the unsaturated groups. We now simplify this (the difference is minimal and would add some complex logic now that the time
        # formulae are detector type dependent). This is simple.
        cr_nframes = (1.0 - ramp_frac * pix_cr_rate * self.exposure_spec.saturation_time) * nframes
