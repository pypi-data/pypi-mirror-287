...
def login(username, password, print='no'):
    login_data1 = {"identity":username,"scene":"","pid":"65edCTyg","deviceId":"","timestamp":time.time()}
    print(requests.post('https://open-service.codemao.cn/captcha/rule/v3',json=login_data1))
...
def comment_w(workid, data, print='no'):
    ...
    result = session().post("https://api.codemao.cn/creation-tools/v1/works/%d/comment" % workid,json=data)
    global get
