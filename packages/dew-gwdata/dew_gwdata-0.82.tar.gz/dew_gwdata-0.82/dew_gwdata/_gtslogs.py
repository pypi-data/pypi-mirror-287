...
def list_geophys_job_files(
    ...
    if sageodata_conn is None:
        df = list_geophys_job_files_from_gtslogs(job_nos=job_nos, job_paths=job_paths)
    else:
        df = sageodata_conn.list_geophys_log_db_files(job_nos)
    ...
    def apply_las_metadata(row):
        if Path(row.path).is_file() and row.file_type == "LAS":
            ...
            if contents:
                ...
                print(f"Error getting cotents of {row.path}")
                las_md = None
        else:
            las_md = None
...
def list_geophys_job_files_from_gtslogs(job_nos=None, job_paths=None):
    files_dfs = []
