...
class LDataFile:
    ...
    def open(self):
        ...
        self.fs.path_for_node_cache[node_id].add(str(self.path))
...
class LDataFS(Fuse):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.path_for_node_cache: dict[str, set[str]] = defaultdict(set)
        ...
        while not self.exiting:
            ...
            while not self.ws_ctx.sock.closed:
                try:
                    ...
                    self.path_for_node_cache = defaultdict(set)
                    break
                except Exception:
                    traceback.print_exc()
    ...
    async def load_subtree_by_path_async(self, path: str, *, use_cache: bool = True):
        ...
        self.path_for_node_cache[node_id].add(path)
    ...
    def invalidate_cache(self, path: str):
        ...
        while len(to_process) > 0:
            ...
            for path in self.path_for_node_cache[node_id]:
                self.ldata_path_cache.pop(path, None)

            if node_id in self.ldata_node_cache:
                for child in self.ldata_node_cache[node_id].values():
                    to_process.append(child["id"])
                self.ldata_node_cache.pop(node_id, None)

            self.path_for_node_cache.pop(node_id, None)
