...
def heaveCF(om0,beta_deg,U,L,B0,T,C_B=1):
    ...
    for i, beta_i in enumerate(beta):
        ...
        eta = 1/np.sqrt((1-2*k*T*alpha**2)**2+(A**2/(k*B*alpha**2))**2)
...
def pitchCF(om0,beta_deg,U,L,B0,T,C_B=1):
    ...
    for i, beta_i in enumerate(beta):
        ...
        G = kappa*f*24/((ke*L)**2*L)*(np.sin(ke*L/2)-ke*L/2*np.cos(ke*L/2))
        eta = 1/np.sqrt((1-2*k*T*alpha**2)**2+(A**2/(k*B*alpha**2))**2)
...
def rollCF(om0,beta_deg,U,L,B0,T,C_B,C_WP,GM_T,kappa,mu,T_N=0):
    ...
    for i, beta_i in enumerate(beta):
        ...
        b44_0 = rho*A0*B0**2*np.sqrt(2*g/B0)*aBT0*np.exp(bBT0*ome**(-1.3))*ome**dBT0
        b44_0[np.where(b44_0<1e-100)] = 1e-100
        b44_1 = rho*A1*B1**2*np.sqrt(2*g/B1)*aBT1*np.exp(bBT1*ome**(-1.3))*ome**dBT1
        kappa = np.sqrt(1+(b44_1-b44_0)/b44_0)
        B44_invisc = L*b44_0*(delta+kappa**2*(1-delta)) # Roll hydrodynamic damping
        B44_tot = B44_invisc+mu*B44_crit
        
        # Amplitude of the roll excitation moment:
        M_abs = np.abs(np.sin(beta_i))*np.sqrt(rho*g**2/ome)*2/ke*\
                np.sqrt(b44_0)*np.sqrt(np.sin(0.5*delta*L*ke)**2+\
                kappa**2*np.sin(0.5*(1-delta)*L*ke)**2+\
                2*kappa*np.sin(0.5*delta*L*ke)*np.sin(0.5*(1-delta)*L*ke)*\
                np.cos(0.5*L*ke))
        
        roll[start_om:,i] = M_abs/np.sqrt((-ome**2*(T_N/(2*np.pi))**2+1)**2*C44**2+ome**2*B44_tot**2)
