...
.. dropdown:: Copyright (C) 2024 Technical University of Denmark, R.E.G. Mounet
    :color: primary
    :icon: law
...
def norm_resp(Rxy_ReIm,TRF,S1D,f,mn=None,ReIm=None,Na=None,axy=None):
    ...
    if mn==None:                        # Indicate the indices of considered response pairs
        mn = np.array([[ix,ix] for ix in range(Nresp)]+[list(x) for x in itertools.combinations(range(Nresp), 2)]*2)

    if ReIm==None:                      # Indicates whether the real or the imaginary part is considered
        ReIm = np.array(['R']*2*Nresp+['I']*Nresp).reshape((-1,1))  
    else:
        ReIm = np.reshape(ReIm,(-1,1))
    
    if Na==None:                        # Number of the ensembled average
        Na = np.ones((Nxy,))
    elif np.size(Na)==1:
        Na = Na*np.ones((Nxy,))                                                
    
    if axy==None:                       # Additional weights
        axy = np.zeros((Nxy,))
    else:
        axy = np.abs(axy)
    ...
    for ixy in range(Nxy):
        mn_ixy = mn[ixy]
        index_xx_Re = np.where(np.all((mn==[mn_ixy[0],mn_ixy[0]])*(ReIm=='R'),axis=1))[0][0]
        index_yy_Re = np.where(np.all((mn==[mn_ixy[1],mn_ixy[1]])*(ReIm=='R'),axis=1))[0][0] 
        ...
        if ReIm[ixy] == 'R': # Case 1: real part
            ...
            Wmn[:,ixy] = np.sqrt((Rxy_ReIm[:,index_xx_Re]*Rxy_ReIm[:,index_yy_Re] +\
                                    np.square(Rxy_ReIm[:,index_xy_Re]) -\
                                    np.square(Rxy_Im)) / (2*(Na[ixy]+axy[ixy])))
            ...
            Wmn[:,ixy] = np.sqrt((Rxy_ReIm[:,index_xx_Re]*Rxy_ReIm[:,index_yy_Re] -\
                                    np.square(Rxy_Re) +\
                                    np.square(Rxy_ReIm[:,index_xy_Im])) / (2*(Na[ixy]+axy[ixy])))
    ...
    for ixy in range(Nxy):
        mn_ixy = mn[ixy]
        if ReIm[ixy] == 'R':
            Hn[:,:,ixy] = np.real(TRF[mn_ixy[0],:,:]*np.conj(TRF[mn_ixy[1],:,:]))/np.expand_dims(Wmn[:,ixy],0)
        if ReIm[ixy] == 'I':
            Hn[:,:,ixy] = np.imag(TRF[mn_ixy[0],:,:]*np.conj(TRF[mn_ixy[1],:,:]))/np.expand_dims(Wmn[:,ixy],0)
...
def emep(Sxyn, Hn, theta, fi, k, opt=None):
    ...
    XY = np.zeros((2*maxModelOrder, M))      # Matrix to store the negative Jacobian in the iteration
    ...
    for ff in k:
        ...
        while not stop:
            ...
            exponent = np.dot(coef[0:N], cosn[0:N,:]) + np.dot(coef[N:2*N], sinn[0:N,:])
            a0 = -np.max(exponent)
            Fn = np.exp(a0+exponent)
            Dn = np.tile(Fn.reshape((-1,1))/(trapezoid(Fn)*dtheta), (1,M))  # Fn(theta|f)/norm(Fn)=Dn(theta|f)
            PhiHD = (Phij - Hj) * Dn
            ...
            while not (stop or modelFound):
                ...
                for ix in range(N):
                    XY[ix,:] = \
                        (Z*trapezoid(Dn*cosnt[:,:,ix], axis=0) - trapezoid(PhiHD*cosnt[:,:,ix], axis=0))*dtheta
                    XY[N+ix,:] = \
                        (Z*trapezoid(Dn*sinnt[:,:,ix], axis=0) - trapezoid(PhiHD*sinnt[:,:,ix], axis=0))*dtheta

                deltaCoefOld = deltaCoef[0:2*N]

                # Perform the least-square method for sum((Z - deltacoef.T @ XY)**2):
                deltaCoef[0:2*N] = np.linalg.lstsq(XY[0:2*N,:].T,Z,rcond=None)[0]
                ...
                if maxCoef < np.inf:
                    ...
                    if len(k0) > 0:
                        deltaCoef[k0] = \
                            (np.sign(deltaCoef[k0])*maxCoef-(coef[k0]-lambda_*deltaCoef[k0]))/lambda_
                ...
                if np.all(np.abs(deltaCoef[0:2*N]) < coefAbsTol) or (error1 <= errorTol):
                    modelFound = 1
                    if display>0:
                        print('Model found.')
                        
                elif count > maxIter or (
                        (error2 < error1) and (np.any(np.abs(deltaCoef[:2*N] - deltaCoefOld) > coefAbsTol2))):
                    if lambda_ > Li*2**(-4):
                        lambda_ = lambda_*0.5

                        if display>0:
                            print('Coefficient divergence detected. Relaxing to lamdba_= %.3f'%lambda_)

                        count = 0
                        deltaCoef[0:2*N] = 0
                        coef[:2*N] = np.zeros((2*N,)) # np.concatenate((coefOld[:(N-1)],np.array([0]),coefOld[(N-1):2*(N-1)],np.array([0])))
        ...
        if display > 0:
            print(f'f = {fi[ff]} \t \t Model order = {Nbest} \t \t error = {np.max(np.abs(Z))}')

        exponent = np.dot(coef[0:Nbest], cosn[0:Nbest, :]) + np.dot(coef[Nbest:2*Nbest], sinn[0:Nbest,:])
        a0 = -np.max(exponent) # Trick in order to avoid infinities
