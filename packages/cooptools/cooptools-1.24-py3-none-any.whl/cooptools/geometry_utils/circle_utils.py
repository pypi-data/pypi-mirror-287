...
def rads_of_point_around_origin(a: vec.FloatVec,
                                origin: vec.FloatVec = None) -> float:
    if origin is None:
...
def degrees_of_point_around_origin(a: vec.FloatVec,
                                   origin: vec.FloatVec = None) -> float:
    return rads_to_degrees(rads_of_point_around_origin(a, origin))


def rads_between(v: vec.FloatVec,
                 start: vec.FloatVec = None,
                 origin: vec.FloatVec = None,
...
def degrees_between(v: vec.FloatVec,
                    start: vec.FloatVec= None,
                    origin: vec.FloatVec = None,
...
def circles_from_2tanline_and_tanpoint(
    ...
    intersect = line_intersection_2d(line.points_on_a_slope_intercept(tan_line1), line.points_on_a_slope_intercept(tan_line2), extend=True)
...
def point_of_right_triangle(o: vec.FloatVec,
    ...
    rads = rads_of_point_around_origin(vec.vector_between(o, base), o)
...
if __name__ == "__main__":
    ...
    def test_3():
        ...
        ####### UNDER TEST
        #######
        pt_on_line1 = line.point_is_on_line_2d(desired_pt, linepts=(desired_pt, vec.add_vectors([desired_pt, desired_v])))
        ...
        _plot(
            ...
            lines={
                ((desired_pt[0] - 3, line.eval(bisecting_line_1, desired_pt[0] - 3)), (desired_pt[0] + 3, line.eval(bisecting_line_1, desired_pt[0] + 3))): ((),{'linestyle': 'dotted', 'color': 'red'}),
                ((desired_pt[0] - 3, line.eval(bisecting_line_2, desired_pt[0] - 3)), (desired_pt[0] + 3, line.eval(bisecting_line_2, desired_pt[0] + 3))): ((), {'linestyle': 'dotted', 'color': 'red'}),
