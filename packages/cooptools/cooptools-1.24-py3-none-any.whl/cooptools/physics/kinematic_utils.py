...
def _accel_to_stop(v,
    ...
    return -v ** 2 / (2 * (stopping_distance - increment_buffer * v))
...
def _calculate_seconds_to_decel_data(
    ...
    for ii, val in enumerate(vector_to_goal):
        ...
        s2d = seconds_until_decel(
            v=velocity[ii] * mod + increment_buffer * accel_magnitude,
            a=accel_magnitude,
            x=0 + (velocity[ii] * mod) * increment_buffer + .5 * accel_magnitude * increment_buffer ** 2,
...
def decide_how_to_accel_decel_towards_a_goal_without_overshoot(
    ...
    ideal_accel = []
    magnitude_pool = accel_magnitude
    small_adjustment = 0.0001
    for ii in range(len(vector_to_goal)):
        mod = 1
        if vector_to_goal[ii] < 0:
            mod = -1


        if math.isclose(magnitude_pool, 0, abs_tol=1e-6):
            ideal_accel.append(0)
            logger.info(f"no acccel in pool at ind {ii}")
        elif seconds_until_decel_vector[ii] is None:
            ideal_accel.append(0)
            logger.info(f"have reached dest in dimension at ind {ii}")
        elif math.isclose(vector_to_goal[ii], 0, abs_tol=reached_threshold) and math.isclose(velocity[ii], 0, abs_tol=stopped_threshold):
            ideal_accel.append(0)
            logger.info(f"close and stopped at ind {ii}")
        elif abs(vector_to_goal[ii]) < dist_linear_stop_threshold and abs(velocity[ii]) < velo_linear_stop_threshold:
            a = _accel_to_stop(
                v=velocity[ii],
                stopping_distance=abs(vector_to_goal[ii])
            )
            used = min(a, magnitude_pool) * mod - small_adjustment
            logger.info(f"accel to stop: {a} [out of avail {magnitude_pool}] at ind {ii}")
            ideal_accel.append(used)
            magnitude_pool -= used
        elif seconds_until_decel_vector[ii] > 0:
            ideal_accel.append(magnitude_pool * mod)
            logger.info(f"throw all avail pool at ind {ii}")
            magnitude_pool = 0
        else:
            #TODO: Theoretically, we should be able to use the exact stopping accel, rather than the -a_amplitude. However, didtn work first try and didnt care to spend more
            # time on it
            # a = _accel_to_stop(
            #     v=velocity[ii],
            #     stopping_distance=abs(vector_to_goal[ii])
            # ) * mod
            # used = min(a, magnitude_pool)
            # ideal_accel.append(used)
            # magnitude_pool -= used
