...
class GoalSeeker:
    ...
    def update(self,
        ...
        if goal_pos is None:
            goal_pos = self._mover.Transform.Translation.Vector
        self._goal = goal_pos

        ''' Calculate Vector to goal'''
        vector_to_goal = vec.vector_between(self._mover.Transform.Translation.Vector, goal_pos)
        ...
        for ii in range(len(self.Velocity.CurrentVelocity_M_S)):
            if (self.Velocity.CurrentVelocity_M_S[ii] < slow_enough_tolerance and
                ...
                self.Transform.Translation.update(idx_val_map={ii: goal_pos[ii]})

        if close and stopped:
            # self._mover.Transform.Translation.update(goal_pos)
            # self._mover.Velocity.set(m_s_vec=(0, 0))
            # self._mover.Acceleration.set(m_s2_vec=(0, 0))
            logger.info(f'Mover: {self.Name} has reached goal: {goal_pos}')
            return True
        ...
        self._mover.Acceleration.set(m_s2_vec=kin.decide_how_to_accel_decel_towards_a_goal_without_overshoot(
            ...
            reached_threshold=close_enough_tolerance,
            stopped_threshold=slow_enough_tolerance
        ))
