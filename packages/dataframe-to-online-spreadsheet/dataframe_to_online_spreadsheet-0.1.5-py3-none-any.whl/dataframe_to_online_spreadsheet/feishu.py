        ...
        for i in range(0, self._obj.shape[0], max_size):
            ...
            data = {
                "valueRanges": [
                    {
                        "range": body_range,
                        "values": [[(int(x) if isinstance(x, np.int64) else (None if pd.isnull(x) else (None if isinstance(x, pd.Timestamp) and pd.isnull(x) else (x.strftime('%Y-%m-%d %H:%M:%S') if isinstance(x, pd.Timestamp) else x)))) for x in df.loc[index]] for index in df.index]
                    }
                ]
            }
