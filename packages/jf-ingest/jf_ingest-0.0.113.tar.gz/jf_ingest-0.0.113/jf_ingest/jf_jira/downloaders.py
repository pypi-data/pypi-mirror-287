...
def search_users(
    ...
    if gdpr_active and not using_connect_as_primary_auth:
        ...
        jira_users = _search_users_by_letter(
            jira_connection, search_users_by_letter_email_domain, page_size=page_size, using_connect_as_primary_auth=using_connect_as_primary_auth
        )
...
def _search_users_by_letter(
    ...
    jira_users_dict = {_jira_user_key(u, using_connect_as_primary_auth): u for u in non_deduped_jira_users}
...
def _expand_changelog(
    ...
    if not server_info.get('deploymentType') == 'Cloud':
        logger.info(f'Jira server at {jira_connection.server_url} does not support expanding changelogs')
        return jira_issues
