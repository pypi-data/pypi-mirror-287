...
def _get_jellyfish_custom_fields_default_response() -> dict:
    return json.loads(
        get_fixture_file_data(
            fixture_path=os.path.join('api_responses', 'jellyfish_custom_fields.json')
        )
    )
    ...
    return json.loads(
        get_fixture_file_data(
            fixture_path=os.path.join(
                'api_responses', 'jellyfish_custom_fields_empty_response.json'
            )
        )
    )
...
def test_custom_fields_max_issues_to_retrieve():
    config = _get_ingestion_config()
    jf_response = _get_jellyfish_custom_fields_default_response()
    empty_response = _get_jellyfish_custom_fields_empty_response()
    ...
    with requests_mock.Mocker() as mocker:
        ...
        q = queue.Queue()
        _retrieve_custom_fields_from_jellyfish(config, q, max_issues_to_retrieve=1)
        assert q.qsize() == 1
        output0 = q.get()
...
def test_custom_fields_raise_for_status():
    config = _get_ingestion_config()
