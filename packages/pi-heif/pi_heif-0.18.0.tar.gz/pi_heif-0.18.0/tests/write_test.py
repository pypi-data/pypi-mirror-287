...
def test_heif_no_encoder():
    ...
    with mock.patch("pi_heif.heif._pi_heif.get_lib_info", side_effect=get_lib_info):
...
def test_input_chroma_value():
    ...
    im = pi_heif.open_heif(Path("images/heif_other/RGB_8_chroma444.heif"))
    assert im.info["chroma"] == 444
    im = Image.open(Path("images/heif_other/pug.heic"))
    assert im.info["chroma"] == 420
    im = pi_heif.open_heif(Path("images/heif_other/pug.heic"))
    assert im.info["chroma"] == 420
...
def test_invalid_encoder():
    ...
    buf = BytesIO()
    try:
        pi_heif.options.PREFERRED_ENCODER["AVIF"] = "invalid_id"
        pi_heif.options.PREFERRED_ENCODER["HEIF"] = "invalid_id"
        im_rgb.save(buf, format="AVIF")
        im_rgb.save(buf, format="HEIF")
    finally:
        pi_heif.options.PREFERRED_ENCODER["AVIF"] = ""
        pi_heif.options.PREFERRED_ENCODER["HEIF"] = ""


@pytest.mark.skipif("svt" not in pi_heif.libheif_info()["encoders"], reason="Requires SVT AVIF encoder.")
@pytest.mark.skipif("aom" not in pi_heif.libheif_info()["encoders"], reason="Requires AOM AVIF encoder.")
def test_svt_encoder():
    ...
    buf_svt = BytesIO()
    try:
        pi_heif.options.PREFERRED_ENCODER["AVIF"] = "svt"
...
@pytest.mark.skipif("rav1e" not in pi_heif.libheif_info()["encoders"], reason="Requires RAV1E AVIF encoder.")
@pytest.mark.skipif("aom" not in pi_heif.libheif_info()["encoders"], reason="Requires AOM AVIF encoder.")
def test_rav1e_encoder():
    ...
    buf_rav1e = BytesIO()
    try:
        pi_heif.options.PREFERRED_ENCODER["AVIF"] = "rav1e"
