...
class Cardano(Blockchain):
    def __init__(self, preprod: bool = False, gomaestroorg_api_key: str = None):
        super().__init__(
            name=CARDANO,
            currency=ADA,
    ...
    def check_account(self, account: str):
        if self.preprod:
            is_account = utils.is_preprod_account(account)
        else:
            is_account = utils.is_account(account)
    ...
    def get_account(self, wallet: str) -> Optional[str]:
        if not utils.is_account(wallet):
            return None

        return self.get_account_stake_address(wallet)
    ...
    def get_account_balance(self, account: str) -> Tuple[float, str]:
        stake_address = self.get_account_stake_address(account)
    ...
    def get_account_transactions(self, account: str, last: int = 10) -> Iterable[TransactionDTO]:
        def parse_input_output(items: List[dict]) -> Iterable[TransactionInputOutputDTO]:
            for item in items:
        ...
        for address, tx in gomaestroorg.get_account_transactions(stake_address, api_key=self.gomaestroorg_api_key, preprod=self.preprod):
            transactions.append((address, tx))
        ...
        for address, tx in sorted_transactions[:last]:
            ...
            if tx.get("metadata"):
                if tx["metadata"].get("674"):
                    if tx["metadata"]["674"].get("msg"):
                        message = "\n".join(tx["metadata"]["674"]["msg"])
    ...
    def get_asset_rank(self, collection_id: str, asset_id: str, *args, **kwargs) -> RankDTO:
        ...
        number = int("".join((c for c in decoded_asset_name if c.isdigit())))
        ...
        **kwargs,
        ...
        for asset in collection_assets:
            ...
            yield AssetDTO(
                ...
                supply=int(asset.get("total_supply", 1)),
            )
    ...
    def get_collection_snapshot(self, collection_id: str, discriminator: str = None, max_threads: int = 3, *args, **kwargs) -> Iterable[HoldingDTO]:
        ...
        collection_owners = gomaestroorg.get_policy_owners(
            collection_id,
            api_key=self.gomaestroorg_api_key,
            preprod=self.preprod,
        )
    ...
    def verify_otp(self, account: str, otp: int) -> TransactionDTO:
        for transaction in self.get_account_transactions(account):
            ...
            for tx_output in transaction.outputs:
                if tx_output.address != account:
                    continue

                for tx_asset in tx_output.assets:
                    if tx_asset.unit != "lovelace":
                        continue
