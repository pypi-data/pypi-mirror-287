...
class PositionScheduler(NamedTuple):
    ...
    def from_report(cls, report, total_weight):
        return cls(report.weights, report.next_weights,
                   report.actions, report.next_trading_date,
                   report.trade_at, report.market_info, report.stop_loss or 1, report.take_profit or np.inf,
    ...
    def _from_json_leaf(cls, pdata):
        ...
        weight.name = 'weight'
        nextWeight = pp_df.nextWeight
        nextWeight.name = 'next_weight'
        ...
        market_info = TWMarketInfo()
        # else:
        #     raise ValueError('Unknown market')


        return cls(weight, nextWeight, actions, next_trading_date, pdata['positionConfig']['entryTradePrice'], market_info, 
                   pdata['positionConfig'].get('sl', 1), pdata['positionConfig'].get('tp', np.inf), pdata['positionConfig'].get('ts', np.inf),
                   total_weight)
...
class Portfolio(Report):
    ...
    def __init__(self, reports: Union[Dict[str, Tuple[Report, float]], Report]):
        """建構 Portfolio 物件。
            ...
            Example:
                ...
                **B. 將單一策略轉換成 Portfolio**
                ...
                report_strategy = sim(...)
                portfolio = Portfolio(report_strategy)
                ```
        """
        ...
        market_info = sample_report.market_info

        super().__init__(creturn, position, fee_ratio, tax_ratio,
                         trade_at, next_trading_date, market_info)
    ...
    def from_weight_function(cls, reports, weight_function):
        ...
        weight_function: Callable[[Dict[str, Report]], Dict[str, float]]
        """
    ...
    def position_info2(self):
        ...
        for name, (report, weight) in self.portfolio.items():
            ...
            pinfo['market'] = report.market_info.get_name()
            ...
            # handle recursive position
                ...
                for sub_name, sub_position in pinfo.items():
                    ...
                    position[f'{name}_{sub_name}'] = sub_position
