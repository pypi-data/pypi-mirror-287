...
class PortfolioSyncManager():
    ...
    Examples:
        ```py
        from finlab.online.order_executor import Portfolio

        # 初始化投資組合
        portfolio = Portfolio()

        # 設定投資組合
        portfolio.set({'策略A': (1000000, report1), '策略B': (1000000, report2)})
    ...
    def __init__(self, data=None):
        ...
        market_name = self.data.get('market', None)

        if market_name == 'tw_stock':
            self.market = TWMarketInfo()
        else:
            self.market = None
    ...
    def from_local(cls, name='default'):
        ...
        if not os.path.exists(path):
            raise ValueError(f'path {path} not existed')
    ...
    def from_cloud(cls, name='default'):
        ...
        data = res.json()
        if data == None:
            raise ValueError(f'path {name} not existed')
        return cls(data)
    ...
    @staticmethod
    def calc_position_value(position, market):
        """
        計算持倉價值。
        ...
        ret = 0
        lot = 1000  # market.get_odd_lot()
        price = market.get_price('close', adj=False).iloc[-1]

        for p in position:
            if p['stock_id'] in price:
                ret += p['quantity'] * price[p['stock_id']] * lot
            else:
    ...
    def update(self, portfolio: Portfolio, total_balance=0, rebalance_safety_weight=0.2, smooth_transition=True, force_override_difference=False, **kwargs):
        """
        設定投資組合的函數。
        ...
        Args:
            ...
            smooth_transition (bool): 是否平滑過渡，當新增減少策略，以及權重更新時，部位不會立即更新，而是在每個策略的到期日才進行更新，預設為 True。
        ...
        for sid in position:
            if sid not in portfolio:
                ...
                if smooth_transition and next_trading_time < now:
                    logger.info(f'{sid} not in portfolio, wait for next trading time to remove')
                    continue

                logger.info(f'{sid} not in portfolio, remove it')
                position.pop(sid)
        ...
        for sid, allocation in portfolio.items():
            ...
            market = allocation.market_info
            weight = allocation.total_weight
            self.data['market'] = market.get_name()
            ...
            if (now > next_trading_time) or (not smooth_transition and soft_condition):
        ...
        for sid, info in position.items():
            if sid not in updated_strategies:
                asset_value += self.calc_position_value(
                    info['position'], market=market)
        ...
        free_value = liquid_value - max(0, rebalance_safety_fund - cash)
        ...
        for sid in updated_strategies:
            ...
            fund = free_value * weight / total_update_weight

            # calc next trading time
            next_trading_time = report.market_info.market_close_at_timestamp(
        ...
        for sid, allocation in portfolio.items():
            ...
            next_trading_time = allocation.market_info.market_close_at_timestamp(
                allocation.next_trading_date)
            ...
            if allocation.actions.isin(['sl_', 'tp_', 'sl', 'tp']).any():
                ...
                for s in allocation.actions[allocation.actions.isin(['sl_', 'tp_', 'sl', 'tp'])].index.tolist():
                    for p in position[sid]['position']:
                        if p['stock_id'] == s and p['quantity'] != 0:
                            ...
                            position[sid]['stop'][-1]['reason'] = allocation.actions.loc[s]
                            p['quantity'] = 0
    ...
    def get_position(self, combined=True):
        """
        獲取持倉資訊。

        Args:
            combined (bool, optional): 是否合併持倉資訊。預設為 True。
        ...
        if combined:
            position = self.data['position']
            return sum([Position.from_list(p['position']) for p in position.values()], Position({}))

        return copy.deepcopy(self.data['position'])
    ...
    def clear(self):
        ...
        Returns:
            None
        """
        self.data = {'position': {}, 'history': {}, 'market': None}

    def sync(self, account, consider_margin_as_asset=True, **kwargs):
        """
        同步持倉資訊。
        ...
        Returns:
            None
        """
        acp = account.get_position()
        idp = self.get_position()

        if consider_margin_as_asset:
            idp = self.margin_cash_position_combine(acp, idp)

        OrderExecutor(idp, account).create_orders(**kwargs)

    def create_order_executor(self, account, same_direction_preserve=False, **kwargs):
        """
        同步持倉資訊。
        ...
        if same_direction_preserve:
            idp = self.margin_cash_position_combine(acp, idp)

        return OrderExecutor(idp, account)
    ...
    def __repr__(self):
        ...
        if self.market:
            ref_price = self.market.get_reference_price()
            close_price = self.market.get_price(
                'close', adj=False).iloc[-1].to_dict()
            volume = self.market.get_price(
                'volume', adj=False).iloc[-1].to_dict()

        total_allocation = sum([v['allocation'] for v in self.data['position'].values()])

        pos = self.get_position(combined=False)
        ...
        if self.market:
            ...
            df = df[["stock_id", "quantity", "strategy", "type"]]\
                .sort_values('stock_id').set_index('stock_id')

        if self.market:
            ret = f'Estimate value {int((df.price * df.quantity * 1000).sum())}'
        else:
            ret = 'Position'
        ...
        df2['weight'] = (df2.allocation / df2.allocation.sum()).map('{:.2%}'.format) +  '/' + df2['weight'].map('{:.2%}'.format)
