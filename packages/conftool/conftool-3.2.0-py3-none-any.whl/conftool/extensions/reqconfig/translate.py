...
class HAProxyDSLTranslator:
    ...
    def from_expression(self, expression: str) -> str:
...
class HAProxyACLTranslator:
    ...
    def from_ipblocks(self, ipblocks: List[str]) -> str:
        ...
        for ipblock in sorted(ipblocks):
            ...
            out.append(f'acl ipblock_{scope}_{value} src,map_ip({scope}) -m str "{value}"')
        return "\n".join(out)
...
class HAProxyExpression:
    ...
    def haproxy_expression(self, patterns_registry: Dict[str, List[str]]) -> str:
        ...
        # Step 1: expand all patterns and convert the expression to a symbolic one
        # format
        symbolic = self.to_symbol(self.expanded_expression(patterns_registry))
        # Step 2: resolve all parentheses and distribute and over or
        # using sympy
        try:
            parsed = parse_expr(symbolic)
            dnf = to_dnf(parsed, True)
