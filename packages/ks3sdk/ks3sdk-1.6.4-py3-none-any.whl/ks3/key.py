...
class Key(object):
    ...
    def size(self):
        if self._size is None and self.bucket:
            key = self.bucket.get_key(self.name)
            if key:
                self._size = key.size
        return self._size
    ...
    async def upload_file_async(self, filename, headers=None, replace=True, cb=None, num_cb=10, policy=None,
                                md5=None,
        ...
        return await self.set_contents_from_data_async(data, headers, replace, cb, num_cb, policy, md5,
                                                       reduced_redundancy,
                                                       encrypt_key=encrypt_key, calc_encrypt_md5=calc_encrypt_md5,
                                                       query_args=query_args)
    ...
    async def set_contents_from_data_async(self, data, headers=None, replace=True,
                                 cb=None, num_cb=10, policy=None, md5=None,
                                 reduced_redundancy=False, method="PUT",
                                 encrypt_key=False, calc_encrypt_md5=True, query_args=None):
        """
        Store an object in S3 using the name of the Key object as the
        key in S3 and the string 's' as the contents.
    ...
    async def download_file_async(self, filename, byte_range=None, headers=None,
                                 ...
                                 res_download_handler=None,
                                 response_headers=None):
        data = await self.get_contents_as_data_async(byte_range, headers, cb, num_cb, torrent=torrent,
                                                     version_id=version_id, response_headers=response_headers)
        await _write_file_async(filename, data)

    async def get_contents_as_data_async(self, byte_range=None, headers=None,
                               cb=None, num_cb=10,
                               torrent=False,
                               version_id=None,
    ...
    def restore_object(self, headers=None):
        self.bucket.restore_object(self.name, headers=headers)
