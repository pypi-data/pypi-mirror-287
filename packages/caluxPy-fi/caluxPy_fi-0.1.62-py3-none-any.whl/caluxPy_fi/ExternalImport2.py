...
class ExternalImport:
    ...
    def __init__(self, file = '', df = '', file_type = '', initial_row = 0, columns = ''):#, repos = False, repos_attributes = [], medida = False):
        ...
        columna = list(self.df.columns.values)
        dicErrores = {}
        ...
        for i in range(len(self.df.columns)):
            ...
            if columna[i][0] == 'd': #--> Dates
                ...
                self.df[columna[i]] = pd.to_datetime(self.df[columna[i]], format='%Y-%m-%d', errors='coerce').dt.date # 1
            ...
            #1 - Primero se convierte todo a float, si hay errores se convierten a NaM
                ...
                self.df[columna[i]] = pd.to_numeric(self.df[columna[i]], errors = 'coerce') # 1
                for cell in self.df[columna[i]]:
                    if str(cell) == 'nan':
                        if columna[i] == 'n_amortizationTimes':
                            if self.df['t_paymentType'][counter] == 'bullet':
                                self.df.iloc[counter, i] = np.nan
                            else:
                ...
                for cell in self.df[columna[i]]:
                    if cell == '':
                        if columna[i] == 'p_coupon' or columna[i] == 'p_ytm':
                            ...
                            if columna[i] == 'p_porcentaje_amortizable' and self.df['t_paymentType'][counter] != 'bullet':
                                logger.info(f'MissingValue: -línea {counter}- en -{columna[i]}- no hay tasa especificada - Registro se sacará de la data.\n')
                                errores.append(counter)
                    else: 
                        if type(cell) != float and type(cell) != int:
                            ...
                            if '%' in dato:
                                ...
                                self.df.iloc[counter, i] = float(dato.strip('%')) / 100
                            else:
                            ...
                            if cell < 0:
                                logger.info(f'ValueError: -línea {counter}- en -{columna[i]}- es menor que 0 - Registro se convertirá a positivo.\n')
                                self.df.replace(self.df[columna[i]][counter], -cell, inplace = True)
                            elif cell > 1:
                                logger.info(f'ValueError: -línea {counter}- en -{columna[i]}- es mayor que 1 - Registro se dividirá por 100.\n')
                                self.df.replace(df[columna[i]][counter], cell / 100, inplace = True)
                    counter += 1
            elif columna[i] == 't_paymentType':
                for cell in self.df[columna[i]]:
                    if cell == 'bullet':
                        self.df.iloc[counter, i + 4 : i + 4 + 3] = np.nan
                    counter += 1
            elif columna[i] == 's_couponType':
                if cell == 'normal':
                    self.df.replace(self.df.iloc[counter, i + 1], '', inplace = True)
        ...
        #self.importedData = self.df.to_dict('records')
