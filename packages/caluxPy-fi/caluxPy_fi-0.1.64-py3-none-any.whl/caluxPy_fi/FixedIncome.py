...
class FixedIncome:
        ...
        try:
            if self.maturity_date and self.settlement_date and self.maturity_date >= self.settlement_date:
                ...
                if self.coupon_type == 'normal':
                    ...
                    '''if not forward_date in ['', None, np.nan, pd.NaT]: 
                        self.forward_date = self.ensure_date_type(forward_date) 
                    else:
                ...
                if self.payment_type == 'bullet':
                    ...
                    '''if amortization_start_date not in ['', None, pd.NaT] and not np.isnan(amortization_start_date):
                        try:
                            self.amortization_start_date = self.ensure_date_type(amortization_start_date)
                        except Exception as e:
                            self._logger.error(f'Error ensuring date type for amortization start date => {e}')
                            raise ValueError('Invalid amortization start date specified!')
                    else: 
                        error_message = 'Amortization date not specified!'
                    ...
                    '''if amortization_periods not in ['', None] and not np.isnan(amortization_periods):
                        try:
                            # Convert amortization_periods to integer
                            self.amortization_periods = int(amortization_periods)
                        except Exception as e:
                            # Log error if conversion fails
                            self._logger.error(f'Error converting amortization periods to integer => {e}')
                            raise ValueError(f'Invalid amortization periods value: {amortization_periods}')
                    else:
                        # Log error and raise exception if amortization_periods is not specified
                        error_message = 'Amortization periods not specified!'
                        self._logger.error(error_message)
                        raise ValueError(error_message)'''
                    ...
                    '''if amortization_periodicity not in ['', None] and not np.isnan(amortization_periodicity):
                        try:
                            # Convert amortization_periodicity
                            self.amortization_periodicity = Support.periodicityConversion(per = amortization_periodicity, mode = 'amortization')
                        except Exception as e:
                            # Log error if conversion fails
                            self._logger.error(f'Error converting amortization periodicity => {e}')
                    else:
                        # Log error and raise exception if amortization_periodicity is not specified
                        error_message = 'Amortization periodicity not specified!'
                        self._logger.error(error_message)
                        raise ValueError(error_message)'''
                    ...
                    '''if amortizable_percentage not in ['', None] and not np.isnan(amortizable_percentage):
                        try:
                            # Convert amortizable_percentage
                            self.amortizable_percentage = Support.rateConversion(rate = amortizable_percentage)
                        except Exception as e: 
                            # Log error if conversion fails
                            self._logger.error(f'Error converting amortization rate => {e}')
                    else:
                        # Log error and raise exception if amortizable_percentage is not specified
                        error_message = 'Amortization percentage not specified!'
                        self._logger.error(error_message)
                        raise ValueError(error_message)'''
    ...
    def validate_and_convert(self, value, value_name, convert_func=None, logger=True, raise_exception=True):
        ...
        """
        is_invalid = value in ['', None, pd.NaT] or np.isnan(value)
