...
class FixedIncome:
        ...
        try:
            if self.maturity_date >= self.settlement_date:
                ...
                if self.payment_type == 'bullet':
                    ...
                    if not amortization_start_date in ['', None]:
    ...
    def ensure_date_type(self, date):
        ensuredDate = None
        
        try:
            if isinstance(date, str):
                ensuredDate = datetime.strptime(date, self.date_format).date()
            elif isinstance(date, datetime):
                ensuredDate = date.date()
            elif isinstance(date, pd.Timestamp):
                ensuredDate = date.date()
            elif isinstance(date, datetime.date):
                ensuredDate = date
            else:
                raise TypeError("Unsupported type for issuance_date")
        except (ValueError, TypeError) as e:
            self._logger['error'](f"Error processing issuance_date: {e}")
        finally:
            return ensuredDate
