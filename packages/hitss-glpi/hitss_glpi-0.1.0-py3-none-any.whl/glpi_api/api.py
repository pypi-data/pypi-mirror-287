...
class GLPIAPI:
    ...
    def create_ticket(self, title, content, urgency=3, requester_id=None, assigned_id=None):
        ...
        print("Response text after redirect:", response_text)
        
        # Verificar si la respuesta contiene el mensaje de éxito y extraer el ID del ticket
    ...
    def extract_ticket_id_from_message(self, response_text):
        ...
        print("Toast container found:", toast_container is not None)
        if toast_container:
            toast_body = toast_container.find('div', {'class': 'toast-body'})
            print("Toast body found:", toast_body is not None)
            if toast_body:
                ticket_link = toast_body.find('a', href=True)
                print("Ticket link found:", ticket_link is not None)
                if ticket_link:
                    print("Ticket link href:", ticket_link['href'])
