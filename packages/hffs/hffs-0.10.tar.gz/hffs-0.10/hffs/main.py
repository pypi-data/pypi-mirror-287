...
def download_model(local_dir):
    ...
    def fn(model_dropdown, callback):
        id = model_dropdown
#        if len(model_id) > 0:
#            id = model_id
...
def downloaded(local_dir):
    def fn():
        ...
        dirs = [[item] for item in files if os.path.isdir(os.path.join(local_dir, item))]
        return dirs
    return fn
...
#                            model_input = gr.Textbox(label="Enter Hugging Face Model ID", elem_classes="invisible")
                        if len(choices) > 0:
                            ...
                            model_dropdown = gr.Dropdown(label="HuggingFace ID (example: cocktailpeanut/stable-audio-open-1.0)", choices=choices, allow_custom_value=True)
                        else:
                            model_dropdown = gr.Dropdown(label="HuggingFace ID (example: cocktailpeanut/stable-audio-open-1.0)", choices=choices, elem_classes="invisible", allow_custom_value=True)
                ...
                button.click(fn=download_model(local_dir), inputs=[model_dropdown], outputs=[dataFrame])
