...
def ipaddr(value, query = '', version = False, alias = 'ipaddr'):
    ...
    if not query:
        ...
        if (v.version == 4 and v.is_ipv4_private_use()) \
            or (v.version == 6 and v.is_ipv6_unique_local()):
            return value

    elif query == 'public':
        if v_ip.is_unicast() \
            and not ((v_ip.version == 4 and v_ip.is_ipv4_private_use()) \
                     or (v.version == 6 and v_ip.is_ipv6_unique_local())) and \
