        ...
        while retries <= MAX_RETRIES:
            ...
            try:
                ...
                if 'MySQL server has gone away' in str(ex) and retries < MAX_RETRIES:
                    if log:
                        log.error(f"Erreur de connexion détectée : {ex}. Tentative de reconnexion {retries + 1}/{MAX_RETRIES}.", ex=ex)
                    retries += 1
                    time.sleep(DELAY_SECONDS)
                    # Optionnel: réinitialiser ou reconfigurer l'engine ici si nécessaire
                else:
                    if log:
                        log.error("Nombre maximal de tentatives atteint ou erreur non récupérable. Abandon.", ex=ex)
