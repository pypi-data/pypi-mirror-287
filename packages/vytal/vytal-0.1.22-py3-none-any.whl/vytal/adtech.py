...
def analyze_eye_tracking_data(results, aois, fps, fixation_threshold_sec=0.5, distance_threshold=50):
    ...
    Parameters:
    results (list of dict): A list of dictionaries, each containing a 'PoG' key
    ...
    for i, result in enumerate(results):
        if not isinstance(result, dict) or 'PoG' not in result:
            raise ValueError(f"Result at index {i} is not a valid dictionary with 'PoG' key.")
        if not isinstance(result['PoG'], torch.Tensor) or result['PoG'].shape != (2,):
            raise ValueError(f"Result at index {i} contains invalid 'PoG' value. Expected a tensor of shape (2,).")
    ...
    general_metrics = {
        'Entry_Point': tuple(results[0]['PoG'].tolist()),
        'Exit_Point': tuple(results[-1]['PoG'].tolist())
    }
            ...
            if point_in_rect(x, y, aoi_rect):
                ...
                if current_fixation is None:
                    current_fixation = (x, y, i)
                        ...
                        (i < len(results) - 1 and
                         ((results[i + 1]['PoG'][0] - x) ** 2 + (
                                 results[i + 1]['PoG'][1] - y) ** 2) ** 0.5 > distance_threshold):
...
def plot_gaze_path(results: List[Dict[str, torch.Tensor]], aois: Dict[str, Tuple[float, float, float, float]],
                   image_path: str):
    """
    ...
    Args:
    results (List[Dict[str, torch.Tensor]]): A list of dictionaries, each containing a 'PoG' key
                            representing the predicted x and y coordinates of the eye gaze as a tensor.
    ...
    x = [r['PoG'][0] for r in results]
    y = [r['PoG'][1]  for r in results]
...
def generate_heatmap(results: List[Dict[str, torch.Tensor]], image_path: str, sigma: float = 20):
    """
    Creates a Gaussian heatmap of gaze intensity overlaid on the advertisement image.

    This function generates a Gaussian heatmap visualization of the gaze data, showing areas of high and low
    ...
    sigma (float): Standard deviation for Gaussian kernel. Default is 20.

    The function will:
    1. Load and display the background image.
    2. Create a Gaussian heatmap of the gaze data.
    3. Overlay the heatmap on the image using a rainbow colormap with partial transparency.
    ...
    - The heatmap uses a rainbow colormap where red indicates areas of high gaze concentration.
    ...
    if any(r['PoG'][0] < 0 or r['PoG'][1] < 0 for r in results):
    ...
    if any(r['PoG'][0] > width or r['PoG'][1] > height for r in results):
        logger.warning(
    ...
    # Add Gaussian around each gaze point
    for r in results:
        x, y = int(r['PoG'][0]), int(r['PoG'][1])
    ...
    # Overlay the heatmap
    heatmap_overlay = ax.imshow(heatmap, cmap='rainbow', alpha=0.6)

    plt.title("Gaussian Gaze Intensity Heatmap")
...
def aoi_significance_test(group1_results: List[Dict[str, torch.Tensor]], group2_results: List[Dict[str, torch.Tensor]],
    ...
    Args:
    group1_results (List[Dict[str, torch.Tensor]]): Gaze data for the first group. Each dict should contain
                                             the 'PoG' key for gaze coordinates.
    group2_results (List[Dict[str, torch.Tensor]]): Gaze data for the second group. Same format as group1_results.
    aois (Dict[str, Tuple[float, float, float, float]]): A dictionary where keys are AOI names and values
                                                         are tuples representing the bounding box of each AOI
                                                         in the format (x1, y1, x2, y2).
    ...
    def get_aoi_metrics(results, aois):
        ...
        for r in results:
            for aoi, (x1, y1, x2, y2) in aois.items():
                if x1 <= r['PoG'][0] <= x2 and y1 <= r['PoG'][1] <= y2:
...
def main():
    ...
    simulated_results = [
        {'PoG':torch.Tensor([np.random.uniform(0, 1920), np.random.uniform(0, 1080)])}
