...
from typing import Tuple, Optional, Generator, Union, Dict
...
class Dataset:
    ...
    def __init__(
        ...
        metadata_url: Optional[Union[DataUrl, str]] = None,
    ):
        self._data = None
    ...
    def _extract_hdf5_metadata(
        ...
        for name, dset in datasets.items():
            if name in Dataset._DATASETS_USED_FOR_TRANSFORMATION:
                ...
                # This happens when the 1D datasets have less points than there are images (CTRL-C of scan)
                _logger.warning(
                    f"Unable to access index {frame_index} of the dataset {dset.name}"
                )
                metadata[name] = None
            else:
                metadata[name] = dset[frame_index]
...
def extract_positioners(positioners: h5py.Group) -> Dict[str, h5py.Dataset]:
    ...
    for name in positioners:
        ...
        if isinstance(dset, h5py.Dataset):
            if (
                dset.ndim == 0 and name in Dataset._DATASETS_USED_FOR_TRANSFORMATION
            ) or (dset.ndim == 1):
