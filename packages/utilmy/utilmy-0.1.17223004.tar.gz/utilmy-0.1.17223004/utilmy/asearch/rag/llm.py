...
import dask.dataframe as dask_d
import dspy
from box import Box
...
class LLM(object):
    ...
    def get_batch_df(self, df, prompt_template: str, prompt_map_dict: dict, dirout: str = None, dirname: str = None,
        ...
        if npool == 1:
            ...
            # s_time = time.time()
            df = pd_parallel_apply(df, llm_get_sync, n_threads=npool, colout="llm_json",  prompt_request=promptRequest(api_key=self.api_key,
                                                                 modelid=self.modelid,
                                                                 service=self.service,
                                                                 json_schema=output_schema))
            # log("Time for parallel processing: ", time.time() - s_time)
        #### Extract
...
def pd_parallel_apply(df, func, colout="llm_json", n_threads=4, **kwargs):
    """
