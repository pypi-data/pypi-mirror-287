...
Example usage of `get_table_from_file_or_folder`:
```python
from evidi_fabric.fs import get_table_from_file_or_folder
...
def get_table_paths(
    ...
    return [
        f"abfss://{workspace_name}@onelake.dfs.fabric.microsoft.com/{lakehouse_name}.Lakehouse/Tables/{table_name}"
        for table_name in table_names
    ]
...
def drop_all_tables_and_shortcuts(lakehouse: str, workspace: str = None, included_tables: list = None):
    """
    ...
    """
    # Tables to delete
    tables_df = get_lakehouse_tables_info(lakehouse=lakehouse, workspace=workspace)

    # Filter to only existing tables
    existing_tables = tables_df[tables_df.apply(lambda row: mssparkutils.fs.exists(row["Location"]), axis=1)]

    if included_tables is None:
        included_tables = [table for table in existing_tables["Table Name"]]

    for _, table_row in existing_tables.iterrows():
        table_name, _ = table_row["Table Name"], table_row["Location"]
        if table_name not in included_tables:  # or not mssparkutils.fs.exists(table_path):
            print("Skipping", table_name)
            continue

        # Trying to drop either shortcut or table
        drop_table_or_shortcut(table_name, lakehouse, workspace)
...
def replace_all_tables_or_shortcuts(
    ...
    # Tables to delete
    dst_tables_df = get_lakehouse_tables_info(lakehouse=dst_lakehouse, workspace=dst_workspace).sort_values(
        "Table Name"
    )

    # Filter to only existing tables
    existing_tables = dst_tables_df[dst_tables_df.apply(lambda row: mssparkutils.fs.exists(row["Location"]), axis=1)]

    if included_tables is None:
        included_tables = [table for table in existing_tables["Table Name"]]

    # Only set a control time there are any tables to load
    start_time = time.time() if existing_tables.shape[0] > 0 else 0
    ...
    for _, table_row in existing_tables.iterrows():
        ...
        # Trying to drop either shortcut or table
        drop_table_or_shortcut(table_name, dst_lakehouse, dst_workspace)
    ...
    if only_replace:
        src_tables_df = dst_tables_df

    else:
        # Re-setting the shortcuts aka tables to shortcut to
        src_tables_df = get_lakehouse_tables_info(lakehouse=src_lakehouse, workspace=src_workspace).sort_values(
            "Table Name"
        )

    for _, table_row in src_tables_df.iterrows():
        table_name = table_row["Table Name"]
...
def create_shortcut_onelake(
    ...
    try:
        ...
        if response.status_code == 201:
            print(
                f"The shortcut '{shortcut_name}' was created in the '{dst_lakehouse}' lakehouse within the '{dst_workspace} workspace. It is based on the '{table_name}' table in the '{src_lakehouse}' lakehouse within the '{src_workspace}' workspace."
            )
        else:
