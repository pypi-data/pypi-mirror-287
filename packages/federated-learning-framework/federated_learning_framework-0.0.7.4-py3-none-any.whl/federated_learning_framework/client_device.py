...
class ClientDevice:
    ...
    async def send_message(self, message):
        try:
            serialized_message = pickle.dumps(message)
            await self.connection.send(serialized_message)
            self.logger.info(f"Client {self.client_id}: Sent message: {message}")
        except Exception as e:
            self.logger.error(f"Client {self.client_id}: Error sending message: {e}")

    async def receive_message(self):
        try:
            message = await asyncio.wait_for(self.connection.recv(), timeout=10)  # Timeout added
            deserialized_message = pickle.loads(message)
            self.logger.info(f"Client {self.client_id}: Received message: {deserialized_message}")
            return deserialized_message
        except asyncio.TimeoutError:
            self.logger.error(f"Client {self.client_id}: Timeout waiting for message")
            return None
        except Exception as e:
            self.logger.error(f"Client {self.client_id}: Error receiving message: {e}")
            return None
    ...
    async def federated_learning(self, x_train, y_train):
        try:
            while True:
                message = await self.receive_message()
                if message is None:
                    self.logger.info(f"Client {self.client_id}: No message received, exiting")
                    break
                
                weights = message.get('weights', None)
                if weights is None:
                    self.logger.info(f"Client {self.client_id}: No weights found in message")
                    break

                self.logger.info(f"Client {self.client_id}: Processing weights")
                self.model.set_weights(decrypt_weights(self.context, weights))
                self.model.train(x_train, y_train, epochs=1)
                new_weights = self.model.get_weights()
                await self.send_message({'weights': encrypt_weights(self.context, new_weights)})
