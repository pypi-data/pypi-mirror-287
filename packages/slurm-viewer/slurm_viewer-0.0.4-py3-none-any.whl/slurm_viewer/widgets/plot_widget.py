...
from slurm_viewer.data.models import JobStateCodes, Job, Slurm, SlurmError
...
class PlotWidget(Static):
    ...
    def _update(self, old: Config, new: Config) -> bool:
        if old.ui != new.ui:
            return True

        name = self.slurm.cluster().name
        if old.get_cluster(name) != new.get_cluster(name):
            return True

        return False

    @work
    async def watch_config(self, old: Config, new: Config) -> None:
        if not self._update(old, new):
            return

        with Loading(self):
            try:
                self.gpu_usage = await self.slurm.jobs(self.gpu_history)
