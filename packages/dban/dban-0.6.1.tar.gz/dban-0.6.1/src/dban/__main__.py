...
print("Dumping all tables into Excel...")
...
with pd.ExcelWriter("dban-generated-data-dump.xlsx") as ew:
    ...
    for tablename in tablenames_alphabetical:
        ...
        rows = src_session.query(database_table).all()
        for row in rows:
            #print(row._mapping) oder so
            res = {}
            for fk in database_table.foreign_keys:
                res[fk.name] = str(getattr(row, fk.name))
            for col in database_table.columns:
                res[col.name] = str(getattr(row, col.name))
            
            data_row = []
            for k in allcols:
                data_row.append(res[k])
