...
class NXdetector(NXobject):
    ...
    def set_transformation_from_x_flipped(self, flipped: Optional[bool]):
        ...
        if len(current_lr_transfs) == 0:
            self.transformations.add_transformation(
                DetZFlipTransformation(flip=flipped)
            )
        elif len(current_lr_transfs) == 1 and current_lr_transfs[
            0
        ] == DetZFlipTransformation(flip=flipped):
            ...
            raise RuntimeError(
                "setting x_flipped requested but get already some transformations along Z. Unable to do it. Please provide NXtransformations directly"
            )
    ...
    def set_transformation_from_y_flipped(self, flipped: Optional[bool]):
        ...
        if len(current_ud_transfs) == 0:
            self.transformations.add_transformation(
                DetYFlipTransformation(flip=flipped)
            )
        elif len(current_ud_transfs) == 1 and current_ud_transfs[
            0
        ] == DetYFlipTransformation(flip=flipped):
            # we consider it is already there
            return
            ...
            raise RuntimeError(
                "setting y_flipped requested but get already some transformations along Y. Unable to do it. Please provide NXtransformations directly"
            )
