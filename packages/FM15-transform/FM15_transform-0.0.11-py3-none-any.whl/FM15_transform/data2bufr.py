...
def transform(data: str, year: int, month: int) -> Iterator[dict]:
    ...
    for metar in messages[is_bulletin:]:
        ...
        try:
            ...
            station_info = requests.get(f'http://wis2_api/collections/mlid_station/items?icao={icao}').json()['features'][0]
            LOGGER.debug(f"Station info: {station_info}")
        except Exception:
            conversion_success[icao] = False
            LOGGER.warning(f"Station {icao} not found in MLID station list")
            continue
        ...
        try:
            ...
            station_height = station_info["elevaiton"]
            ...
            if icao == "":
                ...
                LOGGER.warning((f"Invalid ICAO ({icao}) found in MLID station list,"
                                " unable to parse"))
            continue
