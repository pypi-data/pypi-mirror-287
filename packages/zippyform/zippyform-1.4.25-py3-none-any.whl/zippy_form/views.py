...
from zippy_form.models import Account, FormField, Form, FormStep, FormSubmission, FormSubmissionData, FormFieldOption, Webhook,WebhookForm, AccountPaymentSettings,FormPaymentSettings, PaymentGatewayWebhook, FormSubmissionPaymentDetails, \
    FormMetaData, FormMetaDataOption, FormSubmissionMetaDataValue, FormPageViews, FormSettings, GoogleCredentials, GsheetMappingFields, GSheetSyncFields
...
def update_form_settings_details(req, form_id):
    ...
    if form_settings:
        # Update Form Setting Details
        if gsheet_enabled == 'True':
            if str(form_settings.gsheet_enabled) != gsheet_enabled:
                form_settings.gsheet_enabled = True
                form_settings.gsheet_type = gsheet_type
                create_update_gsheet(req, form, gsheet_type=gsheet_type)
            else:
                if str(gsheet_type) != str(form_settings.gsheet_type):
                    form_settings.gsheet_type = gsheet_type
                    create_update_gsheet(req, form, gsheet_type=gsheet_type)
        else:
        ...
        # Create Form Setting Details
        if gsheet_enabled == 'True' and recaptcha_enabled == 'True':
            ...
            create_update_gsheet(req, form, gsheet_type=gsheet_type)
        elif recaptcha_enabled == 'True':
            form_settings = FormSettings.objects.create(form=form, recaptcha_enabled=recaptcha_enabled,
                                                        site_key=site_key,
                                                        secret_key=secret_key)
        elif gsheet_enabled == 'True':
...
def create_update_gsheet(req, form, gsheet_type=GSHEET_TYPES[0][0], sheet_name=None):
    """
    ...
    if gsheet_type == GSHEET_TYPES[0][0]:
        ...
        gsheet = threading.Thread(target=create_gsheet_map_form, args=(form, form_field_ids_to_append, gsheet_type))
        gsheet.start()
    elif gsheet_type == GSHEET_TYPES[1][0]:
        gsheet = threading.Thread(target=create_spreadsheet_in_user_google_account, args=(req, form,form_field_ids_to_append, gsheet_type))
        gsheet.start()
    elif gsheet_type == GSHEET_TYPES[2][0]:
        gsheet = threading.Thread(target=update_existing_gsheet_url, args=(req,form))
        gsheet.start()

def update_existing_gsheet_url(req,form):
    """
...
def google_sheet_oauth2callback(request):
    ...
    return render(request, 'google_oauth.html', context={"url": ZF_GSHEET_AFTER_OAUTH_REDIRECT_TO})
...
def get_sheet_columns(request, spreadsheet_id, sheet_id):
    ...
    try:
        ...
        form_fields = FormField.objects.select_related('form', 'form_step').filter(
            form=form_id, form__status=FORM_STATUS[1][0], form_step__status=FORM_STEP_STATUS[1][0]).exclude(status=FORM_FIELD_STATUS[0][0])
...
def map_gsheet_fields(request):
    ...
    if gsheet_enabled:
        ...
        if active_form_fields_list:
            spreadsheet=None
            gsheet = threading.Thread(target=add_field_labels_to_gsheet, args=(active_form_fields_list, form, GSHEET_TYPES[2][0], form.gsheet_url, spreadsheet))
            gsheet.start()
...
@api_view(['DELETE'])
def delete_mapped_gsheet_field(request, gsheet_map_field_id):
    """
    Delete Mapped GSheet Field
    """
    try:
        gsheet_map_field_id = GsheetMappingFields.objects.get(id=gsheet_map_field_id)

    except GsheetMappingFields.DoesNotExist:
        return Response({"status": 'error', "msg": "Invalid GsheetMapFieldId"}, status=status.HTTP_400_BAD_REQUEST)

    gsheet_map_field_id.status = GSHEET_MAPPING_FIELDS_STATUS[0][0]
    gsheet_map_field_id.save()

    return Response({"status": "success", "msg": "GsheetMapField Deleted Successfully"})
