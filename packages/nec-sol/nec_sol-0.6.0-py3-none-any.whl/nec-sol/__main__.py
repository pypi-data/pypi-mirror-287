...
s_available_frameworks = ['torch', 'tensorflow', 'numpy', 'onnx', 'vllm']
...
def initialize():
	...
	if args.features == ['all']:
		...
		s_dependencies['adv-sdk']	= s_version
		s_dependencies['tests']		= s_version
	else:
		for f in args.features:
			if f in ['adv-sdk', 'sdk', 'tests']:	s_dependencies[f] = s_version
	...
	for x in s_python_packages:
		...
		if d in args.frameworks:
			s_dependencies[d] = f'~={v}.0' # padding to enforce >=1.2.3.0, <1.2.3, which only allows .post!
...
def install(args, plugins = None):
	...
	cmd = ['python3', '-m', 'pip', 'install' if is_install else 'wheel']
