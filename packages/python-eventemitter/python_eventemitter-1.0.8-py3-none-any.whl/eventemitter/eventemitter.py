...
class AbstractEventEmitter(ABC, Generic[L, H]):
    ...
    def add_listener(self, event: Hashable, listener: L) -> Self:
        ...
        Returns:
            (Self): An instance of the `EventEmitter`, so that calls can be chained.
        """
    ...
    def on(self, event: Hashable, listener: Optional[F] = None) -> Union[Self, Callable[[F], F]]:
        """Add the `listener` function to the end of the listeners list for the event named `event` if a `listener` is provided, or returns a decorator to add the decorated function as a `listener` if no `listener` is provided.
    ...
    def once(self, event: Hashable, listener: Optional[F] = None) -> Union[Self, Callable[[F], F]]:
        """Add a **one-time** `listener` function for the event named `event` if a `listener` is provided, or returns a decorator that adds the decorated function as a **one-time** `listener` if no `listener` is provided.
