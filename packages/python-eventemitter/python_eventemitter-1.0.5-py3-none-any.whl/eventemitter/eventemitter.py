...
from typing import Any, Generic, Hashable, List, Optional, Type, TypeVar, Union

from typing_extensions import Self, overload

from eventemitter.events import Events
from eventemitter.handlers import AbstractHandler, AsyncHandler, Handler
from eventemitter.types import AsyncListenable, Callable, Listenable, Returns
...
class AbstractEventEmitter(ABC, Generic[L, H]):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        """Initializes an instance of [`AbstractEventEmitter`][eventemitter.AbstractEventEmitter].
    ...
    def add_listener(self, event: Hashable, listener: L) -> Self:
        """Adds the `listener` function to the end of the listeners list for the event named `event`. Multiple calls passing the same combination of `event` and `listener` will result in the `listener` being added, and called, multiple times.
        ...
        Returns:
            (Self): A reference to the `EventEmitter`, so that calls can be chained.
        """
    ...
    def prepend_listener(self, event: Hashable, listener: L) -> Self:
        """Adds the `listener` function to the *beginning* of the listeners list for the event named `event`. Multiple calls passing the same combination of `event` and `listener` will result in the `listener` being added, and called, multiple times.
    ...
    def prepend_once_listener(self, event: Hashable, listener: L) -> Self:
        """Adds a **one-time** `listener` function for the event named `event` to the *beginning* of the listeners list. The next time `event` is triggered, this `listener` is removed, and then invoked.
    ...
    def emit(self, event: Hashable, *args: Any, **kwargs: Any) -> Returns[bool]:
        """Calls each of the listeners registered for the event named `event`, passing the supplied arguments to each.
    ...
    def events(self) -> List[Hashable]:
        ...
        Returns:
            (list[Hashable]): A list of the `event`s
        """
    ...
    def listeners(self, event: Hashable) -> List[L]:
        ...
        Returns:
           (list[L]): A copy of the list of listeners for the event named `event`.
        """
    ...
    def on(self, event: Hashable, listener: Optional[F] = None) -> Union[Self, Callable[[F], F]]:
        """Adds the `listener` function to the end of the listeners list for the event named `event` if a `listener` is provided, or returns a decorator to add the decorated function as a `listener` if no `listener` is provided.
        ...
        Returns:
            (Self): A reference to the `EventEmitter`, so that calls can be chained if a `listener` is provided.
    ...
    def once(self, event: Hashable, listener: Optional[F] = None) -> Union[Self, Callable[[F], F]]:
        """Adds a **one-time** `listener` function for the event named `event` if a `listener` is provided, or returns a decorator that adds the decorated function as a **one-time** `listener` if no `listener` is provided.
...
class EventEmitter(AbstractEventEmitter[Listenable, Handler]):
    ...
    **Event: `"remove_listener"`**
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        """Initializes an instance of `EventEmitter`.
    ...
    def emit(self, event: Hashable, *args: Any, **kwargs: Any) -> bool:
        """Calls each of the listeners registered for the event named `event`, in the order they were registered, passing the supplied arguments to each.
...
class AsyncIOEventEmitter(AbstractEventEmitter[Union[Listenable, AsyncListenable], AsyncHandler]):
    ...
    **Event: `"remove_listener"`**
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        """Initializes an instance of `AsyncIOEventEmitter`.
    ...
    async def emit(self, event: Hashable, *args: Any, **kwargs: Any) -> bool:
        """Calls each of the listeners registered for the event named `event`, simultaneously, passing the supplied arguments to each.
