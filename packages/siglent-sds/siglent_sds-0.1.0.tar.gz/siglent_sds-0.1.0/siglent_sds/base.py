...
__all__ = ["SDS800X_Base"]


class SDS800X_Base:
    """
    Base class for the Siglent SDS800X HD oscilloscope.
    ...
    socket<siglent_sds.socket.SDS800X_Socket>`, USB, USBTMC, VISA) is left up to a sub-class,
    and thus there should be no reason to instantiate this class directly.
    ...
    def __init__(self, loop=None, open_device=True):
    ...
    async def _open_connection(self):
        ...
        In :class:`SDS_Base` class, this method does nothing. It is up to a sub-class to determine
        the type of connection to make (e.g. network socket, USB). The sub-class should make the
        connection and set values for the :data:`_connection_name`, :data:`_reader`, and
        :data:`_writer` properties.

        The :data:`_connection_name` should be a string describing the connection, such as IP
        address. The :data:`_reader` and :data:`_writer` properties should point to instances of
        ...
        ``await self._handle_comms()`` to the pass control to the communications coroutine.
        """
        self._log.warning(
            "Base class does not implement a connection method, no device communication will occur."
        )
    ...
    async def _handle_comms(self):
        ...
        configured at this point. This coroutine should be called by a sub-class once a connection
        has been established.
        """
        self._log.info(f"Connection established with {self._connection_name}.")
        ...
        while True:
            try:
                ...
                command, delimiter, binary, callback, error_callback = await self._q.get()
                # Flush read buffer if errors have occurred previously
                if last_error:
                ...
                self._log.error("Error writing command.")
                last_error = ex
            else:
                ...
                if delimiter or binary:
                    ...
                    try:
                        if not binary and delimiter:
                            ...
                            # self._log.debug(
                            #     f"Waiting for data block from {self._connection_name} delimited by {delimiter}."
                            # )
                            async with asyncio.timeout(2.0):
                                data_bytes = await self._reader.readuntil(separator=delimiter)
                        elif binary:
                            ...
                            async with asyncio.timeout(1.0):
                                data_header = await self._reader.readuntil(b"#")
                            ...
                            async with asyncio.timeout(1.0):
                                data_header = await self._reader.readexactly(1)
                            try:
                                ...
                                # )
                                async with asyncio.timeout(1.0):
                                    data_header = await self._reader.readexactly(header_length)
                                try:
                                    ...
                                    async with asyncio.timeout(1.0):
                                        data_bytes = await self._reader.readexactly(
                        ...
                        self._log.error(
                            f"Data block from {self._connection_name} exceeded buffer size."
                        )
                        data_bytes = await self._reader.read(self._buffer_size)
                        ...
                        self._log.info(
                            f"Connection with {self._connection_name} closed, but read {len(data_bytes)} bytes."
                        )
                        last_error = ex
                    except (ConnectionResetError, BrokenPipeError, OSError) as ex:
                        self._log.info(f"Connection with {self._connection_name} closed.")
                        last_error = ex
                    else:
                        last_error = None

                    # self._log.debug(f"Received {len(data_bytes)} bytes from {self._connection_name}.")
    ...
    def open(self):
        ...
        if self._connection_task is None or self._connection_task.done():
            self._connection_task = self._loop.create_task(
                self._open_connection(), name=f"{__class__.__name__} Connector"
            )
        # Queue some configuration and update device parameters
        self.waveform_width("WORD")
        self.update_waveform_maxpoints()
        self.update_waveform_points()
    ...
    def send(
        ...
        Once the command has completed, any result will be returned as raw :class:`bytes`. If no
        reply is received from the device, empty bytes (``b""``) will be returned. If an error
        occurs during communications or processing of the response, the exception will be raised. If
        the function takes longer than :data:`block_timeout` seconds, a :class:`TimeoutError` will
        be raised.
        ...
        will be a :class:`bytes` object with the delimiter still appended.
        ...
        if not block:
            ...
            self._loop.call_soon_threadsafe(
                self._q.put_nowait, (command, delimiter, bool(binary), callback, error_callback)
            )
        else:
            ...
            self._loop.call_soon_threadsafe(
                self._q.put_nowait,
                (command, delimiter, bool(binary), handle_send, handle_send_error),
            )
            self._unblock.clear()
            command_result = None
            command_exception = None
            # Wait for command to complete
            if not self._unblock.wait(self.block_timeout):
    ...
    def ready(self):
        ...
        """
        return self._q.empty() and (not self._reader is None) and (not self._writer is None)
    ...
    def waveform_data(self, block=True, callback=None, error_callback=None):
        ...
        def handle_waveform_data(data):
            ...
            if len(data) <= 2:
                ...
                return
            data = np.frombuffer(data[:-2], dtype=np.int16)
            data = (
    ...
    def get_waveforms(self, channels=("C1",), block=True, callback=None):
        ...
        channel specifications are listed in the :data:`utils.waveform_sources` array, but note that
        not all sources are available on every model of oscilloscope.

        Analog channels are specified using the form ``"C1"``, ``"C2"``, ``"C3"``, ``"C4"``. Math
        function channels are specified using ``"F1"``, ``"F2"`` etc.

        Lists of channels may be specified in a number of ways:

        - List of channel specifiers, e.g. ``channels=["C1", "C2", "F1"]``.
        - String separated by spaces or commas, e.g. ``channels="C1 C2 F1"`` or ``channels="C1,C2,
          F1"``.
        - Integer specifying a single analog channel, eg ``channels=3``.
        - List of integers specifying analog channels, eg ``channels=[1, 2, 4]``.
    ...
    def waveform_start(
        ...
        :param t: Starting time point to return in waveform requests.
        :param block: Block function return until command completed.
        :param callback: Function to call once command has completed.
        :param error_callback: Function to call if an error occurs.
        """
    ...
    def waveform_points(
        ...
        :param t: Time interval to return in waveform requests.
        :param block: Block function return until command completed.
        :param callback: Function to call once command has completed.
        :param error_callback: Function to call if an error occurs.
        """
    ...
    def waveform_interval(
        ...
        :param t: Time interval between data points to return in waveform requests.
        :param block: Block function return until command completed.
        :param callback: Function to call once command has completed.
        :param error_callback: Function to call if an error occurs.
        """
    ...
    def autoset(self, block=True, callback=None, error_callback=None):
        ...
        self.update_waveform_preamble(
            block=block, callback=handle_png, error_callback=error_callback
        )
