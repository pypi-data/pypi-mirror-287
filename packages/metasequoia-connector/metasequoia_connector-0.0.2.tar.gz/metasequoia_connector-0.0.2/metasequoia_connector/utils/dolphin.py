...
def conn_get_down_process_by_process(conn: pymysql.Connection,
                                     process: DSProcess
                                     ) -> List[DSProcess]:
...
def conn_get_task_code_list_by_process_code_list(conn: pymysql.Connection, project_code: int,
...
def conn_get_process_task_list_by_process_list(conn: pymysql.Connection, process_list: List[DSProcess]
                                               ) -> List[DSProcessTask]:
...
def conn_get_dependence_process_set(conn: pymysql.Connection, process_list: List[DSProcess]) -> Set[DSProcess]:
    """获取当前任务依赖的所有上游任务的集合"""
    ...
    while process_list:
        ...
        for project_code, group_process_list in grouped_process_dict.items():
            ...
            task_code_list = conn_get_task_code_list_by_process_code_list(conn, project_code, process_code_list)
...
def conn_get_schedule_release_state_by_process_list(conn: pymysql.Connection, process_list: List[DSProcess]
                                                    ) -> Dict[DSProcess, int]:
    """获取当前工作流集合的定时上线状态"""
...
def conn_get_schedule_priority_state_by_process_list(conn: pymysql.Connection, process_list: List[DSProcess]
                                                     ) -> Dict[DSProcess, int]:
    """获取当前工作流集合的定时优先级"""
    process_hash = {process.process_code: process for process in process_list}
    process_list_str = ",".join(str(process.process_code) for process in process_list)
...
def conn_get_process_name_dict_by_process_list(conn: pymysql.Connection,
...
def conn_get_task_name_dict_by_task_list(conn: pymysql.Connection,
...
def conn_filter_process_task_by_condition(conn: pymysql.Connection,
                                          task_list: List[GeneralTask],
...
def conn_get_process_info_by_process(conn: pymysql.Connection, process: DSProcess) -> dict:
    """获取海豚调度指定工作流的名称"""
...
def conn_get_process_info_by_process_list(conn: pymysql.Connection,
...
def conn_get_scheduler_info_by_process_list(conn: pymysql.Connection,
                                            process_list: List[DSProcess],
                                            need_columns: Optional[List[str]] = None) -> Iterable[Dict[str, Any]]:
