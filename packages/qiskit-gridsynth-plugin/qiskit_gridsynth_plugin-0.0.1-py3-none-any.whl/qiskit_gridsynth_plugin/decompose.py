...
def call_gridsynth(angle,approx_exp):
    ...
    if angle < 0:
        angle_str = f"({angle})"
    else: angle_str = str(angle)
    result = subprocess.run([executable_path, angle_str, f'--digits={-approx_exp}', '-p'], capture_output=True)
...
class GridSynth(TransformationPass):
    ...
    def __init__(
        ...
        Args:
        approx_exp : the exponent on epsilon, the degree of approximation
        """
        super().__init__()
...
def clifford_t_transpile(circ : QuantumCircuit, approx_exp : float = -10)  -> QuantumCircuit:
    '''
    ...
    2. Decompose the RZ gates using ``gridsynth`` (This transformation is approximate, with error up to 10^(approx_exp))
    '''
    pm = PassManager([BasisTranslator(equivalence_library=sel, target_basis=["cx", "h", 'rz']), GridSynth(approx_exp)])
    return pm.run(circ)