...
class ImageInferenceDataset(torch.utils.data.Dataset):
    '''input image paths or folders containing images, output transformed tensor for inference.
    '''
    def __init__(self,*imgs_or_dirs,suffix=['jpg','jpeg','png','bmp'],transform=None,loader=default_loader):
        self.__dict__.update(locals())
        self.img_path_list = self.traverse_files()
    ...
    def __getitem__(self,index):
        i = index % len(self)
        img = self.loader(self.img_path_list[i])
        if self.transform:
            img = self.transform(img)
        return img

    def check_suffix(self,fname,suffix):
        fname = os.path.basename(fname)
        if '.' not in fname:
            return False
        else:
            return (fname.split('.')[-1]).lower() in suffix

    def traverse_files(self):
        img_list = []

        for path in self.imgs_or_dirs:
            if os.path.isfile(path):
                img_list.append(path)
                if not self.check_suffix(path,self.suffix):
                    tf.ui.warn(f"the given file {path} does not conform to the suffix requirement, but will be added to the file list.")
            elif os.path.isdir(path):
                for topdir,dirnames,filenames in os.walk(path,followlinks=True):
                    filenames = sorted(filename)
                    for filename in filenames:
                        if self.check_suffix(filename,self.suffix):
                            img_list.append(os.path.join(topdir,filename))
        return img_list
