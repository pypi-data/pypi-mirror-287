...
def pil_resize(pil_image,size_wh=None,scale=None,width=None,height=None,center_crop_size_wh=None):
    """
    Given a PIL image instance, and one of the size argument, this function helps you to call PIL.Image.resize and feed corresponding W,H to its naive function.
    center_crop_size_wh: scale the image to cover the desired size, then crop from the center and return this center patch.
    return type: the result pil image instance.
    """
    out = pil_image
    if size_wh:
        out = pil_image.resize(size_wh)
    elif scale is not None:
        w = round(scale * pil_image.width)
        h = round(scale * pil_image.height)
        out = pil_image.resize((w,h))
    elif width:
        scale = width / pil_image.width
        h = round(scale * pil_image.height)
        out = pil_image.resize((width,h))
    elif height:
        scale = height / pil_image.height
        w = round(scale * pil_image.width)
        out = pil_image.resize((w,height))
    elif center_crop_size_wh:
        out_w,out_h = center_crop_size_wh
        scale = max(out_w/pil_image.width, out_h/pil_image.height)
        scaled_w = round(scale * pil_image.width)
        scaled_h = round(scale * pil_image.height)
        scaled_image = pil_image.resize((scaled_w,scaled_h))
        w_start = (scaled_w - out_w)//2
        h_start = (scaled_h - out_h)//2
        center_patch = scaled_image.crop((w_start,h_start,w_start+out_w,h_start+out_h))
        out = center_patch
    else:
        print('No parameter is given, returning the input image')
    return out
