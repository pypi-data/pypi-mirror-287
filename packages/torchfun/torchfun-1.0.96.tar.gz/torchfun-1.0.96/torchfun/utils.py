...
def record_experiment(exp_dir='not-specified',record_top_dir='records',logfilename='record.txt',comment='',encoding='utf-8'):
    '''Arguments:
    ...
    try:
        ...
        if not os.path.exists(logfilepath):
            f = open(logfilepath,'w',encoding=encoding)
            f.write('科研实验开始::\n')
            f.close()
