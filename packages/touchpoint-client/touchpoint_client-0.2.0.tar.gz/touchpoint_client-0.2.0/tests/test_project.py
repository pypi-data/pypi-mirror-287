...
def test_project(default_client):
    ...
    keys = ['id', 'account_id', 'username', 'global_role', 'created_on', 'active', 'is_confirmed', 'two_factor',
            'modified_on', 'personal_settings', 'projects', 'group_projects']
    for k in keys:
...
def test_account(default_client):
    ...
    keys = ['id', 'account_settings', 'global_account', 'created_on', 'modified_on',
            'active', 'name', 'type', 'limits']
    for k in keys:
