import time
from multiprocessing import Process

class ProcessWrapper:
    def __init__(self, target_func, name, *args):
        self.target_func = target_func
        self.args = args
        self.name = name

    def create_process(self):
        return Process(target=self.target_func, args=self.args, name=self.name)

def monitor_processes(LOG, process_list, stop_event):
    """
    Monitor and restart processes if they are not alive.

    Parameters:
    - LOG: process monitor log
    - process_list: List of ProcessWrapper objects to monitor.
    - stop_event: Event object to signal when to stop monitoring.
    """
    while True:
        for p in process_list:
            proc = p.process
            if not proc.is_alive():
                LOG.error('Process:{} with args:{} is not alive. Restarting...'.format(proc.name, proc.args))
                # Restart the process with the same target function and arguments
                new_process = p.create_process()
                new_process.start()
                # Replace the terminated process with the new process
                p.process = new_process
        if stop_event.is_set():
            break
        time.sleep(10)
    LOG.info('Monitoring has been stopped.')
