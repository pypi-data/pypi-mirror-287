...
def directoryListing(spark, directory_path, recursive, pattern):
    df= spark.sparkContext._jvm.io.prophecy.abinitio.ScalaFunctions._directory_listing_v2(spark._jsparkSession, directory_path, recursive, pattern)
    return DataFrame(df, SQLContext(spark.sparkContext, sparkSession=spark))
