...
def _apply_top_k_top_p(
    ...
    for i in range(logits.size(0)):
        # Apply top-k
        logits_topk, logits_idx = torch.topk(logits[i], k[i])
        # Apply top-p
        probs_topk = logits_topk.softmax(dim=-1)
        probs_sum = probs_topk.cumsum(dim=-1)
        index = torch.searchsorted(probs_sum, p[i], right=True).item()
        # at least one
        if index == 0:
            index = 1
        logits_idx = logits_idx[:index]
        result[i][logits_idx] = logits[i][logits_idx]
    return result
