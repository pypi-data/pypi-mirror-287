...
class NeuralNetwork:
    ...
    def _update_batch(
        self, batch: List[DataPoint], learning_rate: float, momentum: float
    ):
        ...
        for data_point in batch:
            ...
            for layer in self.layers:
                ...
                zs.append(z)
                activation = layer.calculateOutputs(z)
            ...
            for l in range(2, len(self.layers) + 1):
                z = zs[-l]
                sp = self.layers[-l]._activationFunctionDerivative(z)
    ...
    def learn(
        ...
        with ThreadPoolExecutor() as executor:
            future_to_batch = {
                executor.submit(
                    self._update_batch, batch, learning_rate, momentum
                ): batch
            ...
            for future in as_completed(future_to_batch):
                ...
                for i, layer in enumerate(self.layers):
                    self.velocity_w[i] = (
                        ...
                        - (learning_rate / len(batch)) * nabla_w[i]
                    )
                    self.velocity_b[i] = (
                        momentum * self.velocity_b[i]
                        - (learning_rate / len(batch)) * nabla_b[i]
                    )
