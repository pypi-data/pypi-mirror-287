...
def submit_uhc_claim(client, x12_request_data):
    """
    Submits a UHC claim using the provided x12 837p data.

    :param client: An instance of APIClient
    :param x12_request_data: The x12 837p data as a string
    :return: Response from the API or a dummy response if in test mode
    """
    endpoint_name = 'UHCApi'
    url_extension = client.config['MediLink_Config']['endpoints'][endpoint_name]['additional_endpoints']['claim_submission']

    # Headers for the request
    headers = {'Content-Type': 'application/json', 'env': 'sandbox'}  # TODO Sandbox needs to come out at some point

    # Request body
    body = {'x12RequestData': x12_request_data}

    # Check if TestMode is enabled in the configuration
    if client.config.get("MediLink_Config", {}).get("TestMode", True):
        # TestMode is enabled, return a dummy response
        print("Test Mode is enabled! API Call not executed.")
        MediLink_ConfigLoader.log("Test Mode is enabled! Simulating API response for claim submission.")
        dummy_response = {
    ...
    # Make the API call using the existing make_api_call method
    try:
        response = client.make_api_call(endpoint_name, 'POST', url_extension, data=body, headers=headers)
        return response
    except Exception as e:
        print("Error during claim submission: {}".format(e))
        raise
...
if __name__ == "__main__":
    ...
    try:
        ...
        if test_config.get('test_claim_submission', False):
            try:
                x12_request_data = (
                    "ISA*00* *00* *ZZ*BRT219991205 *33*87726 *170417*1344*^*00501*019160001*0*P*:~GS*HC*BRT219991205*B2BRTA*20170417*134455*19160001*X*005010X222A1~"
                    "ST*837*000000001*005010X222A1~BHT*0019*00*00001*20170417*134455*CH~NM1*41*2*B00099999819*****46*BB2B~PER*IC*NO NAME*TE*1234567890~"
                    "NM1*40*2*TIGER*****46*87726~HL*1**20*1~NM1*85*2*XYZ ADDRESS*****XX*1073511762~N3*123 CITY#680~N4*STATE*TG*98765~REF*EI*943319804~"
                    "PER*IC*XYZ ADDRESS*TE*8008738385*TE*9142862043*FX*1234567890~NM1*87*2~N3*PO BOX 277500~N4*STATE*TS*303847000~HL*2*1*22*0~"
                    "SBR*P*18*701648******CI~NM1*IL*1*FNAME*LNAME****MI*00123456789~N3*2020 CITY~N4*STATE*TG*80001~DMG*D8*19820220*M~NM1*PR*2*"
                    "PROVIDER XYZ*****PI*87726~CLM*TOSRTA-SPL1*471***12:B:1*Y*A*Y*Y*P~REF*D9*H4HZMH0R4P0104~HI*ABK:Z12~NM1*DN*1*DN*SKO****XX*1255589300~"
                    "LX*1~SV1*HC:73525*471*UN*1***1~DTP*472*RD8*0190701-20190701~REF*6R*2190476543Z1~SE*30*000000001~GE*1*19160001~IEA*1*019160001~"
                )
                response = submit_uhc_claim(client, x12_request_data)
                print("TEST API: Claim Submission Response:", response)
            except Exception as e:
                print("TEST API: Error in Claim Submission Test:", e)
