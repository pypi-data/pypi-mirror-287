...
def submit_claims(detailed_patient_data_grouped_by_endpoint, config):
    ...
    for endpoint, patients_data in tqdm(detailed_patient_data_grouped_by_endpoint.items(), desc="Progress", unit="endpoint"):
        ...
        method = config.get('MediLink_Config', {}).get(endpoint, {}).get('submission_method', 'winscp')
...
def handle_transmission_result(transmission_result, config, operation_type, method):
    ...
    if method == "winscp":
        ...
        for response in transmission_result:
            try:
                # Check if the API response indicates a successful submission
                success = response.get('message') == "Claim validated and sent for further processing"
                # Store the success status and message for each transaction ID
                success_dict[response.get('transactionId')] = (success, response.get('message'))
            except Exception as e:
                # Handle any errors encountered while processing the API response
                log("Error processing API response: {}".format(e))
                success_dict[response.get('transactionId')] = (False, str(e))
...
def prepare_receipt_data(submission_results):
    ...
    for endpoint, files in submission_results.items():
        ...
        for file_path, file_result in files.items():
            # Log the current file result to understand its structure
            log("File path: {}".format(file_path), level="INFO")
            log("File result: {}".format(file_result), level="INFO")
            
            try:
                # Unpack the tuple structure to get status and message
                status = file_result.get('status', False)
                message = file_result.get('message', 'No message provided')
            except AttributeError as e:
                log("AttributeError: {}".format(e), level="ERROR")
                continue
            except Exception as e:
                log("Unexpected error: {}".format(e), level="ERROR")
                continue

            log("Status: {}, Message: {}".format(status, message), level="INFO")
            ...
            # Parse patient details and add the result status
            patient_data, _ = parse_837p_file(file_path)
            for patient in patient_data:
                patient['status'] = 'SUCCESS' if status else 'FAILED'
