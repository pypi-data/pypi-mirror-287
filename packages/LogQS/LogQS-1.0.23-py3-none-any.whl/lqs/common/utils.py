...
def get_record_image_ros(record_data, renormalize=True) -> ImagePIL.Image:
    img_modes = {
        "16UC1": "I;16",
        "mono8": "L",
        "mono16": "I;16",
    ...
    if record_image_format is None:
        ...
        if encoding in ["mono16", "16UC1", "32FC1"] and renormalize:
...
def get_record_image(
    ...
    if _encoding is not None:
        ...
        img = get_record_image_ros(record_data=record_data, renormalize=renormalize)
    elif _format is not None:
        # ROS sensor_msgs/CompressedImage
        img = get_record_image_ros(record_data=record_data, renormalize=renormalize)
    elif _data_format is not None:
        # ARK ark::image::Image
        img = get_record_image_ark(record_data=record_data, renormalize=renormalize)
    elif _image is not None:
        # ROS stereo_msgs/DisparityImage
        img = get_record_image_ros(record_data=_image, renormalize=renormalize)
    else:
        # we default to LQS image formats
        img = get_record_image_lqs(record_data=record_data)
