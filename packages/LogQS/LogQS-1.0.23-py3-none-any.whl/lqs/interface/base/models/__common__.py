...
class ResourceModel(BaseModel, Generic[T]):
    ...
    def fetch(cls: Type[T], *args, **kwargs) -> T:
        ...
        if id is not None:
            fetch_method = getattr(
                cls.app.fetch, convert_camel_to_snake_case(cls.__name__)
            )
            ...
            list_method = getattr(
                cls.app.list, convert_camel_to_snake_case(cls.__name__)
            )
    ...
    def list(cls: Type[T], **kwargs) -> List[T]:
        list_method = getattr(cls.app.list, convert_camel_to_snake_case(cls.__name__))
    ...
    def create(cls: Type[T], **kwargs) -> T:
        create_method = getattr(
            cls.app.create, convert_camel_to_snake_case(cls.__name__)
        )
    ...
    def delete(cls, **kwargs) -> None:
        delete_method = getattr(
            cls.app.delete, convert_camel_to_snake_case(cls.__name__)
        )
