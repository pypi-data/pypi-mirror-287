...
agent = Agent(name="New Default bot")
(
    agent.add(GroqLlama370b(config={"groq_api_key": GROQ_API_KEY}))
    # .add(SystemPromptConfig(config={"system_prompt": "ragprompt"}))
    .save()
)
print(agent.id)


ans = agent.invoke(question="What is the difference between eating an apple and eating a cake?", metadata=metadata)
print(ans)
