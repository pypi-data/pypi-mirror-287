...
class RegMap:
    ...
    def load(self, filename):
        ...
        for reg in regs:
            ...
            # Store default value
            reg['Default'] = reg['Value']
            # Guard some possible missing entries
            if 'Min' not in reg:
                reg['Min'] = 0
                reg['Max'] = 0
            if 'Label' not in reg:
    ...
    def write_default(self, name):
        """
        Write default register value as stored in reg map json
        :param name: Register name
        :return: 0 on write success
        :return: None on write error
        """
        reg = self.get_by_name(name)
        # Get default value from register map
        value = reg['Default']
        return self.write_by_name(name, value)
