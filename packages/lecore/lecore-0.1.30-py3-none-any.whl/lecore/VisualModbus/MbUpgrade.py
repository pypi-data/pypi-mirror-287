...
class MbUpgrade:
    ...
    def load_file(self, file_name):
        ...
        # Add a comment field, if needed - extended header
        if self.comment:
            comment = (self.comment[:31]) if len(self.comment) > 31 else self.comment
            hdr_ext_b = b'\x00\x00\x00\x00\x00\x00' + bytes(comment, 'UTF-8')
            if hdr_ext_b[-1] != 0:
                hdr_ext_b += bytes([0])
            if len(hdr_ext_b) % self.align != 0:
                hdr_ext_b += ((0).to_bytes(self.align - (len(hdr_ext_b) % self.align), byteorder='little'))
            for i in range(len(hdr_ext_b) // 2):
                header.append(int.from_bytes(hdr_ext_b[i * 2:i * 2 + 2], byteorder='little', signed=False))
