...
def get_all_actions(names_only=False, label=None, status_only=False):
    try:
        ...
        if names_only:
            params['name'] = ''
        if label:
            endpoint += f"/label/{label}"  # Changed this line
            if status_only:
...
def handle_action_command(args):
    if args.action_command == "list":
        result = get_all_actions(args.names_only, args.label, args.status_only)
        if 'error' not in result:
            if args.status_only:
                # Format the output as a list of label-status pairs
                formatted_result = [f'"{label}": "{status}"' for label, status in result.items()]
