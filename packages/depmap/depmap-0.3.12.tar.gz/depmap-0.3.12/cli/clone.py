...
def clone_repo(url, temp_dir):
    try:
        ...
        if gh_token:
            # If token is available, use it in the URL
            parsed_url = url.replace('https://', f'https://{gh_token}@')
        else:
            # If no token, use the original URL
            parsed_url = url
        
        subprocess.run(['git', 'clone', '--depth', '1', parsed_url, repo_path], check=True)
        return repo_path
