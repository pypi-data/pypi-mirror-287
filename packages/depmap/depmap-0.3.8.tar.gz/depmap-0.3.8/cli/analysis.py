...
def get_analysis_results(label, file=None, details=True, action=None, flatten=False):
    try:
        headers = get_headers()
...
def start_and_poll_analysis(label, model):
    try:
        ...
        delete_result = delete_analysis_results(label)
        ...
        while True:
            ...
            if action_count > 0:
                ...
                pbar.n = min(int(average_progress), 100)
                pbar.set_postfix_str(f"Status: {status['status']}")
                pbar.refresh()
