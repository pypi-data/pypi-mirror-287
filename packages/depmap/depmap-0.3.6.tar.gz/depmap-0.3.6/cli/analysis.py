        ...
        while True:
            ...
            for action, status in actions.items():
                ...
                if action not in action_progress:
                    action_progress[action] = tqdm(total=100, desc=f"Action: {action}", unit="%")

                action_progress[action].n = progress
                action_progress[action].refresh()

                print(f"Action: {action}, Status: {status['status']}, Progress: {progress:.2f}%")

                if status['status'] not in ['COMPLETED', 'FAILED']:
                    all_completed = False

            if all_completed:
                for pbar in action_progress.values():
                    pbar.close()
