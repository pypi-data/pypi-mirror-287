...
from ....common.types import CrawlerBackTask, CrawlerContent, DatapoolContentType, CrawlerDemoUser, StudyUrlTask
...
class FreesoundOrgPlugin(BasePlugin):
    ...
    async def study(self, task: StudyUrlTask, timeout: Optional[int] = 3000) -> List[BaseTag]:
        ...
        return await super().study(task, timeout=10000)
                ...
                if full_local_url:
                    ...
                    if self.is_supported(full_local_url) and not await self.ctx.session.has_url(full_local_url):
                        ...
                        if u.path[0:8] == "/search/":
                            ...
                            added += 1
            logger.info(f"yielded {added} back tasks")
