...
class CrawlerWorker(Stoppable):
    ...
    todo_queues_r: Dict[str, GenericQueue]
    todo_queues_w: Dict[str, GenericQueue]
    ...
    async def wait(self):
        ...
        waiters = [self.todo_queues_r[session_id].until_empty() for session_id in self.todo_queues_r] + [
    ...
    async def stop(self):
        ...
        for session_id in self.todo_queues_r:
            await self.todo_queues_r[session_id].stop()
        logger.info("todo queues_r stopped")
        for session_id in self.todo_queues_w:
            await self.todo_queues_w[session_id].stop()
        logger.info("todo queues_w stopped")
        ...
        try:
            ...
            while not await self.is_stopped():
                ...
                if time.time() - last_queues_update > 5:
                    ...
                    new_queues = {}
                    for session_id, queue in self.todo_queues_r.items():
                        if session_id not in session_ids:
                            logger.info(f"deleting queue {queue.queue.queue_name}")
                            await queue.stop()
                            await queue.delete()
                        else:
                            new_queues[session_id] = queue
                    self.todo_queues_r = new_queues

                    # create new queues
                    for session_id in session_ids:
                        if session_id not in self.todo_queues_r:
                            await self._try_running_queue(
                                session_id,
                                role=QueueRole.Receiver,
                                container=self.todo_queues_r,
                                size=self.cfg.MAX_HIGH_PRIORITY_PROCESSING_TASKS,
                            )

                if len(self.todo_queues_r) > 0:
                    if len(self.todo_tasks) < self.cfg.MAX_HIGH_PRIORITY_PROCESSING_TASKS:
                        # choose random input queue from the list
                        t = int(str(time.time()).split(".")[1])
                        i = t % len(self.todo_queues_r)
                        session_id = list(self.todo_queues_r.keys())[i]
                        # logger.info(f"reading queue for {session_id=} {t=}")
                        queue = self.todo_queues_r[session_id]
    ...
    async def _ack_message(self, message, session_id: Optional[str] = None):
        ...
        elif session_id in self.todo_queues_r:
            await self.todo_queues_r[session_id].mark_done(message)
    ...
    async def _reject_message(self, message, session_id: Optional[str] = None, requeue: Optional[bool] = True):
        if message.routing_key == self.cfg.WORKER_STUDY_URL_TASKS_R_QUEUE_NAME:
            ...
            await self.todo_queues_r[session_id].reject(message, requeue)
        else:
        ...
        if cc.content:
            ...
            if cc.type:
                ...
                if not cc.priority_timestamp:
                    # for now DO NOT PARSE tag from content by default because it's possible way of cheating
                    # if cc.type == DatapoolContentType.Image:
    ...
    async def _try_running_queue(
        self, session_id, role: QueueRole, container: dict, size: Optional[int] = None
    ) -> GenericQueue:
        if session_id not in container:
            container[session_id] = GenericQueue(
                role=role,
                url=self.cfg.QUEUE_CONNECTION_URL,
            ...
            await container[session_id].run()
    ...
    async def _add_delayed_task(self, session_id, task: WorkerTask | DelayedWorkerTask, message):
        ...
        q = await self._try_running_queue(session_id, role=QueueRole.Publisher, container=self.todo_queues_w)
        await q.push(QueueMessage(session_id=session_id, message_type=QueueMessageType.DelayedTask, data=task))
