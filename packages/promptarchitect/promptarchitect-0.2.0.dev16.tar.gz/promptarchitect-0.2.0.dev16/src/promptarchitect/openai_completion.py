...
class OpenAICompletion:
    ...
    def __init__(self, system_role: str = "", model=DEFAULT_MODEL, parameters={}):
        ...
        with open(price_file_path, "r") as pricing_file:
            self.openai_pricing = json.load(pricing_file)

        # Check if the parameters are valid
        if model not in self.openai_pricing.keys():
    ...
    def _calculate_cost(self, response: dict) -> float:
        ...
        cost = (
            input_tokens * self.openai_pricing[self.model]["input_tokens"]
            + output_tokens * self.openai_pricing[self.model]["output_tokens"]
        )
