...
class CalculatedCompletion(Completion):
    def __init__(self, parameters={}):
        ...
        self.system_role = SYSTEM_ROLE  # Override the system role
    ...
    def completion(self, prompt: str) -> str:
        ...
        self.latency = default_timer() - start
        return response
    ...
    def from_dict(cls, data):
        ...
        obj.system_role = data.get("system_role", SYSTEM_ROLE)
...
class FormatCheckCompletion:
    ...
    @classmethod
    def from_dict(cls, data):
        # format_str = data.get("format", "TEXT").upper()
        # format_enum = Format[format_str]
        obj = cls(parameters=data.get("parameters", {}))
...
if __name__ == "__main__":
    format_checker = FormatCheckCompletion()
    comment = "FORMAT == html"
    completion = "<HTML><BODY><H1>Hello World</H1></BODY></HTML>"

    prompt = f"{comment}\n{completion}"
    if format_checker.format_check_used(prompt):
        response = format_checker.completion(prompt)
        print(response)
