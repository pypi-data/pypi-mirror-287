...
from tensorkube.services.knative_service import list_deployed_services, get_knative_service, \
    get_ready_condition, get_pods_for_service
from tensorkube.services.k8s_service import start_streaming_pod, ssh_into_pod_with_podman




def list_tensorkube_deployments(env_name: Optional[str] = None, all: bool = False):
    ...
    services = list_deployed_services(env_name=env_name, all=all)['items']
    for service in services:
        ...
        table.add_row(service['metadata']['name'], service['status']['url'],
                    latest_ready_revision,
                    ready_condition['status'], service['metadata']['namespace'],ready_condition.get('reason', None))
...
def describe_deployment(service_name: str, env_name: Optional[str] = None):
    ...
    tree.add("[bold]URL:[/] [cyan]" + deployment['status']['url'])
    tree.add("[bold]Latest Creaded Revision:[/] " + deployment['status']['latestCreatedRevisionName'])
