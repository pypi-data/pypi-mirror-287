...
def create_aws_secret(credentials, namespace: str = DEFAULT_NAMESPACE):
    config.load_kube_config()
    v1 = client.CoreV1Api()
...
def delete_aws_secret(namespace: str = DEFAULT_NAMESPACE):
    config.load_kube_config()
    v1 = client.CoreV1Api()
    try:
...
def apply_k8s_buildkit_config(sanitised_project_name: str, image_tag: str, region: str = REGION,
    ...
    namespace_to_use = env_name if env_name else DEFAULT_NAMESPACE  # Replace 'default' with your default namespace if needed
    ...
    buildkit_config['spec']['template']['spec']['containers'][0]['command'] = ["/bin/sh", "-c", f"""
        mkdir -p /data/cache
...
def apply_image_cleanup_job(sanitised_project_name: str, image_tags: List[str], env: Optional[str] = None):
    ...
    k8s_client = client.ApiClient()
    utils.create_from_dict(k8s_client, cleanup_config)
...
def find_and_delete_old_job(job_name: str, namespace: str = DEFAULT_NAMESPACE):
    config.load_kube_config()
    v1 = client.CoreV1Api()
...
def delete_all_jobs_in_namespace(namespace: str):
    config.load_kube_config()
    v1 = client.BatchV1Api()
    jobs = v1.list_namespaced_job(namespace=namespace)
    for job in jobs.items:
...
def start_streaming_pod(pod_name, namespace, status=None, container_name=None, 
                        retry_number: int=0, max_retries: int=4):
    config.load_kube_config()
    v1 = client.CoreV1Api()
    ...
    try:
        ...
        while True:
            logs = v1.read_namespaced_pod_log(name=pod_name, namespace=namespace, container=container_name, since_seconds=5)
...
def evict_pods_from_node(node_name):
    config.load_kube_config()
...
def drain_and_delete_node(node_name):
    core_api = client.CoreV1Api()
    # Cordon the node
...
def get_nodes_not_using_bottlerocket(ec2_node_class_label):
    config.load_kube_config()
    v1 = client.CoreV1Api()
...
def delete_pv_using_name(pv_name: str):
    config.load_kube_config()
    v1 = client.CoreV1Api()
...
def delete_pvc_using_name_and_namespace(pvc_name: str, namespace: str):
    config.load_kube_config()
    v1 = client.CoreV1Api()
    timeout = 60  # Timeout in seconds
    start_time = time.time()
...
def check_pvc_exists_by_name(claim_name, namespace):
...
def get_pod_name_corresponing_to_job(job_name: str, namespace: str = DEFAULT_NAMESPACE) -> Optional[str]:
    config.load_kube_config()
    v1 = client.CoreV1Api()
    pods = v1.list_namespaced_pod(namespace=namespace)
...
def create_new_namespace(env_name: str):
    config.load_kube_config()
    namespace = client.V1Namespace()
...
def delete_namespace(env_name: str):
...
def ssh_into_pod_with_podman(pod_name: str, namespace: str):
    command = ["kubectl", "exec", "-it", pod_name, "-n", namespace, "-c", "user-container", "--", "sh", "-c", "podman exec -it $(podman ps -q) /bin/bash"]
