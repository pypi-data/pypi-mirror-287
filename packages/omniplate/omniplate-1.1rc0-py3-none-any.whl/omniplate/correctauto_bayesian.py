        ...
        for c in sunder.getset(
            ...
            for s in sunder.getset(
                ...
                if (
                    ...
                    # check if predicted_fl present
                    df = self.s.query(
                        "experiment == @e and condition == @c "
                        "and strain == @s"
                    )
                    if f"predicted_{f[0]}" in df.columns:
                        predicted_fl = df[f"predicted_{f[0]}"].dropna().values
                    ...
                    print(f"{e}: {s} in {c}")
                    if (
                        "predicted_fl" in locals()
                        and predicted_fl.size == t.size
                    ):
                        print(
                            "Using previous estimates for corrected fluorescence."
                        )
                    else:
                    ...
                    if True:
                        ...
                        flperod_samples = predicted_fl.reshape(
                            t.size, 1
                        ) * np.exp(-lod_samples)
                        ff, _ = runfitderiv(
                            self,
                            t,
                            np.mean(flperod_samples, axis=1),
                            "flperod",
                            "d/dt_flperod",
                            experiment=e,
                            condition=c,
                        ...
                        autofdict = {
                            ...
                            f"predicted_{f[0]}": predicted_fl,
                            f"{bname}perOD": ff.f,
                            f"{bname}perOD_err": np.sqrt(ff.fvar),
                        }
                        # add to data frames
                        omcorr.addtodataframes(self, autofdict)
