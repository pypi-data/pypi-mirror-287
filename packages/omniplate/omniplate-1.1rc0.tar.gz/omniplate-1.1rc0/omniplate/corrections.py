...
def findODcorrection(
    wdirpath, ODfname, figs, bd, gp_results, odmatch_min, max_nunchaku_segments
):
    """
...
def correctauto(
    ...
    if usebayesian:
        ...
        if max_data_pts is not None:
            print(
                "correctauto: the bayesian method does not use max_data_pts."
            )
        if not useGPs:
            print("correctauto: the bayesian method does not use use_GPs.")
    ...
    for e in exps:
        for c in cons:
            if not usebayesian and self.progress["negativevalues"][e]:
                for datatype in f:
                    if (
...
def addtodataframes(self, datadict):
    ...
    gu.merge_df_into(self.s, newdf, key_cols)
...
def sample_ODs_with_GP(self, experiment, condition, strain, t, od, nosamples):
...
def sample_flperod_with_GPs(
    ...
    if np.any(fl[~np.isnan(fl)]):
        ...
        f_samples = ff.fitderivsample(nosamples)[0]
        if logs:
