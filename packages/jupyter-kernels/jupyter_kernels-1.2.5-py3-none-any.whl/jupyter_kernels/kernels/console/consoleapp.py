...
from ...application_base import (
    JupyterKernelsBaseApp,
    jupyter_kernels_aliases,
    jupyter_kernels_flags,
)
...
flags = dict(jupyter_kernels_flags)
flags.update(
    boolean_flag(
        "simple-prompt",
...
# copy flags from mixin
aliases = dict(jupyter_kernels_aliases)
...
class KernelConsoleApp(JupyterKernelsBaseApp):
    ...
    def initialize(self, argv=None):
        ...
        if getattr(self, "_dispatching", False):
            return
        JupyterKernelsBaseApp.initialize(self)
