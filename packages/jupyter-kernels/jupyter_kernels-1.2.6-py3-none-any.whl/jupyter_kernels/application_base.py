...
jupyter_kernels_aliases["kernels-url"] = "JupyterKernelsBaseApp.kernels_url"
jupyter_kernels_aliases["token"] = "JupyterKernelsBaseApp.token"
jupyter_kernels_aliases["external-token"] = "JupyterKernelsBaseApp.external_token"
...
class JupyterKernelsBaseApp(DatalayerApp):
    ...
    def initialize(self, argv=None):
        ...
        self.log.debug(
            "Jupyter Kernels - Version %s - Connected as %s on %s",
            self.version,
            self.username,
            self.kernels_url,
        )
        console = Console()
        console.print()
        console.print(f"Jupyter Kernels - Version [bold cyan]{self.version}[/bold cyan] - Connected as [bold yellow]{self.username}[/bold yellow] on [i]{self.kernels_url}[/i]")
        console.print()
            ...
            if ans is None:
                ...
                try:
                    ...
                    keyring.set_password(
                        self.kernels_url, "access_token", self.token
                    )
                ...
                if stored_token:
                    ...
                    try:
                        ...
                        if error.response.status_code == 401:
                            ...
                            self.log.debug(
                                f"Delete invalid cached token for {self.kernels_url}"
                            )
                            self._log_out()
                        else:
                            self.log.warning(
