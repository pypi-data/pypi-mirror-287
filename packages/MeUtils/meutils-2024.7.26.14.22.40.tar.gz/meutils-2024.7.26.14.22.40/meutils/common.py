...
def storage_to_cookie(storage: Union[str, dict]):
    if isinstance(storage, str) and len(storage) < 256 and Path(storage).is_file():
...
def url2fileview(url):
    ...
    return f"https://file.chatfire.cn/onlinePreview?url={encoded_url}"
