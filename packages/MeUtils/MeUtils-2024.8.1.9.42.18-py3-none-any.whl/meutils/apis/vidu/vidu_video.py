...
async def upload(file, token: Optional[str] = None):  # todo: 统一到 file object
    ...
    async with httpx.AsyncClient(base_url=UPLOAD_BASE_URL, headers=headers, timeout=60) as client:
        ...
        if response.is_success:
            ...
            response = await client.put(put_url, content=file, headers={**headers, "Content-Type": "image/*"})
...
if __name__ == '__main__':
    ...
    # file = Path('/Users/betterme/PycharmProjects/AI/QR.png').read_bytes()
    ...
    # arun(get_task('vidu-2369758774774058', token=token))
