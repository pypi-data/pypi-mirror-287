...
def affinity_benchmark(result_path, benchmark_name="BTK"):
    ...
    if result_path is None:
        ...
        smiles_keys = set(benchmark_affinity.keys()) & set(result.keys())
        values_benchmark = [benchmark_affinity[smiles] for smiles in smiles_keys]
        values_result = [result[smiles] for smiles in smiles_keys]

        pearson_corr, _ = scipy.stats.pearsonr(values_benchmark, values_result)
        spearman_corr, _ = scipy.stats.spearmanr(values_benchmark, values_result)
        kendall_corr, _ = scipy.stats.kendalltau(values_benchmark, values_result)
