    ...
    async def opt_vol_anal(self, headers, ids):
        try:
            ...
            async with aiohttp.ClientSession(headers=headers) as session:
                ...
                contract_info = [self.fetch_contract_data(session,id) for id in ids]
                contract_results = await asyncio.gather(*contract_info)
                ...
                for data, contract_data in zip(fetch_results, contract_results):
                        ...
                        ask = contract_data[0].get('ask')
                        bid = contract_data[0].get('bid')
