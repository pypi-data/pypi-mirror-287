...
references裡的資料，是依據你回答的內容所參考的「知識庫」內容，格式如下，下面只是示意，不要直接用：
{
    "ori_file_name":"",
    "page_number:1,
    "google_file_id":"",
...
ori_file_name 的內容，請參考「知識庫」回傳的 ori_file_name tag 內的結果，如果辨別不出來，就放空字串
page_number 的內容，請參考「知識庫」回傳的 page_number tag 內的結果，只要放數字，如果辨別不出數字，就放空字串
google_file_id 的內容，請參考「知識庫」回傳的 google_file_id tag 內的結果，如果辨別不出來，就放空字串
gen_page_imgs 的內容，請參考「知識庫」回傳的 gen_page_imgs tag 內的結果，可能會是 True, False 或是空字串，如果辨別不出來，就放空字串

如果回答時有參考多個「知識庫」的內容，請依序列加在 references 的 array 裡面

因此整個回答的範例會為，以下只是範例，不要直接使用：
原本要回答的內容
...
<<Botrun.ai波特人的沉穩參考來源>>
{
    "references":[
    {
        "ori_file_name":"filename.pdf",
        "page_number:"2",
        "google_file_id":"123456ABC",
        "gen_page_imgs":"True",
...
# 從使用者問提產生關鍵字清單
@timing_decorator
def generate_keywords(user_input: str, model="gpt-4o-mini") -> list:
    # 達成生成關鍵字的邏輯
    ...
    response = completion(
        ...
        messages=[{"role": "user",
                   "content":
                       f'''找出問題中1個用來搜尋的專有名詞，以 "keyword1,keyword2" 的格式回傳，優先選中文專有名詞，英文的專有名詞給我全大寫、全小寫、第一個字母與連接號後第一個字母大寫三種字串格式'''
                       f"上下文不要多廢話，只給專有名詞就好，純粹給我關鍵字清單就好:\n\n{user_input}"}]
    )
...
def query_qdrant_knowledge_base_keyword(qdrant_host,
    ...
    try:
        ...
        user_input_keywords = generate_keywords(user_input, model=chat_model) 
        ...
        open("./users/botrun_ask_folder/str_knowledge_base_keyword.txt", "w").write(str_knowledge_base)
