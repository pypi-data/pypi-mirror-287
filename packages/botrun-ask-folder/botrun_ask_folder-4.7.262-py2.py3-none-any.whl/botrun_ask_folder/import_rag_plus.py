...
def import_rag_plus(websocket, chat_context, session_memory, websocket_send) -> List[Dict[str, Any]]:
    ...
    for json_block in json_blocks:
        try:
            str_knowledge_base = "" #初始化向量搜尋的結果字串
            ...
            is_keyword_search=json_block.get("is_keyword_search", False) #是否啟用專有名詞關鍵字搜尋機制
            ...
            if is_keyword_search:
                str_knowledge_base_keyword = query_qdrant_knowledge_base_keyword(
                    ...
                    top_k=json_block.get("keyword_top_k", 4), #單獨設定關鍵字全文檢索回傳資料最大筆數
                    is_show_ref_info=json_block.get("is_show_ref_info", False), #單獨設定關鍵字全文檢索是否回傳引證來源
                    hnsw_ef=json_block.get("hnsw_ef", 256)
                )
                #用字串加到 向量搜尋的結果字串中
