...
use taiao_storage_py::record::PythonRecord;
...
pub fn python_service_main(
    ...
    let service_main = move || {
        ...
        match storage {
            ...
            OwnedStorageTypeInner::Stream { state_schema, output_schema } => taiao_declare_service_macro::declare_service!(
                    @service_main_impl
                    client=client,
                    project=project,
                    service=service,
                    storage=storage_type,
                    body=PythonService::<OwnedStream<PythonRecord, PythonRecord>>::new(body, locals),
                    privacy=privacy,
                    interval=periodicity,
                    listen=listen,
                    storage-init=(state_schema, output_schema)
                )
        }
    };
