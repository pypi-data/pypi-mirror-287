...
class TextProcessor:
    ...
    def url_decode(input_text):
        ...
        while urllib.parse.unquote(processed_text) != processed_text:
            processed_text = urllib.parse.unquote(processed_text)
    ...
    def remove_repetitions(text):
        # Define a regular expression pattern to match emoji descriptions
        rep_ptrn = r"((.+?)(?:[?.,;:!])?)\s(?:\1\s){1,}\2(\s|[?.,;:!]|$)"
        rep_repl = r"\g<2>\g<3>"
        text = re.sub(rep_ptrn, rep_repl, text, flags=re.IGNORECASE)
        return text
    
    @staticmethod
    def preprocess_for_lang_id(input_text):
        if input_text:
            processed = str(input_text)
            processed = TextProcessor.url_decode(processed)
            processed = TextProcessor.remove_emojis(processed)
    ...
    # For now no difference between lang id and translation, but that might chance over time
    @staticmethod
    def preprocess_for_translation(input_text):
        return TextProcessor.preprocess_for_lang_id(input_text)
    ...
    def postprocess(input_text):
        if input_text:
            processed = str(input_text)
            return TextProcessor.remove_repetitions(processed)
        else:
            return ""
    
    @staticmethod
    def postprocess_for_translation(input_text):
        if input_text:
            processed = str(input_text)
            return TextProcessor.remove_repetitions(processed)
        else:
