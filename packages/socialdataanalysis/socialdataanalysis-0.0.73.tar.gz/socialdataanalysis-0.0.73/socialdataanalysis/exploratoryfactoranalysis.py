...
from factor_analyzer.confirmatory_factor_analyzer import ConfirmatoryFactorAnalyzer, ModelSpecificationParser
...
def calculate_fit_indices(df, variables, method='ml', n_factors=3, rotation='varimax'):
    ...
    if np.linalg.cond(corr_matrix) < 1 / np.finfo(corr_matrix.dtype).eps:
        ...
        # Criar a especificação do modelo para CFA
        model_dict = {f"Factor{i+1}": variables[i::n_factors] for i in range(n_factors)}
        ...
        def calculate_gfi(cfa, data):
            s = np.cov(data.T)
            sigma_hat = cfa.get_model_implied_cov()
            return 1 - (np.sum((s - sigma_hat) ** 2) / np.sum(s ** 2))
        
        def calculate_agfi(gfi, n, p):
            return gfi - (p * (p + 1) / (2 * n * (n - 1))) * (1 - gfi)
        
        def calculate_rmsr(cfa, data):
            sigma_hat = cfa.get_model_implied_cov()
            s = np.cov(data.T)
            residuals = s - sigma_hat
            return np.sqrt(np.sum(residuals[np.tril_indices_from(residuals)] ** 2) / (s.shape[0] * (s.shape[0] - 1) / 2))
        
        def calculate_srmr(cfa, data):
            sigma_hat = cfa.get_model_implied_cov()
            s = np.cov(data.T)
            residuals = s - sigma_hat
            return np.sqrt(np.sum(residuals ** 2) / (s.shape[0] * (s.shape[0] + 1) / 2))
        
        gfi = calculate_gfi(cfa, data)
        agfi = calculate_agfi(gfi, data.shape[0], data.shape[1])
        rmsr = calculate_rmsr(cfa, data)
        srmr = calculate_srmr(cfa, data)
