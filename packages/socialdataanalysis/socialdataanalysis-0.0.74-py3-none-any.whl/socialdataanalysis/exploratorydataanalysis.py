...
def gerar_tabela_frequencias(df, meta, variaveis):
    ...
    for var in variaveis:
        ...
        frequency_table = df[var].value_counts(dropna=False).sort_index().reset_index()
    ...
    # Calcular os percentuais
    total_cases = len(df) * len(variaveis)  # total de casos é multiplicado pelo número de variáveis
...
def casos_menores_desvios_padrao(df, variaveis, id_coluna=None, limite_desvio=0.5):
    ...
    df['Desvio padrão'] = df[variaveis].std(axis=1, ddof=0)
    
    # Filtrar os casos com desvio padrão menor que o limite especificado
    menores_desvios = df[df['Desvio padrão'] < limite_desvio]
    
    # Ordenar pelo desvio padrão em ordem crescente
    menores_desvios = menores_desvios.sort_values(by='Desvio padrão')
    
    # Reordenar as colunas para mostrar o ID (se fornecido) e o desvio padrão ao final
    if id_coluna:
        cols = [id_coluna] + variaveis + ['Desvio padrão']
    else:
        cols = variaveis + ['Desvio padrão']
    
    menores_desvios = menores_desvios[cols]
    
    return menores_desvios