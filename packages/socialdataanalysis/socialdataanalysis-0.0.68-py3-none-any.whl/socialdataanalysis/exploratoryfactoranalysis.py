...
def calculate_fit_indices(observed_corr_matrix, reproduced_corr_matrix):
    """
    Calcula os índices de bondade do ajustamento GFI, AGFI e RMSEA.

    Parâmetros:
    observed_corr_matrix (np.ndarray): Matriz de correlações observadas.
    reproduced_corr_matrix (np.ndarray): Matriz de correlações reproduzidas.

    Retorna:
    dict: Índices GFI, AGFI e RMSEA.
    """
    # Número de variáveis
    p = observed_corr_matrix.shape[0]
    ...
    # Calcular o GFI
    gfi_numerator = np.sum(np.square(observed_corr_matrix - reproduced_corr_matrix))
    gfi_denominator = np.sum(np.square(observed_corr_matrix))
    gfi = 1 - (gfi_numerator / gfi_denominator)

    # Calcular o AGFI
    df_model = p * (p + 1) / 2
    agfi = 1 - (df_model / (df_model - p)) * (1 - gfi)
    
    # Calcular o RMSEA
    chi_squared = np.sum(np.square(residual_matrix))
    rmsea = np.sqrt(chi_squared / (df_model * (p - 1)))

    # # Calcular o RMSR
    rmsr = np.sqrt(np.mean(np.square(residual_matrix)))
    ...
    # Exibir tabela formatada
    headers = ["Index", "Value"]
    table = [[key, f"{value:.4f}"] for key, value in fit_indices.items()]
    print("Fit Indices:")
