...
def cvt_copy_model(transform_config_path, src_model_path, dst_model_path, src_dataset_uri: str=''):
    """ cvt copy model
        Args:
    ...
    bcelogger.info(f"src_model {src_model_path}, dst_model {dst_tmp_path}, cfg_path {transform_config_path}")
    img_list_file = ''
    if len(src_dataset_uri) > 0:
        with open(src_dataset_uri + "/val.json", 'r') as file:
            data = json.load(file)

            # 获取量化图片列表, 并保存到文件/root/tmp_img_list_path/img_list.txt
            quant_img_list = []
            for image in data['images']:
                quant_img_list.append(image['file_name'])
            if len(quant_img_list) == 0:
                bcelogger.error("quant img list is empty")
            img_list_tmp_path = '/root/tmp_img_list_path'
            if(os.path.exists(img_list_tmp_path) is False):
                os.makedirs(img_list_tmp_path)
            img_list_file = img_list_tmp_path + '/img_list.txt'
            fw = open(img_list_file, 'w')
            fw.write('\n'.join(quant_img_list))
            fw.close()

    bcelogger.info(f"img_list_file {img_list_file}")
    run = Runner(src_path=src_model_path, dst_path=dst_tmp_path, \
        cfg_path=transform_config_path, src_dataset_list=img_list_file)
    run.run()
