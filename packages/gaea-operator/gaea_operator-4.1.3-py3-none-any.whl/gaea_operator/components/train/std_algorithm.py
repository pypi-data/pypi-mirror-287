...
def _modify_detection_annotations(src_name, dst_path, abs_work_path, all_image_folders):
    """
    ...
    for img in images:
        img["file_name"] = _cvt_2_abs_path(
                img["file_name"], abs_work_path, default_prefix, abs_work_folders, abs_work_default_folders)
...
def _modify_classify_annotations(src_name, dst_path, abs_work_path):
    """
    ...
    with open(src_name) as f:
        ...
        with open(dst_name, 'w') as dst_f:
            for l in lines:
                ...
                frags[0] = _cvt_2_abs_path(frags[0], abs_work_path)
...
def _get_txt_annotation_type(name):
    ...
    """
    with open(name) as f:
        last_val = f.readlines()[0].split(NX_SPLIT_CHAR)[-1]
...
def _modify_annotation(src_name, dst_path, abs_work_path, all_image_folders):
    ...
    if annotation_type == NX_ANNOTATION_TYPE_DET:
        return _modify_detection_annotations(src_name, dst_path, abs_work_path, all_image_folders)
    elif annotation_type == NX_ANNOTATION_TYPE_CLS:
        return _modify_classify_annotations(src_name, dst_path, abs_work_path)
...
def std_algorithm_train(args):
    ...
    config['data_load']['eval'][key_sample_prob] = 1
    ...
    train_command = (
                ...
                f'python -m v2x_model_standardization ; '
                f'cd /root/{task_name} &&'
                f'bash train.sh 2>&1 | tee -a std_alg_log_name && '
                f'bash export.sh'
                )
