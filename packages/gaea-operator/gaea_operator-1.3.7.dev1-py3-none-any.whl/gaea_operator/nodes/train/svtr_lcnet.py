...
from gaea_operator.config import SVTRLCNetConfig, KEY_NETWORK_ARCHITECTURE
from gaea_operator.config.svtr_lcnet.template.modify_train_parameter import KEY_TEXT_SCENE
from gaea_operator.metric.types.metric import LOSS_METRIC_NAME, \
    PRECISION_METRIC_NAME, \
    RECALL_METRIC_NAME, \
    OCR_ACCURACY_METRIC_NAME
...
def ppyoloe_plus_train(args):
    ...
    model_template = ModelTemplate(algorithm=ModelTemplate.SVTR_LCNET_NAME)
    ...
    # 3. 下载预训练模型
    pretrain = {"text_english": "en_PP-OCRv4_rec_train",
                "text_chinese": "ch_PP-OCRv4_rec_train"}
    pretrain_name = pretrain[train_advanced_parameters[KEY_TEXT_SCENE]]
    ...
    config = SVTRLCNetConfig(windmill_client=windmill_client, tracker_client=tracker_client)
    ...
    metric_names = [LOSS_METRIC_NAME, PRECISION_METRIC_NAME, RECALL_METRIC_NAME, OCR_ACCURACY_METRIC_NAME]
    ...
    # 6. 创建模型
    metric_name = OCR_ACCURACY_METRIC_NAME
