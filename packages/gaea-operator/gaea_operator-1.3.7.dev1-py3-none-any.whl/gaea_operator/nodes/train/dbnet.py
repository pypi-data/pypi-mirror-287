...
from gaea_operator.config import DBNetConfig, KEY_NETWORK_ARCHITECTURE
...
def dbnet_train(args):
    ...
    model_template = ModelTemplate(algorithm=ModelTemplate.DBNET_NAME)
    ...
    # 3. 下载预训练模型
    pretrain = {"dbnet_student": "PPLCNetV3_x0_75_ocr_det",
                "dbnet_teacher": "PPHGNet_small_ocr_det"}
    ...
    config = DBNetConfig(windmill_client=windmill_client, tracker_client=tracker_client)
    ...
    metric_names = [LOSS_METRIC_NAME, PRECISION_METRIC_NAME, RECALL_METRIC_NAME, HMEAN_METRIC_NAME]
