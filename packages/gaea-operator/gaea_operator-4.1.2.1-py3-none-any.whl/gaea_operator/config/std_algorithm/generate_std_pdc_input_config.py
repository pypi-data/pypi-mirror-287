...
def parse_args():
    ...
    parser.add_argument('--gpu_num', type=int, required=True, help="gpu num")
...
def generate_hyperparameters(batch_size, learning_rate, epochs, advanced_parameters):
    ...
    hyperparameters = {'auto': auto, 'batch_size': batch_size, 'lr': lr, 'epochs': epochs}
...
def generate_input_config(advanced_parameters, generate_config, label_description, input_config):
    ...
    ## 1. 任务名称
    paddle_framework = generate_config['base_and_pretrain'][advanced_parameters["base_model"]]['paddle_framework']
    task_name = paddle_framework + '_task'
    ## 2. 训练数据默认为空，等待产线自动填写，无需处理

    ## 3. 训练的模板和预训练权重，由外部指定 base 并关联到唯一预训练权重
    base, pretrained_model = get_base_and_pretrain(advanced_parameters["base_model"], generate_config)

    ## 4. 根据 label_description 自动生成任务配置
    supported_tasks = generate_config['base_and_pretrain'][advanced_parameters["base_model"]].get('supported_tasks', [])
    ...
    ## 7. 设置训练显卡
    CUDA_VISIBLE_DEVICES = list(range(args.gpu_num))
...
def main(args):
    ...
    dataset_relative_path = artifact_path.replace("s3://" + filesystem["endpoint"] + "/", "")
    ...
    check_ret = check_label_description(advanced_parameters["base_model"], generate_config, label_description)
    ...
    label_description = label_description_reorder(advanced_parameters["base_model"], label_description, generate_config)
