...
class SharedData:
    ...
    def __init__(self, source, 
                 user='guest',access_key_id=None,secret_access_key=None):
        self.source = source
        self.user = user
        ...
        if not self.islogged:
            self.islogged = True
            try:
                version = pkg_resources.get_distribution("shareddata").version
    ...
    ######### LIST ############    
    def listall(self, keyword='', user='master'):
        tables = pd.DataFrame()
        ...
        if arrobj.size>0:
            ...
            dfremote['last_modified'] = pd.to_datetime(dfremote['last_modified']).dt.tz_localize(None)
        ...
        if len(ls)>0:
            ...
            ts = list(timeseries.itertuples())[0]
