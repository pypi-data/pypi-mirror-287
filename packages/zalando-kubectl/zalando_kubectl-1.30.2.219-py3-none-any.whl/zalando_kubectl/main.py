...
def login(config, cluster_or_url: str):
    ...
    if looks_like_url(cluster_or_url):
        ...
        if cluster["provider"] == "zalando-eks":
            ...
            ca = cluster["config_items"]["eks_certficate_authority_data"]
...
def credentials(env: Environment, cluster):
    kube_config.get_auth(env, cluster, access_request._has_any_state_file(cluster))
...
def do_login(env: Environment, cluster, okta, force_refresh):
    ...
    url, alias, ca = login(env.config, cluster)
    env.store_config()

    token = auth_token()
    if okta:
        token = kube_config.get_auth_token(env, alias, force_refresh)

    zalando_kubectl.kube_permissions.check_cluster_permissions(url, alias, token)

    with Action("Writing kubeconfig for {}..".format(url)):
        kube_config.update(url, alias, token, okta, ca=ca)
...
@click.pass_obj
def request_cluster_access(env: Environment, emergency, incident, user, reason, okta, wait_for_approval, timeout):
    ...
    if emergency:
        ...
        make_emergency_access_request(env, reference_url, user, reason, okta, wait_for_approval, timeout)
    else:
        make_manual_access_request(env, user, reason, okta, wait_for_approval, timeout)


def make_emergency_access_request(env: Environment, reference_url, user, reason, okta, wait_for_approval, timeout):
    if okta:
        access_request.create_with_okta(env, "emergency", reference_url, " ".join(reason), wait_for_approval, timeout)
        return
...
def make_manual_access_request(env: Environment, user_option, reason, okta, wait_for_approval, timeout):
    if okta:
        access_request.create_with_okta(env, "privileged", None, " ".join(reason), wait_for_approval, timeout)
        return
...
def list_cluster_requests(env: Environment, okta, request_key, output):
    ...
    if okta:
        access_request.list_with_okta(env, request_key, output)
        return
