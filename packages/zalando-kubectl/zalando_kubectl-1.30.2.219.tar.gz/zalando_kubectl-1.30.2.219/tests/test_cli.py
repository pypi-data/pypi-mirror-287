...
def test_login(monkeypatch):
    ...
    mock_cluster = {"api_server_url": api_url, "alias": "foo", "provider": "zalando-aws"}
    ...
    url, alias, ca = zalando_kubectl.main.login(config, "aws:123:eu-west-1:my-kube-1")
    assert api_url == url
    assert alias == "foo"
    assert ca is None

    url, alias, ca = zalando_kubectl.main.login(config, "foo.example.org")
    assert "https://foo.example.org" == url
    assert alias is None
    assert ca is None

    url, alias, ca = zalando_kubectl.main.login(config, "my-alias")
    assert api_url == url
    assert alias == "foo"
    assert ca is None

    url, alias, ca = zalando_kubectl.main.login(config, "https://foo.example.org")
    assert "https://foo.example.org" == url
    assert alias is None
    assert ca is None
...
def test_login_eks(monkeypatch):
    ...
    mock_cluster = {
        ...
        "config_items": {
            ...
            "eks_certficate_authority_data": eks_ca,
        },
    }
    ...
    url, alias, ca = zalando_kubectl.main.login(config, "aws:123:eu-west-1:my-kube-eks-1")
    assert eks_url == url
    assert alias == "foo-eks"
    assert ca == eks_ca

    with pytest.raises(zalando_kubectl.main.ClusterAccessUnsupported):
        zalando_kubectl.main.login(config, "foo-eks.example.org")

    url, alias, ca = zalando_kubectl.main.login(config, "my-eks-alias")
    assert eks_url == url
    assert alias == "foo-eks"
    assert ca == eks_ca

    with pytest.raises(zalando_kubectl.main.ClusterAccessUnsupported):
        zalando_kubectl.main.login(config, "https://foo-eks.example.org")
