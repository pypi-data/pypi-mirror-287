...
class Issue(Base):
	...
	def get_by_project_function(self, project, last_hour=False, **kwargs):
		result = []
		issues = []
		...
		try:
			...
			result = self.find_by_project(project['key'], last_hour)
			for issue in result:
				value = issue.__dict__
				if 'parent' in value['raw']['fields']:
					value['raw']['fields']['parent'] = self.find_by_id(value['raw']['fields']['parent']['id']).__dict__
	...
	def find_by_project(self, project_key, last_hour=False):
		"""
		...
		try:
			...
			if last_hour:
				jql_query = (f'project = {project_key} AND sprint = {sprint_id} AND (status = "In Progress" OR (status = "Done"'
							 f'AND updated >= -1h))')
