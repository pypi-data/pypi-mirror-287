...
DEFAULT_AGENT_OPT_DATASET = "default agent optimization"
DEFAULT_LLM_OPT_DATASET = "default llm optimization"
...
class DBConnector(Task):
  ...
  def _initialize_default_datasets(self):
    """
    Check if the default datasets exist and create them if they don't.
    """
    with self.session_local() as session:
      self.add_if_not_exist(
        session, DEFAULT_AGENT_OPT_DATASET, "This is the default dataset for agent optimization."
      )
      self.add_if_not_exist(
        session, DEFAULT_LLM_OPT_DATASET, "This is the default dataset for LLM optimization."
      )
      session.commit()

  def add_if_not_exist(self, session: Session, name: str, description: str) -> None:
    """
  ...
  def add_example_to_dataset_by_name(self, dataset_name: str, input_data: str, output_data: str):
    with self.session_local() as session:
      ...
      self.add_examples_to_dataset(dataset.id, examples)
  ...
  def log_run(
    ...
    """
    Logs a run to the database with the provided agent details and outputs.
  ...
  def add_examples_to_dataset(self, dataset_id: str, examples: List[dict]):
    """
...
class Run(Base):
  ...
  user_email = Column(String, nullable=False, default="anonymous")
