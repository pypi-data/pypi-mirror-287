if __name__ == '__main__':
    ...
    from . import *
from psplpyProject.psplpy.file_utils import *
...
def test_get_file_paths():
    ...
    rel_list = ['test.txt', 'inner/inner_test.txt', 'inner/inner_test2.txt']

    paths_list = get_file_paths(folder_path)
    assert paths_list == abs_list

    paths_list = get_file_paths(folder_path, relative=True)
    assert paths_list == rel_list

    paths_list = get_file_paths(folder_path, generator=True)
