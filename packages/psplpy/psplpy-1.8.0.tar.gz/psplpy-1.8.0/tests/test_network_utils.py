if __name__ == '__main__':
    ...
    from . import *
from psplpyProject.psplpy.network_utils import *
...
def test_mp_http():
    ...
    s = TestServer(workers=6)
    s.run_server(new_thread=True)
    time.sleep(0.25)
    c = MpHttpClient()
    load_results = futures.ThreadPoolExecutor().map(show_load, [c])
    ...
    future = c.delay_get(0.5)
    time.sleep(0.5)
    t = time.perf_counter()
    result = future.result()
    elapsed = time.perf_counter() - t
    assert result == {'result': 0.5, 'num': 1}, result
