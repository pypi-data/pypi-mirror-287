...
<script lang="ts">
	...
	function setCookieMap() {
		...
		accessKey = appAccessKey ?? cookieMap.get('appAccessKey') ?? 'sk-3fa4e645488f42b6ab2c130d4a44b0ad' ?? 'sk-cf76953ad52a471587cda01547170e09'
		xAppKey = clientName ?? cookieMap.get('clientName') ?? 'brm-file-uuid1722309865' ?? 'opensdk-demo--test-uuid1720691302'
	}
	...
	onMount(() => {
		...
		window.addEventListener("message", async function(e: any) {
			...
			if (res.type === 'selectFromBohrium' && res.status === 'succeed') {
				value = decodeURIComponent(res.data.url).replaceAll(' ', '%20');
				iframeRef.contentWindow.postMessage({
					id: '1',
					type: 'clear',
					data: {},
...
>
	{#if loading_status}
		...
		/>
	{/if}
	<iframe src="https://bohrium.test.dp.tech/app/function-panel" bind:this={iframeRef} style="width:{width}px;height: {height}px;padding: 24px;background: #ffffff;{visible ? '' : 'display: none;'}"></iframe>
</Block>
