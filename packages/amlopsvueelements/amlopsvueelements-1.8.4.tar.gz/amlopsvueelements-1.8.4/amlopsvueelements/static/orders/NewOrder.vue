    ...
    <OrderForm :is-loading="isLoading" add-default-classes>
        ...
        <div class="content-wrap flex flex-col">
          ...
          <div class="w-full flex">
            ...
            <div class="flex items-center justify-center mt-4 w-1/12">
              <CheckboxField v-model="formModel.aircraft_sub_allowed"
                :disabled="!!meta && !isAdmin || !formModel.client" class="mb-0 mr-1" />
      ...
      <template #content class="grid-cols-1" v-if="!isFirstStep && formModel?.type?.is_fuel">
        ...
        <SelectField class="w-11/12" v-model="releaseType" label-text="Release Type"
          ...
          label="aircraft" :options="['Standart', 'Open']" :loading="false" />
        ...
        <div class="flex items-start w-11/12">
          <div class="w-full flex gap-x-3">
            ...
            <div class="w-6/12">
              ...
              <SelectField v-model="formModel.fuel_order!.fuel_uom" placeholder="Please select fuel uplift category"
...
import type { IOrganisation, IPerson } from '@/types/order/order-reference.types'
...
import type { IClient, IFuelUom, IGroundHandler, IOperator, IOrderStatus, IOrderType } from '@/types/order/order.types'
...
const releaseType = ref('Standart');
...
const departureDateTime = ref({
  ...
  time: dayjs().add(1, 'day').format('HH:mm'),
  timezone: 'Local'
})
...
} = useFetch<IOrganisation[], () => Promise<IOrganisation[]>>(async () => {
  const res = await OrderReferences.fetchOrganisations();
  return res;
})
const {
...
} = useFetch<IClient[], () => Promise<IClient[]>>(async () => {
  return await OrderReferences.fetchClients()
})
...
} = useFetch<IOperator[], () => Promise<IOperator[]>>(async () => {
  return await OrderReferences.fetchOperators()
})
...
onBeforeMount(async () => {
  await Promise.allSettled([
    getIsAdmin() ? fetchOrganisations() : fetchMeta(),
    fetchOrderStatuses(),
    fetchOrderTypes(),
