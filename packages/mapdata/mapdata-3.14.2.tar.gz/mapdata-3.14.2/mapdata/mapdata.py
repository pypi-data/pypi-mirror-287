...
def add_help_ok_cancel_buttons(dlg, container, help_action, ok_action, cancel_action, ok_enabled=False):
	...
	help_btn = new_help_button(dlg, container, help_action)
	ok_btn = new_ok_button(dlg, container, 1, ok_action, ok_enabled)
	cancel_btn = new_cancel_button(dlg, container, 2, cancel_action)
	return help_btn, ok_btn, cancel_btn
...
def add_help_src_plot_close_btns(dlg, container, help_action, data_action, plot_action, close_action):
	...
	close_btn = new_close_button(dlg, container, 3, close_action)
	return help_btn, data_btn, plot_data_btn, close_btn
...
def add_help_src_close_btns(dlg, container, help_action, data_action, close_action):
	...
	help_btn = new_help_button(dlg, container, help_action)
	data_btn = new_data_button(dlg, container, 1, data_action)
	close_btn = new_close_button(dlg, container, 2, close_action)
	return help_btn, data_btn, close_btn
	...
	def add_menu(self, table_object):
		...
		def show_selected():
			if table_object.selection():
				...
				show_table(self.win, "Selected Rows", "Selected rows from the main data table.", rowset, self.headers, "Selected data", grab=False, dlg_args={}, tv_args={})
		def save_table():
			if table_object.selection():
				rowset = []
				for sel_row in table_object.selection():
		...
		def save_entire_table():
			export_data_table(self.headers, [table_object.item(r)["values"] for r in table_object.get_children()],
      ...
      Copyright %s, R Dreas Nielsen
		...
		tbl_menu.add_command(label="Find duplicate rows", command=self.find_duprows, underline=6)
		tbl_menu.add_command(label="Recode data", command=self.recode_data, underline=0)
		tbl_menu.add_command(label="Aggregate data", command=self.aggregate_data, underline=0)
...
class TableHideColsDialog(object):
	def __init__(self, parent, column_list, displayed_columns):
		...
		self.canceled = False
		self.help_btn, self.ok_btn, self.cancel_btn = add_help_ok_cancel_buttons(self.dlg, btn_frame, self.do_help, self.do_select, self.do_cancel, ok_enabled=True)
	...
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/hide_cols.html", new=2, autoraise=True)
...
class MapConfigDialog(object):
	def __init__(self, parent, column_list):
		...
		# Buttons
		self.canceled = False
		self.help_btn, self.ok_btn, cancel_btn = add_help_ok_cancel_buttons(self.dlg, btn_frame, self.do_help, self.do_select, self.do_cancel, ok_enabled=True)
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/map_settings.html", new=2, autoraise=True)
	def do_cancel(self, *args):
		self.canceled = True
		self.dlg.destroy()
	def do_select(self, *args):
...
class ImportSymbolDialog(object):
	def __init__(self):
		...
		fn_btn = new_button(prompt_frame, "Browse", 1, 2, get_fn)
		# Buttons
		self.canceled = False
		help_btn, self.ok_btn, cancel_btn = add_help_ok_cancel_buttons(self.dlg, btn_frame, self.do_help, self.do_select, self.do_cancel)
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/import_symbol.html", new=2, autoraise=True)
	def do_cancel(self, *args):
		self.canceled = True
		self.dlg.destroy()
	def do_select(self, *args):
...
class DataFileDialog(object):
	def __init__(self):
		...
		# Buttons
		self.canceled = False
		help_btn, ok_btn, cancel_btn = add_help_ok_cancel_buttons(self.dlg, btn_frame, self.do_help, self.do_select, self.do_cancel)
		self.dlg.resizable(False, False)
	def do_help(self, *args):
...
class NewCrsDialog(object):
	def __init__(self, current_crs):
		...
		# Buttons
		self.canceled = False
		help_btn, ok_btn, cancel_btn = add_help_ok_cancel_buttons(self.dlg, btn_frame, self.do_help, self.do_select, self.do_cancel, True)
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/change_crs.html", new=2, autoraise=True)
	def do_cancel(self, *args):
		self.canceled = True
		self.dlg.destroy()
	def do_select(self, *args):
...
class QueryDialog(object):
	def __init__(self, parent, column_headers, db_conn, init_sql=""):
		...
		# Buttons
		self.canceled = False
		help_btn = new_help_button(self.dlg, btn_frame, self.do_help)
		self.apply_btn = new_button(btn_frame, "Apply", 0, 1, self.do_apply, tk.E, (3,3), (0,0), 0)
		if init_sql is None or len(init_sql) == 0:
			self.apply_btn["state"] = tk.DISABLED
		self.dlg.bind('<Alt-a>', self.do_apply)
		self.ok_btn = new_ok_button(self.dlg, btn_frame, 2, self.do_select)
		if init_sql is None or len(init_sql) == 0:
			self.ok_btn["state"] = tk.DISABLED
		self.cancel_btn = new_cancel_button(self.dlg, btn_frame, 3, self.do_cancel)
	...
	def check_enable(self, args=None):
		...
		if curr_sql != self.sql:
			self.applied = False
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/sql_select.html", new=2, autoraise=True)
...
class RecodeDialog(object):
	def __init__(self, parent, column_specs, prohibited_columns, init_sql=None):
		...
		# Buttons
		self.canceled = False
		help_btn, self.ok_btn, cancel_btn = add_help_ok_cancel_buttons(self.dlg, btn_frame, self.do_help, self.do_select, self.do_cancel, not (init_sql is None or len(init_sql) == 0))
		self.dlg.bind("<Control-Return>", self.do_select)

	def check_enable(self, args=None):
		enable_if(self.ok_btn, self.col_var.get() != '' and self.sql_text.get("1.0", "end-1c") != '')
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/recode.html", new=2, autoraise=True)
...
class FindCandKeysDialog(object):
	def __init__(self, parent, table_rows, column_specs):
		...
		data_frame.rowconfigure(0, weight=4)
		...
		self.results_text = tk.Text(data_frame, width=50, height=5, state=tk.DISABLED)
		...
		self.canceled = False
		self.help_btn = new_help_button(self.dlg, btn_frame, self.do_help)
		self.data_btn = new_data_button(self.dlg, btn_frame, 1, self.show_data)
		self.dups_btn = new_button(btn_frame, "Duplicates", 0, 2, self.show_dups, tk.W, (3,3), state=tk.DISABLED)
		self.seldups_btn = new_button(btn_frame, "Select", 0, 3, self.select_dups, tk.W, (3,3), state=tk.DISABLED)
		close_btn = new_close_button(self.dlg, btn_frame, 4, self.do_close)

		center_window(self.dlg)
		raise_window(self.dlg)

	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/candkeys.html", new=2, autoraise=True)
	...
	def check_cols(self):
		...
		if len(has_nulls) > 0 or n_dups > 0:
			...
			if len(has_nulls) > 0:
				...
				self.results_text.insert(tk.END, f"There are nulls in column{'s' if len(has_nulls) > 1 else ''}\n{badcols}.\n")
			if n_dups > 0:
				if n_dups == 1:
...
class FindDupRowsDialog(object):
	def __init__(self, parent, table_rows, column_specs, prohibited_columns):
		...
		prompt_frame.grid(row=0, column=0, sticky=tk.N+tk.EW, pady=(3,3))
		prompt_frame.columnconfigure(0, weight=1)
		msg_lbl = ttk.Label(prompt_frame, width=50, text="The table below lists all duplicated rows, for all columns that are not candidate keys.")
		msg_lbl.grid(row=0, column=0, sticky=tk.W, padx=(6,6), pady=(3,3))
		def wrap_msg(event):
			msg_lbl.configure(wraplength=event.width - 5)
		msg_lbl.bind("<Configure>", wrap_msg)
		...
		ctrl_frame.grid(row=1, column=0, sticky=tk.N+tk.EW)
		self.sel_only_var = tk.StringVar(ctrl_frame, "0")
		self.sel_only_ck = ttk.Checkbutton(ctrl_frame, text="Selected data only", command=self.recalc, variable=self.sel_only_var,
		...
		self.data_frame = tk.Frame(self.dlg, borderwidth=5)
		self.data_frame.rowconfigure(0, weight=4)
		self.data_frame.columnconfigure(0, weight=1)
		self.data_frame.grid(row=2, column=0, columnspan=2, sticky=tk.NSEW)
		self.dlg.rowconfigure(2, weight=1)
		self.dlg.columnconfigure(0, weight=1)
		frame_panes = ttk.PanedWindow(self.data_frame, orient=tk.HORIZONTAL)
		...
		# Content frame for table of duplicates.
		...
		self.out_tbl_frame, self.output_table = treeview_table(self.content_frame, [["No duplicates found"]], column_headers=["Result"], select_mode="none", nrows=10)
		...
		self.canceled = False
		self.help_btn = new_help_button(self.dlg, btn_frame, self.do_help)
		self.add_columns_btn = new_addcol_button(self.dlg, btn_frame, 1, self.add_column)
		close_btn = new_close_button(self.dlg, btn_frame, 2, self.do_close)
	...
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/findduplicates.html", new=2, autoraise=True)
	...
	def recalc(self, *args):
		...
		if len(sel_columns) > 0:
			...
			if len(dupdata) > 0:
				...
				self.out_tbl_frame, self.output_table = treeview_table(self.content_frame, dupdata, column_headers=hdrs, select_mode="none", nrows=10)
				self.out_tbl_frame.grid(row=0, column=0, columnspan=2, sticky=tk.NSEW, padx=(6,6), pady=(3,3))
...
class AddRowIDDialog(object):
	def __init__(self, parent, column_specs):
		...
		# Buttons
		self.canceled = False
		self.ok_btn = new_ok_button(self.dlg, btn_frame, 0, self.do_select)
		self.cancel_btn = new_cancel_button(self.dlg, btn_frame, 1, self.do_cancel)
...
class AddCoordKeyDialog(object):
	def __init__(self, parent, column_specs):
		self.parent = parent
		self.canceled = True
		self.curr_colnames = [c[0].lower() for c in column_specs]
		self.db_names = db_colnames(self.curr_colnames)
		self.dlg = tk.Toplevel()
		self.dlg.title("Add Candidate Key for Coordinates")
		self.dlg.rowconfigure(0, weight=1)
		self.dlg.columnconfigure(0, weight=2)
		prompt_frame = tk.Frame(self.dlg)
		prompt_frame.grid(row=0, column=0, sticky=tk.EW, padx=(6,6), pady=(6,6))
		prompt_frame.rowconfigure(0, weight=1)
		prompt_frame.columnconfigure(0, weight=2)
		msg_lbl = ttk.Label(prompt_frame, wraplength=100, text="This will create a new column with a unique identifier for every unique combination of coordinates.  Enter a name, and optionally a prefix, for the new column.  The new column must have a name different from any existing column.")
		msg_lbl.grid(row=0, column=0, sticky=tk.EW, padx=(6,6), pady=(3,3))
		def wrap_msg(event):
			msg_lbl.configure(wraplength=event.width - 5)
		msg_lbl.bind("<Configure>", wrap_msg)
...
class DistanceDialog(object):
	def __init__(self, parent, coord_keys):
		...
		self.dlg = tk.Toplevel()
		self.dlg.title("Distances Between Locations")
		self.dlg.rowconfigure(0, weight=0)
		self.dlg.rowconfigure(1, weight=0)
		self.dlg.rowconfigure(2, weight=1)
		...
		msg_lbl = ttk.Label(prompt_frame, width=65, text=msg)
		msg_lbl.grid(row=0, column=0, sticky=tk.W, padx=(6,6), pady=(3,3))
		def wrap_msg(event):
			msg_lbl.configure(wraplength=event.width - 5)
		msg_lbl.bind("<Configure>", wrap_msg)
		...
		ctrl_frame.grid(row=1, column=0, sticky=tk.N+tk.EW)
		self.sel_only_var = tk.StringVar(ctrl_frame, "0")
		self.sel_only_ck = ttk.Checkbutton(ctrl_frame, text="Selected data only", command=self.reset_locid_list, variable=self.sel_only_var,
				onvalue="1", offvalue="0")
		self.sel_only_ck.grid(row=0, column=0, sticky=tk.W, padx=(6,3), pady=(3,3))

		# Distance units
		self.dist_var = tk.StringVar(ctrl_frame, "meters")
		ttk.Label(ctrl_frame, text="Units:").grid(row=0, column=1, sticky=tk.W, padx=(12,3), pady=(3,3))
		dist_sel = ttk.Combobox(ctrl_frame, state="readonly", textvariable=self.dist_var, width=11)
		dist_sel["values"] = ["meters", "kilometers", "feet", "miles"]
		dist_sel.grid(row=0, column=2, sticky=tk.W, padx=(3,3), pady=(3,3))
		...
		self.var_frame = tk.Frame(frame_panes, borderwidth=2, relief=tk.RIDGE)
		...
		# Content frame for table of distances
		self.content_frame = tk.Frame(frame_panes, borderwidth=3, relief=tk.RIDGE)
		...
		# Buttons
		self.canceled = False
		self.help_btn, self.data_btn, close_btn = add_help_src_close_btns(self.dlg, btn_frame, self.do_help, self.show_data, self.do_close)
	...
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/distances.html", new=2, autoraise=True)
	...
	def clear_output(self):
		for ctl in self.content_frame.winfo_children():
			ctl.destroy()
		tframe, tdata = treeview_table(self.content_frame, [], ["Location 1", "Location 2", "Distance"])
	...
	def recalc(self):
		...
		tframe, tdata = treeview_table(self.content_frame, self.calc_result, self.calc_result_labels)
		tframe.grid(row=0, column=0, sticky=tk.NSEW)
		self.data_btn["state"] = tk.NORMAL
		self.dlg.bind("<Control-s>", self.save_data)
		self.loading_dlg.hide()
...
class AggregateDialog(object):
	def __init__(self, parent, column_specs):
		self.parent = parent
		self.column_specs = column_specs
		self.dlg = tk.Toplevel()
		self.dlg.title("Numerical Aggregates")
		self.dlg.rowconfigure(0, weight=0)
		self.dlg.rowconfigure(1, weight=0)
		self.dlg.rowconfigure(2, weight=1)
		self.dlg.rowconfigure(3, weight=0)
		...
		prompt_frame.grid(row=0, column=0, sticky=tk.N+tk.EW, pady=(3,3))
		prompt_frame.columnconfigure(0, weight=1)
		msg_lbl = ttk.Label(prompt_frame, width=65, text="Select one or more variables from the left, and a grouping variable, to see the selected aggregates.  Use Ctrl-click or Shift-click to select multiple rows.")
		msg_lbl.grid(row=0, column=0, sticky=tk.W, padx=(6,6), pady=(3,3))
		def wrap_msg(event):
			msg_lbl.configure(wraplength=event.width - 5)
		msg_lbl.bind("<Configure>", wrap_msg)
		...
		self.aggreg_var = tk.StringVar(ctrl_frame, "Arithmetic mean")
		ttk.Label(ctrl_frame, text="Aggregate rows by:").grid(row=0, column=1, sticky=tk.W, padx=(12,3), pady=(3,3))
		aggreg_sel = ttk.Combobox(ctrl_frame, state="readonly", textvariable=self.aggreg_var, width=24)
		aggreg_sel["values"] = ["Count", "Minimum", "Maximum", "Median", "Arithmetic mean", "Geometric mean", "Harmonic mean", "Sum"]
		aggreg_sel.grid(row=0, column=2, sticky=tk.W, padx=(3,3), pady=(3,3))
		...
		# Content frame for table of aggregates
		self.content_frame = tk.Frame(frame_panes, borderwidth=3, relief=tk.RIDGE)
		self.content_frame.grid(row=0, column=1, sticky=tk.NSEW)
		self.content_frame.rowconfigure(0, weight=1)
		self.content_frame.columnconfigure(0, weight=1)
	...
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/aggregates.html", new=2, autoraise=True)
	...
	def clear_output(self):
		for ctl in self.content_frame.winfo_children():
			ctl.destroy()
		tframe, tdata = treeview_table(self.content_frame, [], ["Group"])
		tframe.grid(row=0, column=0, sticky=tk.NSEW)
		self.dataset = None
		self.agg_data = None
	...
	def recalc(self):
		...
		if self.agg_data is not None and len(self.agg_data) > 0 and len(self.agg_data[0]) > 0:
			...
			tframe, tdata = treeview_table(self.content_frame, columns_to_rows(self.agg_data), self.agg_data_labels)
			tframe.grid(row=0, column=0, sticky=tk.NSEW)
			self.loading_dlg.hide()
...
class UniqueValuesDialog(object):
	def __init__(self, parent, column_specs):
		self.parent = parent
		self.column_specs = column_specs
		self.variables = [c[0] for c in column_specs]
		self.dlg = tk.Toplevel()
		self.dlg.title("Unique Values")
		self.dlg.rowconfigure(0, weight=0)
		self.dlg.rowconfigure(1, weight=1)
		...
		prompt_frame.grid(row=0, column=0, sticky=tk.N+tk.EW, pady=(3,3))
		prompt_frame.columnconfigure(0, weight=1)
		msg_lbl = ttk.Label(prompt_frame, width=65, text="Select one or more variables from the left to see all unique values.")
		msg_lbl.grid(row=0, column=0, sticky=tk.W, padx=(6,6), pady=(3,3))
		def wrap_msg(event):
			msg_lbl.configure(wraplength=event.width - 5)
		msg_lbl.bind("<Configure>", wrap_msg)
		...
		data_frame.rowconfigure(0, weight=1)
		data_frame.columnconfigure(0, weight=1)
		data_frame.grid(row=1, column=0, sticky=tk.NSEW)
		frame_panes = ttk.PanedWindow(data_frame, orient=tk.HORIZONTAL)
		frame_panes.grid(row=0, column=0, sticky=tk.NSEW)
		...
		# Content frame for table of unique values
		self.content_frame = tk.Frame(frame_panes, borderwidth=3, relief=tk.RIDGE)
		self.content_frame.grid(row=0, column=1, sticky=tk.NSEW)
		self.content_frame.rowconfigure(0, weight=1)
		self.content_frame.columnconfigure(0, weight=1)
	...
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/unique_values.html", new=2, autoraise=True)

	def clear_output(self):
		for ctl in self.content_frame.winfo_children():
			ctl.destroy()
		tframe, tdata = treeview_table(self.content_frame, [], [])
		tframe.grid(row=0, column=0, sticky=tk.NSEW)
		self.dataset = None
		self.agg_data = None
		clear_dlg_hotkeys(self.dlg)
	...
	def recalc(self):
		...
		tframe, tdata = treeview_table(self.content_frame, self.dataset, self.data_labels)
		tframe.grid(row=0, column=0, sticky=tk.NSEW)
		self.dlg.bind("<Control-s>", self.save_data)
...
class ColocatedDialog(object):
	def __init__(self):
		...
		self.help_btn, self.ok_btn, cancel_btn = add_help_ok_cancel_buttons(self.dlg, btn_frame, self.do_help, self.do_select, self.do_cancel, ok_enabled=True)
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/select_colocated.html", new=2, autoraise=True)
	def do_cancel(self, *args):
		self.canceled = True
		self.dlg.destroy()
	def do_select(self, *args):
...
class PlotDialog(object):
	def __init__(self, parent, column_specs):
		...
		self.type_sel = ttk.Combobox(ctrl_frame, state="readonly", textvariable=self.type_var, width=20, height=21, values=plot_types)
		...
		self.x_lbl = ttk.Label(ctrl_frame, text="X column:")
		...
		self.y_lbl = ttk.Label(ctrl_frame, text="Y column:")
		...
		self.groupby_lbl = ttk.Label(ctrl_frame, text="Group by:")
		...
		self.help_btn = new_help_button(self.dlg, btn_frame, self.do_help)
		self.data_btn = new_data_button(self.dlg, btn_frame, 1, self.show_data)
		self.plot_data_btn = new_plot_button(self.dlg, btn_frame, 2, self.show_plot_data)
		self.clone_btn = new_button(btn_frame, "Clone", 0, 3, self.clone_plot, tk.W, (3,6), (0,0), 3)
	...
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/plot_general.html", new=2, autoraise=True)
		...
		if self.plot_data is not None and len(self.plot_data) > 0 and len(self.plot_data[0]) > 0:
			if plot_type == "Count by category":
				...
				if len(self.plot_data) > self.n_dataset_columns:
					...
					def hover(event):
						hoverer.hover(event)
					self.canvas_conn_id = self.plot.canvas.mpl_connect("motion_notify_event", hover)
				else:
...
class PairPlotDialog(object):
	def __init__(self, parent, column_specs):
		...
		self.log10_ck = ttk.Checkbutton(ctrl_frame, text="Log10 transform data", command=self.q_redraw, variable=self.log10_var,
				onvalue="1", offvalue="0")
		self.log10_ck.grid(row=0, column=1, sticky=tk.W, padx=(12,3), pady=(3,3))
		...
		self.content_frame.columnconfigure(0, weight=1)
		frame_panes.add(self.content_frame, weight=12)
		self.pairplot = Plot(self.content_frame, 4, 4, layout="tight")
	...
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/plot_pairplot.html", new=2, autoraise=True)
	...
	def redraw(self):
		...
		if self.data_labels is not None and self.n_data_columns > 1:
			...
			if groupby != '':
				...
				colkey = list(cmap.values())
				symbs = [matplotlib.lines.Line2D([], [], marker='o', color=colkey[i], label=lbls[i], linestyle='None') for i in range(len(lbls))]
				self.pairplot.figure.legend(handles=symbs, labels=lbls)
			self.pairplot.draw()
...
class VariableDistDialog(object):
	def __init__(self, parent, column_specs):
		self.parent = parent
		self.column_specs = column_specs
		self.numeric_columns = sorted([c[0] for c in self.column_specs if c[1] in ("int", "float")])
		...
		self.ylabel = None
		self.dlg = tk.Toplevel()
		self.dlg.title("Variable Distribution Plot")
		self.loading_dlg = LoadingDialog(self.dlg)
		self.alpha = 0.45
		self.rotated = False
		self.xlabel = None
		...
		prompt_frame.grid(row=0, column=0, sticky=tk.N+tk.EW, pady=(3,3))
		prompt_frame.columnconfigure(0, weight=1)
		msg_lbl = ttk.Label(prompt_frame, width=80, text="Select one or more variables from the list below.")
		msg_lbl.grid(row=0, column=0, sticky=tk.W, padx=(6,6), pady=(3,3))
		def wrap_msg(event):
			msg_lbl.configure(wraplength=event.width - 5)
		msg_lbl.bind("<Configure>", wrap_msg)
		...
		ckb_frame = tk.Frame(ctrl_frame)
		ckb_frame.grid(row=0, column=0, sticky = tk.NSEW)

		self.sel_only_var = tk.StringVar(ckb_frame, "0")
		self.sel_only_ck = ttk.Checkbutton(ckb_frame, text="Selected data only", variable=self.sel_only_var, command=self.q_redraw,
				onvalue="1", offvalue="0")
		self.sel_only_ck.grid(row=0, column=0, sticky=tk.W, padx=(6,3), pady=(3,3))

		self.autoupdate_var = tk.StringVar(ckb_frame, "1")
		self.autoupdate_ck = ttk.Checkbutton(ckb_frame, text="Auto-update", command=self.set_autoupdate, variable=self.autoupdate_var,
				onvalue="1", offvalue="0")
		...
		btn_frame.grid(row=3, column=0, sticky=tk.EW, pady=(3,3))
		self.canceled = False
		self.help_btn, self.data_btn, self.plot_data_btn, close_btn = add_help_src_plot_close_btns(self.dlg, btn_frame,
	...
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/plot_vardist.html", new=2, autoraise=True)
...
class PctStackedAreaDialog(object):
	def __init__(self, parent, column_specs):
		self.parent = parent
		self.column_specs = column_specs
		self.numeric_columns = sorted([c[0] for c in self.column_specs if c[1] in ("int", "float")])
		...
		self.ylabel = None
		self.dlg = tk.Toplevel()
		self.dlg.title("Percentage Stacked Area Plot")
		self.loading_dlg = LoadingDialog(self.dlg)
		self.alpha = 0.45
		self.rotated = False
		self.xlabel = None
		...
		prompt_frame.grid(row=0, column=0, sticky=tk.N+tk.EW, pady=(3,3))
		prompt_frame.columnconfigure(0, weight=1)
		msg_lbl = ttk.Label(prompt_frame, width=80, text="Select an X variable and two or more Y variables from the list below.")
		msg_lbl.grid(row=0, column=0, sticky=tk.W, padx=(6,6), pady=(3,3))
		def wrap_msg(event):
			msg_lbl.configure(wraplength=event.width - 5)
		msg_lbl.bind("<Configure>", wrap_msg)
		...
		# Content frame for Y variable selection and output plot
		self.content_frame = tk.Frame(self.dlg)
		self.content_frame.grid(row=2, column=0, sticky=tk.NSEW)
		self.dlg.rowconfigure(2, weight=1)
		self.dlg.columnconfigure(0, weight=1)
	...
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/pctstackedarea.html", new=2, autoraise=True)
...
class MultiVariableLinePlotDialog(object):
	def __init__(self, parent, column_specs):
		self.parent = parent
		self.column_specs = column_specs
		...
		self.ylabel = None
		self.dlg = tk.Toplevel()
		self.dlg.title("Multi-Variable Line or Scatter Plot")
		self.xlabel = None
		self.ylabel = None
		self.plot_title = None
		# Data
		...
		prompt_frame.grid(row=0, column=0, sticky=tk.N+tk.EW, pady=(3,3))
		prompt_frame.columnconfigure(0, weight=1)
		msg_lbl = ttk.Label(prompt_frame, width=80, text="Select an X variable and then one or more Y variables from the list below.")
		msg_lbl.grid(row=0, column=0, sticky=tk.W, padx=(6,6), pady=(3,3))
		def wrap_msg(event):
			msg_lbl.configure(wraplength=event.width - 5)
		msg_lbl.bind("<Configure>", wrap_msg)
	...
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/multivarlineplot.html", new=2, autoraise=True)
...
class RankAbundDialog(object):
	def __init__(self, parent, column_specs, prohibited_columns):
		...
		prompt_frame.grid(row=0, column=0, sticky=tk.N+tk.EW, pady=(3,3))
		prompt_frame.columnconfigure(0, weight=1)
		msg_lbl = ttk.Label(prompt_frame, width=80, text="Select two or more variables from the list below.")
		msg_lbl.grid(row=0, column=0, sticky=tk.W, padx=(6,6), pady=(3,3))
		def wrap_msg(event):
			msg_lbl.configure(wraplength=event.width - 5)
		msg_lbl.bind("<Configure>", wrap_msg)
	...
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/plot_rankabund.html", new=2, autoraise=True)
...
class UnivarStatsDialog(object):
	...
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/univarstats.html", new=2, autoraise=True)
...
class BivarStatsDialog(object):
	...
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/bivariatestats.html", new=2, autoraise=True)
...
class CorrMatrixDialog(object):
	def __init__(self, parent, column_specs):
		...
		self.content_frame = tk.Frame(frame_panes, borderwidth=3, relief=tk.RIDGE)
		self.content_frame.grid(row=0, column=1, sticky=tk.NSEW)
		self.plot = Plot(self.content_frame, 2.5, 2.5)
		frame_panes.add(self.content_frame, weight=12)
		...
		self.help_btn = new_help_button(self.dlg, btn_frame, self.do_help)
		self.data_btn = new_data_button(self.dlg, btn_frame, 1, self.show_data)
		self.corr_data_btn = new_button(btn_frame, "Correlations", 0, 2, self.show_corr_data, tk.W, (3,3), (0,0), None, tk.DISABLED)
		close_btn = new_close_button(self.dlg, btn_frame, 3, self.do_close)
	...
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/corrmatrix.html", new=2, autoraise=True)
...
class CosineSimilarityDialog(object):
	def __init__(self, parent, column_specs):
		self.parent = parent
		self.column_specs = column_specs
		self.dlg = tk.Toplevel()
		self.dlg.title("Cosine Similarity")
		self.dlg.rowconfigure(0, weight=0)
		self.dlg.rowconfigure(1, weight=0)
		self.dlg.rowconfigure(2, weight=1)
		self.dlg.rowconfigure(3, weight=0)
		...
		prompt_frame.grid(row=0, column=0, sticky=tk.N+tk.EW, pady=(3,3))
		prompt_frame.columnconfigure(0, weight=1)
		msg_lbl = ttk.Label(prompt_frame, width=65, text="Select two or more variables from the left, and a grouping variable, to see the cosine similarity matrix.  Use Ctrl-click or Shift-click to select multiple rows.")
		msg_lbl.grid(row=0, column=0, sticky=tk.W, padx=(6,6), pady=(3,3))
		def wrap_msg(event):
			msg_lbl.configure(wraplength=event.width - 5)
		msg_lbl.bind("<Configure>", wrap_msg)
		...
		self.cleantype_var = tk.StringVar(ctrl_frame, "Drop cases")
		ttk.Label(ctrl_frame, text="Remove missing values by:").grid(row=0, column=1, sticky=tk.W, padx=(12,3), pady=(3,3))
		self.cleantype_sel = ttk.Combobox(ctrl_frame, state=tk.NORMAL, textvariable=self.cleantype_var, values=["Drop cases","Drop variables"], width=15)
		...
		# Content frame for cosine similarity matrix figure
		self.content_frame = tk.Frame(frame_panes, borderwidth=3, relief=tk.RIDGE)
		self.content_frame.grid(row=0, column=1, sticky=tk.NSEW)
		self.plot = Plot(self.content_frame, 3.5, 3.5)
		frame_panes.add(self.content_frame, weight=12)
		# initialize content frame with an empty plot
		self.clear_output()
		...
		self.help_btn = new_help_button(self.dlg, btn_frame, self.do_help)
		self.data_btn = new_data_button(self.dlg, btn_frame, 1, self.show_data)
		self.agg_data_btn = new_button(btn_frame, "Aggregates", 0, 2, self.show_agg, padx=(3,3), state=tk.DISABLED)
		self.sim_data_btn = new_button(btn_frame, "Similarities", 0, 3, self.show_sim, padx=(3,3), state=tk.DISABLED)
		close_btn = new_close_button(self.dlg, btn_frame, 4, self.do_close)
	...
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/cossimmatrix.html", new=2, autoraise=True)
...
class ANOVADialog(object):
	def __init__(self, parent, column_specs):
		...
		self.help_btn = new_help_button(self.dlg, btn_frame, self.do_help)
		self.data_btn = new_data_button(self.dlg, btn_frame, 1, self.show_data)
		self.group_btn = new_button(btn_frame, "Group Data", 0, 2, self.show_group_data, padx=(3,3), state=tk.DISABLED)
		close_btn = new_close_button(self.dlg, btn_frame, 3, self.do_close)
	...
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/anova.html", new=2, autoraise=True)
...
class TSNEDialog(object):
	...
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/tsne.html", new=2, autoraise=True)
...
class UMAPDialog(object):
	...
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/umap.html", new=2, autoraise=True)
...
class FitDistDialog(object):
	...
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/fitunivardist.html", new=2, autoraise=True)
...
class ContTableDialog(object):
	def __init__(self, parent, column_specs):
		...
		self.canceled = False
		self.help_btn = new_help_button(self.dlg, btn_frame, self.do_help)
		close_btn = new_close_button(self.dlg, btn_frame, 1, self.do_close)

		center_window(self.dlg)
		raise_window(self.dlg)

	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/conttable.html", new=2, autoraise=True)
...
class ROCCurveDialog(object):
	...
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/roccurve.html", new=2, autoraise=True)
...
class PCADialog(object):
	def __init__(self, parent, column_specs, id_columns):
		self.parent = parent
		self.column_specs = column_specs
		self.id_columns = id_columns
		self.dlg = tk.Toplevel()
		self.dlg.title("Principal Component Analysis")
		self.dlg.rowconfigure(0, weight=0)
		self.dlg.rowconfigure(1, weight=0)
		self.dlg.rowconfigure(2, weight=1)
		self.dlg.rowconfigure(3, weight=0)
		...
		prompt_frame.grid(row=0, column=0, sticky=tk.N+tk.EW, pady=(3,3))
		prompt_frame.columnconfigure(0, weight=1)
		msg_lbl = ttk.Label(prompt_frame, width=100, text="Select three or more variables from the left for PCA analysis.  Use Ctrl-click or Shift-click to select multiple rows.")
		msg_lbl.grid(row=0, column=0, sticky=tk.W, padx=(6,6), pady=(3,3))
		def wrap_msg(event):
			msg_lbl.configure(wraplength=event.width - 5)
		msg_lbl.bind("<Configure>", wrap_msg)
		...
		ctrl_frame = tk.Frame(self.dlg, borderwidth=5)
		ctrl_frame.rowconfigure(0, weight=0)
		ctrl_frame.columnconfigure(0, weight=0)
		ctrl_frame.grid(row=1, column=0, sticky=tk.N+tk.EW)
		self.sel_only_var = tk.StringVar(ctrl_frame, "0")
		self.sel_only_ck = ttk.Checkbutton(ctrl_frame, text="Selected data only", command=self.q_recalc, variable=self.sel_only_var,
				onvalue="1", offvalue="0")
		...
		self.zscore_var = tk.StringVar(ctrl_frame, "1")
		self.zscore_ck = ttk.Checkbutton(ctrl_frame, text="Use Z scores", command=self.q_recalc, variable=self.zscore_var,
				onvalue="1", offvalue="0")
		...
		self.content_frame.columnconfigure(0, weight=1)
		frame_panes.add(self.content_frame, weight=12)
		self.output_pages = ttk.Notebook(self.content_frame)
		...
		self.help_btn = new_help_button(self.dlg, btn_frame, self.do_help)
		self.data_btn = new_data_button(self.dlg, btn_frame, 1, self.show_data)
		self.addcols_btn = new_addcol_button(self.dlg, btn_frame, 2, self.add_columns)
	...
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/pca.html", new=2, autoraise=True)
	...
	def pcplot_redraw(self, *args):
		...
		if self.scores is not None and xvar != '' and yvar != '':
			...
			def hover(event):
				hoverer.hover(event)
			self.canvas_conn_id = self.pcplot.canvas.mpl_connect("motion_notify_event", hover)
...
class NMFUnmixingDialog(object):
	def __init__(self, parent, column_specs, id_columns):
		self.parent = parent
		self.column_specs = column_specs
		self.id_columns = id_columns
		self.dlg = tk.Toplevel()
		self.dlg.title("NMF Unmixing")
		self.dlg.rowconfigure(0, weight=0)
		self.dlg.rowconfigure(1, weight=0)
		self.dlg.rowconfigure(2, weight=1)
		self.dlg.rowconfigure(3, weight=0)
		...
		prompt_frame.grid(row=0, column=0, sticky=tk.N+tk.EW, pady=(3,3))
		prompt_frame.columnconfigure(0, weight=1)
		msg_lbl = ttk.Label(prompt_frame, width=100, text="Select three or more variables from the left for unmixing to find end member patterns.  Use Ctrl-click or Shift-click to select multiple rows.  Use the 'Unmix' button to initiate the calculation.")
		msg_lbl.grid(row=0, column=0, sticky=tk.W, padx=(6,6), pady=(3,3))
		def wrap_msg(event):
			msg_lbl.configure(wraplength=event.width - 5)
		msg_lbl.bind("<Configure>", wrap_msg)
		...
		self.em_var = tk.IntVar(var_frame, 0)
		em_lbl = ttk.Label(var_frame, text="End members:")
		em_lbl.grid(row=3, column=0, sticky=tk.W, padx=(6,3), pady=(3,3))
		self.em_sel = ttk.Combobox(var_frame, textvariable=self.em_var, values=[], width=5)
		self.em_sel.grid(row=4, column=0, sticky=tk.E, padx=(6,3), pady=(3,3))
		self.em_sel["state"] = tk.DISABLED

		# 'Unmix' button
		self.calc_btn = ttk.Button(var_frame, text="  Unmix  ", command=self.recalc, state=tk.DISABLED)
		self.calc_btn.grid(row=5, column=0, sticky=tk.E, padx=(3,18), pady=(3,3))
	...
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/unmixing.html", new=2, autoraise=True)
	...
	def recalc(self):
		...
		self.loading_dlg.display("Unmixing data")
		if self.dataset is not None and len(self.dataset[0]) > 2:
			# If the EM-number dropdown is disabled, do multiple NMF runs to find the optimum EM number based on AIC,
			...
			if self.em_var.get() == 0:
				...
				for em_no in test_ems:
					...
					if n_rows - k - 1 == 0:
						...
						fit_aic = 2*k - 2 * math.log(fit_rss/n) + 2 * k + ((2 * k + (k + 1)) / (n_rows - k - 1))
					unmixes.append((W, H))
					...
					diagnostics.append([em_no, fp_display(arraysum(fit_ds)-arraysum(ds), 4), fp_display(fit_rss,5),
						fp_display(fit_rmse,4), fp_display(frobnorm,4), fp_display(fit_aic,4)])
				if len(rmses) > 1:
				...
				self.em_diagnostics_labels = ["EMs", "Tot. resid.", "RSS", "RMSE", "Frob. norm", "AICc"]
				self.out_tables[3] = (self.em_diagnostics_labels, diagnostics, "Diagnostics")
...
class CategCorrespDialog(object):
	...
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/categcorresp.html", new=2, autoraise=True)
...
class CategSimDialog(object):
	def __init__(self, parent, column_specs, id_columns):
		self.parent = parent
		self.id_columns = id_columns
		self.column_specs = column_specs
		self.dlg = tk.Toplevel()
		self.dlg.title("Categorical Similarity")
		self.dlg.rowconfigure(0, weight=0)
		self.dlg.rowconfigure(1, weight=0)
		self.dlg.rowconfigure(2, weight=1)
		self.dlg.rowconfigure(3, weight=0)
		...
		prompt_frame.grid(row=0, column=0, sticky=tk.N+tk.EW, pady=(3,3))
		prompt_frame.columnconfigure(0, weight=1)
		msg_lbl = ttk.Label(prompt_frame, width=65, text="Select one or more variables from the left to see the similarity matrix.  Use Ctrl-click or Shift-click to select multiple rows.")
		msg_lbl.grid(row=0, column=0, sticky=tk.W, padx=(6,6), pady=(3,3))
		def wrap_msg(event):
			msg_lbl.configure(wraplength=event.width - 5)
		msg_lbl.bind("<Configure>", wrap_msg)
		...
		# Content frame for similarity matrix figure
		self.content_frame = tk.Frame(frame_panes, borderwidth=3, relief=tk.RIDGE)
		self.content_frame.grid(row=0, column=1, sticky=tk.NSEW)
		frame_panes.add(self.content_frame, weight=12)
		self.plot = Plot(self.content_frame, 2, 2)
		...
		self.help_btn = new_help_button(self.dlg, btn_frame, self.do_help)
		self.data_btn = new_data_button(self.dlg, btn_frame, 1, self.show_data)
		self.sim_data_btn = new_button(btn_frame, "Similarities", 0, 2, self.show_sim_data, tk.W, (3,3), state=tk.DISABLED)
		close_btn = new_close_button(self.dlg, btn_frame, 3, self.do_close)
	...
	def do_help(self, *args):
		webbrowser.open("https://mapdata.readthedocs.io/en/latest/categsimmatrix.html", new=2, autoraise=True)
...
class OneEntryDialog(object):
	def __init__(self, parent, title, prompt, obscure=False):
		...
		self.canceled = False
		self.ok_btn = new_ok_button(self.dlg, btn_frame, 0, self.do_select, ok_enabled=True)
		cancel_btn = new_cancel_button(self.dl6, btn_frame, 1, self.do_cancel)
