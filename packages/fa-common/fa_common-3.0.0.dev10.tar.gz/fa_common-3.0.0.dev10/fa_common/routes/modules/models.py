...
from fa_common.routes.modules.types import Module, ModuleResponse, ModuleVersion
...
class ModuleDocument(Module, Document, TimeStampedModel):
    ...
    async def get_all(cls) -> List[str]:
        ...
        return [module.name for module in lst_modules]

    @classmethod
    async def get_version(cls, name: str, version: str) -> ModuleResponse:
    ...
    @classmethod
    async def insert_new(cls, new_module: Module, sku: str):
        module_doc = ModuleDocument(sku=sku, **new_module.model_dump())
