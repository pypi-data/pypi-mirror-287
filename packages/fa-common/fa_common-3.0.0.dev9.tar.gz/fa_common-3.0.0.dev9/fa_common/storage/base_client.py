...
class BaseClient(abc.ABC):
    ...
    @classmethod
    def add_user_base_path(cls, location_id: str, sub_path: str) -> str:
        ...
        trim_path = f"{st.BUCKET_USER_FOLDER}/{location_id}/{sub_path}".replace("//", "/")
        return trim_path

    @classmethod
    def add_project_base_path(cls, location_id: str, path: str) -> str:
        # @REVIEW: This is just a common function across all
        # clients. So, I've put the logic here.
        st = get_settings()
        # Added below trim as sometime an extra "/" could mess things up.
        trim_path = f"{st.BUCKET_PROJECT_FOLDER}/{location_id}/{path}".replace("//", "/")
        return trim_path

    @classmethod
    def get_standard_workflow_upload_path(cls, location_id):
        st: FACommonSettings = get_settings()
        return cls.add_project_base_path(location_id, st.WORKFLOW_UPLOAD_PATH)
