...
def _run_model_server(
    ...
    app = FastAPI(
        title=api_name,
        description="This is a simple model serving API. "
        + (
            "Add `X-AUTH-KEY` header you set as `SERVING_AUTH_KEY` in your environment. "
            if remote
            else ""
        )
        + "You can access the metrics at `/metrics`. "
        + "You can access the API at `POST /`. "
        + "You can access the API documentation at `GET /`. "
        + "If you want to present the request/response type on this openapi document, you should import [`pydantic.BaseModel`(#)](https://fastapi.tiangolo.com/tutorial/body/#import-pydantics-basemodel) when defining your input output class. ",
        docs_url="/",
    ...
    REQUEST_DURATION = Histogram("request_duration_seconds", "Time spent processing request")
    ...
    inputType = inputType if inputType else Request
    outputType = outputType if outputType else Response

    api_key_header = APIKeyHeader(name="X-AUTH-KEY")
    security_if_remote = Security(api_key_header) if remote else None

    def verify_key(token: str):
        if not remote:
            return True

        if token is None or token != os.environ.get("SERVICE_AUTH_KEY"):
    ...
    @app.post("/", tags=["model"])
    async def index(data: inputType, token: str = security_if_remote) -> outputType:
        REQUEST_TOTAL.inc()

        if not remote or verify_key(token):
            print("Received:", data)
