...
Classes and Enums Imported:
    ...
    Integrations: Enum or class encapsulating integration-specific constants or configurations.
...
class HttpClient:
    ...
    _integration_name: Integrations = None
    _connection_id: str = None
    _resource_list: List[str] = None
    _is_third_party_call: bool = False
    ...
    def __init__(self):
        ...
        """
        input = Input()
        self.client_id = input.get_value(InputVariables.CW_OPEN_API_CLIENT_ID)
        self.client_secret = input.get_value(InputVariables.CW_OPEN_API_CLIENT_SECRET)
        self.scopes = input.get_value(InputVariables.CW_PARTNER_API_SCOPE)
        self.cw_api_url = input.get_value(InputVariables.CW_OPEN_API_URL_KEY)
        self.site_id = input.get_value(InputVariables.CW_SITE_ID)
        self.company_id = input.get_value(InputVariables.CW_COMPANY_ID)
        self._token: Token = None

    def third_party_integration(self, integration_name: Integrations, connection_id: str = None, resource_list: List[str] = None):
        """
        ...
        """
        # create the copy of the current instance  
        third_party = copy.copy(self)
        third_party._integration_name = integration_name
        third_party._connection_id = connection_id
        third_party._resource_list = resource_list
        third_party._is_third_party_call = True
        return third_party
    ...
    def generate_integration_payload(self, integration_name: Integrations, method: HttpMethod, url: str, body=None, resource_list: List[str] = None, connection_id: str = None) -> dict:
        """
        ...
        if not resource_list and integration_name == Integrations.MICROSOFT_CSP:
    ...
    def send_request(self, method: HttpMethod, url: str, headers: dict = {}, **kwargs) -> requests.Response:
        """
        ...
        if self._integration_name and self._is_third_party_call:
            body = kwargs.get("json")
            kwargs["json"] = self.generate_integration_payload(self._integration_name, method, url, body, self._resource_list, self._connection_id)
        ...
        if self.is_token_required(url):
            ...
            if self._token and self._token.is_expired() == False:
    ...
    def get(self, url: str, headers: dict = {}, **kwargs) -> requests.Response:
        ...
        - The response from the request.
        """
        return self.send_request(HttpMethod.GET, url, headers, **kwargs)

    def post(self, url: str, headers: dict = {}, **kwargs) -> requests.Response:
        """
        ...
        - The response from the request.
        """
        return self.send_request(HttpMethod.POST, url, headers, **kwargs)

    def put(self, url: str, headers: dict = {}, **kwargs) -> requests.Response:
        """
        Sends a PUT request.
        ...
        - The response from the request.
        """
        return self.send_request(HttpMethod.PUT, url, headers, **kwargs)
    ...
    def delete(self, url: str, headers: dict = {}, **kwargs) -> requests.Response:
        ...
        - The response from the request.
        """
        return self.send_request(HttpMethod.DELETE, url, headers, **kwargs)

    def patch(self, url: str, headers: dict = {}, **kwargs) -> requests.Response:
        """
        ...
        - The response from the request.
        """
        return self.send_request(HttpMethod.PATCH, url, headers, **kwargs)
