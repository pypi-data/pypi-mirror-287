...
--------------------------------------
    ...
    optional arguments:
      ...
      --bypass-tax                  Bypass the download_taxonomy step. The necessary taxonomy files can                                                              
                                    be found on our GitHub and downloaded by users.(required for kmer
                                    database)
      -t THREADS                    Number of threads to use (default: 1)
      --db-name DB_NAME             DB name of yourself(required for kmer database)
      --add-to-library REFERENCE_GENOME.fasta
                                    Please update your FASTA file headers to include the prefix in the 
                                    following format: >GenomeID|kraken:taxid|39947. For more details, 
                                    please refer to our GitHub help document.(required for kmer database)
