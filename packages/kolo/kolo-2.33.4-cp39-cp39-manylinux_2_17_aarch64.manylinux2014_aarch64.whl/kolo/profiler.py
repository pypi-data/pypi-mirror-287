...
class Enabled:
    ...
    def save_trace_profiler(self):
        ...
        if self.upload_token:
            self._profiler.upload_trace_in_thread(self.upload_token)
        else:
            self._profiler.save_trace_in_thread()
    ...
    def save_trace_monitor(self):
        ...
        if self.upload_token:
            self._monitor.upload_trace_in_thread(self.upload_token)
        else:
            self._monitor.save_trace_in_thread()
...
def enable(
    ...
    *,
    ...
    enabled = Enabled(
        config, source, _one_trace_per_test, _save_in_thread, _upload_token
    )
