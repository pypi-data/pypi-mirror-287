...
impl PluginProcessor {
    ...
    fn new(plugin_data: &Bound<'_, PyDict>, context: &Bound<'_, PyDict>) -> Result<Self, PyErr> {
        Ok(Self {
            filename_finder: Finder::new(&get_filename(plugin_data)?).into_owned(),
    ...
    fn frame_id(&self, pyframe: &Bound<'_, PyFrame>, event: &str) -> String {
        match event {
            "call" => self.create_frame_id(pyframe),
            "return" => self.get_frame_id(pyframe),
            _ => unreachable!(),
        }
    }
