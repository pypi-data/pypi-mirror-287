...
if sys.version_info >= (3, 12):
    ...
    class KoloMonitor:
        def __init__(self, db_path, *, config=None, one_trace_per_test=False, source):
            ...
            self.event_map = {
                sys.monitoring.events.PY_START: self.monitor_pystart,
                sys.monitoring.events.PY_RETURN: self.monitor_pyreturn,
                sys.monitoring.events.PY_UNWIND: self.monitor_pyunwind,
                sys.monitoring.events.PY_RESUME: self.monitor_pyresume,
                sys.monitoring.events.PY_YIELD: self.monitor_pyyield,
                sys.monitoring.events.PY_THROW: self.monitor_pythrow,
            }
            if self.config.get("line_events", False):
                self.event_map[sys.monitoring.events.INSTRUCTION] = (
                    self.monitor_instruction
                )

            self.main_frames = []
            self.thread_frames = defaultdict(list)
        ...
        def monitor(self):
            if sys.monitoring.get_tool(self.tool_id):
                return
            self.active = True
            sys.monitoring.use_tool_id(self.tool_id, "kolo")

            all_events = functools.reduce(
                operator.or_, self.event_map, sys.monitoring.events.NO_EVENTS
            )
            sys.monitoring.set_events(self.tool_id, all_events)
            for event, callback in self.event_map.items():
        ...
        def start_test(self):
            self.trace_id = f"trc_{ulid.new()}"
            self.start_test_index = len(self.main_frames)
            self.start_test_indices = {
                thread_id: len(frames)
                for thread_id, frames in self.thread_frames.items()
            }
        ...
        def end_test(self):
            ...
            self.save(
                self.main_frames[self.start_test_index :],
                test_frames,
            )
        ...
        def monitor_instruction(self, code, instruction_offset):  # pragma: no cover
            ...
            if instruction is None or not instruction.opname in (
                "STORE_FAST",
                "STORE_GLOBAL",
                "STORE_DEREF",
            ):
        ...
        def push_frame_data(self, data):
            native_id = data["thread_native_id"]
            data = self.dump_msgpack(data)
            if native_id == self.main_thread_id:
                self.main_frames.append(data)
            else:
                self.thread_frames[native_id].append(data)  # pragma: no cover
        ...
        def build_trace(self, main_frames=None, thread_frames=None):
            data = {
        ...
        def save(self, main_frames=None, thread_frames=None):
            serialized_data = self.build_trace(main_frames, thread_frames)
        ...
        def save_trace_in_thread(self):
            if platform.machine() == "wasm32":
                ...
                threading.Thread(target=self.save, name=name).start()
