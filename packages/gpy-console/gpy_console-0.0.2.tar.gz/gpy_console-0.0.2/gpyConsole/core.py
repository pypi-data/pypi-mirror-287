...
from guilded.ext.commands.cooldowns import (
    BucketType,
    Cooldown,
    CooldownMapping,
...
def console_command(name: str = None, cls=ConsoleCommand, **kwargs):
    """A decorator that transforms a function into a :class:`.ConsoleCommand`
    or if called with :func:`console_group`, :class:`.ConsoleGroup`.
...
class ConsoleGroup(ConsoleCommand):
    invoke_without_command: bool = False
    case_insensitive: bool = False
    all_commands: typing.Dict[str, ConsoleCommand]
    ...
    def copy(self):
        """Creates a copy of this :class:`.ConsoleGroup`.
    ...
    def console_command(
        self,
        name: str = None,
        cls: typing.Type[ConsoleCommand] = ConsoleCommand,
        *args,
        **kwargs,
    ):
        """A shortcut decorator that invokes :func:`.console_command` and adds it to
        the internal command list via :meth:`~.ConsoleGroup.add_command`.

        Returns
        --------
        Callable[..., :class:`ConsoleCommand`]
            A decorator that converts the provided method into a ConsoleCommand, adds it to the bot, then returns it.
        """
    ...
    def console_group(
        ...
        **kwargs,
        ...
        """A shortcut decorator that invokes :func:`.console_group` and adds it to
        the internal command list via :meth:`.add_command`.

        Returns
        --------
        Callable[..., :class:`ConsoleGroup`]
            A decorator that converts the provided method into a ConsoleGroup, adds it to the bot, then returns it.
        """

        def decorator(func: typing.Callable):
            kwargs.setdefault("parent", self)
            result = group(
                name=name or func.__name__, cls=cls or ConsoleGroup, *args, **kwargs
            )(func)
...
def console_group(name: str = None, cls=ConsoleGroup, **kwargs):
    """A decorator that transforms a function into a :class:`.ConsoleGroup`.

    This is similar to the :func:`.console_command` decorator but the ``cls``
    parameter is set to :class:`ConsoleGroup` by default.
    """
...
def check(predicate: Check) -> Callable[[T], T]:
    r"""A decorator that adds a check to the :class:`.ConsoleCommand` or its
    subclasses. These checks could be accessed via :attr:`.ConsoleCommand.checks`.
...
def cooldown(
    ...
    """A decorator that adds a cooldown to a :class:`.ConsoleCommand`
