...
def df_to_html_table(
    ...
    **kwargs: Any,
    ...
    """Convert a pandas Styler to a svelte table, HTML, or SVG image.

    Args:
        styler (Styler): Styler object to export.
        file_path (str): Path to the file to write the svelte table, HTML, or SVG to.
    ...
    Returns:
        str: pandas Styler as HTML or path to SVG file.
    """
    ...
    if file_path:
        ...
        if file_path.suffix.lower() == ".svg":
            ...
            # Create SVG
            dwg = Drawing(file_path, size=("100%", "100%"))

            # Add table content to SVG
            y_offset = 0
            for row in table.find_all("tr"):
                x_offset = 0
                for cell in row.find_all(["td", "th"]):
                    text = dwg.text(cell.text, insert=(x_offset, y_offset))
                    dwg.add(text)
...
def df_to_svg(
    ...
    **kwargs: Any,
    ...
    If the pdf2svg CLI is installed, this function will use df_to_pdf followed by pdf2svg
    conversion which offers superior qualiry and margin cropping. Otherwise, it falls
    back to translating the Styler to a matplotlib figure which is then saved to SVG.
    ...
    Args:
        ...
        **kwargs: Passed to matplotlib.figure.Figure.savefig() or df_to_pdf().

    Returns:
        str: The SVG file contents.

    Raises:
        subprocess.CalledProcessError: If SVG compression or conversion fails.
    """
    file_path = Path(file_path)
    ...
    if which("pdf2svg"):
        ...
        try:
            df_to_pdf(obj, temp_pdf_path, **kwargs)

            # Convert PDF to SVG
            subprocess.run(["pdf2svg", str(temp_pdf_path), str(file_path)], check=True)
        finally:
            temp_pdf_path.unlink()  # Remove temporary PDF file

    else:  # Fall back to original matplotlib-based SVG generation
        matplotlib_df_to_svg(
            obj, file_path, font_size=font_size, dark_mode=dark_mode, **kwargs
        )

    # Compress SVG if requested and svgo is available
    if compress and (svgo := which("svgo")):
        subprocess.run(
...
def matplotlib_df_to_svg(
    ...
    **kwargs: Any,
    ...
    """Convert a pandas DataFrame or Styler to matplotlib Figure in order to save it as
    SVG and optionally compress it.
