...
class OptimizeDatas:
    ...
    def optimize_bak(self):
        ...
        now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S ")
        print(f'{now}mongodb({self.host}: {self.port}) 数据库优化中...')

        self.client = pymongo.MongoClient(self.link)  # 连接数据库
        collections = self.my_collection_names(db_name=self.db_name)  # 所有集合名称
        for collection_name in collections:
            print(f'任务开始: {collection_name}')
            before = time.time()
            datas = self.get_collection_datas(db_name=self.db_name, collection_name=collection_name)
            now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S ")
            print(f'{now}mongodb({self.host}: {self.port}) -> {collection_name} 获取数据, 长度: {len(datas)}')
            if not datas:
                continue
            cost_time1 = round(time.time() - before, 2)
            ...
            now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S ")
            print(f'{now}mongodb({self.host}: {self.port}) -> {collection_name} 完成排重')
            ...
            print(f'{now}{self.db_name}/{collection_name} -> '
                  f'{collection_name} 移除重复数据: {len(duplicate_id)}条, 保留: {collection.count_documents({})},  '
                  f'用时: {cost_time}秒(其中读取: {cost_time1}秒)')
            # break

        self.client.close()  # 断开连接
        now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S ")
        print(f'{now}mongodb({self.host}: {self.port}) 数据库优化完成!')
    ...
    def delete_duplicate(self, collection_name, date,):
        ...
        print(f'{now}{self.db_name}/{collection_name} -> '
              ...
              f'移除重复数据: {len(duplicate_id)}条, 保留: {collection.count_documents({})},  用时: {cost_time}秒')
