...
def upload_one_dir():
    ...
    name_str = '商品_全部'
    p.db_name = '生意参谋数据1'
    p.collection_name = f'生意参谋_商品排行'
    path = pathlib.Path(r'C:\同步空间\BaiduSyncdisk\原始文件2', r'生意参谋/商品排行123123123')

    for root, dirs, files in os.walk(path, topdown=False):
        for name in files:
            if name.endswith('.csv') and 'baidu' not in name and name_str in name:
...
def main():
    m = OptimizeDatas(username='linle', password='188988', host='192.168.1.7', port=27017)
    m.db_name_lists = [
        '京东数据1',
        '天猫数据1',
        '市场数据1',
    ...
    m = OptimizeDatas(username='linle', password='188988', host='8.138.27.216', port=27017)
    m.db_name_lists = [
        'pandas数据源'
    ]
    m.days = 63
