...
def set_environment_variables(config):
    environment = config['DEFAULT'].get('environment')
    aws_account_id = config['DEFAULT'].get('aws_account_id')
    sam_deployment_bucket = config['DEFAULT'].get('sam_deployment_bucket')
    buildkite_org_slug = config['DEFAULT'].get('buildkite_org_slug')
    buildkite_pipeline_slug = config['DEFAULT'].get('buildkite_pipeline_slug')
...
def deploy(service_type, action, service_name):
    ...
    config = get_config()
    base_path = config['DEFAULT'].get('base_path')
    ...
    sam_deployment_bucket = os.environ['SAM_DEPLOYMENT_BUCKET']
    buildkite_org_slug = os.environ['BUILDKITE_ORG_SLUG']
    ...
    if service_type == 'ecs':
        ...
        check_versioning_script_path = '{}/lambda/{}/.buildkite/scripts/check-versioning'.format(env_path, service_name)
        stack_name = extract_stack_name('{}/lambda/{}/template.yaml'.format(env_path, service_name), environment)
        if not stack_name:
        ...
        s3_command = 'aws s3 cp s3://{}/{}/buildkite/sam/config/{}/{}/{}/samconfig.toml {}/lambda/{}/samconfig.toml'.format(
            sam_deployment_bucket, environment, buildkite_org_slug, buildkite_pipeline_slug, stack_name, env_path, service_name)
        try:
    ...
    if action == 'deploy':
        if service_type == 'ecs':
            ...
            print("Running deploy script: {}".format(deploy_script_path))
            run_script(deploy_script_path, os.environ, base_path)
            if not os.path.exists(check_versioning_script_path):
                print("Check versioning script not found. Please provide the correct service name.")
                return
...
def migrate(db_type):
    print("Starting migrate function for db_type: {}".format(db_type))
    config = get_config()
    base_path = config['DEFAULT'].get('base_path')
