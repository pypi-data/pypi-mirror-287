...
from message_local.ChannelProviderConstants import (
    ...
    INFORU_MESSAGE_PROVIDER_ID, VONAGE_MESSAGE_PROVIDER_ID, FACEBOOK_MESSAGE_SELENIUM_PROVIDER_ID)
...
class MessagesLocal(metaclass=MetaLogger, object=logger_object_message):
    ...
    def __send(recipient: Recipient, message_local: MessageLocal, importance: MessageImportance,
        ...
        if (message_recipient_channel == MessageChannel.SMS and
            ...
            # TODO FacebookMessageSelenium doesn't work with the same format as the other send methods. Why? Seems like I need to do it like this?
            # if message_recipient_provider_id == FACEBOOK_MESSAGE_SELENIUM_PROVIDER_ID:
            message_local = FacebookMessageSelenium()
            message_ids = message_local.send(recipients=[recipient], body=str(body))
