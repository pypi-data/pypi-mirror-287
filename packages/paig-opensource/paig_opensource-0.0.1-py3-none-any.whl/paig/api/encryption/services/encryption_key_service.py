...
from core.utils import validate_id
...
class EncryptionKeyRequestValidator:
    ...
    def __init__(self, encryption_key_repository: EncryptionKeyRepository = Depends(EncryptionKeyRepository)):
...
class EncryptionKeyService(BaseController[EncryptionKeyModel, EncryptionKeyView]):
    ...
    def __init__(self,
                 encryption_key_repository: EncryptionKeyRepository = Depends(EncryptionKeyRepository),
                 secure_encryptor: SecureEncryptor = Depends(SecureEncryptorFactory().get_secure_encryptor),
                 encryption_key_request_validator: EncryptionKeyRequestValidator = Depends(EncryptionKeyRequestValidator)):
        super().__init__(
            encryption_key_repository,
            EncryptionKeyModel,
            EncryptionKeyView
        )
        self.secure_encryptor = secure_encryptor
    ...
    async def list_encryption_keys(self, filter: EncryptionKeyFilter, page_number: int, size: int, sort: List[str]) -> (
        ...
        for record in result.content:
            record.public_key = self.secure_encryptor.decrypt(record.public_key)
            record.private_key = self.secure_encryptor.decrypt(record.private_key)
    ...
    async def create_encryption_key(self, key_type: EncryptionKeyType = EncryptionKeyType.MSG_PROTECT_SHIELD) -> (
        ...
        encrypted_public_key = self.secure_encryptor.encrypt(rsa_key_info.public_key_encoded_str)
        encrypted_private_key = self.secure_encryptor.encrypt(rsa_key_info.private_key_encoded_str)
    ...
    async def get_public_encryption_key_by_id(self, id: int) -> EncryptionKeyView:
        ...
        result.public_key = self.secure_encryptor.decrypt(result.public_key)
    ...
    async def get_encryption_key_by_id(self, id: int) -> EncryptionKeyView:
        ...
        encryption_key.public_key = self.secure_encryptor.decrypt(encryption_key.public_key)
        encryption_key.private_key = self.secure_encryptor.decrypt(encryption_key.private_key)
