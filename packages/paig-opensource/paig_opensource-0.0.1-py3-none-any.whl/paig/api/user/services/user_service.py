...
from core.utils import verify_password, get_password_hash
...
def _update_user_model_dict(user_info, user_params):
    user_dict = dict()
    user_dict["password"] = user_params.get("password", user_info.password)
    user_dict["first_name"] = user_params["first_name"]
    user_dict["last_name"] = user_params["last_name"]
    user_dict["status"] = user_params["status"]
    user_dict["is_tenant_owner"] = True if user_params.get('roles')[0] == users_constants.OWNER_ROLE else False
    return user_dict
...
class UserService:
    ...
    def __init__(
            ...
            group_repository: GroupRepository = Depends(GroupRepository),
            gov_service_validation_util: GovServiceValidationUtil = Depends(GovServiceValidationUtil)
    ):
    ...
    async def update_user(self, id: int, user: dict, user_params: dict):
        ...
        new_groups = user_params.get('groups', [])
