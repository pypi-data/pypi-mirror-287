...
class BaseUserModel(BaseModel):
    status: Literal[0, 1] = Field(..., description="User status 0 or 1")
    first_name: str = Field(..., description="First Name of the user", pattern=r"^[a-zA-Z0-9_'.-]*$", examples=["John"], alias="firstName")
    last_name: str = Field(..., description="Last Name of the user", pattern=r"^[a-zA-Z0-9_'.-]*$", examples=["Doe"], alias="lastName")
...
class UsernameModel(BaseModel):
    username: str = Field(None, description="Unique username of the user", pattern=r"^[a-zA-Z0-9_.-]{1,255}$", examples=["JohnDoe"])
