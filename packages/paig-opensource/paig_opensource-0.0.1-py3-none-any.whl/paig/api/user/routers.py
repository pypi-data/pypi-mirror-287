...
from api.user.api_schemas.user_schema import CreateUserModel, UserLoginRequest, UserLoginResponse, \
    BaseUserModel, GetUsersFilterRequest
...
@user_router.get("/api/users/tenant")
...
@user_router.put("/api/users/{id}")
async def update_user(
        request: Request,
        response: Response,
        id: int,
        body_params: BaseUserModel,
        user: dict = Depends(get_auth_user),
        user_controller: UserController = Depends(UserController)
):
...
@user_router.put("/api/groups/{group_id}")
...
@user_router.delete("/api/groups/{group_id}")
async def update_user_group(
        request: Request,
        group_id: int,
        user: dict = Depends(get_auth_user),
...
@user_router.put("/api/groups/{group_id}/users")
...
@user_router.post("/api/data-protect/decrypt")
async def decrypt_messages(
        request: Request,
        response: Response,
        body_params: DecryptListMessagesByID = DecryptListMessagesByID,
        user: dict = Depends(get_auth_user),
        encryption_key_controller: EncryptionKeyController = Depends(EncryptionKeyController),
):
