...
@publish_invocation
def cmd_pod_load(name=_A,remote=_A,merge=_A,yes=_B,secret=_A):
	B=remote;A=name;D=CloudPodsClient(_C);_assert_host_reachable()
	try:E,F=get_pod_name_and_version(A);G=RemoteConfigParams(remote_name=B)if B else _A;D.load(pod_name=E,remote=G,version=F,merge_strategy=merge,ignore_version_mismatches=yes,secret=secret)
