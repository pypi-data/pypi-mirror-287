...
from .stopwords import remove_stopwords_extended
...
def arabica_freq(text: str,                 # Text
                 ...
                 stopwords_extened: [],     # Languages for extended stop words lists
    ...
    if date_format == 'eur':
        ...
        data['time']=data['time'].astype(str)
        data['time'] = pd.to_datetime(data['time'], dayfirst=False, errors = 'coerce')
    ...
    if stopwords_extened is not None:
        data['text'] = data.text.map(lambda x: remove_stopwords_extended(x,stopwords=[stopwords]))
