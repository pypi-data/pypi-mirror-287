...
def transform(data: str, year: int,
    ...
    for gts_msg in gts_messages:
        ...
        for message in messages:
            ...
            # set WSI
            try:
                # client = OpenSearchClient('aos-2313f07e907e-nirwnoshjgg3cpzom2vnlcfyiy.us-east-1.es.amazonaws.com', '', 'fm12_features')
                #client = Elasticsearch('http://localhost:9200')
                # station_info = client.search(index='wigos_stations', 
                #                          body={"query": 
                #                                 {"match": 
                #                                     {"properties.traditional_station_identifier": tsi}}})['hits']['hits'][0]['_source']
                # station_info = client.station_info_lookup(tsi)

                station_info = requests.get(f'http://wis2_api/collections/mlid_station/items?wmo={tsi}').json()['features'][0]

                wsi = station_info['wigos_id']
            except Exception:
                conversion_success[tsi] = False
                LOGGER.warning(f"Station {tsi} not found in MLID station list")
                warning_msgs.append(f"Station {tsi} not found in MLID station list")
                result = {
                    "_meta": {
                        "id": None,
                        "geometry": None,
            ...
            try:
                ...
                station_height = station_info['properties']['elevaiton']
                barometer_height = station_info['properties']['elevaiton']
