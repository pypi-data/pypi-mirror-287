    ...
    try:
        ...
        while True:
            ...
            if error_message:
                ...
                status_with_color = f"{GREEN}passed{RESET}" if status == "passed" else f"{RED}failed{RESET}"

                if msg_string == 'complete':
                    print('Test complete')
                    break
                elif status == 'failed':
                    print(f"\r{date} | {status_with_color} | {msg_string}\r")
                    sys.exit(1)
                else:
                    print(f"\r{date} | {status_with_color} | {msg_string}", end="\r", flush=True)
    except Exception as e:
        print(f"\r{RED}Error: {e}{RESET}\r")
        sys.exit(1)
    finally:
        spinner.stop()
        spinner.join()
