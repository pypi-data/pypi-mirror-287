...
# from flet import(
# UserControl,Control,Alignment,FilePickerResultEvent,Page,app,
...
# Container,TextButton,Column,Tab,Tabs,AlertDialog,MainAxisAlignment,
# DataTable, TextThemeStyle,IconButton, icons, DataColumn,
...
class Atalhos:
    def __init__(self,page):
        super().__init__()
        self.page = page
        self.arq_abrev = self.ler_json('abreviacoes.json')
    def Aplicar_atalhos(self):
        ...
        # kb.add_abbreviation('f.', 'Fórum de Salgado')
        # kb.add_abbreviation('F.', 'Fórum de Salgado')
        # kb.add_abbreviation('e.', 'Expedi Mandados')
        # kb.add_abbreviation('E.', 'Expedi Mandados')    
        kb.wait('shift+esc')
...
'''
    ...
    def Infos(self):
        ...
        for i in self.info.split('\n')[1:]:
            try:
                ...
                pass
        self.arq_abrev = Ler_json('abreviacoes.json')
        for i, j in self.arq_abrev.items():
            a += f'{i}: {j} \n\n'

            # print(i,j)

        self.tabelaInfo = Text(a, width=800, selectable=True, size = 18)
    ...
    def Add_nova_abreviacao_lista(self, texto):
        ...
        self.saidad.controls.append(Text(texto, selectable=True))
        self.update()
    ...
    def Save_abreviation(self, e,nome_json):
        ...
        if abrev != '' and subst != '':
            ...
            self.arq_abrev = Ler_json(nome_json)
            self.arq_abrev[abrev] = subst
            Escrever_json(self.arq_abrev, nome_json)
    ...
    def CriarAbreviacao(self, e):
        lin1 = Row(alignment='center')
        col1 = Column(tight=True)
        ...
        def sv(e):
            self.Save_abreviation(e, 'abreviacoes.json')
        # salvar = ElevatedButton('Salvar',on_click=sv)
        # fechar =  ElevatedButton('Fechar',on_click=self.Sair)

        # col1.controls.append(Row([salvar, fechar], alignment='center'))
        ...
        def Func(e):
            ...
            match e.control.data[0]:
                ...
                case 'save':
                    ...
                    Escrever_json(self.arq_abrev, 'abreviacoes.json')
...
class Saida(ft.Column):
    def __init__(self, height=150, page = None):
        ...
        self.controls.append(ft.Container(ft.ListView([self.saidad],auto_scroll = True, height=self._height,  ),bgcolor='white,0.03' ))
    ...
    def __init__(self, json,nomejson, page):
        ...
        for i in self.dic.keys():    
            if i in ["loc_digitar_mensagen", "loc_cruz", "loc_enquet", "loc_documento", "loc_pesquisar_contato"]:
                ...
                self.dic_objetos[i] = ft.TextField(label = i, value = self.dic[i] , content_padding=12, dense=True, width=400)
        ...
        for l, i in enumerate(df_lista):
            ...
            for k, j, num_col in zip(list(self.df.columns), i, range(200)):
                ...
                if k in ['Destinatario do mandado:']:
                    ...
                    cell.append(DataCell(Row([Text(j, selectable=True, text_align='center',
                                              width=100),
                                        IconButton(icon = icons.PICTURE_AS_PDF, scale = 1, icon_size = 13, data = [indx[l],'ver_pdf',j], on_click=self.Func, tooltip = 'Visualizar PDF')
