...
def find_integer_columns(*, df: pd.DataFrame) -> list[str]:
    ...
    >>> import dawgdad as dd
    >>> df = dd.create_dataframe()
    >>> columns_int = dd.find_integer_columns(df=df)
    >>> columns_int
    ['i', 'y', 'yn']
    """
...
def number_empty_cells_in_columns(*, df: pd.DataFrame) -> None:
    ...
    ...     'Z': ['a', 'b', np.nan, 'd', 'e']
    ... })
    >>> dd.number_empty_cells_in_columns(df=df) # doctest:+NORMALIZE_WHITESPACE
...
def process_columns(
    *, df: pd.DataFrame
    ...
    ...     columns_timedelta_count = dd.process_columns(df=df) # doctest:+SKIP
    columns_in_count       : 12
...
def process_rows(*, df: pd.DataFrame) -> tuple[pd.DataFrame, int, int, int]:
    ...
    ...     dd.process_rows(df=df) # doctest: +SKIP
    rows_in_count   : 42
    rows_out_count  : 42
    rows_empty_count: 0
    """
...
def read_file(
    *,
    ...
    >>> file_name = "file.csv"
    >>> df = dd.read_file(file_name=file_name) # doctest: +SKIP
    >>> df.dtypes # doctest: +SKIP
    a    float64
    b       bool
    c     object
    ...
    >>> df = dd.read_file(file_name=path)
...
def feature_percent_empty(
    *, df: pd.DataFrame, columns: list[str], threshold: float
    ...
    >>> import dawgdad as dd
    >>> features = dd.feature_percent_empty(
    ...     df=data,
    ...     columns=features,
...
def create_directory(
    *, directories: list[str], ignore_errors: bool = True
    ...
    >>> dd.create_directory(directories=directory_list)
    """
...
def delete_directory(
    *, directories: list[str], ignore_errors: bool = True
    ...
    >>> import dawgdad as dd
    >>> directory_list = ['directory_one', 'directory_two']
    >>> dd.delete_directory(directories=directory_list)
    """
    for directory in directories:
        rmtree(path=directory, ignore_errors=ignore_errors)
...
def rename_directory(
    *, sources: list[str], destinations: list[str], ignore_errors: bool = True
    ...
    >>> dd.rename_directory(sources=sources, destinations=destinations) \
    ...     # doctest: +SKIP
    """
...
def list_files(
    *,
    ...
    >>> import dawgdad as dd
    >>> files = dd.list_files(directory=path) # doctest: +SKIP
...
def print_list_by_item(*, list_to_print: list[str], title: str = None) -> None:
    ...
    >>> import dawgdad as dd
    >>> dd.print_list_by_item(list_to_print=my_list_to_print) # doctest: +SKIP
    """
    if title:
        print(title)
...
def ask_directory_path(
    *,
    ...
    >>> from tkinter import Tk
    >>> import dawgdad as dd
    >>> path = dd.ask_directory_path(title='your message') # doctest: +SKIP
    """
    rootwindow = Tk()
...
def ask_open_file_name_path(
    *,
    ...
    >>> from tkinter import Tk
    >>> import dawgdad as dd
    >>> path = dd.ask_open_file_name_path(title='message') # doctest: +SKIP

    >>> path = dd.ask_open_file_name_path(
    ...     title='your message',
...
def ask_save_as_file_name_path(
    *,
    ...
    >>> from tkinter import Tk
    >>> import dawgdad as dd
    >>> path = dd.ask_save_as_file_name_path(title='message') # doctest: +SKIP

    >>> path = dd.ask_save_as_file_name_path(
    ...     title='your message',
    ...     filetypes=[('csv files', '.csv .CSV')]
    ... ) # doctest: +SKIP
    """
...
def list_directories(
    *,
    ...
    >>> import dawgdad as dd
    >>> path = "path"
    >>> directory_list = dd.list_directories(path=path) # doctest: +SKIP

    >>> path = "path"
    >>> pattern_startswith = ["job aidd.]
    >>> directory_list = dd.list_directories(
    ...     path=path,
    ...     pattern_startswith=pattern_startswith
    ... ) # doctest: +SKIP

    >>> path = "path"
    >>> pattern_startswith = ["job aidd., "cheatsheet"]
    >>> directory_list = dd.list_directories(
    ...     path=path,
    ...     pattern_startswith=pattern_startswith
    ... ) # doctest: +SKIP
...
def remove_punctuation(*, list_dirty: list[str]) -> list[str]:
    ...
    >>> import dawgdad as dd
    >>> list_clean = dd.remove_punctuation(list_dirty=list_dirty) \
    ...     # doctest: +SKIP
    """
    list_clean = [
        "".join(
...
def list_change_case(*, list_dirty: list[str], case: str) -> list[str]:
    ...
    >>> import dawgdad as dd
    >>> list_clean = dd.list_change_case(
...
def listone_contains_all_listtwo_substrings(
    *, listone: list[str], listtwo: list[str]
    ...
    >>> listwo = [ 'CMJG', '2020-21']
    >>> matches = dd.listone_contains_all_listtwo_substrings(
    ...     listone=listone,
    ...     listtwo=listtow
    ... ) # doctest: +SKIP
...
def list_one_list_two_ops(
    *,
    ...
    >>> list_one_unique = dd.list_one_list_two_ops(
    ...     list_one=list_one,
    ...     list_two=list_two,
    ...     action="list_one"
    ... ) # doctest: +SKIP
...
def file_size(path: Path | str) -> int:
    ...
    >>> import dawgdad as dd
    >>> path = "myfile.feather"
    >>> size = dd.file_size(path=path) # doctest: +SKIP
    """
...
def delete_empty_rows(
    ...
    >>> import dawgdad as dd
    >>> df = dd.delete_empty_rows(df=df) # doctest: +SKIP

    >>> list_columns = ["column_x", "column_y", "column_z"]
    >>> df = dd.delete_empty_rows(
    ...     df=df,
...
def delete_empty_columns(
    *, df: pd.DataFrame, list_empty_columns: list[str] | None = None
    ...
    >>> import dawgdad as dd
    >>> df = dd.delete_empty_columns(df=df) # doctest: +SKIP

    >>> list_empty_columns = ["mixed", "nan_none"]
    >>> df = dd.delete_empty_columns(
    ...    df=df,
...
def optimize_float_columns(
    ...
    --------
    >>> import dawgdad as dd
    >>> df = dd.optimize_float_columns(df=df) # doctest: +SKIP

    >>> float_columns = ["column A", "column B"]
    >>> df = dd.optimize_float_columns(
    ...     df=df,
    ...     float_columns=float_columns
    ... ) # doctest: +SKIP
    """
...
def optimize_columns(
    ...
    --------
    >>> import dawgdad as dd
    >>> df = dd.optimize_columns(df=df) # doctest: +SKIP
...
def series_memory_usage(s: pd.Series, suffix: str = "B") -> str:
    ...
    >>> import dawgdad as dd
    >>> memory_usage = dd.series_memory_usage(
    ...     s=s,
    ...     suffix="B"
    ... ) # doctest: +SKIP
    """
...
def print_dictionary_by_key(
    *, dictionary_to_print: dict[str, list[str]], title: str = None
    ...
    >>> import dawgdad as dd
    >>> dd.print_dictionary_by_key(dictionary_to_print=mydict) # doctest: +SKIP
    """
    if title:
        print(title)
