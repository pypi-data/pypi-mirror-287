...
from typing import Dict, Tuple, Any, Optional, List, Iterator, Type
...
class Table:
    def __init__(self, db, table_name):
        ...
        dataclass_fields: List[Tuple[str, Any]] = []  # Fields for dataclass
        for col in columns:
            if col['kind'] != 'partition_key' and col['kind'] != 'clustering':
                model_fields[col["column_name"]] = (Optional[get_pydantic_type(col["type"])], None)
                dataclass_fields.append((col["column_name"], Optional[get_pydantic_type(col["type"])]))
            else:
                model_fields[col["column_name"]] = (get_pydantic_type(col["type"]), ...)
                dataclass_fields.append((col["column_name"], get_pydantic_type(col["type"])))
