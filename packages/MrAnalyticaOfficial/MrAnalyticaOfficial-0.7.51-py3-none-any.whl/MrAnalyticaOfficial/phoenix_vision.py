...
class PhoenixVision:
    ...
    def _criar_modelo(self):
        ...
        # Congele apenas as primeiras 100 camadas
    ...
    def treinar(self):
        ...
        self.model.fit(
            ...
            epochs=50,  # Aumente o número de épocas
    ...
    def _process_frame(self):
        while not self.stopped:
            if self.frame is not None:
                # Processe em lotes para melhorar o FPS
                batch_size = 4
                frames = [self.frame.copy() for _ in range(batch_size)]
                results = [self._detect_and_recognize(f) for f in frames]
                self.processed_frame, num_faces = results[-1]  # Use o último resultado
                self.total_faces_detected += sum(r[1] for r in results)
