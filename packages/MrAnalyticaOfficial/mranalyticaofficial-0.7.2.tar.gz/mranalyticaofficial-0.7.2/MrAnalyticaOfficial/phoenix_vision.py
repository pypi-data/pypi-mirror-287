...
class PhoenixVision:
    ...
    def treinar(self):
        ...
        self.model.fit(
            self.data_augmentation.flow(X_train, y_train, batch_size=64),
            epochs=20,
    ...
    def start(self):
        ...
        while True:
            ...
            if self.show_stats:
                stats_text = [
                    f"Modo de deteccao: {self.detection_mode}",
                    f"FPS: {fps:.2f}",
                    f"Faces detectadas: {num_faces}",
                    f"Total de faces detectadas: {self.total_faces_detected}",
                    f"Resolucao: {frame.shape[1]}x{frame.shape[0]}",
                    f"Tempo decorrido: {elapsed_time:.2f}s",
                    f"Frames processados: {self.frame_count}",
                    f"Media de faces por frame: {self.total_faces_detected / self.frame_count:.2f}",
                    f"Uso de memoria: {self._get_memory_usage():.2f} MB"
                ]
