...
class PhoenixVision:
    ...
    def enable_face_landmarks(self, enable, landmark_predictor_path=None):
        ...
        if enable:
            if landmark_predictor_path and os.path.exists(landmark_predictor_path):
                ...
                print(f"Detecção de pontos de referência facial ativada")
            else:
                print("Arquivo de preditor de pontos de referência não encontrado. Desativando a funcionalidade.")
                self.enable_landmarks = False
        else:
            print("Detecção de pontos de referência facial desativada")
    ...
    def _detect_and_draw(self, frame, gray):
        faces = self.face_cascade.detectMultiScale(
            ...
            scaleFactor=1.1, 
            minNeighbors=5, 
            minSize=self.face_size_range[0], 
            maxSize=self.face_size_range[1]
        )
        ...
        for (x, y, w, h) in faces:
            ...
            if self.enable_landmarks and self.landmark_predictor:
