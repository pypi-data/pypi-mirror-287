    ...
    def _carregar_fotos(self):
        ...
        for nome_pessoa in os.listdir(self.fotos_dir):
            ...
            for nome_imagem in os.listdir(pessoa_dir):
                ...
                # Lê a imagem como BGR
                imagem = cv2.imread(imagem_path)
                if imagem is None:
                    logging.warning(f"Não foi possível carregar a imagem: {imagem_path}")
                    continue
                ...
                if imagem_rgb.dtype != np.uint8:
                    logging.warning(f"A imagem {imagem_path} não está no formato uint8. Convertendo...")
