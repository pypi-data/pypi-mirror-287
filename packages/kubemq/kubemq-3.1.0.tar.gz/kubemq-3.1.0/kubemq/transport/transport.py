...
def _get_call_options(connection: Connection) -> Sequence:
    ...
    if connection.keep_alive and isinstance(connection.keep_alive, KeepAliveConfig) and connection.keep_alive.enabled:
        options.append(('grpc.keepalive_time_ms', connection.keep_alive.ping_timeout_in_seconds * 1000))
        options.append(('grpc.keepalive_timeout_ms', connection.keep_alive.ping_interval_in_seconds * 1000))
        options.append(('grpc.keepalive_permit_without_calls', 1))
        options.append(('grpc.http2.min_time_between_pings_ms', connection.keep_alive.ping_timeout_in_seconds * 1000))
        options.append(
            ('grpc.http2.min_ping_interval_without_data_ms', connection.keep_alive.ping_interval_in_seconds * 1000))
    return options
...
class Transport:
    ...
    def initialize(self) -> 'Transport':
        try:
            auth_interceptor = _AuthInterceptors(self._opts.auth_token)
            interceptors = [auth_interceptor]
            credentials = _get_ssl_credentials(self._opts.tls) if self._opts.tls.enabled else None
            self._channel = grpc.secure_channel(self._opts.address, credentials, options=_get_call_options(
                self._opts)) if credentials else grpc.insecure_channel(self._opts.address,
...
class AsyncTransport:
    ...
    async def initialize(self) -> 'AsyncTransport':
        ...
        if self._opts.tls.enabled:
            try:
                credentials: ChannelCredentials = _get_ssl_credentials(self._opts.tls)
                self._channel = grpc.aio.secure_channel(self._opts.address, credentials,
                                                        options=_get_call_options(self._opts),
                                                        interceptors=interceptors_async)
            except Exception as e:
                raise e
        else:
            self._channel = grpc.aio.insecure_channel(self._opts.address, options=_get_call_options(self._opts),
                                                      interceptors=interceptors_async)
