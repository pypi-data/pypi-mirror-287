...
from .utils import decode_JWT, get_logger, generate_token, create_temporary_file
...
class Cortex:
    ...
    def client(
        ...
        **Example**
        ...
        :param profile: (optional) Use the specfied profile, will default to current profile in ~/.cortex.config if not provided
        ...
        tkn = _Token(token)
        cert_data = env.cert
        # If verifying certs and cert data is provided, dump to tempfile and set values and env vars == temp file name
        if verify_ssl_cert and cert_data is not None:
            # 1) write cert to temporary location
            temp_cert_file_name = create_temporary_file(suffix=".crt")
            with open(temp_cert_file_name, "w+b") as temp_cert_file:
                temp_cert_file.write(base64.b64decode(cert_data))
                temp_cert_file.close()
            # 2) set verify and CURL_CA_BUNDLE
            verify_ssl_cert = temp_cert_file_name
            # Add this JIC for other libs using Sensa URL
            os.environ.CURL_CA_BUNDLE = temp_cert_file_name
