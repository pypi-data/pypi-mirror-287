...
def config_file_path():
    ...
    """
    default_path = os.path.join(str(Path.home()), CONFIG_FOLDER)
    return os.path.abspath(os.path.join(os.getenv("CORTEX_CONFIG_DIR", default_path), "config"))
...
def create_temporary_file(suffix=None):
    ...
    with NamedTemporaryFile(suffix=suffix, delete=False) as temporary_file:
        ...
        CLEANUP.append(temporary_filename)
        return temporary_filename

def file_cleanup():
    """
    Cleanup temporary files created by create_temporary_file.
    """
...
atexit.register(file_cleanup)
signal.signal(signal.SIGTERM, file_cleanup)
signal.signal(signal.SIGINT, file_cleanup)
