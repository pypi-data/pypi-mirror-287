...
# Only for debugging, otherwise PyCharm crashes TODO: Remove before uploading
import matplotlib
...
def get_global_delay(data, original_time, signal_interpolated, consider_all_traces, debug=False):
    ...
    if not consider_all_traces and data["number_of_traces"] > number_of_traces_tested:
        ...
        trace_cut_idx = data["trace_cut_index"][(data["trace_cut_index"] >= data["trace_cut_index"][2]) &
                                                 (data["trace_cut_index"] <= data["trace_cut_index"][number_of_traces_tested - 2])]
    ...
    res = direct(func=_minimize,
                 bounds=bounds,
                 len_tol=5e-6,
                 args=(data, original_time_short, original_position_short, trace_cut_idx, signal_interpolated, interpolated_delay))
    # print(res)
    end = timer()
...
def _minimize(delay, data, original_time_short, original_position_short, trace_cut_idx, signal_interpolated, interpolated_delay):
