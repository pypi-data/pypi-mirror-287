...
# %% auto 0
__all__ = ['get_user', 'GetUser_Config', 'modify_config_user_email_for_upsert_user_fn', 'upsert_user', 'UpsertUser_Config',
           'delete_user', 'DeletetUser_Config', 'modify_config_is_force_regenerate_based_on_expiration_threshold_fn',
           'post_config_fail_if_no_token', 'regenerate_user_accesstoken', 'RegenerateUserAccessToken_Config']
...
async def upsert_user(
    ...
    **kwargs,
    ...
    if modify_config_fn:
        ...
        user_display_name = config.domo_user.display_name
    ...
    if domo_user:
        try:
            property_ls = [
                dmdu.UserProperty(dmdu.UserProperty_Type.display_name, user_display_name),
            ...
            await domo_user.update_properties(property_ls=property_ls, session = session, debug_api = debug_api)
...
async def post_config_fail_if_no_token(config: FactoryConfig, step: FactoryMessage, **kwargs):
...
async def regenerate_user_accesstoken(
    ...
    **kwargs,
    ...
    try:
        ...
        # step 1
        if modify_config_fn:
            step = FactoryMessage(stage="executing modify_config_fn", 
                                stage_num=1)
            step.is_success = False
            res.add_message(step)
            await modify_config_fn(
        ...
        if post_config_fn:
            step = FactoryMessage(stage="executing post_config_fn", 
                                stage_num=4, is_success = False)
