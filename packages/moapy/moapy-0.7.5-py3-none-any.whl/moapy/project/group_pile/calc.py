import moapy.project.group_pile.foundations_modeling as fm
import json
...
class CreateGroupPile(MBaseModel):
    ...
    """
    pile_matl_ID: int = dataclass_field(default=3, description="pile material ID")
    pile_sect_ID: int = dataclass_field(default=6, description="pile section ID")
    cap_matl_ID: int = dataclass_field(default=3, description="cap material ID")
    cap_sect_ID: int = dataclass_field(default=5, description="cap section ID")
    pile_start_nb: int = dataclass_field(default=1000, description="pile start node number")
    cap_start_nb: int = dataclass_field(default=100, description="cap start node number")
    pile_array_long: int = dataclass_field(default=2, description="pile array long")
    pile_array_tran: int = dataclass_field(default=3, description="pile array tran")
    pile_spacing_long: float = dataclass_field(default=2.5, description="pile spacing long")
    pile_spacing_tran: float = dataclass_field(default=2.5, description="pile spacing tran")
    cap_edge_spacing_long: float = dataclass_field(default=1.25, description="cap edge spacing long")
    cap_edge_spacing_tran: float = dataclass_field(default=1.25, description="cap edge spacing tran")
    spacing_style_int: int = dataclass_field(default=1, description="spacing style int")
    pile_dia: float = dataclass_field(default=1.2, description="pile diameter")
    pile_length: float = dataclass_field(default=15.5, description="pile length")
    cap_height: float = dataclass_field(default=2, description="cap height")
    cap_modeling: str = dataclass_field(default="true", description="cap modeling")
    grup_ID: int = dataclass_field(default=5, description="group ID")
    bngr_ID: int = dataclass_field(default=2, description="boundary group ID")
