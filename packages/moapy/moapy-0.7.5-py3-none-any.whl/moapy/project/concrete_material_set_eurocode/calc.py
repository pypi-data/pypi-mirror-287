...
class ConcreteMaterial:
	"""
	Concrete Material Input Class
	"""
	grade: float = dataclass_field(
    default=25, metadata={
        "description": "Concrete Grade"})


@auto_schema
def concrete_properties(gradeConc: ConcreteMaterial):
	# Density and Weight of Cocnrete
	# EN 1991-1-1:2002
	# Table A.1
	# nomarl weight = 24.0 / Increase by 1 kN/m3 for normal persentage of
	# reinforcing and pre-stressing steel
	grade = gradeConc.grade
	density = 25.0 / 9.806
...
@dataclass
class ConcreteMaterialInfo:
	"""
	Concrete Material Info Input Class
	"""
	grade: float = dataclass_field(
    default=25, metadata={
        "description": "Concrete Grade"})
...
@auto_schema
def stain_stress_curve(input: ConcreteMaterialInfo) -> str:
	# EN 1992-1-1:2004
	# §3.1.5 Stress-strain relation for non-linear structural analysis
	grade = input.grade
	partial_factor = input.partial_factor
	if check_numbers(partial_factor):
		json.dumps({ "error": "Partial Factor must be a number" })
	elif math.isnan(partial_factor):
		return json.dumps({ "error": "Partial Factor must be a number" })
	elif partial_factor<=1:
		return json.dumps({ "error": "Partial Factor must be greater than 1" })
...
def Create_material(input_data):
	...
	# Material Properties
	# ---------------------------------------------------------------------------------------------
	civil = MidasAPI.create_instance(Product.CIVIL, "KR")
