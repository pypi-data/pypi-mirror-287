...
class AppDbDocument:
    ...
    def update_config(self):
        self.content = {key: value for key, value in self.__dict__.items() if key not in ['auth', 'content'] and not key.startswith('_')}
        return self.content
...
class AppDbCollection:
    ...
    async def search_documents(
        ...
        self.domo_documents= await ce.gather_with_concurrency(*[AppDbDocument.get_by_id(collection_id= self.id,
                                document_id= doc['id'] , auth = auth ) for doc in res.response], n = 5)
...
# %% ../../nbs/classes/50_DomoAppDb.ipynb 18
@patch_to(AppDbDocument, cls_method=True)
async def upsert(
    cls,
    auth: dmda.DomoAuth,
    ...
    filters = {f"content.{col}": content[col] for col in identity_columns}

    domo_collection = await AppDbCollection.get_by_id(
        auth=auth, collection_id=collection_id, return_raw=False
    )

    res = await domo_collection.search_documents(filters=filters, debug_api=debug_api)
...
# %% ../../nbs/classes/50_DomoAppDb.ipynb 22
@dataclass
