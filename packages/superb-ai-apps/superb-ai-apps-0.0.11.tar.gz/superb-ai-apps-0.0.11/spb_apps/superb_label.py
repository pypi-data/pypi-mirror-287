...
class SuperbLabel(SuperbApps):
    def __init__(
        ...
        try:
            ...
            if data_type == "image":
                new_label_interface = phy_credit.imageV2.LabelInterface.get_default()
            elif data_type == "video":
                new_label_interface = phy_credit.video.LabelInterface.get_default()
    ...
    def upload_video(self, video_path: str, dataset_name: str, num_threads: int = 3):
        """
        ...
        if len(file_names) > 1000:
            print("[INFO] Large video detected, splitting into chunks of 1000 frames")
    ...
    def upload_large_video(
        ...
        chunks = [file_names[i : i + 1000] for i in range(0, len(file_names), 1000)]
    ...
    def upload_small_video(
        ...
        self.process_video_chunk(file_names, video_path, dataset_name, key, num_threads)
    ...
    def process_video_chunk(
        ...
        self.upload_files_concurrently(file_infos, file_names, num_threads, video_path)
    ...
    def upload_images(self, image_paths: list, dataset_name: str, ignore: bool = False):
        """
        ...
        if overwrite:
            ...
            for anno in annotations:
                ...
                if "class_name" not in bbox or "annotation" not in bbox:
                    raise Exception("[ERROR]: No annotations found for the label")
                call_with_retry(
    ...
    def add_object_classes_to_project(
        ...
        if data_type == "image":
            ...
            if existing_label_interface:
                ...
                object_tracking = phy_credit.pointclouds.ObjectTrackingDef.from_dict(
                    existing_label_interface.get("object_tracking")
                )
            else:
                label_interface = phy_credit.pointclouds.LabelInterface.get_default()
                object_tracking = phy_credit.pointclouds.ObjectTrackingDef.get_default()
