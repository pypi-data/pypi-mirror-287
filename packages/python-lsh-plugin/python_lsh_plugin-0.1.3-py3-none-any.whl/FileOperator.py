...
LastEditTime: 2024-07-30 15:34:04
FilePath: \my_plugin\plugin\FileOperator.py
Description: 
'''
#!/usr/bin/env python
# coding=utf-8
...
class YamlFileOperator(FileOperator):
    def read(self, file_path):
        with open(file_path, 'r') as f:
...
if __name__ == '__main__':
    json_file = JsonFileOperator()
    json_file.write('test.json', [{'name': 'test', 'age': 18}, {'name': 'test2', 'age': 20}, {'name': 'test3', 'age': 21}])
    print(json_file.read('test.json'))

    # yaml_file = YamlFileOperator()
    # yaml_file.write('test.yaml', [{'name': 'test', 'age': 18}, {'name': 'test2', 'age': 20}, {'name': 'test3', 'age': 22}])
    # print(yaml_file.read('test.yaml'))

    # excel_file = ExcelFileOperator()
