...
class Dataset(SendableMixin):
    ...
    def run(
        ...
        **run_args,
        ...
        runners_to_update = []
        master_content = [
            f"export rootdir=$PWD",
            f"source {self.bash_repo.name}",
        ]
        ...
        for runner in self.runners:
            ...
            runline = runner.generate_runline(submitter=submitter)
        ...
        if self.dependency is None:
            ...
            self._write_to_repo()
            self._write_to_bash_repo(bash_cache)
            ...
            cmd = [f"cd {self.remote_dir}"]  # enter remote dir

            if self._fresh_dataset:
                cmd.append(f"rm -f {self.manifest_log.name}")

            cmd += [
                # apply root dir config to repo
                f'sed -i -e "s#{{rootdir}}#$(pwd)#" "{self.bash_repo.name}"',
                # enable function calls
                f"source {self.bash_repo.name}",
                # exec the master script
                f"exec_and_log {launch_cmd} {self.master_script.name}",
            ]
            cmd = " && ".join(cmd)
            if not dry_run:
    ...
    def _write_to_bash_repo(self, bash_cache: list):
        content = [
            create_log_function(
                manifest_filename=self.manifest_log.name, dataset_uuid=self.short_uuid
            )
        ]
    ...
    def _is_finished(
        self, check_dependency: bool = True, dependency_call: bool = False
    ) -> list:
        """
    ...
    def fetch_results(
        ...
        self._is_finished(check_dependency=False)
    ...
    def update_runners(
        self, runners: Union[list, None] = None, dependency_call: bool = False
    ):
        """
        ...
        if log_collection.succeeded:
            ...
            if self.short_uuid in runner_data["stderr"]:
                dataset_stderr_data = runner_data["stderr"][self.short_uuid]

                stderr = "\n".join(
                    [
                        line[1]
                        for line in dataset_stderr_data
                        if line[0] >= self._last_run
                    ]
                )
                if stderr != "":
                    raise ValueError(f"Dataset encountered an issue:\n{stderr}")
            ...
            for runner in runners:
                ...
                if runner.short_uuid in runner_data["stdout"]:
                    stdout = "\n".join(
                        line[1]
                        for line in runner_data["stdout"][runner.short_uuid]
                        if line[0] >= runner.last_submitted_utc
                    )
                    runner.stdout = stdout
                if runner.short_uuid in runner_data["stderr"]:
                    stderr = "\n".join(
                        line[1]
                        for line in runner_data["stderr"][runner.short_uuid]
                        if line[0] >= runner.last_submitted_utc
                    )
...
submit_job_{submitter} () {
    export r_uuid=$1
    write_to_log submitted
    sed -i -e "s#{run_rootdir}#{rootdir}#" $2
    {submission_section}
}"""

    submission_normal = """exec_and_log {submitter} $2 || write_to_log failed
"""
    submission_script = """if exec_and_log {submitter} $2 ; then
...
def create_log_function(manifest_filename: str, dataset_uuid: str) -> str:
    template = """write_to_log () {
\techo $(date -u +'%Y-%m-%d %H:%M:%S %Z') [$r_uuid] $1 >> {rootdir}/{manifest_filename}
}
exec_and_log () {
    if [ -z "$r_uuid" ]; then
        r_uuid={dataset_uuid}
    fi
    {
        "$@" 2> >(while IFS= read -r line; do
            echo "$(date -u '+%Y-%m-%d %H:%M:%S %Z') [$r_uuid] [stderr] $line"
        done >> {rootdir}/{manifest_filename}) \\
        > >(while IFS= read -r line; do
            echo "$(date -u '+%Y-%m-%d %H:%M:%S %Z') [$r_uuid] [stdout] $line"
        done >> {rootdir}/{manifest_filename})
    }
}
"""
    template = template.replace("{manifest_filename}", manifest_filename)
    template = template.replace("{dataset_uuid}", dataset_uuid)
    return template
