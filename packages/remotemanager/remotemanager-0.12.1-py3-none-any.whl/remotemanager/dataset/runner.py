...
class Runner(SendableMixin):
    ...
    def __init__(
        ...
        **run_args,
        ...
        # stdout can be a string, however stderr needs to be a list for dependencies
        self._history = {}
        self.stdout = ""
        self._stderr = []
    ...
    def generate_runline(
        ...
        runline = [f"submit_job_{submitter} {self.short_uuid} {jobfile}"]
    ...
    def stage_python(
        ...
        runscript = [
            ...
            "from datetime import datetime, timezone",
            "remote_path = os.path.expandvars('$rootdir')",
        ...
        # jobscript writing
        submit = [
            f"export rootdir={{run_rootdir}}\n",
            f"source $rootdir/{self.parent.bash_repo.name}\n",
        ]
        if self.run_path != self.remote_dir:
            ...
            submit += [
                ...
                f"{parent_check}"
                f"cd {self.run_dir} && "
                f"exec_and_log {python} ${{pydir}}/{self.runfile.name}"
            ]
        else:
            submit += [f"{parent_check}\nexec_and_log {python} {self.runfile.name}"]

        submit += [f" || write_to_log failed"]
        submit = "".join(submit)
        logger.debug("using script %s", submit)
    ...
    def read_local_files(self) -> None:
        ...
        if not self.state.success:
            logger.info("Runner has not succeeded: state=%s", self.state)
            return
    ...
    def result(self):
        ...
        if self.parent.function is None and self.stdout != "":
            return self.stdout
    ...
    def error(self):
        ...
        try:
            ...
            # legacy run mode, we're probably here because an update was performed
            # in the middle of a run
            if self.errorfile.local_mtime_utc > self.last_submitted_utc:
                content = self.errorfile.content

                if content != "":
                    self._stderr.append(content)
                    warnings.warn(
                        "Warning: Detected a valid error file. This means that "
                        "the intended error method encountered an error. Did you "
                        "perform an update mid run? While every effort has been "
                        "made to ensure the stability between versions, it cannot "
                        "be guaranteed for major/minor updates."
                    )
