...
class PostgresDB(DatabaseManagement, LLMIntegration, QueryHandling):
    ...
    def _create_db_engine(self) -> str:
        ...
        """
        try:
            default_db = self.postgres_connection_string.split('/')[-1].split('?')[0]
            self.db_uri = self.postgres_connection_string.replace(default_db, self.name)
            self.engine: Engine = create_engine(self.db_uri, isolation_level="AUTOCOMMIT")
        except SQLAlchemyError as e:
