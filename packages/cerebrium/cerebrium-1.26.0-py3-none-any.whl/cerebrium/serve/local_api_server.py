...
async def save_predict_data(request: Request, id: str | None):
    ...
    assert isinstance(session["workingDir"], str)
    out_file = os.path.join(session["workingDir"], "_cerebrium_predict.json")
    if os.path.exists(out_file):
        os.remove(out_file)
    with open(out_file, "w") as f:
        f.write(json.dumps(data))
    return {"message": "Data sent successfully. You should see the response in your cli logs"}
...
async def get_session(id: str | None = None) -> dict[str, JSON] | None:
    """
...
def stop_local_server(server: uvicorn.Server | None):
