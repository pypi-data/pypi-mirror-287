...
def upload_cortex_files(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if zip_size > ERROR_SIZE_LIMIT:
            cerebrium_log(
                level="ERROR",
                message="Your project zip file is over 2GB. Please reduce the file size and try again.",
                prefix="Error uploading app to Cerebrium:",
            )
            ...
            cerebrium_log(
                level="WARNING",
                message="Your project zip file is over 1GB. Your deployment should work but might encounter issues. Please try to reduce the file size if you encounter issues.",
                prefix="Warning:",
            )
