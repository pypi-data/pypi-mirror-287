    ...
    def returnSearchPriceParam(self, data):
        ...
        def full2half(s):
            ...
            for char in s:
                ...
                n += num
            return n
        if("iPhone XS" in data["model"]):
            data["model"] = "苹果 "+"iPhone XS"
        data["model"] = str(data["model"]).lower()
        if(type(data["model"]) != int):
            try:
                model_temp = data["model"]
                data["model"] = self.model_id_dic[data["model"]]
            except KeyError as e:
                return {"msg":"不存在对应机型product_id","model":data["model"]}
        ...
        try:
            ...
            if(post_CN_li == [] and post_id_li == []):
                return {"msg":"pjt_inspection不存在对应product_id AllJson数据","product_id":model,"model_CN":model_temp}
            else:
                return {
                    "productId": model,
...
if __name__ == '__main__':
    ...
    #     , charset='utf8'  # 字符编码
    # )
    # sql_word = f"select id,model,brand,ram,storage,color,purchase_channel,network,quality_desc,stick_code,grade,warranty_period,patch from ware_price where patch = '{today_patch}'"
    # # sql_word = f"-- select id,model,brand,ram,storage,color,purchase_channel,network,quality_desc,stick_code,grade,warranty_period,patch from ware_price"
    ...
    # data_li = []
    # lacked_model_li = []
    # for i in range(df_chx_ware_price.shape[0]):
    #     data = {
    ...
    # df_lacked_model = df_lacked_model.drop_duplicates().dropna(how="any")
    # df_lacked_model.to_excel("./2024年7月30日lacked_models.xlsx",index=False)
    # # pool = Pool(1)
    # # pool.map(do,data_li)
    # # pool.close()
    # # pool.join()
