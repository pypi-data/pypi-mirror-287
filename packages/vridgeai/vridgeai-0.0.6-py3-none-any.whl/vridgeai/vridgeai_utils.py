...
from test import DATSET_UPLOAD_BODY_FOR_TEST
...
def upload_dataset(api_key):
    ...
    try:
        # 테스트를 위해서 모든 데이터들이 들어있는 DATSET_UPLOAD_BODY_FOR_TEST 변수 사용 
        response = response = requests.post(url, params=params, headers=headers, data=json.dumps(DATSET_UPLOAD_BODY_FOR_TEST)).json()
