...
from src.audio_base import AudioBase
from src.utils.audio_utils import resample, get_energy_level, write_frames_to_file, calculate_audio_duration
from src.utils.auga import normalize_rms
from src.utils.error_utils import RecordingError, RecognizingError
from src.utils.input_utils import get_bucket_name, get_name, get_mode
from src.utils.logger_utils import get_logger


class JabraAudioBase(AudioBase):
    """The Jabra Audio Base analyzes the audio recorded from the built-in or usb-wired audio recorder. The
    base communicates with services running on audio server, and it communicates with the database server to send the
    recognition results.
    ...
    def __init__(self, root_dir: str = None, config_path: str = None, mode: str = 'full', local: bool = True,
        ...
        """
        super().__init__(root_dir=root_dir, config_path=config_path, mode=mode, local=local, vad=vad, nr=nr, tr=tr,
                         sp=sp, store=store)
    ...
    def _continuous_recognizing(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                write_frames_to_file(segment_audio_path, frames)
    ...
    def _assemble_chunk_without_hsr(self, final_speakers, record_start_time, final_signals):
        ...
        last speaker.
        :param final_speakers: a list of finalized recognized speakers
        :param record_start_time: segment start time
        :param final_signals: a list of finalized audio signals
        :return:
        """
            ...
            for speaker in self.last_speaker:
                if speaker in final_speakers:
                    ...
                    if self.store:
                        ...
                        write_frames_to_file(chunk_audio_path, chunk_frames, framerate=8000)
                        if speaker != 'silent':
    ...
    def _assemble_chunk_with_hsr(self, speaker, record_start_time, frames):
        """
        Assemble the chunk of audio frames if the current recognized speaker is the same as the last recognized speaker,
        if not, do speaker recognition on half-segment before and after the border of the speaker turn. Update the
        speaker audio dictionary and last speaker.

        :param speaker: recognized speaker of the current segment
        ...
        if not self.last_speaker:
            ...
            if self.last_speaker == speaker:
                ...
                if chunk_frames:
                    ...
                    write_frames_to_file(left_temp_path, chunk_frames[-number_frames:], framerate=fr)
                    write_frames_to_file(right_temp_path, frames[:number_frames], framerate=fr)
                ...
                if chunk_frames:
                    ...
                    if self.store:
                        ...
                        write_frames_to_file(chunk_audio_path, chunk_frames, framerate=fr)
