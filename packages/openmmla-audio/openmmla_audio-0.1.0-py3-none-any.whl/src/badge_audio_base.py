...
from src.utils.audio_utils import get_energy_level, read_byte_from_wav, write_frames_to_file, calculate_audio_duration, \
    resample
from src.utils.auga import normalize_rms, apply_gain
from src.utils.error_utils import RecordingError, RecognizingError
from src.utils.input_utils import get_mode, get_bucket_name, get_name
from src.utils.logger_utils import get_logger
from src.utils.socket_utils import read_frames_tcp, clear_socket_udp, read_frames_udp
...
class BadgeAudioBase(AudioBase):
    ...
    def _continuous_recording(self, sock):
        ...
        :param sock: the socket object for receiving audio stream
        """
            ...
            while not self.stop_event.is_set():
                try:
                    ...
                    output_path = os.path.join(self.audio_dir, sub_dir,
                                               f'badge_{self.id}_record_{base_start_time:.4f}.wav')
    ...
    def _continuous_recognizing_sp(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                resampled_segment_bytes = read_byte_from_wav(segment_audio_path)
    ...
    def _assemble_chunk_with_hsr(self, speaker, record_start_time, origin_frames,
                                 separate_frames=None):
        """
        ...
        :param origin_frames: audio frames of the current segment
        :param separate_frames: speech separated frames of the current segment, default to None
        """
