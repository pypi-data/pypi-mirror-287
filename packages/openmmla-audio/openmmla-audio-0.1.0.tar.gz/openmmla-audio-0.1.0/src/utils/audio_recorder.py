...
from src.utils.auga import apply_gain
from src.utils.logger_utils import get_logger
from src.utils.request_utils import request_speech_enhancement, request_voice_activity_detection
from src.utils.socket_utils import clear_socket_udp, read_frames_udp, read_frames_tcp
        ...
        if self.vad_enable:
            if self.vad_local:
                ...
                if not speech_timestamps:
                    return None
                if inplace:
                    self.save_audio(input_path, self.collect_chunks(speech_timestamps, wav),
            ...
            try:
                ...
                try:
                    self.vad_model, utils = torch.hub.load(repo_or_dir='snakers4/silero-vad', model='silero_vad',
                                                           force_reload=True, onnx=self.use_onnx)
                except Exception as e:
                    self.logger.warning(f'Error {e} happens with silero-vad, use local cached instead.')
                    self.vad_model, utils = torch.hub.load(repo_or_dir=os.path.join(self.root_dir, 'silero-vad'),
                                                           model='silero_vad', source='local', force_reload=True,
                                                           onnx=self.use_onnx)
