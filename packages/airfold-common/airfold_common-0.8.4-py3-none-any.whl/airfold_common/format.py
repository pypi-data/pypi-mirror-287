...
class ChFormat(Format):
    ...
    def normalize_using(self, data: dict) -> dict:
        ...
        if not self.SUPPORTED_NAME_RE.fullmatch(using["database"]):
            raise FormatError(f"Imported table database is invalid: '{using['database']}'")

        if not self.SUPPORTED_NAME_RE.fullmatch(using["table"]):
            raise FormatError(f"Imported table name is invalid: '{using['table']}'")
