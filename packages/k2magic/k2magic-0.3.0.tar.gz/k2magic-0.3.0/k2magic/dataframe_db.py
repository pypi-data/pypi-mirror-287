...
import pandas as pd
from sqlalchemy import create_engine, Table, MetaData, text, Column, Integer, Float, String, select, make_url
from sqlalchemy.dialects import registry
from sqlalchemy.exc import SQLAlchemyError
from sqlalchemy.orm import sessionmaker

from k2magic.dataframe_db_exception import DataFrameDBException
from k2magic.dialect.k2a_repo_discloser import disclose
...
class DataFrameDB:
    ...
    def __init__(self, db_url, schema=None):
        ...
        """
        registry.register("k2assets", "k2magic.dialect.calcite_dialect", "CalciteDialect")
        # registry.register("repo", "k2magic.dialect.repo_dialect", "RepoDialect")

        db_url_obj = make_url(db_url)
        if db_url_obj.drivername == 'repo':
            db_url_obj = disclose(db_url_obj)

        self.schema = schema
        self.engine = create_engine(db_url_obj)
