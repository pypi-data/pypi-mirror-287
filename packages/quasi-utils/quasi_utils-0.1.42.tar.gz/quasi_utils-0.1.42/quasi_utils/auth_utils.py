...
def request_verification_flow(headers, body=None):
    if body is not None:
        body = json.loads(body)
        
        if body['marker'] != 1421185:
            return {'statusCode': 406, 'body': json.dumps({'message': 'Invalid marker'}),
                    'headers': {ACAO: '*', ACAC: True}}
        else:
            return 'dummy_token', 'dummy_origin'
