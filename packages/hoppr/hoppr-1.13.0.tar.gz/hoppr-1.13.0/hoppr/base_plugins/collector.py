...
from hoppr import Hash, __version__, net, plugin_utils
...
class BaseCollectorPlugin(HopprPlugin):
    ...
    def _get_artifact_hashes(path: Path) -> Iterator[Hash]:
        yield from (
            Hash(alg=cdx_alg, content=net.get_file_hash(path, hashlib_alg))
            for cdx_alg, hashlib_alg in net.HASH_ALG_MAP.items()
        )
