...
token = user_external.get_auth_details_by_system_id_and_profile_id(
            profile_id=user.get_effective_profile_id(), system_id=SYSTEM_ID)
external_id = token[0]
username1 = scraper1.get_user_by_external_id(external_id)
password = scraper1.get_password_clear_text_by_system_id_and_profile_id(username=username1)
scraper = FacebookScraper(username1, password, is_test_data=True)
def main(self):
    
    scraper._move_to_url(url=TEST_URL)
    scraper.get_num_friends()
    scraper.click_friend_by_index(1)
    scraper.click_about_profile(url=TEST_URL)
    scraper.click_about_basic_info_friend(url=TEST_URL)
    scraper.get_num_friends()
    scraper.scrape_friends()
    scraper._move_to_url(url=TEST_URL)
    scraper.get_intro()
    scraper.click_about_profile()
    scraper.get_about_details()
    scraper.insert_to_database()
