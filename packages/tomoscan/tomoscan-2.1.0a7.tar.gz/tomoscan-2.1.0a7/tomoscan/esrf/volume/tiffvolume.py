...
class TIFFVolume(VolumeSingleFrameBase):
    ...
    def _get_slices_from_disk(
        ...
        for slice_ in slices:
            if slice_.index >= volume_shape[slice_.axis]:
                raise IndexError(
        ...
        def read_n_lines_in_file(file_name, line_indices) -> tuple:
            return tuple(
                [
        ...
        def read_n_columns_in_file(file_name, column_indices):
            return tuple(
                [
        ...
        for i_axis_0, file_name in enumerate(self.browse_data_files()):
            ...
            if i_axis_0 in indices_to_read_per_axis[0]:
                data_cache = tifffile.imread(file_name)
