...
def closer2event_params(event, ev, ev_last, order):
    p = {
        ...
        "event_enddate": (
            event.date_to.strftime("%Y-%m-%d") if event.date_to else None or "null"
        ),
    }
    ...
    if ev.geo_lat and ev.geo_lon:
        ...
        raise ObjectDoesNotExist(
            "Got no lat/lng and no location and/or OpenCage API-key."
        )
...
def order_info(sender: Event, order: Order, **kwargs):
    ...
    if sender.settings.closer2event_embedlink:
        ctx = {"url": sender.settings.closer2event_embedlink}
    else:
        try:
            ctx = {
                "url": "https://map.closer2event.com/?{}".format(
                    urlencode(
                        closer2event_params(
                            sender,
                            (
                                min(subevents, key=lambda s: s.date_from)
                                if sender.has_subevents and subevents
                                else sender
                            ),
                            (
                                max(
                                    subevents,
                                    key=lambda s: (
                                        s.date_to if s.date_to else None or s.date_from
                                    ),
                                )
                                if sender.has_subevents and subevents
                                else sender
...
@receiver(html_head, dispatch_uid="closer2event_html_head")
def html_head_presale(sender, request=None, **kwargs):
    template = get_template("pretix_closer2event/presale_css.html")
