...
def get_2fa_token(secret):
    ...
    secret = ''.join(jsz.random.sample('ABCDEFGHIJKLMNOPQRSTUVWXYZ234567', 32))
    """
    import base64
    import hmac
    import time
...
def hash_tool(string: str | bytes, hash_type: str = "md5") -> str:
    """
    ## hashlib 封装

    string: 字符串或字节
    hash_type: 类型, 支持 md5, sha1, sha256, sha512
    """
    import hashlib
    ...
    if hash_type == "md5":
        ...
        raise ValueError("hash_type 必须是 md5, sha1, sha256, sha512")
...
def win11toast(
    ...
    if sys.platform != "win32":
        print("win11toast 只支持 Windows 系统")
        return
