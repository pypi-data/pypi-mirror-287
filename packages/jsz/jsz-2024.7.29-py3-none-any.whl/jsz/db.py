...
def oss2_upload_file(bucket, filename, filepath, proxies=None):
    """
    上传文件到 oss
    """
    import oss2
    ...
    try:
        z = bucket.put_object_from_file(filename, filepath)
...
def oss2_upload_content(bucket, filename, content, proxies=None):
    """
    上传 content 到 oss
    ...
    if proxies:
        bucket.proxies = proxies
    try:
        z = bucket.put_object(filename, content)
        print(f"文件 {filename} 上传成功")
        return z
    except Exception as e:
