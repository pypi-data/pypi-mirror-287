...
def obtener_capa_activa(index):
    ...
    logger.info(f"Trabajando en la capa: {nombre_capa}")
    return capa_activa
...
def validate_attributes(layer):
    ...
    if (where_clause == "1=0"):
        logger.info("No hay entidades seleccionadas.")
        return
    ...
    try:
        ...
        if mensajes:
            ...
            logger.info("Todos los atributos se encuentran debidamente llenos.")
        ...
        if decision == 'si':
            ...
            logger.info("Los cambios han sido confirmados.")
        else:
            edit.stopOperation()
            edit.stopEditing(False)
            logger.info("Los cambios han sido abortados.")
    except Exception as e:
        edit.stopOperation()
        edit.stopEditing(False)
        logger.error(f"Ocurrio un error: {e}. Los cambios han sido abortados.")
    finally:
        if edit.isEditing:
            edit.stopEditing(False)
