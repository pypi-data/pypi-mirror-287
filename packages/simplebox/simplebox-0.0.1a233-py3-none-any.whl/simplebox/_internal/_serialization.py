...
class _SerializeField(Generic[T]):
    ...
    def __init__(self, value: T, *, name=None, autoname: bool = False, camel: bool = False,
                 hooks: list[Callable[[T], tuple[bool, Any]]] = None):
        ...
        self.__hooks: list[Callable[[T], tuple[bool, Any]]] = hooks or []
        self.__value: T = self.__hook_handler(value)

    @final
    def __hook_handler(self, value) -> T:
...
class _Serializable(Generic[T]):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        instance.__prefix__ = prefix
        return instance

    @final
    def __getattribute__(self, item):
        result = super().__getattribute__(item)
        if isinstance(result, _SerializeField):
            return result.value
        return result
                ...
                if not name:
                    ...
                    if value.autoname or self.autoname or SerializeConfig.autoname:
                        if key.startswith(self.__prefix__):
                            name = key.replace(self.__prefix__, "")
