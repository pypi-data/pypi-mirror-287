...
class PEP604(PEP):
    ...
    def _sort_operands(self, annotation: Expression) -> Expression:
        ...
        def func(operation: ExprBinOp) -> Expression:
            ...
            operands = sorted(operands, key=lambda operand: self._priorities.get(canonical_path_of(operand), 2))

            chained = self._chain_operands("|", operands)
            transformed.update(id(operation) for operation in chain(chained, "left"))
            return chained
