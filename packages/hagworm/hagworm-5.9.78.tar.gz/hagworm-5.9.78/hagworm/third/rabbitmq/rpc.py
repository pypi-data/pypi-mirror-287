...
class RpcServer:
    ...
    async def _reply_message(self, incoming: aio_pika.IncomingMessage, func_name: str, data: typing.Any):

        await self._producer.publish(
            aio_pika.Message(
                body=Utils.msgpack_encode(
                    {
                        r'name': func_name,
                        r'data': data,
                    }
    ...
    async def _reply_error(self, incoming: aio_pika.IncomingMessage, error: str):

        await self._producer.publish(
            aio_pika.Message(
                body=ERROR_HEAD + error.encode(),
                correlation_id=incoming.correlation_id
            ),
            routing_key=incoming.reply_to
        )
        ...
        with catch_error():
            ...
            try:
                ...
                if _func_type == FunctionType.Unary.value and inspect.iscoroutinefunction(_function):
                    await self._reply_message(message, _func_name, await _function(*_func_args, **_func_kwargs))
                    ...
                    async for _item in _function(*_func_args, **_func_kwargs):
                        await self._reply_message(message, _func_name, _item)
                    else:
                ...
                await self._reply_error(message, str(err))
                raise err
...
class RpcClient:
    ...
    def __init__(
        ...
        self._unary_response: typing.Dict[str, asyncio.Future] = {}
        self._stream_response: typing.Dict[str, asyncio.Queue[asyncio.Future]] = {}
    ...
    async def _consume_message(self, message: aio_pika.IncomingMessage):
        ...
        with catch_error():
            ...
            if correlation_id in self._unary_response:

                future = self._unary_response[correlation_id]
            ...
            elif correlation_id in self._stream_response:
                ...
                self._stream_response[correlation_id].put_nowait(future)
    ...
    async def call(
        ...
        future = self._unary_response[correlation_id] = asyncio.Future()
        ...
        try:
            ...
            async with async_timeout.timeout(request_timeout):
                ...
                return (await future)[r'data']

        except asyncio.TimeoutError as err:
            Utils.log.error(f'rpc call timeout: {func_name=} {func_args=} {func_kwargs=}')
            raise err

        finally:
            del self._unary_response[correlation_id]
    ...
    async def call_stream(
        ...
        queue = self._stream_response[correlation_id] = asyncio.Queue()
        ...
        try:
            ...
            del self._stream_response[correlation_id]
