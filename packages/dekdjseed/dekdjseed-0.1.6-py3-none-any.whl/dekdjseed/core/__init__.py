...
def create_project_entities(seed=None):
    ...
    for app_config in apps.get_app_configs():
        done = create_app_entities(app_config, guesser)
        if done:
...
def create_app_entities(app_config, guesser):
    loader = SeedLoader.from_module(f'{app_config.name}.seed', guesser)
    for model in sorted_models(app_config):
        create_model_entities(model, loader)
    return loader.seed_funcs.get('done')
        ...
        while True:
            ...
            for field in model_cls._meta.fields:
                ...
                if isinstance(field, (models.OneToOneField, models.ForeignKey)):
                    ...
                    if array:
                        faker_data[field.name] = array if isinstance(array, models.Model) \
                            else loader.guesser.random_sample(array, 1)[0]
                    continue
