...
def get_author_ids(paper_url: str) -> List[str]:
    '''
    Find the Google Scholar IDs of authors from a Google Scholar paper url.
    '''
    ...
    # Loop through the citation results
...
def scrape_citations(citation_url, headers):
    response = requests.get(citation_url, headers=headers)
    if response.status_code != 200:
        raise Exception('Failed to fetch the citations page')
