...
class OverallDataManager(MolDataManager):
    ...
    def save_graphs(self,col):
        ...
        save_graphs(os.path.join(self.config['GRAPH_PATH'],self.data+"_"+col+"_"+self.graph_type+".bin"), self.molecule_graphs[col],{'overall':overall_features,'smiles':torch.tensor([hash(smi) for smi in self.molecule_smiles[col]])})


    def load_graphs(self,col):
        self.molecule_graphs[col],_= load_graphs(os.path.join(self.config['GRAPH_PATH'],self.data+"_"+col+"_"+self.graph_type+".bin"))
