...
def init_path(config):
    ...
    init_graph_path(config)
    init_network_path(config)
    init_frag_ref_path(config)
    init_net_path(config)
...
def init_graph_path(config):
    if 'grpha_path' in config:
        GRAPH_PATH = config['graph_path']
    elif os.path.exists(work_dir+"/_Graphs"):
        GRAPH_PATH = work_dir+"/_Graphs"
    else:
...
def init_network_path(config):
    if 'network_refer_path' in config:
        NET_REFER = config['network_refer_path']
...
def init_frag_ref_path(config):
    if 'frag_ref_path' in config:
        FRAG_REF = config['frag_ref_path']
...
def init_net_path(config):
    if 'network_path' in config:
        NET_PATH = config['network_path']
    elif os.path.exists(work_dir+"/networks"):
        NET_PATH = work_dir+"/networks"
...
def check_path(config):
    ...
    if not os.path.exists(config['NET_PATH']):
