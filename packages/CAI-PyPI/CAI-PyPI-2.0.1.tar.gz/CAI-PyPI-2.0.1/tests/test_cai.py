...
def test_cai():
    ...
    assert (
        CAI("AAC", reference=["AAC"])
        == CAI("AAC", RSCUs=RSCU(["AAC"]))
        == CAI("AAC", weights=relative_adaptiveness(sequences=["AAC"]))
        == 1.0
    )
...
def test_stop_codon():
    ...
    assert (
        CAI("AAT", reference=["AAC"])
        == CAI("AATTAA", reference=["AAC"])
        == CAI("TAAAAT", reference=["AAC"])
    )


def test_alternate_genetic_code():
    # the differing stop codon
    assert CAI("AATTGA", reference=["AAC"]) != CAI(
        "AATTGA", reference=["AAC"], genetic_code=10
    )
