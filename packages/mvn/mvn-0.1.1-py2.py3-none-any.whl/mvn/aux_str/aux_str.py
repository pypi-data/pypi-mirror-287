...
def is_ascii(s: str) -> bool:
    ...
    >>> from mvn.aux_str import is_ascii
...
def is_ascii_alt(s: str) -> bool:
    ...
    >>> from mvn.aux_str import is_ascii_alt
...
def clean_str(
    ...
    mappings : List[Dict[str, List[Union[str, Pattern[str]]]]], default=[{' ': ['\\n', '\\r', '\\t']}, {'-': ['−', '–', '—', '―', '﹣', '－']}]
        ...
        in dictionary key value. The default value is sourced from ``mvn.aux_str.clean_str_mappings.CLEAN_STR_MAPPINGS_TINY``.
    ...
    >>> from mvn.aux_str import clean_str
    ...
    :python:`mvn.aux_str` also provides few usefull mappings:

    >>> # Import example mappings:
    >>> from mvn.aux_str import CLEAN_STR_MAPPINGS_TINY
    >>> from mvn.aux_str import CLEAN_STR_MAPPINGS_LARGE
    >>> from mvn.aux_str import CLEAN_STR_MAPPINGS_HUGE
    >>> from mvn.aux_str import CLEAN_STR_MAPPINGS_SPACE
    >>> from mvn.aux_str import CLEAN_STR_MAPPINGS_DROP_HASHTAGS
    ...
    >>> from mvn.aux_str import CLEAN_STR_MAPPINGS_DROP_HASHTAGS as map0
    >>> from mvn.aux_str import clean_str
