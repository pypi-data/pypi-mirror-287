...
class SecurityHub(o7lib.aws.base.Base):
    ...
    def load_standards(self):
        ...
        self.df_standards['Short'] = self.df_standards['Short'].replace('NIST 5', 'NIST 800-53')
    ...
    def load_findings(self, standard_arn : str = None):
        ...
        df['WorkflowNote'] = df['Note'].apply(lambda x: x.get('Text', '') if isinstance(x,dict) else '')
        ...
        if df_results is not None:
            ...
            params = TableParam(
                ...
                columns = [
                    ...
                    ColumnParam(title = 'Crit',     type = 'int',  data_col = 'Critical', group = 'Controls', footer = 'sum', critical_hi=1),
                    ColumnParam(title = 'High',     type = 'int',  data_col = 'High', group = 'Controls', footer = 'sum', alarm_hi=1),
                    ColumnParam(title = 'Med',     type = 'int',  data_col = 'Medium', group = 'Controls', footer = 'sum', warning_hi=1),
                    ...
                    ColumnParam(title = 'Crit',     type = 'int',  data_col = 'FindingsCritical', group = 'Findings', footer = 'sum', critical_hi=1),
                    ColumnParam(title = 'High',     type = 'int',  data_col = 'FindingsHigh', group = 'Findings', footer = 'sum', alarm_hi=1),
                    ColumnParam(title = 'Med',     type = 'int',  data_col = 'FindingsMedium', group = 'Findings', footer = 'sum', warning_hi=1),
                    ColumnParam(title = 'Low',     type = 'int',  data_col = 'FindingsLow', group = 'Findings', footer = 'sum'),
                ]
    ...
    def display_per_account(self):
        ...
        params = TableParam(
            ...
            columns = [
                ...
                ColumnParam(title = 'Finding'   ,  type = 'int',  data_col = 'Other', footer = 'sum', group = "Other"),
    ...
    def compile_findings_per_accounts(self) -> pd.DataFrame:
        ...
        df = self.df_findings[['AwsAccountId', 'AccountName','ProductName', 'SeverityN', 'SeverityL']].copy()
        ...
        df['IsHigh']     = df['IsSecurityHub'] & (df['SeverityN'] >= 70) & (df['SeverityN'] < 90)
        df['IsMedium']   = df['IsSecurityHub'] & (df['SeverityN'] >= 40) & (df['SeverityN'] < 70)
        df['IsLow']      = df['IsSecurityHub'] & (df['SeverityN'] < 40) & (df['SeverityN'] > 0)

        df['IsGuardDutyHigh']  = df['IsGuardDuty'] & (df['SeverityL'] == 'HIGH') & df['IsFailed']
    ...
    def generate_html_report(self):
        ...
        print('')
        report.add_section(title='Findings Summary per Account', html=findigs_per_account_html)
        ...
        section_params = TableParam(
            columns = [
                ...
                ColumnParam(title = 'Note',   type = 'str',  data_col = 'WorkflowNote', max_width = 15),
                ColumnParam(title = 'ResType',   type = 'str',  data_col = 'ResType', max_width = 15),
                ColumnParam(title = 'ResName',   type = 'str',  data_col = 'ResName', max_width = 15),
                ColumnParam(title = 'Title',     type = 'str',  data_col = 'Title'),
            ]
        ...
        findings.loc[findings['Started'].isna(), 'Started'] = findings['CreatedAt'].str.slice(0,10)
        ...
        for section in sections:
            ...
            if count < 1 :
                continue
            section_html += Table(section_params, section_findings[0:section_max].to_dict(orient='records')).generate_html()
            Table(section_params, section_findings[0:10].to_dict(orient='records')).print()
            print('')
        ...
        if count > 0 :
            ...
            section_html = f"Current Count: {count}<br>"
            section_html += Table(section_params, other_findings[0:10].to_dict(orient='records')).generate_html()
    ...
    def write_to_dynamodb(self, table_name : str) :
        ...
        for account in stats_per_account.to_dict(orient='records') :
            ...
            for index, row in self.df_standards[self.df_standards['StandardsStatus'] == 'READY'].iterrows():
                std_short = row['Short']
