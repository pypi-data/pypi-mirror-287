...
_Orm = orm.Resource | orm.Source | orm.FeatureView | orm.Space | orm.FeatureViewSource
_ID = (
    orm.ResourceID
    | orm.SourceID
    | orm.FeatureViewID
    ...
    match orm_obj:
        ...
        case orm.Source():
            return models_pb2.Source(
                ...
                name=orm_obj.name,
                table_op_graph=op_graph.op.from_bytes(
                    orm_obj.table_op_graph
                ).to_proto(),
                room_id=str(orm.RoomID.from_orm(orm_obj.room_id)),
                resource_id=str(
...
def _source_proto_to_orm(
    ...
    return Ok(
        orm.Source(
            id=ids.obj_id,
            name=proto_obj.name,
            table_op_graph=proto_obj.table_op_graph.SerializeToString(),
            room_id=ids.room_id,
            resource_associations=[
                orm.SourceResourceAssociation(
                    source_id=ids.obj_id, resource_id=resource_id
                )
            ],
        )
    )
