...
class DuckDBStaging(StagingDB):
    ...
    def query_for_vector_search(
        ...
        if num_results < 1:
            raise BadArgumentError("'num_results' parameter must be greater than 0")
        ...
        return (
            ...
            .from_(vector_union_table)
            .order_by(similarity_func)
            .limit(num_results)
            .with_(vector_union_table, as_=union)
        )
    ...
    def run_select_query(
        ...
        if not order_exp:
            query.set(
                "order", sqlglot.parse_one("ORDER BY ALL", into=sqlglot.exp.Order)
            )
        else:
