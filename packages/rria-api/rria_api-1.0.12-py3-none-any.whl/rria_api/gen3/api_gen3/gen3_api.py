...
class Gen3Api:
    ...
    def gripper_close_percentage(
            base,
            finger_value: float,
            actuation_time: float,
    ) -> None:
        """Controls the gripper oppening by a value between
        0 and 1.

        Args:
            base (`BaseClient`)
            finger_value (float): floating value between 0 and 1,
            where lower values represent a gripper openned and
            higher values a closer gripper.
            actuation_time (float): time in seconds to wait after
            the command is sent to the Kinova API.
        """
        if finger_value > 1:
            finger_value = 1.0
        if finger_value < 0:
