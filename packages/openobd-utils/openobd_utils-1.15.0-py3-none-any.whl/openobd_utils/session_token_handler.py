...
class SessionTokenHandler:
    ...
    def _authenticate(self):
        print(f" [+] Authenticating the openOBD session with id [{self.openobd_session.id()}]")
        ...
        print(f" [+] Start refreshing session tokens for session [{self.openobd_session.id()}]")
    ...
    def _refresh_session_token(self):
        ...
        print(f" [+] Stopped refreshing session tokens for session [{self.openobd_session.id()}]")
