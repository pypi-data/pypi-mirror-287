from openobd import *
from .stream_handler import StreamHandler, requires_active_stream
...
class UiHandler:
    ...
    def __init__(self, openobd_session: OpenOBDSession, target: InterfaceType = InterfaceType.INTERFACE_USER):
        ...
        """
        self.stream_handler = StreamHandler(openobd_session.open_control_stream, outgoing_stream=True)
        self._target = target
    ...
    def show_ui(self, control: _control_types) -> None | int | bool | str:
        ...
        self.stream_handler.send(Control(**kwargs), flush_incoming_messages=True)
        while True:
    ...
    def stop_stream(self) -> None:
        """
        Closes the gRPC stream if it is not already closed. A new UiHandler object will have to be created to start
        another stream.
        """
