...
from .stream_exceptions import OpenOBDStreamException, OpenOBDStreamStoppedException, OpenOBDStreamTimeoutException
...
def requires_active_stream(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if stream_handler.stream_finished:
...
class StreamHandler:
    ...
    def __init__(self, stream_function: Callable[[Any], Any], request: Any = None, **kwargs):
        ...
        self._stream_finished = ThreadSafeVariable(False)
    ...
    def stream_finished(self):
        return self._stream_finished.value

    @stream_finished.setter
    def stream_finished(self, value):
        self._stream_finished.value = value
    ...
    def receive(self, block: bool = True, timeout: float | None = None) -> Any:
        ...
        try:
            ...
            if block:
                raise OpenOBDStreamTimeoutException
            else:
                return None
    ...
    def stop_stream(self, send_remaining_messages: bool = True) -> None:
        ...
        """
        # Indicate the stream is finished to prevent attempts at using the stream immediately after this call
        ...
        if self._response_iterator is not None:
            ...
            if hasattr(self._response_iterator, "cancel"):   # Method defined by gRPC. Should always be present (sanity check).
                ...
                raise OpenOBDStreamException("Unable to stop the stream, as the stream object does not have a `cancel` method.")
