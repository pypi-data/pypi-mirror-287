...
from robotnikmq import RobotnikConfig, Subscriber, Message, RpcClient
...
@click.group()
def rpc():
    """A set of utilities for working with Legion RPC commands"""


@rpc.command()
@click.argument('ip_address', type=str)
def princeps_registry_pingable_from(ip_address: str):
    print(pformat(RpcClient().call('princeps_registry.pingable_from', {'ip_addr': ip_address})))


CLI = click.CommandCollection(sources=[cli, rpc])
