...
def preprocess(img):
    ...
    p2 = NormalizeImage(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    p3 = PrepareForNet()
    img = p1({"image": img})["image"]
    img = p2({"image": img})["image"]
    img = p3({"image": img})["image"]
    return img
...
def da_onnx_infer(img: str, model: str, viz: bool = True):
