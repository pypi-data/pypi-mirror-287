...
Rules:s
1- srt files must be in the following format (and only has one line of subtitles/lyrics.):
1
00:00:01,918 --> 00:00:03,378
Hello, How are you?

2- lrc files must be in the following format:
...
def srt_to_lrc(srt_file, lrc_file):
    '''
    This function converts srt files to lrc based on regular expressions.
    srt_file and lrc_file should be the paths to the files.
    ...
    Therefore, all the timings will be the off by 0.001 to 0.005 seconds. (Thanks to round_ms(ms) function. Normally it'd be 0.001 to 0.009)
    This timing inaccurance would not be significant nor cause problems in captions/subtitles.
    '''
...
def lrc_to_srt(lrc_file, srt_file):
    '''
    This function converts lrc files to srt based on regular expressions.
    lrc_file and srt_fle should be the paths to the files.
    input file/path is lrc_file. output path/file is srt_file.
    lrc files use 2 digits for milliseconds while srt files use 3 digits. Therefore, all the timings will be the off by 0.001 to 0.009 seconds.
    This timing inaccurance would not be significant nor cause problems in captions/subtitles.
