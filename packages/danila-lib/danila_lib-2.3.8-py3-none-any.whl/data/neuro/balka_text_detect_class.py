...
class Balka_text_detect_class:
    ...
    def __init__(self, model_path, yolo_path, balka_prod):
        ...
        zip_path = 'balka_' + balka_prod.text + '_text_detect.zip'
        ...
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
            zip_ref.extractall()
        weights_file_path = 'balka_' + balka_prod.text + '_text_detect.pt'
        ...
        for image_text_areas in img_text_areas_in_two_balkas:
            for class_im in image_text_areas.areas:
                for rect in image_text_areas.areas[class_im]:
                    cv2.rectangle(imgs[0], (rect.xmin, rect.ymin), (rect.xmax, rect.ymax), color=colors[class_im.value], thickness=2)
                    cv2.putText(imgs[0], class_im.name, (rect.xmin, rect.ymin), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color=colors[class_im.value], thickness=1)
        return imgs
    ...
    def work_text_areas_for_altai(self, left_half_balka, right_half_balka):
        if len(left_half_balka.areas['year']) > 0:
            left_half_balka.areas['year'] = []
        if len(right_half_balka.areas['prod']) > 0:
            right_half_balka.areas['prod'] = []
        return [left_half_balka, right_half_balka]

    def work_text_areas_for_begickaya(self, left_half_balka, right_half_balka):
        if len(right_half_balka.areas['year']) > 0:
            right_half_balka.areas['year'] = []
        if len(right_half_balka.areas['prod']) > 0:
            right_half_balka.areas['prod'] = []
        return [left_half_balka, right_half_balka]
    ...
    def work_text_areas_for_promlit(self, left_half_balka, right_half_balka):
        if len(right_half_balka.areas['year']) > 0:
            right_half_balka.areas['year'] = []
        if len(left_half_balka.areas['prod']) > 0:
            left_half_balka.areas['prod'] = []
        return [left_half_balka, right_half_balka]
