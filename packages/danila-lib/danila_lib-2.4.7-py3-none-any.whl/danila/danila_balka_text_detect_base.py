...
class Danila_balka_text_detect_base:
    def __init__(self, yolov5_dir, balka_detect_version, balka_text_detect_version):
        if (balka_detect_version == 1) & (balka_text_detect_version == 1):
            ...
            self.balka_detect_objects[balka_prod.Balka_Prod.altai] = Balka_detect_class(BALKA_ALTAI_DETECT_MODEL_ADDRESS, yolov5_dir)

            print('reading and loading - BALKA_BEGICKAYA_DETECT_MODEL')
            self.balka_detect_objects[balka_prod.Balka_Prod.begickaya] = Balka_detect_class(BALKA_BEGICKAYA_DETECT_MODEL_ADDRESS, yolov5_dir)

            print('reading and loading - BALKA_PROMLIT_DETECT_MODEL')
            self.balka_detect_objects[balka_prod.Balka_Prod.promlit] = Balka_detect_class(BALKA_PROMLIT_DETECT_MODEL_ADDRESS, yolov5_dir)

            print('reading and loading - BALKA_RUZHIMMASH_DETECT_MODEL')
            self.balka_detect_objects[balka_prod.Balka_Prod.ruzhimmash] = Balka_detect_class(BALKA_RUZHIMMASH_DETECT_MODEL_ADDRESS, yolov5_dir)

            print('reading and loading - BALKA_TIHVIN_DETECT_MODEL')
            self.balka_detect_objects[balka_prod.Balka_Prod.tihvin] = Balka_detect_class(BALKA_TIHVIN_DETECT_MODEL_ADDRESS,
                                                                                      yolov5_dir)
            ...
            self.balka_text_detect_objects[balka_prod.Balka_Prod.altai] = Balka_text_detect_class(
                BALKA_ALTAI_TEXT_DETECT_MODEL_ADDRESS, yolov5_dir, balka_prod.Balka_Prod.altai)
            ...
            self.balka_text_detect_objects[balka_prod.Balka_Prod.begickaya] = Balka_text_detect_class(
                BALKA_BEGICKAYA_TEXT_DETECT_MODEL_ADDRESS, yolov5_dir, balka_prod.Balka_Prod.begickaya)
            ...
            self.balka_text_detect_objects[balka_prod.Balka_Prod.promlit] = Balka_text_detect_class(
                BALKA_PROMLIT_TEXT_DETECT_MODEL_ADDRESS, yolov5_dir, balka_prod.Balka_Prod.promlit)
            ...
            self.balka_text_detect_objects[balka_prod.Balka_Prod.ruzhimmash] = Balka_text_detect_class(
                BALKA_RUZHIMMASH_TEXT_DETECT_MODEL_ADDRESS, yolov5_dir, balka_prod.Balka_Prod.ruzhimmash)
            ...
            self.balka_text_detect_objects[balka_prod.Balka_Prod.tihvin] = Balka_text_detect_class(
                BALKA_TIHVIN_TEXT_DETECT_MODEL_ADDRESS,
    ...
    def balka_text_detect(self, img, balka_prod):
        ...
        if len(images_drawn_text_areas) == 1:
            ...
            dsize = (w0 + w1, max(h0,h1), 3)
            new_img = cv2.resize(images_drawn_text_areas[0],dsize)
            left_image = cv2.resize(images_drawn_text_areas[0], ( w0, max(h0,h1),3))
            for i in range(max(h0,h1)):
                for j in range(w0):
                    new_img[i,j] = left_image[i,j]
                for k in range(w0,w0+w1):
                    new_img[i,k] = images_drawn_text_areas[1][i,k - w0]
        os.remove(initial_image_path)
        return new_img
