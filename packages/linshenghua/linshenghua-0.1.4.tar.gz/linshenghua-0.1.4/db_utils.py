...
class DBUtils(metaclass=ABCMeta):
    def __init__(self, database:str, is_ssh: bool=False, curson_type: bool=True, user_env: bool=True, **kwargs):
        """
        初始化数据库连接
...
class MysqlUtils(DBUtils):
    def __init__(self, database:str, is_ssh: bool=False, curson_type: bool=True, user_env: bool=True, **kwargs):
        super().__init__(database, is_ssh, curson_type, user_env, **kwargs)
...
if __name__ == '__main__':
    ...
    b = MysqlUtils("base", user_env=False)
    print(get_db_data(b, "select name from user"))
    print(get_db_data(b, "select name from user", is_one=True))