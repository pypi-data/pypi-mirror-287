...
<div>
	...
	<TabGroup>
		...
		<Tab bind:group={ current_tab } name="tab2" value={1}>
			<span longevity="UT_hexadecimal_string_button">Hexadecimal</span>
		</Tab>
			...
			{#if current_tab === 0}
				<div>
					...
					<div class="card variant-soft-primary p-4">
						<UT_Stringified 
							unsigned_tx_stringified={ freight.Unsigned_Transaction_Fields.Aptos_object_fiberized }
						/>
					</div>
				</div>
				...
				<div>
					...
					<p
						...
						style="
							...
							border-radius: 4px;
						"
					>{ freight.Unsigned_Transaction_Fields.hexadecimal_string }</p>
				</div>
			{/if}
		</svelte:fragment>
	</TabGroup>
