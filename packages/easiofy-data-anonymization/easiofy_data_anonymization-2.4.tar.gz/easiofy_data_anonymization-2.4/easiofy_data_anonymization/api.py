...
async def save_patient_data_to_firebase(adminName1, patientKey, rootFolderPathortho, patientName, studyType, patientID, patientBirthDate, patientSex, studyDate, referringPhysicianName, modality, technicianName, StudyInstanceUid, seriesInstanceUid, PatientDataId):
    ...
    sanitizedPatientName = extract_name(patientName)
    patientKey = f"Patient-{sanitize_firebase_key(sanitizedPatientName)}"
    ...
    # Determine gender
    genderLabel = "Male" if patientSex == "M" else "Female" if patientSex == "F" else "Unknown"

    # Prepare patient data
    patientData = {
        "Age": age or "NA",
        "Refered_By_Doctor": referringPhysicianName or "NA",
        "Dicom_3D": "",
        "Dicom_3DURL": "",
        "Dicom_Report": "",
        "Dicom_ReportURL": "",
        "StudyDate": studyDate or "",
        "DOB": formatted_patient_birth_date,
        "MetaData_Orthanc":{"ID":PatientDataId,"MainDicomTags":{"PatientID":patientID,"PatientName":patientName,"PatientBirthDate":patientBirthDate,"PatientSex":patientSex},"SeriesInstanceUID": seriesInstanceUid,
        "StudyInstanceUID": StudyInstanceUid,},
        "FolderPath": rootFolderPathortho,
        "FullName": sanitizedPatientName,
        "Gender": genderLabel or "",
        "ID": patientID or "",
        "Timestamp": datetime.now().strftime("%d/%m/%Y, %H:%M:%S"),
        "Type_of_Dicom_Data": studyType or "",
        "Prescription": "",
        "PrescriptionURL": "",
        "Role": "Patient",
        "Version": "02",
    ...
    await increment_upload_count(adminName1, modality)

    # Log upload event
    await log_upload_event(adminName1, technicianName, sanitizedPatientName, modality)
