...
class PredictorParser(InputFileParser):
    ...
    information: Union[None, PredictorInformation]

    def __init__(
        self, geom_area: RectGeom, information: Union[None, PredictorInformation] = None
    ):
    ...
    def _check_first_line(self, comments, delimiter):
        if not comments[0][0] == "#":
            if self.information is None:
                raise FileValidationError(
                    f"First line '{comments[0]}' is not a comment line."
                )
            ...
            if match is None:
                raise FileValidationError(
                    f"Comment line '{comments[0]}' is not a valid JSON string."
                )
            json_string = match.group(1)
            try:
                information_from_header = PredictorInformation(
                ...
                raise FileValidationError(
                    f"Predictor information does not match the schema.\n"
            ...
            if self.information:
                if information_from_header != self.information:
                    raise FileValidationError(
                        f"Information from header '{information_from_header}' "
    ...
    def get_file_information(self):
        ...
        return self.information.dict()

    def _check_validty_area(self):
        area_input = self.input_geom_area.get_area()
        area_file = self.parse_geom_area.get_area()
        percentage = (area_file / area_input) * 100
        if percentage < 50:
            logging.warning(
                f"Predictor file covers only {percentage:.2f}% of the input area."
            )
        else:
            logging.info(f"Predictor file covers {percentage:.2f}% of the input area.")
