...
function gererateXML(xml_type) {
  ...
  let jsObj = { 'base_xml': 'TA010_TFLXAUTH' };
  ...
  Object.assign(jsObj, { ["encrypt_write"]: xml_type !== "proto_xml"});
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (keyLoadConfig[i] == "noLoad") {
      ...
      if (certOptValue === 'custCert' && i === 1) {
        if ((document.getElementById(dev_sig_inputs[0] + "certname").value == '') ||
          ...
          if ((document.getElementById("4certname").value == '') ||
      ...
      if (certOptValue === "custCert") {
        //device
        Object.assign(jsSlot, { ['d_cert_org']: document.getElementById(i + "certname").value });
        Object.assign(jsSlot, { ['d_cert_cn']: 'sn0123030405060708EE' });
        Object.assign(jsSlot, { ['d_cert_cn']: document.getElementById(i + "certcommonname").value });
        Object.assign(jsSlot, { ['d_cert_expiry_years']: document.getElementById(i + "certyear").value });
        //signer
        Object.assign(jsSlot, { ['s_cert_org']: document.getElementById(dev_sig_inputs[1] + "certname").value });
        Object.assign(jsSlot, { ['s_cert_cn']: 'sn0123030405060708EE' });
        Object.assign(jsSlot, { ['s_cert_cn']: document.getElementById(dev_sig_inputs[1] + "certcommonname").value });
        Object.assign(jsSlot, { ['s_cert_expiry_years']: document.getElementById(dev_sig_inputs[1] + "certyear").value });
        //ca
        Object.assign(jsSlot, { ['signer_ca_org']: document.getElementById("4certname").value });
        Object.assign(jsSlot, { ['signer_ca_cn']: document.getElementById("4certcommonname").value });
  ...
  if(partNumberString == "")
      partNumberString = "TA010-MAHAA-T"
  ...
  if (useCaseValid === false && slotDataValidity === true && isManIdValid === true) {
    ...
    if (xml_type === "proto_xml" || xml_type === "prod_xml") {
        ta010_tflexauth_json(JSON.stringify(jsObj));
    }
    else {
      ta010_tflexauth_proto_prov(JSON.stringify(jsObj));
    }
  }
}
