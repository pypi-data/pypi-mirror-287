    ...
    try:
        for _ in range(num_rounds):
            ...
            try:
                ...
                rounds += 1
                print(Fore.WHITE + f"[fck] Codes and complaints successfully sent!")
                print(Fore.WHITE + f"[fck] Total rounds sent: {rounds}")
            except requests.exceptions.RequestException as e:
...
def loading_animation(duration=3):
    ...
    while time.time() < end_time:
        for symbol in symbols:
            sys.stdout.write(f"\r{Fore.YELLOW}Loading {symbol}")
            sys.stdout.flush()
            time.sleep(0.1)
    sys.stdout.write("\r")
...
╚██████╗   ██║   ██████╔╝███████╗██║  ██║    ╚██████╔╝███████║██║██║ ╚████║   ██║   
 ╚═════╝   ╚═╝   ╚═════╝ ╚══════╝╚═╝  ╚═╝     ╚═════╝ ╚══════╝╚═╝╚═╝  ╚═══╝   ╚═╝   
                        .:Code by - Cyber Stalker:.
                       ::Telegram - @CYB3R_ST4LK3R::
                       ::Subscribe me in  telegram::                                      
""", speed=0.002)
        animated_print(f"\nWelcome back, {name}!", speed=0.05)
    else:
        animated_print('Enter your real name or your nickname', speed=0.05)
        name = input(f"Enter your name: ")
        animated_print('You need to get the token according to the guide from the telegram channel - t.me/CYB3R_ST4LK3R\n search post with #TokenCO', speed=0.05)
