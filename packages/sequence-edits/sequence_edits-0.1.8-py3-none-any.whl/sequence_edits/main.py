...
V =  TypeVar('V')    
def decompress(edits: Iterable[Skip|Insert[V]], start: int = 0, end: int | None = None) -> Iterable[int|Inserted[V]]:
...
@overload
def apply(edits: Iterable[Skip|Insert], xs: list[A], start: int = 0, *, fill: V) -> Iterable[A | V]:
  """Applies `edits` to an actual list `xs[start:]`, replacing insterts by `fill`"""
  ...
@overload
def apply(edits: Iterable[Skip|Insert[V]], xs: list[A], start: int = 0) -> Iterable[A | V]:
  """Applies `edits` to an actual list `xs[start:]`"""
  ...
def apply(edits: Iterable[Skip|Insert], xs: list[A], start: int = 0, *, fill = None): # type: ignore
