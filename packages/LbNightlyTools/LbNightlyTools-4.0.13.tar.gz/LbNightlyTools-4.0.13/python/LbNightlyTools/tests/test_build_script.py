...
def test_simple_build_env_search_path():
    ...
    try:
        ...
        loc = {}
        exec(
            open(join(artifacts_dir, "confSummary.py")).read(),
            {"__file__": "/new/root/confSummary.py"},
            loc,
        )
        assert loc["cmtProjectPathList"] == expected_cmt, "expected %r, found %r" % (
            expected_cmt,
            loc["cmtProjectPathList"],
        )
        ...
        proc = Popen(
            ["cmake", "-P", join(tmpd, "new_loc", "searchPath.cmake")], stdout=PIPE
        )
        expected_cmake = [
    ...
    try:
        ...
        call(
            [
                ...
                "-q",
                "-o",
                join(os.pardir, "artifacts", "TestProject.HEAD.testing-slot.src.zip"),
            ],
            cwd="build",
        )
