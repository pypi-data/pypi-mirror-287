...
class Test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._manifestxml = normpath(
            join(*([__file__] + [os.pardir] * 4 + ["testdata", "rpm", "manifest.xml"]))
        )
        ...
        self._fullbinspecname = normpath(
            join(
                *([__file__] + [os.pardir] * 4 + ["testdata", "rpm", self._binspecname])
            )
        )
    ...
    def testSharedSpec(self):
        ...
        with ZipFile(
            os.path.join(
                artifactdir, "packs", "src", "Brunel.v46r0.lhcb-release.999.src.zip"
            ),
            "w",
            ZIP_DEFLATED,
        ):
        ...
        newlines = [
            l
            for l in open(newspecfilename, "U").readlines()
            if "%define buildarea" not in l
        ]
        oldlines = [
    ...
    def testGlimpseSpec(self):
        ...
        oldlines = [
            l
            for l in open(self._fullindexspecname, "U").readlines()
            if "%define buildarea" not in l
        ]
    ...
    def testDatapkgSpec(self):
        ...
        oldlines = [
            l
            for l in open(self._fulldatapkgspecname, "U").readlines()
            if "%define buildarea" not in l
        ]
    ...
    def testDatapkgSpecNewRelease(self):
        ...
        oldlines = [
            l
            for l in open(self._fulldatapkgspecnamenewrel, "U").readlines()
            if "%define buildarea" not in l
        ]
        ...
        d = Differ()
        print(
            "Comparing %s and %s" % (self._fulldatapkgspecnamenewrel, newspecfilename)
        )
        result = unified_diff(
            oldlines,
            newlines,
