...
class Test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._manifestfile = normpath(
            join(
                *([__file__] + [os.pardir] * 4 + ["testdata", "tools", "manifest.xml"])
            )
        )
    ...
    #         nl = [l.strip() for l in newspectxt.splitlines()]
    #         ol = [l.strip() for l in oldspectxt.splitlines()]
...
"""
    ...
    def testGlimpseSpecBuilder(self):
        ...
        spec = LHCbGlimpseRpmSpec(
            project, version, "/tmp/toto.zip", rpmbuildarea, manifest
        )
