...
def validateDaysOfWeek(daysstr):
    ...
    if len(unknown_days) > 0:
        raise ValueError("Unknown days in schedule: %s" % ",".join(unknown_days))
...
def validateDaysOfMonth(daysstr):
    ...
    if len(incorrect_days) > 0:
        raise ValueError(
            "Days have to be between 1 and 31. Not allowed: %s"
            % ",".join(incorrect_days)
        )
...
def _fromXML(etree):
    ...
    if elem == None:
        raise Exception("Missing <test> with test definition in %s" % tostring(etree))
    ...
    if len(missing) > 0:
        raise Exception(
            "Missing values for: %s in \n%s" % ((",".join(missing)), tostring(etree))
        )
    ...
    if len(extra) > 0:
        raise Exception(
            "Unknown attribute(s): %s in \n%s" % ((",".join(extra)), tostring(etree))
        )
...
class PeriodicTest(object):
    ...
    def __str__(self):
        ...
        Convert to string
        """
        return "/".join(["%s=%s" % (k, getattr(self, k)) for k in TEST_ATTRIBUTES])
    ...
    def isForDate(self, date_start, date_end):
        ...
        if self.scheduletype == SCHED_TYPE_WEEK:
            ...
            daymatches = dayname.lower() in [
                d.lower() for d in self.dates
    ...
    def getBuildMatches(self, buildlist):
        ...
        for buildinfo in buildlist:
            ...
            __log__.debug(
                self.testgroup + "Checking against build: %s" % buildinfo["slot"]
            )
            ...
            for proj in buildinfo["projects"]:
                if (
                    proj["name"].lower() != self.project.lower()
                ):  # pylint: disable=E1101
                    continue
                # Project matches !
                ...
                for platf in buildinfo["platforms"]:
                    ...
                    if not (
                        fullmatch(self.platform, platf) or fnmatch(platf, self.platform)
                    ):
                        continue
                    ...
                    if not self.label or any(fullmatch(self.label, x) for x in labels):
...
class ScheduledTest(object):
    ...
    def __str__(self):
        ...
        Convert to string
        """
        return "/".join(["%s=%s" % (k, getattr(self, k)) for k in SCHED_ATTRIBUTES])
