...
from sqlalchemy import BigInteger, Column, Integer, String, ForeignKey, DateTime, Boolean  # , Enum
...
class User(TimeStampedModel):
    ...
    # user_type = Column(Enum(UserType, values_callable=lambda x: [i.value for i in x]))
    user_type = Column(postgresql.ENUM(UserType))
...
class Chat(TimeStampedModel):
    ...
    # chat_type = Column(Enum(ChatType, values_callable=lambda x: [i.value for i in x]))
    chat_type = Column(postgresql.ENUM(ChatType))
...
class ChatSetting(TimeStampedModel):
    ...
    # chat_type_override = Column(Enum(ChatType, values_callable=lambda x: [i.value for i in x]), nullable=True)
    chat_type_override = Column(postgresql.ENUM(ChatType), nullable=True)
...
class AppRole(TimeStampedModel):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    # role_type = Column(Enum(AppRoleType, values_callable=lambda x: [i.value for i in x]))
    role_type = Column(postgresql.ENUM(AppRoleType))
