...
async def select_entity(query: Query, entities: List[Entity]) -> Entity:
    selector = (
    ...
    logger.info(
        ...
        selected_idx,
        entities[selected_idx].reference,
        entities[selected_idx].value,
    )

    return Entity(value=str(value), reference=entities[llm_output["index"]].reference)
