            ...
            if property:
                for prop in property.strip().split(','):
                    ...
                    if num_fields % 2 == 1:
                        ...
                        prop_value = ':'.join(split_prop[2:])
                    else:
                        # even number ==> no type, so set it to the default or blank
                        prop_type = def_type
                        # And recombine all the remaining fields back into the value string
                        prop_value = ':'.join(split_prop[1:])

                    # Remove leading ! as they are not needed in the key
                    valid_key = amu.validate(remove_leading_chars(key.strip(), '!'))
                    if key.startswith('!!'):
                        ...
                        if num_fields > 1 and not key.startswith('!'):
                            validated[valid_key + prop_type] = prop_value
                        else:
