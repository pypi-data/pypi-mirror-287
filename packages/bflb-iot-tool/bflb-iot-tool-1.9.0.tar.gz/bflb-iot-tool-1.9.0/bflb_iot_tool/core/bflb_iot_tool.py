            ...
            for file_name in file:
                ...
                for name in name_list:
                    ...
                    if new_name == "factory":
                        ...
                        continue
                    if (
                        "whole_img_" + new_name + ".bin" in file_name
                        or new_name + ".bin" in file_name
                    ):
                        if file_size > pt_parcel[new_name + "_len"]:
                            # bflb_utils.printf("Error: %s bin size is overflow with partition table" % new_name)
                            raise Exception(
                                "Error: %s bin size is overflow with partition table" % new_name
                            )
                            return False
        except Exception as e:
            bflb_utils.printf(e)
...
class BflbIotTool(object):
    def __init__(self, chipname, chiptype, outdir):
        ...
        eflash_loader_cfg_org = os.path.join(
        ...
        if chiptype == "bl60x":
            self.bh_cfg_file_org = os.path.join(
                chip_path, chipname, "efuse_bootheader/efuse_bootheader_cfg_dp.conf"
            )
        else:
            self.bh_cfg_file_org = os.path.join(
                chip_path, chipname, "efuse_bootheader/efuse_bootheader_cfg.conf"
            )
    ...
    def bl60x_image_gen_cfg(
        ...
        if cfg_ini in [None, ""]:
            if chiptype == "bl60x":
                f_org = os.path.join(
                    chip_path, chipname, "img_create_iot", "img_create_cfg_dp.conf"
                )
            else:
        ...
        if bintype == "fw":
            if group_type is None or group_type == "Group0":
                ...
                bootinfo_file = os.path.join(
                    self.outdir, "bootinfo_{0}.bin".format(group_type.lower())
                )
    ...
    def bl60x_mfg_ota_xz_gen(
        self, ota_path, chipname="bl60x", chiptype="bl60x", cpu_type=None, fw_len=0, hash_pk=None
    ):
        bl60x_xz_filters = [
        ...
        if len(fw_ota_bin_xz) > fw_len:
            ...
            return False
        fw_ota_bin_xz_ota = self.bl60x_mfg_ota_header(
            chipname, fw_ota_bin_xz, use_xz=1, hash_pk=hash_pk
        )
    ...
    def bl60x_mfg_ota_bin_gen(
        ...
        if cpu_type is None or cpu_type == "Group0":
            ...
            bootinfo_fw_path = os.path.join(
                self.outdir, "bootinfo_fw_" + cpu_type.lower() + ".bin"
            )
        ...
        fw_ota_bin_header = self.bl60x_mfg_ota_header(
            chipname, fw_ota_bin, use_xz=0, hash_pk=hash_pk
        )
        if cpu_type is None or cpu_type == "Group0":
            fw_ota_path = os.path.join(ota_path, "FW_OTA.bin")
        else:
    ...
    def bl60x_image_gen(
        ...
        **kwargs,
        ...
        if encrypt or sign:
            ...
            cfg.write(self.eflash_loader_cfg, "w")
        return img_create.create_sp_media_image_file(
            f, chiptype, cpu_type, security_save_efuse, **kwargs
        )
            ...
            if bind_bootinfo is True:
                if "boot2_header" not in pt_parcel or pt_parcel["boot2_header"] == 1:
                    ...
                    if bootinfo_filedata[0:4] == filedata[0:4]:
                        ...
                        whole_img_len = imge_offset + os.path.getsize(
                            os.path.join(self.outdir, "img_boot2.bin")
                        )
                        whole_img_data = bl_create_flash_default_data(whole_img_len)
                    ...
                    if name + "_addr" in pt_parcel:
                        ...
                        if bind_bootinfo is True:
                            if name == "fw":
                                ...
                                fp.close()
                                d_files.append(
                                    os.path.join(self.outdir, "whole_img_" + name + ".bin")
                                )
    ...
    def run_amazon_hook(self, hook):
        p = subprocess.Popen(
            hook, shell=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
    ...
    def flasher_download_cfg_ini_gen(self, chipname, chiptype, cpu_type, config):
        ...
        if chiptype == "bl702" or chiptype == "bl702l":
            if (
                "boot2_download" in config["check_box"]
                and config["check_box"]["boot2_download"] is True
            ):
                boot2_en = False
            else:
                boot2_en = True
        ...
        if (
            "factory_download" in config["check_box"]
            and config["check_box"]["factory_download"] is True
        ):
        ...
        if (
            ...
            if "boot2_len" in pt_parcel and pt_parcel["boot2_len"] == 0:
                ...
                if boot2 is not None and len(boot2) > 0:
                    ...
                    f_org = os.path.join(
                        chip_path, chipname, "img_create_iot", "img_create_cfg_boot2.conf"
                    )
                    f = os.path.join(self.outdir, "img_create_cfg_boot2.ini")
                    if os.path.isfile(f) is False:
                        shutil.copyfile(f_org, f)
        ...
        for i in range(len(name_list)):
            ...
            if pt_parcel[name + "_header"] == 1:
                if (
                    name + "_download" in config["check_box"]
                    and name + "_bin_input" in config["input_path"]
                ):
                    media_sign = 0
                    if name == "media":
                        ...
                        if config["check_box"]["media_download"] is True:
                            ...
                            if media_bin is not None and len(media_bin) > 0:
                                try:
                                    shutil.copyfile(
                                        media_bin, os.path.join(self.outdir, "media.bin")
                                    )
                                except Exception as e:
                                    bflb_utils.printf(e)
                        if config["check_box"]["romfs_download"] is True:
                            ...
                            if romfs_dir is not None and len(romfs_dir) > 0:
                                ret = generate_romfs_img(
                                    romfs_dir, os.path.join(self.outdir, "media.bin")
                                )
                                if ret != 0:
                    ...
                    if (
                        media_sign == 1
                        or media_sign == 2
                        or config["check_box"][name + "_download"] is True
                    ):
                        if name + "_addr" in pt_parcel:
                            if chiptype == "bl808":
                                if media_sign == 2:
                                    ...
                                    temp_obj = (
                                        "%s|UNUSED|UNUSED|"
                                        % config["input_path"][name + "_bin_input"]
                                    )
                                cfg_t.read(self.bh_cfg_file)
                                cfg_t.set(
                            ...
                            if temp_obj is not None and len(temp_obj) > 0:
                                ...
                                if name == "fw" or name == "fw_cpu0":
                                    ...
                                    if chiptype == "bl616":
                                        ...
                                        if cfg_flasher.has_option(
                                            "THIRD_CFG", "amazon_header_enable"
                                        ):
                                            if (
                                                cfg_flasher.get(
                                                    "THIRD_CFG", "amazon_header_enable"
                                                ...
                                                out_encrypt_file = cfg_flasher.get(
                                                    "THIRD_CFG", "out_encrypt_file"
                                                )
                                                after_encrypt_hook = cfg_flasher.get(
                                                    "THIRD_CFG", "after_encrypt_hook"
                                                )
                                                in_signed_header = cfg_flasher.get(
                                                    "THIRD_CFG", "in_signed_header"
                                                )
                                                shutil.copyfile(
                                                    os.path.join(self.outdir, "img.bin"),
                                                    out_encrypt_file,
                                    ...
                                    if in_signed_header != "":
                                        with open(in_signed_header, "rb") as fp:
                                            ...
                                            fp_header = open(
                                                os.path.join(self.outdir, "bootinfo.bin"), "wb+"
                                            )
                ...
                if name == "media":
                    if (
                        "media_download" in config["check_box"]
                        and config["check_box"]["media_download"] is True
                    ):
                        media_bin = config["input_path"]["media_bin_input"]
                        if media_bin is not None and len(media_bin) > 0:
                            try:
                        ...
                        bin_d_list[i] = True
                    if (
                        "romfs_download" in config["check_box"]
                        and config["check_box"]["romfs_download"] is True
                    ):
                        romfs_dir = config["input_path"]["romfs_dir_input"]
                        if romfs_dir is not None and len(romfs_dir) > 0:
                            ret = generate_romfs_img(
                    ...
                    if config["check_box"][name + "_download"] is True:
                        ...
                        if temp_bin is not None and len(temp_bin) > 0:
                            try:
                                ...
                                bflb_utils.printf(e)
                        bin_d_list[i] = True
        return self.bl60x_mfg_flasher_eflash_loader_cfg(
            chipname, chiptype, boot2_d, pt_parcel, bin_d_list, name_list
        )
    ...
    def flasher_download_thread(self, chipname, chiptype, act, config, callback=None):
        ...
        try:
            ...
            self.eflash_loader_bin = os.path.join(
                chip_path, chipname, "eflash_loader/" + get_eflash_loader(bl60x_xtal)
            )
            ...
            if act == "read":
                ...
                cfg.set("FLASH_CFG", "address", addrs_str)
                cfg.write(self.eflash_loader_cfg, "w")
                self.eflash_loader_t = eflash_loader.BflbEflashLoader(
                    chipname, chiptype, self.outdir
                )
                if (
                    not config["param"]["comport_uart"]
                    and config["param"]["interface_type"] == "uart"
                ):
            ...
            # generation partition table/boot2/fw bin files and rf files
                ...
                if act == "build" or act == "download" or act == "erase":
                    ...
                    if (self.encrypt is True or self.sign is True) and (
                        chiptype == "bl602" or chiptype == "bl702"
                    ):
                        if self.encrypt:
                            encrypt = 1
                        else:
                        ...
                        if ret == True:
                            ...
                            z.write(
                                os.path.join(
                                    chip_path, chipname, "eflash_loader", eflash_loader_bin
                                ),
                ...
                if act != "build":
                    ...
                    if error is True:
                        ...
                        if (
                            ...
                            error = (
                                '{"ErrorCode":"FFFF","ErrorMsg":"BFLB INTERFACE HAS NO COM PORT"}'
                            )
                            bflb_utils.printf(error)
                            return error
                        self.bl60x_mfg_flasher_erase_all(False)
                        if "ckb_erase_all" in config["check_box"]:
                            if config["check_box"]["ckb_erase_all"] == "True":
                                ...
                                self.bl60x_mfg_flasher_erase_all(False)
                        if (
                            config["check_box"]["encrypt"] is True
                            or config["check_box"]["sign"] is True
                        ):
                            options.extend(["--efuse"])
                            if (
                                config["check_box"]["encrypt"] is True
...
def iot_download_cmd(args):
    ...
    if act == "build":
        f_org = os.path.join(
            chip_path, args.chipname, config["param"]["outdir"], "whole_flash_data.bin"
        )
        f = "firmware.bin"
        try:
            shutil.copyfile(f_org, f)
