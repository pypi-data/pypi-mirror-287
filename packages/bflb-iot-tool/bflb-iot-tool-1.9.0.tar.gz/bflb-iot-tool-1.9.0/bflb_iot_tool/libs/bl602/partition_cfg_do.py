...
def check_pt_data(data):
    ...
    if data[16 + (36 * table_count) : 16 + (36 * table_count) + 4] != crcarray:
        ...
        return False, 0, 0
    age = (
        bflb_utils.bytearray_to_int(data[8:9])
        + (bflb_utils.bytearray_to_int(data[9:10]) << 8)
        + (bflb_utils.bytearray_to_int(data[10:11]) << 16)
        + (bflb_utils.bytearray_to_int(data[11:12]) << 24)
    )
