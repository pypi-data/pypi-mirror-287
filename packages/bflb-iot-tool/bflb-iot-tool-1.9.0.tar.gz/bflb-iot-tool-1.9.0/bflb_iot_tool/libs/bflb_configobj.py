...
class ConfigObj(Section):
        ...
        while cur_index < maxline:
            ...
            if mat is not None:
                ...
                if cur_depth != sect_close.count("]"):
                    self._handle_error(
                        "Cannot compute the section depth", NestingError, infile, cur_index
                    )
                    continue
                ...
                if cur_depth < this_section.depth:
                    ...
                    try:
                        ...
                        self._handle_error(
                            "Cannot compute nesting level", NestingError, infile, cur_index
                        )
                        continue
            ...
            if mat is None:
                ...
                if value[:3] in ['"""', "'''"]:
                    try:
                        ...
                        self._handle_error(
                            "Parse error in multiline value", ParseError, infile, cur_index
                        )
                        continue
                    else:
                        if self.unrepr:
    ...
    def _quote(self, value, multiline=True):
        ...
        * Obey list syntax for empty and single member lists.
        ...
        hash_triple_quote = (
            multiline and not need_triple and ("'" in value) and ('"' in value) and ("#" in value)
        )
    ...
    def write(self, outfile=None, section=None):
        ...
        if (self.filename is None) and (outfile is None):
            ...
            if self.BOM and (
                (self.encoding is None) or (BOM_LIST.get(self.encoding.lower()) == "utf_8")
            ):
                # Add the UTF8 BOM
                if not out:
                    out.append("")
