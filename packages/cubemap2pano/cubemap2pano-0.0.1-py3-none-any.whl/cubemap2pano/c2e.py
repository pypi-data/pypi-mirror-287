...
def cubemap2pano(width:int, height:int, cubemap_paths: list[tuple[list[str], str]], device: Optional[torch.device]=None):
    ...
    tensor_cubemap_list = [(torch.stack([load_image_as_tensor(cube_path).to(device) for cube_path in cube_paths]).permute(0, 2, 3, 1),result_path) for cube_paths, result_path in cubemap_paths]
    if len(tensor_cubemap_list) == 0:
    ...
    for tensor_cubemap, save_path in tensor_cubemap_list:
