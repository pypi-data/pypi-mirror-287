    ...
    def create_export(
        ...
        while True:
            ...
            for export in exports:
                if (export.status == OperationStatusOverview.FINISHED.value and
                        export.id == new_export.id):
                    return new_export
    ...
    def download_exported_model(
        ...
        for artifact in artifacts:
            ...
            for model in artifact.exports:
                if (model.status == OperationStatusOverview.FINISHED.value and
                        model_id in (model.id, model.id.split("_")[1])):
                    ...
                    return msgspec.json.decode(
                        json.dumps({
                            "download_path":
                                str(download_path),
                            "model_filename":
                                ("saved_model/" if model.format == "TensorFlow"
                                 else model_files[0]),
                            "label_filename":
                                ("label.txt" if model.format == "TFLite" else
                                 "label_map.pbtxt"),
                        }),
