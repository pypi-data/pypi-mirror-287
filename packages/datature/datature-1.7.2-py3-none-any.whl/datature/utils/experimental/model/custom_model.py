...
class CustomModel:
    ...
    def zip(self):
        ...
        with ZipFile(self._zip_path, "w", ZIP_DEFLATED) as zipf:
            ...
            zipf.write(
                ...
                os.path.relpath(self._graph_module_path,
    ...
    def _verify_contents(self):
        ...
        yaml_filename = next((f for f in all_files if f.endswith(".yaml")),
                             None)
        if not yaml_filename:
            raise FileNotFoundError("no yaml file")
        graph_module_filename = next(
            (f for f in all_files if f.endswith(".pth")), None)
        ...
        self._graph_module_path = os.path.join(self._model_folder_path,
                                               graph_module_filename)
