...
def verify_yaml_config(yaml_file: str):
    ...
    assert inputs, ("No input fields found in the YAML file. "
    ...
    for key, value in inputs.items():
        assert isinstance(
            value["name"],
            str), f"Input name '{value['name']}' must be a string"
        assert isinstance(value["key"],
                          str), f"Input key '{value['key']} must be a string"
        assert isinstance(
            value["format"],
            list), f"Input format '{value['format']}' must be a list"
        assert value["format"] in InputFormatOptions, (
            f"Input format '{value['format']}' "
            f"needs to be one of {InputFormatOptions}")
        assert isinstance(
            value["shape"],
            list), f"Input shape '{value['shape']}' must be a list"
    ...
    assert outputs, ("No output fields found in the YAML file. "
    ...
    for key, value in outputs.items():
        assert isinstance(
            value["name"],
            str), f"Output name '{value['name']}' must be a string"
        assert isinstance(value["key"],
                          str), f"Output key '{value['key']} must be a string"

        if key in output_required_fields:
            assert value["format"] in output_format_options[key.upper(
            )].value, (
        ...
        assert isinstance(
            value["shape"],
            list), f"Output shape '{value['shape']}' must be a list"
        assert len(value["shape"]) == len(value["format"]), (
