...
class ImagePredictor(BasePredictor):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._input_names = [
            input["name"] for input in self._model_config["input"]
        ]
        self._output_names = [
            output["name"] for output in self._model_config["output"]
        ]
        self._output_shapes = [
    ...
    def _run_inference_client(self, model_input: np.ndarray) -> Dict:
        ...
        """
        inputs = [
            httpclient.InferInput(input_name,
                                  self._input_shape,
                                  datatype="FP32")
    ...
    def __call__(self) -> None:
        for each_image in glob.glob(os.path.join(self._input_path, "*")):
            ...
            if self._save:
                ...
                specific_output_path = os.path.join(self._output_path,
                                                    image_name)
...
class MaskPredictor(ImagePredictor):
    ...
    def __call__(self) -> None:
        for each_image in glob.glob(os.path.join(self._input_path, "*")):
            ...
            if self._save:
                image_origi = cv2.imread(each_image)
                for box, mask, class_id, score in zip(bboxes, masks, classes,
                                                      scores):
...
class YOLOv8BoxPredictor(YOLOv8Predictor):
    ...
    def _postprocess(self, response_outputs,
                     num_classes: int) -> Tuple[np.ndarray]:
        """Postprocess the model output.
...
class YOLOv8MaskPredictor(YOLOv8Predictor):
    ...
    def _postprocess(self, response_outputs,
        ...
        scores, classes, boxes, masks = postprocess_predictions(
            prediction, num_classes)
        ...
        masks = 1 / (1 + np.exp(-1 * masks)
                    )  # use -1*mask over -mask to avoid overflow
...
class YOLOv8PosePredictor(YOLOv8Predictor):
    ...
    def __call__(self) -> None:
        ...
        for each_image in glob.glob(os.path.join(self._input_path, "*")):
            ...
            _, classes, _, keypoints, keypoint_confidences = self._predict(
                each_image)
        ...
        for i, item in enumerate(self._model_config["input"]):
            if item["name"] == "image":
                input_state = httpclient.InferInput(
                    ...
                    [1] + item["dims"],
                    datatype="FP32"
                    if item["data_type"] == "TYPE_FP32" else "INT32",
                )
                ...
                input_state.set_data_from_numpy(
                    (np.zeros(input_state.shape(), dtype=np.float32)
                     if item["data_type"] == "TYPE_FP32" else np.zeros(
                         input_state.shape(), dtype=np.int32)),
                    binary_data=True,
                )
