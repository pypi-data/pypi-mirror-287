...
def get_model_config(model_name: str) -> Dict:
    ...
    model_config = requests.get(
        f"http://localhost:8000/v2/models/{model_name}/config",
        timeout=(3, 10)).json()
    return model_config
...
def load_skeleton(skeleton_path: str) -> Dict:
    ...
    def _keypoint_positions(skeleton):
        keypoint_name_list = [
            keypoint["name"] for keypoint in skeleton["keypoints"]
        ]
        return {
            "keypoint_names":
                keypoint_name_list,
    ...
    skeleton_dict = {
        skeleton["name"]: _keypoint_positions(skeleton)
...
def draw_yolov8_instance_masks(
    ...
    masked_image = np.where(mask, np.array(color, dtype=np.uint8),
                            masked_image)
    ...
    for kpi, (each_kp, each_score) in enumerate(zip(keypoints, scores)):
        ...
        if x_coord % width != 0 and y_coord % height != 0:
            ...
            cv2.rectangle(
                ...
                (
                    int(x_coord) + len(f"{skeleton['keypoint_names'][kpi]} "
                                       f"{str(round(each_score, 2))}") * 5,
                    int(y_coord),
                ),
                (0, 0, 0),
                -1,
...
def postprocess_predictions(pred: np.array,
...
def xywh2xyxy(boxes_xywh: np.ndarray, height: int, width: int) -> np.ndarray:
    ...
    boxes_xyxy[..., 0] = (boxes_xywh[..., 0] -
                          boxes_xywh[..., 2] / 2) / width  # top left x
    boxes_xyxy[..., 1] = (boxes_xywh[..., 1] -
                          boxes_xywh[..., 3] / 2) / height  # top left y
    boxes_xyxy[..., 2] = (boxes_xywh[..., 0] +
                          boxes_xywh[..., 2] / 2) / width  # bottom right x
    boxes_xyxy[..., 3] = (boxes_xywh[..., 1] +
                          boxes_xywh[..., 3] / 2) / height  # bottom right y
...
def box_diou(boxes: np.ndarray, dtype: str) -> np.ndarray:
    ...
    hei = boxes[:, 3]
    areas = wid * hei
    areas[areas == np.inf] = np.finfo(
        dtype).max  # account for possible overflow
