...
try:
    ...
    raise ModuleNotFoundError(
        ...
        "to learn how to set up your environment.") from import_exc
...
class Converter:
    ...
    def __init__(
        ...
        try:
            self._device_request = docker.types.DeviceRequest(
                device_ids=[f"{self._docker_params.device}"],
                capabilities=[["gpu"]])
    ...
    def setup(self) -> None:
        ...
        if os_name == "Linux" and os.path.exists("/proc/sys/kernel/osrelease"):
            with open("/proc/sys/kernel/osrelease", "r",
                      encoding="utf-8") as os_release:
        ...
        if len(candidate) == 0:
            trt_logger.info("Pulling Docker image %s",
    ...
    def convert(
        ...
        try:
            ...
            self._exec_id = self._api_client.exec_create(
                ...
                environment={
                    ...
                    "POLYGRAPHY_INTERNAL_CORRECTNESS_CHECKS":
                        (1
                         if self._conversion_params.internal_correctness_checks
                         else 0),
                },
            )["Id"]
            self._logs = self._api_client.exec_start(self._exec_id,
                                                     stream=True,
                                                     detach=False)
            ...
            trt_logger.info(
                "TensorRT model conversion successful, model is saved at %s",
                model_path)
            trt_logger.info(
    ...
    def create_export(
        ...
        try:
            ...
            raise error.BadRequestError(
                ...
                "https://developers.datature.io/docs/tensorrt-export") from exc
        except error.InternalServerError as exc:
            if exc.detail["code"] == "ConflictError":
                trt_logger.info(
                    "Model has already been exported, retrieving export")
                is_exported = True
                model_id = [
                    model for model in project.artifacts.list_exported_models(
                        artifact_id) if model.format == target_format
                ][-1].id
            else:
        ...
        if not is_exported:
            target_export = [
                export for export in project.artifacts.list_exported_models(
                    artifact_id) if export.id == model_id
            ][-1]
            ...
            while target_export.status != OperationStatusOverview.FINISHED.value:
                ...
                target_export = [
                    ...
                    project.artifacts.list_exported_models(artifact_id)
    ...
    def download_export(self, project: Project, model_id: str) -> str:
        ...
        model_path = os.path.join(download_details.download_path,
                                  download_details.model_filename)
    ...
    def _get_commands(
        ...
        log_file = os.path.join(self._conversion_params.log_dir,
                                f"{int(time.time())}.log")
        ...
        if self._conversion_params.experimental:
            check_commands, sanitize_commands, conversion_experimental_commands = (
        ...
        convert_verbose_flag = ("--verbosity VERBOSE"
                                if self._conversion_params.verbose else "")
        conversion_commands = (
    ...
    def _get_experimental_commands(
        ...
        if check_params.enabled:
            ...
            check_commands = (f" && polygraphy check lint {model_path}"
        ...
        if sanitize_params.enabled:
            ...
            output_path = (sanitize_params.output_model_path if
                           sanitize_params.output_model_path else os.path.join(
                               root_dir, f"sanitized_{file_name}"))
            cleanup_flag = "--cleanup" if sanitize_params.cleanup else ""
            toposort_flag = "--toposort" if sanitize_params.toposort else ""
            no_shape_inference_flag = ("--no-shape-inference"
                                       if sanitize_params.no_shape_inference
                                       else "")
            force_fallback_shape_inference_flag = (
                "--force-fallback-shape-inference"
                if sanitize_params.force_fallback_shape_inference else "")
            fold_constants_flag = ("--fold-constants"
                                   if sanitize_params.fold_constants else "")
            partitioning_flag = (
                f"--partitioning {sanitize_params.partitioning}"
                if sanitize_params.partitioning else "")
            no_fold_shapes_flag = ("--no-fold-shapes"
                                   if sanitize_params.no_fold_shapes else "")
            no_per_pass_shape_inference_flag = (
                "--no-per-pass-shape-inference"
                if sanitize_params.no_per_pass_shape_inference else "")
            sanitize_commands = (
    ...
    def _get_cuda_version(self) -> str:
        ...
        Returns:
            str: The CUDA version as a string.

        Raises:
            Error: If the CUDA version file cannot be found.
        """
        try:
            with open("/usr/local/cuda/version.json", "r",
                      encoding="utf-8") as cuda_version_file:
                cuda_version = json.load(cuda_version_file)["cuda"]["version"]
                trt_logger.info("CUDA version: %s", cuda_version)
        except FileNotFoundError as f_exc:
            raise error.Error("Could not find CUDA version file.") from f_exc
        return cuda_version
    ...
    def _get_gpu_info(self) -> Dict[str, str]:
        ...
        with subprocess.Popen(command,
            ...
            if process.returncode == 0:
                gpu_name, driver_version = output.decode().strip().split(", ")
                trt_logger.info("GPU name: %s", gpu_name)
                trt_logger.info("Driver version: %s", driver_version)
            else:
