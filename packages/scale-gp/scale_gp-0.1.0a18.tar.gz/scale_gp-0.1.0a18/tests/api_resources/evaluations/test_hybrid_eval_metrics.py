...
class TestHybridEvalMetrics:
    ...
    def test_method_list(self, client: SGPClient) -> None:
        hybrid_eval_metric = client.evaluations.hybrid_eval_metrics.list(
            "evaluation_id",
        )
        assert_matches_type(HybridEvaluationMetricsResponse, hybrid_eval_metric, path=["response"])

    @parametrize
    def test_raw_response_list(self, client: SGPClient) -> None:
        response = client.evaluations.hybrid_eval_metrics.with_raw_response.list(
            "evaluation_id",
        )
    ...
    def test_streaming_response_list(self, client: SGPClient) -> None:
        with client.evaluations.hybrid_eval_metrics.with_streaming_response.list(
            "evaluation_id",
        ) as response:
    ...
    def test_path_params_list(self, client: SGPClient) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `evaluation_id` but received ''"):
            client.evaluations.hybrid_eval_metrics.with_raw_response.list(
                "",
            )
...
class TestAsyncHybridEvalMetrics:
    ...
    async def test_method_list(self, async_client: AsyncSGPClient) -> None:
        hybrid_eval_metric = await async_client.evaluations.hybrid_eval_metrics.list(
            "evaluation_id",
        )
        assert_matches_type(HybridEvaluationMetricsResponse, hybrid_eval_metric, path=["response"])
    ...
    async def test_raw_response_list(self, async_client: AsyncSGPClient) -> None:
        response = await async_client.evaluations.hybrid_eval_metrics.with_raw_response.list(
            "evaluation_id",
        )
    ...
    async def test_streaming_response_list(self, async_client: AsyncSGPClient) -> None:
        async with async_client.evaluations.hybrid_eval_metrics.with_streaming_response.list(
            "evaluation_id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
    ...
    async def test_path_params_list(self, async_client: AsyncSGPClient) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `evaluation_id` but received ''"):
            await async_client.evaluations.hybrid_eval_metrics.with_raw_response.list(
                "",
            )
