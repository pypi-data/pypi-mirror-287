...
class Credential(_APIObject):
    ...
    name = _StringDescr(
        doc="the name of this credential. Unique to the user and how a user will typically reference it"
    )
    type = _StringDescr(
        one_of=valid_types,
    ...
    def from_docker_registry(
        *, name: str, username: str, host: str, email: str, password: str = None
        ...
        Creates a Credential object that allows internal resources to pull a container image from a registry. Most
        registries support this method of pulling images and specifically this is how this is done in Kubernetes in
        most cases.
    ...
    def from_jwt(
        *,
        ...
        Creates a new Credential object for an arbitrary JWT or token endpoint. Note that for OAuth2 flows, this
        should NOT be used. Use from_oauth2_client_credentials or from_oauth2_refresh_token instead.

        Arguments:
            name: the name of the Credential to create
            jwt: the string/encoded JWT. If `None`, user will
                 be prompted via stdin.
            token_url: the token url/uri to request a JWT via an API key exchange. If `jwt` is specified, this is
                       ignored. If `jwt` is not specified, the API key must also be specified and the remaining
