...
@validatetyping
async def get_streams(channel_id:int,user_ids:list=[],login_names:list= [])->dict:
    '''[{
      "id": "26007494656",
      "user_id": "23161357",
      "user_name": "LIRIK",
    ...
    appender = "user_id="+str(channel_id) + "&"
    if user_ids:
    ...
    if login_names:
        for login_name in login_names:
            appender += f"user_login={login_name}&"

    url= f"https://api.twitch.tv/helix/streams?"+appender[:-1]
