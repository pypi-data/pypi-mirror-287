...
class TelegramPeer():
    ...
    def __wrapMsg(self, message:telethon.tl.patched.Message) -> TelegramMessage:
        ...
        if message.from_id:
            ...
            if message.from_id.to_dict()['_'] == "PeerUser":
                ...
                msg.User.IsBot = message.sender.bot
    ...
    def __repr__(self):
        if self.Type == "user":
            return f"TelegramPeer(Type={self.Type}, Name={self.Name}, Username={self.Username}, ID={self.ID}, AccessHash={self.AccessHash}, PhoneNumber={self.PhoneNumber}, IsBot={self.IsBot} GroupOwner={self.GroupOwner})"
        else:
