...
export var gmtime = function (seconds, localtime) {
	...
	return t.__getslice__ (null, 9, 1);
};
	...
	while (ts) {
		...
		if (sep == '') {
			...
			if (d) {
				...
				if (l > -(1)) {
					var lv = [ts.__getslice__ (null, l, 1), ts.__getslice__ (l, null, 1)];
				}
			}
			if (!(lv)) {
				var lv = [ts, ''];
