// Transcrypt'ed from Python, 2024-07-27 18:27:28
...
export var run = function (autoTester) {
	...
	var c = [dict ({'name': 'Defg', 'qty': 99}), dict ({'name': 'Abcd', 'qty': 42}), dict ({'name': 'Hijk', 'qty': 11})];
	var d = [[21, 19, 54], [22, 7, 12, 37]];
	autoTester.check (a);
	autoTester.check (sorted (a));
	autoTester.check (a);
	autoTester.check (sorted (a, __kwargtrans__ ({key: str})));
	autoTester.check (sorted (a, __kwargtrans__ ({key: int})));
	autoTester.check (sorted (b));
	autoTester.check (sorted (c, __kwargtrans__ ({key: (function __lambda__ (k) {
		return k ['qty'];
	})})));
	autoTester.check (sorted (c, __kwargtrans__ ({key: (function __lambda__ (k) {
		return k ['name'];
	...
	autoTester.check (sorted (d, __kwargtrans__ ({key: sum})));
	var e = sorted (d);
	autoTester.check (e);
	d [1] [1] = 9;
	autoTester.check (d);
	...
	autoTester.check (a);
	a.py_sort (__kwargtrans__ ({key: int}));
	autoTester.check (a);
