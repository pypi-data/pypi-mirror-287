...
def extract_plan_and_billing_info(content):
    ...
    def convert_date(date_str):
        formats = ['%B %d, %Y', '%d %B %Y', '%d. %B %Y', '%B %d, %Y.', '%B %d %Y', '%B %dth %Y']
...
def extract_and_process_cookies(file, output_folder, drive, folder_id):
    ...
    if file.endswith(".zip"):
                ...
                if cookies_status == "Active":
                    ...
                    if response.status_code == 200:
                        cookies_status, plan, next_billing_date = (
                            extract_plan_and_billing_info(response.content)
                        )
                    result_list.append(
        ...
        if not (file.endswith(".json") or file.endswith(".txt")):
            print(f"Invalid file format found: {file}. Skipping this file...")
            invalid_files += 1
            return

        cookies, is_netflix_cookie = read_cookies(file)
        if not is_netflix_cookie:
            print(
                f"Invalid cookies file found: {os.path.basename(file)}. Skipping this file..."
            )
            invalid_cookies_files += 1
            return
        cookies_status = check_cookies_valid_netflix(cookies)
        ...
        if cookies_status == "Active":
            response = requests.get(netflix_url, cookies=cookies, allow_redirects=False)
            if response.status_code == 200:
                cookies_status, plan, next_billing_date = extract_plan_and_billing_info(
                    response.content
                )
            result_list.append(
                [os.path.basename(file), cookies_status, plan, next_billing_date]
            )
            unique_id = str(uuid.uuid4())
            zip_filename = f"temp_2/NetCook-{unique_id}.zip"
            with zipfile.ZipFile(zip_filename, "w") as active_zip:
    ...
    if active_cookies > 0:
        print(
            f"\n\033[1mCookies file active save to directory \033[91m{zip_filename}\033[0m"
        )
...
def checked_cookies(netflix_url):
    ...
    while True:
        ...
        if choice == "1":
            ...
            #print("Support me : https://saweria.co/netcook")
            break
        else:
