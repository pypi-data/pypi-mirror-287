...
class SqlDatabase:
    ...
    def __init__(
        self, name, db_name="tango_database.db", history_depth=10, fire_to_starter=True
    ):
        self._db_api = None
        self._db_conn = None
        self.name = name
    ...
    def delete_device(self, dev_name):
        ...
        cursor.execute(
            "DELETE FROM property_attribute_device WHERE device LIKE ?", (dev_name,)
        )
    ...
    def delete_device_property(self, dev_name, prop_name):
        ...
        # Is there something to delete ?
        cursor.execute(
            "SELECT DISTINCT name, count FROM property_device WHERE device=? AND "
            "name LIKE ?",
            (dev_name, prop_name),
        )
        for row in cursor.fetchall():
    ...
    def get_class_attribute_property_hist(self, class_name, attribute, prop_name):
        ...
        for row in cursor.fetchall():
            ...
            stmt = "SELECT strftime('%Y-%m-%d %H:%M:%S', date),value,attribute,name,count FROM property_attribute_class_hist WHERE id =? AND class =?"
    ...
    def get_device_list(self, server_name, class_name):
        cursor = self.cursor
        cursor.execute(
            "SELECT DISTINCT name FROM device WHERE server LIKE ? AND class LIKE ? ORDER BY name",
    ...
    def get_device_wide_list(self, wildcard):
        cursor = self.cursor
        cursor.execute(
            "SELECT DISTINCT name FROM device WHERE name LIKE ? ORDER BY name",
            (wildcard,),
        )
        return [row[0] for row in cursor.fetchall()]
    ...
    def get_device_member_list(self, wildcard):
        cursor = self.cursor
        cursor.execute(
            "SELECT DISTINCT  member FROM device WHERE name LIKE ? ORDER BY member",
            (wildcard,),
        )
        return [row[0] for row in cursor.fetchall()]
    ...
    def get_device_property_hist(self, device_name, prop_name):
        ...
        cursor.execute(stmt, (device_name, tmp_name))

        stmt = "SELECT strftime('%Y-%m-%d %H:%M:%S', date),value,name,count FROM property_device_hist WHERE id =? AND device =? ORDER BY count ASC"
    ...
    def put_attribute_alias(self, attribute_name, attribute_alias):
        ...
        cursor.execute(
            "INSERT attribute_alias SET alias=? ,name=?, device=?,updated=NOW()",
    ...
    def put_class_attribute_property(self, class_name, nb_attributes, attr_prop_list):
        ...
        for i in range(0, nb_attributes):
            ...
            for j in range(k + 2, k + nb_properties * 2 + 2, 2):
                ...
                # then insert the new value for this tuple
                cursor.execute(
                    "INSERT INTO property_attribute_class SET class=? ,attribute=?,name=?,count='1',value=?,updated=NULL,accessed=NULL",
                    (class_name, tmp_attribute, tmp_name, tmp_value),
                )
                # then insert the new value into the history table
                hist_id = self.get_id("class_attribute", cursor=cursor)
                cursor.execute(
                    "INSERT INTO property_attribute_class_hist SET class=?,attribute=?,name=?,id=?,count='1',value=?",
                    (class_name, tmp_attribute, tmp_name, hist_id, tmp_value),
                )
        ...
        for i in range(0, nb_attributes):
            ...
            for jj in range(0, nb_properties, 1):
                ...
                for l in range(j + 1, j + n_rows + 1, 1):
                    ...
                    # then insert the new value for this tuple
                    cursor.execute(
                        "INSERT INTO property_attribute_class SET class=? ,attribute=?,name=?,count=?,value=?,updated=NULL,accessed=NULL",
                        (
                            class_name,
                            tmp_attribute,
                            tmp_name,
                    ...
                    hist_id = self.get_id("class_attribute", cursor=cursor)
                    cursor.execute(
                        "INSERT INTO property_attribute_class_hist SET class=?,attribute=?,name=?,id=?,count=?,value=?",
                        (
                            class_name,
                            tmp_attribute,
                            tmp_name,
    ...
    def put_class_property(self, class_name, nb_properties, attr_prop_list):
        ...
        for i in range(0, nb_properties):
            ...
            for j in range(k + 2, k + n_rows + 2, 1):
                ...
                # then insert the new value for this tuple
                cursor.execute(
                    "INSERT INTO property_class SET class=? ,name=?,count=?,value=?,updated=NULL,accessed=NULL",
                    (class_name, tmp_name, str(tmp_count), tmp_value),
                )
                # then insert the new value into the history table
                hist_id = self.get_id("class", cursor=cursor)
                cursor.execute(
                    "INSERT INTO property_class_hist SET class=?,name=?,id=?,count=?,value=?",
    ...
    def put_device_alias(self, device_name, device_alias):
        ...
        # update the new value for this tuple
        cursor.execute(
            "UPDATE device SET alias=? ,started=NOW() where name LIKE ?",
            (device_alias, device_name),
        )
    ...
    def put_device_attribute_property(self, device_name, nb_attributes, attr_prop_list):
        ...
        for i in range(0, nb_attributes):
            ...
            for j in range(k + 2, k + nb_properties * 2 + 2, 2):
                ...
                # then insert the new value for this tuple
                cursor.execute(
                    "INSERT INTO property_attribute_device SET device=? ,attribute=?,name=?,count='1',value=?,updated=NULL,accessed=NULL",
                    (device_name, tmp_attribute, tmp_name, tmp_value),
                )
                # then insert the new value into the history table
                hist_id = self.get_id("device_attribute", cursor=cursor)
                cursor.execute(
                    "INSERT INTO property_attribute_device_hist SET device=?,attribute=?,name=?,id=?,count='1',value=?",
        ...
        for i in range(0, nb_attributes):
            ...
            for jj in range(0, nb_properties, 1):
                ...
                for l in range(j + 1, j + n_rows + 1, 1):
                    ...
                    # then insert the new value for this tuple
                    cursor.execute(
                        "INSERT INTO property_attribute_device SET device=? ,attribute=?,name=?,count=?,value=?,updated=NULL,accessed=NULL",
                        (
                            device_name,
                            tmp_attribute,
                            tmp_name,
                    ...
                    hist_id = self.get_id("device_attribute", cursor=cursor)
                    cursor.execute(
                        "INSERT INTO property_attribute_device_hist SET device=?,attribute=?,name=?,id=?,count=?,value=?",
                        (
                            device_name,
                            tmp_attribute,
                            tmp_name,
    ...
    def put_device_property(self, device_name, nb_properties, attr_prop_list):
        ...
        k = 0
        hist_id = self.get_id("device", cursor=cursor)
        for _ in range(0, nb_properties):
            tmp_count = 0
            tmp_name = attr_prop_list[k]
            ...
            for j in range(k + 2, k + n_rows + 2, 1):
                ...
                cursor.execute(
                    ...
                    (device_name, tmp_name, str(tmp_count), tmp_value, now, now),
                )
                # then insert the new value into the history table
                cursor.execute(
                    "INSERT INTO property_device_hist (device, name, id, count, value, date) VALUES (?, ?, ?, ?, ?, ?)",
                    (device_name, tmp_name, hist_id, str(tmp_count), tmp_value, now),
                )
            self.purge_property(
                "property_device_hist", "device", device_name, tmp_name, cursor=cursor
            )
            k = k + n_rows + 2
    ...
    def put_property(self, object_name, nb_properties, attr_prop_list):
        ...
        for i in range(0, nb_properties):
            ...
            for j in range(k + 2, k + n_rows + 2, 1):
                ...
                # then insert the new value for this tuple
                cursor.execute(
                    "INSERT INTO property SET object=? ,name=?,count=?,value=?,updated=NULL,accessed=NULL",
                    (object_name, tmp_name, str(tmp_count), tmp_value),
                )
                # then insert the new value into the history table
                cursor.execute(
                    "INSERT INTO property_hist SET object=?,name=?,id=?,count=?,value=?",
                    (object_name, tmp_name, hist_id, str(tmp_count), tmp_value),
                )
            self.purge_att_property(
    ...
    def put_server_info(self, tmp_server, tmp_host, tmp_mode, tmp_level, tmp_extra):
        ...
        cursor.execute(
            "INSERT INTO server SET name=? ,host=? ,mode=? ,level=?",
    ...
    def uexport_device(self, dev_name):
        ...
        cursor.execute(
            "UPDATE device SET exported=0,stopped=NOW() WHERE name LIKE ?", (dev_name,)
        )
    ...
    def uexport_event(self, event_name):
        ...
        cursor.execute(
            "UPDATE event SET exported=0,stopped=NOW() WHERE name LIKE ?", (event_name,)
        )
    ...
    def uexport_server(self, server_name):
        ...
        cursor.execute(
            "UPDATE device SET exported=0,stopped=NOW() WHERE server LIKE ?",
            (server_name,),
        )
    ...
    def delete_all_device_attribute_property(self, dev_name, attr_list):
        ...
        for attr_name in attr_list:
            ...
            for row in rows:
                hist_id = self.get_id("device_attribute", cursor=cursor)
                cursor.execute(
                    "INSERT INTO property_attribute_device_hist SET device=?,attribute=?,name=?,id=?,count='0',value='DELETED'",
                    (dev_name, attr_name, row[0], hist_id),
                )
                self.purge_att_property(
                    "property_attribute_device_hist",
                    "device",
    ...
    def get_alias_attribute(self, alias_name):
        cursor = self.cursor
        cursor.execute(
            "SELECT name from attribute_alias WHERE alias LIKE ? ", (alias_name,)
        )
        return [row[0] for row in cursor.fetchall()]
