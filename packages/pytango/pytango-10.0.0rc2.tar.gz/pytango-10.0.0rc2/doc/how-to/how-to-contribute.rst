...
To test the docs locally with pixi_ (see :ref:`Using pixi for developement <pixi-for-development>`):
      - ``$ cd /path/to/pytango``
      - ``$ pixi run doc``
...
.. _pixi-for-development:

Using pixi for development
---------------------------

If you like to be at the forefront and aren't afraid of trying new tools, you should give pixi_ a try.
...
Refer to `pixi installation <https://pixi.sh/latest/#installation>`_ for more information.
...
To run the tests locally, you first need to install ``TangoTest``. This is done in a separate env
so it doesn't have to depend on the cpptango version used to compile pytango.

Create the `tangotest` environment:
      - ``$ pixi install -e tangotest`` or ``$ pixi run install-tangotest``
...
If you don't want to use `pixi`, you can of course create and work in your own :ref:`Conda environment <build-from-source>`.
