...
Since version 10.0.0 PyTango provides support for distributed tracing and logging via the OpenTelemetry_ framework.
...
E.g., if you run the `Signoz standalone demo <https://signoz.io/docs/install/docker/>`_, there will be a collector
...
..  warning::  Emitting telemetry from a large number of devices servers and clients can generate a high load on
               ...
               See the `benchmarks <https://gitlab.com/tango-controls/TangoTickets/-/issues/109>`_.
...
See the :ref:`PyTango news <pytango-news>` page for which versions of PyTango are packaged
...
on this `issue <https://gitlab.com/tango-controls/tango-doc/-/issues/403>`_ (pending a documentation update).
...
.. note:: The environment variables can be set in a configuration file, similar
          to TANGO_HOST.
          See the `reference documentation <https://tango-controls.readthedocs.io/en/latest/development/advanced/reference.html#environment-variables>`_.
...
`environmental variables <https://opentelemetry-python.readthedocs.io/en/latest/sdk/environment_variables.html>`_
for configuration.
...
`OpenTelemetry instrumentation docs <https://opentelemetry.io/docs/languages/python/instrumentation/>`_.
...
.. code-block:: python
    ...
    def read_attr_value(device_name: str, attr_name: str, request: Request):
        with tracer.start_as_current_span(
               ...
               attributes={"client_host": request.client.host}
           ):
               proxy = tango.DeviceProxy(device_name)
               value = proxy.read_attribute(attr_name).value
               return {"value": value}

.. note:: Creating a span around a very long running task is not recommended.  The
...
feature in OpenTelemetry Python (as at v1.25.0).
See the `logs examples <https://github.com/open-telemetry/opentelemetry-python/tree/v1.25.0/docs/examples/logs>`_.
...
This is the concept of `sampling <https://opentelemetry.io/docs/concepts/sampling/>`_.
You may consider a probabilistic sampler, or a `tail sampler <https://opentelemetry.io/blog/2022/tail-sampling/>`_,
...
The
`prototyping.py <https://gitlab.com/tango-controls/pytango/-/blob/develop/examples/telemetry/prototyping.py>`_
