...
Date: 2024-07-26 (10.0.0rc2)

Type: major release

An overview of the major new features are available in the `Tango v10 and IDLv6 <https://indico.tango-controls.org/event/261/contributions/867/>`_
presentation materials from the Tango Collaboration meeting.

You can find the full changelog in :ref:`revisions history <pytango-version-history>` with the links to related MRs,
...
- PyTango requires at least `cppTango`_ 10.0.0. See the :ref:`migration guide <to10.0_deps_install>`.
...
- When using the Asyncio_ green mode, a :class:`DeprecationWarning` will be emitted during the
  ...
  Note, support of synchronous methods in Asyncio_ servers will be removed in a future release.
  See the :ref:`migration guide <to10.0_asyncio_deprecation>`.
...
- Numpy 2.0 support
- PyTango now supports OpenTelemetry_ for distributed tracing - see the :ref:`telemetry how-to guide <telemetry_howto>`
- Support for alarm events
- Extended device information (DevInfo6 implementation, from IDLv6) - see the :ref:`version info tutorial <version_info>`
- For Asyncio_ green mode servers, added two coroutine functions to be used when adding and
  removing dynamic attributes.  See the :ref:`migration guide <to10.0_asyncio_dyn_attrs>`.
...
- Fixed deadlock when pushing event from attribute read method, introduced in 9.5.1
...
- Various fixes of asyncio :class:`tango.asyncio.DeviceProxy` methods.  High-level reads, and gettar calls
...
- Following cppTango, event type :class:`~tango.EventType.QUALITY_EVENT` was removed from the :class:`~tango.EventType`
  ...
  See the :ref:`migration guide <to10.0_quality_event>`.
...
+========================+==============+==============+==============+
| cpptango               |   10.0.0-rc4 |  10.0.0-rc4  |  10.0.0rc4   |
+------------------------+--------------+--------------+--------------+
...
+------------------------+--------------+--------------+--------------+
| abseil                 |  20240116.2  |      -       | 20240116.2   |
+------------------------+--------------+--------------+--------------+
| protobuf               |  v27.1       |      -       |  v25.3       |
+------------------------+--------------+--------------+--------------+
| c-ares                 | 1.32.2       |      -       |   1.32.3     |
+------------------------+--------------+--------------+--------------+
| re2                    | 2024-07-02   |      -       | 2023.09.01   |
+------------------------+--------------+--------------+--------------+
| OpenSSL                |    3.3.1     |      -       |   3.3.1      |
+------------------------+--------------+--------------+--------------+
| curl                   | curl-8_8_0   |      -       |   8.9.0      |
+------------------------+--------------+--------------+--------------+
| gRPC                   |   v1.65.1    |      -       |   1.62.2     |
+------------------------+--------------+--------------+--------------+
...
- PyTango requires at least `cppTango`_ 9.5.0. See the :ref:`migration guide <to9.5_deps_installation>`.
