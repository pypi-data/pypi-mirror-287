...
def build_get_features(
    ...
    if fdw.compaction_enabled and fdw.is_temporal_aggregate:
        ...
        aggregation_tile_interval = (
            aggregation_tile_interval_override or fdw.get_tile_interval_duration_for_offline_store
        )
        tree = PartialAggNode(
            dialect=dialect,
            compute_mode=compute_mode,
...
def build_aggregated_time_range_validity_query(
    ...
    # TODO(samantha): support sawtooth aggs here.
...
def build_spine_join_querytree(
    ...
    if fdw.is_temporal or fdw.is_feature_table:
        ...
        if fdw.get_min_batch_sawtooth_tile_size():
            ...
            anchor_time_column = sawtooth_agg_data.get_anchor_time_columns()[
                0
            ]  # Each anchor time column contains the same value for the spine.
            columns_to_drop = (
                sawtooth_agg_data.get_anchor_time_columns() + sawtooth_agg_data.get_identifier_partition_columns()
            )
        else:
...
def _build_standard_fv_subtree(
    ...
    if dialect == Dialect.DUCKDB:
        ...
        spine_node = AddUniqueIdNode(dialect, compute_mode, spine_node, column_name=tecton_spine_row_id_col()).as_ref()
...
def build_feature_set_config_querytree(
    ...
    for dac in fsc.definitions_and_configs:
        if dac.feature_definition.is_realtime:
...
def _build_sawtooth_aggregation_subtree(
    ...
    # Build untiled partial aggs
    untiled_partial_aggs_node = build_get_features(
        dialect=dialect,
        compute_mode=compute_mode,
        fdw=fdw,
        from_source=from_source,
    ...
    if explode_partial_aggs_for_non_sawtooths:
        ...
        extra_batch_partial_agg_node = _add_sawtooth_anchor_time_and_partition_columns(
            dialect=dialect,
            compute_mode=compute_mode,
            input_node=untiled_partial_aggs_node,  # Do NOT tile the partial aggs since 5min and 1min tiles are too small and hurt performance.
            sawtooth_aggregation_data=sawtooth_aggregation_data,
            fdw=fdw,
            timestamp_field=anchor_time(),
            should_truncate_timestamps=False,
    ...
    # Stream partial aggs
    # TODO(samantha): Add pushdown filtering for entities for the stream partial aggs
    stream_partial_agg_tree = _add_sawtooth_anchor_time_and_partition_columns(
        dialect=dialect,
        compute_mode=compute_mode,
        input_node=untiled_partial_aggs_node,
        sawtooth_aggregation_data=sawtooth_aggregation_data,
        fdw=fdw,
        timestamp_field=anchor_time(),
        should_truncate_timestamps=False,
