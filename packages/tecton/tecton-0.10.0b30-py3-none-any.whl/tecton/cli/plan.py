...
def show(plan_id):
    ...
    printer.safe_print()
    # TODO(FE-2117): This is fragile, we should construct the URL on the MDS instead
    api_service = conf.get_or_none("API_SERVICE")
    plan_url = f"{api_service}/app/{plan.workspace}/plan-summary/{plan_id}"
    printer.safe_print(f"View your plan in the Web UI: {plan_url}")
    printer.safe_print()
