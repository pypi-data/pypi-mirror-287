...
def compute_aggregate_materialization_schema_from_view_schema(
    ...
    if not is_aggregate and not has_embeddings:
        return view_schema
    ...
    if is_aggregate:
        ...
        for attribute in feature_view_args.materialized_feature_view_args.attributes:
            if attribute.column not in view_schema_column_map:
                msg = f"Attribute {attribute.column} not found in view schema. View schema has columns {','.join(view_schema_column_map.keys())}"
                raise errors.TectonValidationError(msg)
            materialization_schema_columns.append(view_schema_column_map[attribute.column])
