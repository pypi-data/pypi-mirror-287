...
def _initialize_model_cache_dir(model_name: str) -> Tuple[str, bool]:
    model_cache_dir = os.path.join(conf.get_or_raise("MODEL_CACHE_DIRECTORY"), model_name)
    if os.path.exists(os.path.join(model_cache_dir, METADATA_FN)) or os.path.exists(
        os.path.join(model_cache_dir, "model.py")
    ):
...
def retrieve_custom_model_artifacts(model_name: str, s3_path: str) -> str:
