...
class Inference(Feature):
    ...
    def __init__(self, model: str, input_columns: List[Field], name: str):
        ...
        :type name: Optional[str]
        """
        # TODO(jiadong): `column` and `column_dtype` are required right now. Need to refactor `Features` to accommodate new `Inferece` class.
        super().__init__(model=model, input_columns=input_columns, name=name, column="", column_dtype=String)
    ...
    def _to_proto(self) -> feature_view_pb2.Inference:
        ...
        return feature_view_pb2.Inference(
            model=self.model,
            input_columns=[
                schema_pb2.Column(name=field.name, offline_data_type=field.dtype.tecton_type.proto)
                for field in self.input_columns
            ],
            name=self.name,
        )
