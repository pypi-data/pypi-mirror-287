...
class PandasRealtimeFeaturePipeline(RealtimeFeaturePipeline):
    ...
    def _node_to_value(self, pipeline_node: PipelineNode) -> NodeValueType:
        value_df = super()._node_to_value(pipeline_node)

        # For Python Mode, only get the current row of inputs
        if self.is_python_mode and isinstance(value_df, pandas.DataFrame):
            input_dict = value_df.to_dict("records")[self._current_row_index]
            return self._format_datetimes_for_python_mode(input_dict)

        return value_df

    def _format_datetimes_for_python_mode(self, input_dict: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _request_data_node_to_value(self, pipeline_node: PipelineNode) -> Union[Dict[str, Any], pandas.DataFrame]:
        ...
        if isinstance(self._data_df, pandas.DataFrame):
            ...
            try:
                ...
                msg = f"RTFV {self._name} has a dependency on the Request Data Source named '{input_name}', but it didn't pass schema validation: "
        ...
        if isinstance(self._data_df, pandas.DataFrame):
            ...
            for f in select_columns_and_rename_map:
                try:
                    ...
                    msg = f"RTFV {self._name} has a dependency on the Feature View '{input_name}'. Feature {f} of this Feature View is not found in the retrieved historical data. Available columns: {list(self._data_df.column_names)}"
    ...
    def _context_node_to_value(self, pipeline_node: PipelineNode) -> Optional[Union[pandas.DataFrame, RealtimeContext]]:
        ...
        # Use cache for Python Mode
        if input_name in self._input_name_to_df:
            context = self._input_name_to_df[input_name]
            context_dict = context.to_dict("records")[self._current_row_index]
            return RealtimeContext(context_dict[REQUEST_TIMESTAMP_FIELD_NAME])
        ...
        context = data_df[[self._events_df_timestamp_field]].rename(
            columns={self._events_df_timestamp_field: REQUEST_TIMESTAMP_FIELD_NAME}
        )

        self._input_name_to_df[input_name] = context
    ...
    def _rename_pandas_columns(
        ...
        for f in select_columns_and_rename_map.keys():
            if f not in self._data_df.columns:
                msg = f"RTFV {self._name} has a dependency on the Feature View '{input_name}'. Feature {f} of this Feature View is not found in the retrieved historical data. Available columns: {list(self._data_df.columns)}"
                raise TectonValidationError(msg)
    ...
    def _get_request_context_pandas_df(self, request_context_schema: SchemaProto, input_name: str) -> pandas.DataFrame:
        ...
        for f in request_context_fields:
            if f not in self._data_df.columns:
                msg = f"RTFV {self._name} has a dependency on the Request Data Source named '{input_name}'. Field {f} of this Request Data Source is not found in the spine. Available columns: {list(self._data_df.columns)}"
                raise TectonValidationError(msg)
