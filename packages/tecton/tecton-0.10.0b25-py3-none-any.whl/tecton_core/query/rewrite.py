...
class SpineEntityPushdown(Rewrite):
    ...
    def pushdown_entities(self, tree: NodeRef, spine: NodeRef, join_cols: List[str]) -> None:
        ...
        if isinstance(node, can_be_pushed_down):
            ...
            if node.compute_mode != ComputeMode.RIFT:
                tree.node = EntityFilterNode(
                    node.dialect, node.compute_mode, tree.node.as_ref(), entities_node, join_cols
                )
...
class SpineTimePushdown(Rewrite):
    ...
    def pushdown_time_range(self, tree: NodeRef, spine_time_limits: pendulum.Period) -> None:
        node = tree.node
        can_be_pushed_down = (
            RespectFeatureStartTimeNode,
            RenameColsNode,
