...
class StackSession():
    ...
    def parseCommand(self, line):
        ...
        if local:
            ...
            #     words = line.split(" ")
            #     if words[0] in self.subparsers:
            #         help = self.subparsers[words[0]].format_help()
    ...
    def init_args(self):
        ...
        #leave 
        leave_parser = subparsers.add_parser('leave', help='leave help')
    ...
    def toAnsi(self, windowWidth, showAddresses= True, showRegisters = True):
        ...
        regWidth = 80
        stackAnsi = self.stack.toAnsi(showAddress=showAddresses)
        regAnsi = self.conv.registersToAnsi(width=regWidth)

        stackLines = stackAnsi.split("\n")
        regLines = regAnsi.split("\n")
        

        out = ""


        for i in range(max(len(stackLines), len(regLines))):

            underlay = " "*windowWidth
            overlay = ""

            if i < len(stackLines):

                stripped = stripAnsi(stackLines[i])
                if len(stripped) > windowWidth:
                    remove = len(stripped) - windowWidth
                    for j in range(remove):
                        stackLines.pop()

                elif len(stripped) < windowWidth:
                    add = windowWidth - len(stripped)
                    for j in range(add):
                        stackLines[i] += " "
