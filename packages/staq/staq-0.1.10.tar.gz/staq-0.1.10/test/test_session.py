...
def test_html():
    ...
    session.parseCommand("int etc[64] --note my note --note-color green")
    session.parseCommand("call main()")
    session.parseCommand("int buf[16] --note test note")
    session.parseCommand('call printf("Hello %d %d", 1, 2)')
