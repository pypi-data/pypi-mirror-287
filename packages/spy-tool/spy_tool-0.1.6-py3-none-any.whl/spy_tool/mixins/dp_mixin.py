...
from spy_tool.chrome_extension import ChromeExtension
...
class DpMixin(object):
    ...
    def __init__(self, *,
                 ...
                 **co_kwargs):
        self.co: Optional[CO_TYPE] = co
        self.timeout: Optional[TIMEOUT_TYPE] = timeout
        self.is_close_cp: Optional[IS_CLOSE_TYPE] = is_close_cp
        self.cp: Optional[CP_TYPE] = cp
    ...
    def _init_co(self) -> None:
        ...
        port = self.co_kwargs.get('port')
        auto_port = self.co_kwargs.get('auto_port', True)
        add_extension = self.co_kwargs.get('add_extension', False)
        use_system_user_path = self.co_kwargs.get('use_system_user_path', False)
        headless = self.co_kwargs.get('headless', False)
        mute = self.co_kwargs.get('mute', True)
        incognito = self.co_kwargs.get('incognito', False)
        ignore_certificate_errors = self.co_kwargs.get('ignore_certificate_errors', False)
    ...
    def create_cp(self, addr_or_opts, tab_id) -> CP_TYPE:
        cp = ChromiumPage(addr_or_opts, tab_id, self.timeout)
        return cp
    ...
    def _init_cp(self) -> None:
        ...
        cp = self.create_cp(self.co, None)
    ...
    def refresh(self, *refresh_args, cp: Optional[CP_TYPE] = None, **refresh_kwargs) -> None:
        if cp is None:
            cp = self.cp
        js_code = '''function hasOverflowElement() {
