...
from fake_usragent.log import logger
...
def load(fn="browsers.json"):
    data, ret = [], None
    try:
        if fn == "browsers.json":
            json_lines = (
                ilr.files("fake_usragent.data").joinpath(fn).read_text()
            )
            for line in json_lines.splitlines():
                data.append(json.loads(line))
            ret = data
        else:
            ret = ilr.files("fake_usragent.data").joinpath(fn).read_bytes()
        ...
        try:
            ...
            if fn == "browsers.json":
                with open(
                    resource_filename("fake_usragent", "data/"+str(fn))
                ) as file:
...
from fake_usragent import settings  # noqa # isort:skip
from fake_usragent.errors import FakeUserAgentError  # noqa # isort:skip
