...
def prefix_token_fn_generator(tokenizer: PreTrainedTokenizerBase, pattern: str, allow_unicode=False, use_slow_engine=False) -> Callable[[int, torch.Tensor], List[int]]:
    tokens = []
    seen = {}
    for token, token_id in sorted(tokenizer.get_vocab().items(), key=lambda x: x[1]):
        string_token = get_string_token(tokenizer, token)
        if not allow_unicode and not string_token.isascii():
            continue
