...
class TabularIngestion():
    ...
    def process(self, asset_id : int, asset_uri : str, asset_is_full_extract : bool, intake_run_id : str = None, intake_execution_date : datetime = None):
        ...
        self.remove_old_data_if_needed(file_dataframe)
        ...
        if file_dataframe.count() > 0:
            ...
            if (len(incremental_column)>0):
                ...
                delta_column = self.entity_service.get_delta_attribute_name(entity.id, entity.attributes)
            ...
            self.ingest_data(ingestion_start_date, file_dataframe, entity, consolidation_mode)
            
            # Update EntityDeltaLoad in incremental loads
            self.entity_service.update_entity_delta_load_in_incremental_loads(entity, incremental_load_new_max_value)
            ...
            self.generate_data_catalog(entity)
    ...
    def ingest_data(self, ingestion_start_date, file_dataframe, entity, consolidation_mode):
    ...
    def remove_old_data_if_needed(self, file_dataframe):
        ...
        elif self.tabular_service.get_consolidation_mode() == constants.CONSOLIDATION_MODE_OVERWRITE:
            self.tabular_service.truncate_and_vacuum_table()
        elif ((self.tabular_service.get_consolidation_mode() == constants.CONSOLIDATION_MODE_OVERWRITE_IF_NOT_EMPTY) and (file_dataframe.count() > 0)):
            self.tabular_service.truncate_and_vacuum_table()

    def generate_data_catalog(self, entity):
