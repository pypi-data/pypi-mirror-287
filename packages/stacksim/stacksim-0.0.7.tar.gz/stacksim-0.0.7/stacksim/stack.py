...
class StackWord():
    ...
    def toAnsi(self, width = 80, leftMargin = 20, color = "blue", showAddress = True):
        ...
        if showAddress and self.address:
            lAddress = colored(f"{str(self.address).rjust(leftMargin)}", color="dark_grey")
            pass
        
        if self.isHead():
            lAddress = colored(">>>", color="yellow") + colored(f"{str(self.address).rjust(leftMargin-3)}", color="dark_grey")
...
class StackCell():
    ...
    def toAnsi(self, width = 80, leftMargin = 20, color = "blue", showAddress = True, sof = False):
        ...
        for i in range(self.size):
            ...
            if self.words[i].value or self.words[i].note or self.words[i].isHead():
                emptyStreak = 0
            else:
                emptyStreak += 1
...
class Stack():
    ...
    def toAnsi(self, width = 50, showAddress = False, leftMargin = 20):
