...
def _get_endpoint_for_login(endpoint: Optional[str]) -> str:
    ...
    if _unionmeta_byoc_is_installed():
        ...
        if endpoint is not None:
            msg = "--host should not be passed in for login"
            raise click.ClickException(msg)
...
def _check_unionmeta_byoc(config: Optional[str]):
    ...
    unionai_env_var = _get_union_config_env_var()
    config_path = _get_config_path()

    if _UNION_CONFIG.is_direct_unionai_cli_call and unionai_env_var is None and config_path is None:
        msg = (
...
def _get_config_obj(config: Optional[str] = None, default_to_unionai_semantics: bool = False) -> Config:
    ...
    in the following order:
        1. `UNION_SERVERLESS_API_KEY` environment variable
    ...
    if config is not None:
    ...
    elif _UNION_CONFIG.is_direct_unionai_cli_call or default_to_unionai_semantics:
        _check_unionmeta_byoc(config)

        # CLI ran with `unionai` -> Config is _UNION_CONFIG.config and the default is serverless
        ...
        elif getenv(_UNION_SERVERLESS_ENDPOINT_ENV_VAR) is not None:
            # If UNIONAI_SERVERLESS_ENDPOINT is set in `_UnionConfig`, use it instead
