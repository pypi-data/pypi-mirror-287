...
from fastapix.crud._models import create_schema_create, create_schema_read, create_schema_update
...
class SQLAlchemyCrud:
    ...
    def __init__(
        ...
        self.foreign_models = self.__get_foreign_models()
    ...
    async def read_items(
        ...
        sel = self.select(foreign if foreign else [])
        if selector:
        ...
        results = results.all()
        results = [self.table_to_pyobj(r, foreign) for r in results]
