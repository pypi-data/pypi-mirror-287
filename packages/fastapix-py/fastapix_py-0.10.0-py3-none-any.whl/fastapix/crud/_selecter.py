...
class Foreign:
    def __init__(self, model: Type[SQLModel], foreign_models: Optional[List[Type[SQLModel]]] = None):
        self.model = model
        self.foreign_models = foreign_models

    def __call__(self):
        foreign_names = {fm.__tablename__: fm.__tablename__ for fm in self.foreign_models}

        if foreign_names:
            def _foreign(join: List[Enum("enum", foreign_names)] = Query(
                None, title='外连接', description=f'`join`'
            )):
                if join is None:
                    return []
                foreign_models = [foreign.name for foreign in join]
                return [fm for fm in self.foreign_models if fm.__tablename__ in foreign_models]
        else:
            def _foreign():
                pass
        return _foreign
