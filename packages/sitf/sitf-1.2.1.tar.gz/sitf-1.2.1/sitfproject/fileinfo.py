        ...
        for filename_pattern in filename_patterns:
            ...
            if (match):
                if len(filename_pattern[1]) >= 1:
                    ...
                    self.date = datetime.strptime(modified_datetime.strftime('%Y-%m-%d'), filename_pattern[2]).date()
                    self.type = filename_pattern[3]
                break
        else:            
            self.is_null = False
            modified_time = os.path.getmtime(path)
            modified_datetime = datetime.fromtimestamp(modified_time)            
            self.date = datetime.strptime(modified_datetime.strftime('%Y-%m-%d'), filename_patterns[0][2]).date()
            self.type = filename_patterns[0][3]
