...
import pandas as pd
from sqlalchemy import create_engine, Table, MetaData, text, Column, Integer, Float, String, select
...
class DataFrameDB:
    ...
    def __init__(self, db_url, schema=None):
        ...
        # 使用正则表达式匹配 schema 参数的值
        # match = re.search(r'schema=([^&]+)', db_url)
        # if match:
        #     self.metadata = MetaData(schema=match.group(1))
        # else:
